...
self.onmessage = function (event) {
  ...
  try {
    ...
    const prefixNorm = annotationPrefix
      ? normalizeTextForMatching(annotationPrefix).replace(/\s+/g, ' ')
      : '';
    const suffixNorm = annotationSuffix
      ? normalizeTextForMatching(annotationSuffix).replace(/\s+/g, ' ')
      : '';
    ...
    if (foundStartIndex === -1 && searchStrNormalized.length > 0) {
      console.warn(
        `Worker: Context search failed for "${searchStrNormalized.substring(0, 30)}" on page ${pageIndex + 1}. Trying simple search.`
      );
      ...
      while ((simpleMatch = simpleRegex.exec(fullPageTextNormalized)) !== null) {
        if (simpleCount === targetOccurrence) {
          ...
          console.log(
            `Worker: Simple search found match at occurrence ${targetOccurrence} for "${searchStrNormalized.substring(0, 30)}"`
          );
          break;
        }
        simpleCount++;
    ...
    if (foundStartIndex !== -1) {
      ...
      postMessage({
        pageIndex,
        annotationId,
        error: `Text not found for ID ${annotationId} (norm: "${searchStrNormalized.substring(0, 30)}", occ: ${targetOccurrence}) on page ${pageIndex + 1}.`
      });
    }
  } catch (e) {
    console.error(
      `[pdfAnnotationMatcher.worker.js] Error processing annotation ${annotationId} for page ${pageIndex + 1}:`,
      e
    );
