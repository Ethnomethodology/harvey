...
<script>
	...
	import { project, updatePlayerTime } from '$lib/stores/projectStore.js';
	...
	export let isEditingSegment = false; // Received from TranscriptionsView
	...
	/* Local state                                       */
	/* -------------------------------------------------- */
    // activeSegmentIndex is derived from store now
	...
	let lastDrawnDuration = -1;
	let lastDrawnIsEditing = false;
	let lastDrawnEditStart = -1;
	let lastDrawnEditEnd = -1;
	const redrawTimeThreshold = 1/60;
	$: totalLogicalWidth = $project.player?.duration > 0 && visibleCanvasWidth > 0 ? visibleCanvasWidth * zoomLevel : 0;
	...
	let draggingHandle = null; // 'trim-left', 'trim-right', 'edit-left', 'edit-right', null
	...
	/* Animation loop                                     */
	/* -------------------------------------------------- */
	function animationLoop() { if (!isMounted) return; const cur = $project.player?.currentTime || 0; const dur = $project.player?.duration || 0; const buf = $project.audioBuffer; const storeActiveIndex = $project.player?.currentSegmentIndex ?? -1; let needsDraw = forceNextRedraw || (buf && buf !== lastDrawnBuffer) || (storeActiveIndex !== lastDrawnSegmentIndex) || (Math.abs(cur - lastDrawnTime) > redrawTimeThreshold) || (Math.abs(scrollOffsetPx - lastDrawnScrollOffset) > 0.5) || (Math.abs(zoomLevel - lastDrawnZoomLevel) > 0.001) || (dur !== lastDrawnDuration) || (isEditingSegment !== lastDrawnIsEditing) || (isEditingSegment && (editSegmentStartTime !== lastDrawnEditStart || editSegmentEndTime !== lastDrawnEditEnd)); forceNextRedraw = false; if (needsDraw && visibleCanvasWidth > 0 && buf && dur > 0 && totalLogicalWidth > 0 ) { drawTimescale(); drawSegmentWaveformUI(); } else if (needsDraw && (!buf || visibleCanvasWidth <= 0 || dur <= 0 || totalLogicalWidth <= 0)) { clearWaveformCanvases(); lastDrawnTime = cur; lastDrawnScrollOffset = scrollOffsetPx; lastDrawnZoomLevel = zoomLevel; lastDrawnSegmentIndex = storeActiveIndex; lastDrawnBuffer = buf; lastDrawnDuration = dur; lastDrawnIsEditing = isEditingSegment; lastDrawnEditStart = editSegmentStartTime; lastDrawnEditEnd = editSegmentEndTime; } if (autoScrollEnabled && dur > 0 && totalLogicalWidth > visibleCanvasWidth && !isTrimming && !isEditingSegment) { const pxCur_visible = timeToVisiblePx(cur, dur, totalLogicalWidth, scrollOffsetPx); const scrollMarginLeft = visibleCanvasWidth * 0.25; const scrollMarginRight = visibleCanvasWidth * 0.75; let targetScrollOffset = scrollOffsetPx; let needsScrollUpdate = false; if (pxCur_visible < scrollMarginLeft) { targetScrollOffset = timeToLogicalPx(cur, dur, totalLogicalWidth) - scrollMarginLeft; needsScrollUpdate = true; } else if (pxCur_visible > scrollMarginRight) { targetScrollOffset = timeToLogicalPx(cur, dur, totalLogicalWidth) - scrollMarginRight; needsScrollUpdate = true; } if (needsScrollUpdate) { targetScrollOffset = Math.max(0, Math.min(targetScrollOffset, maxScrollPx)); const diff = targetScrollOffset - scrollOffsetPx; const moveAmount = diff * 0.1; let newScrollOffset = scrollOffsetPx + moveAmount; if (Math.abs(diff) < 1) newScrollOffset = targetScrollOffset; newScrollOffset = Math.round(newScrollOffset); if (Math.abs(newScrollOffset - scrollOffsetPx) > 0) { scrollOffsetPx = newScrollOffset; if (waveformScrollContainerRef) waveformScrollContainerRef.scrollLeft = scrollOffsetPx; } } } animationFrameId = requestAnimationFrame(animationLoop); }

	/* -------------------------------------------------- */
	/* Lifecycle                                          */
	/* -------------------------------------------------- */
	onMount(() => { isMounted = true; webAudioApiSupported = typeof window.AudioContext !== 'undefined' || typeof window.webkitAudioContext !== 'undefined'; unsubscribeAudioBuffer = project.subscribe(p => { const newAudioBuffer = p.audioBuffer; if (newAudioBuffer !== lastDrawnBuffer) { zoomLevel = 1; scrollOffsetPx = 0; autoScrollEnabled = true; clearTimeout(autoScrollEnableTimer); autoScrollEnableTimer = null; if (waveformScrollContainerRef) waveformScrollContainerRef.scrollLeft = 0; lastDrawnTime = -1; lastDrawnScrollOffset = -1; lastDrawnZoomLevel = -1; lastDrawnSegmentIndex = -1; lastDrawnBuffer = null; lastDrawnDuration = -1; lastDrawnIsEditing = false; lastDrawnEditStart = -1; lastDrawnEditEnd = -1; clearWaveformCanvases(); requestRedraw(true); } }); unsubscribePlayer = project.subscribe(p => { /* State read in loop */ }); unsubscribeSegments = project.subscribe(p => { /* State read in loop */ }); tick().then(() => { if (isMounted) { if (waveformScrollContainerRef) { visibleCanvasWidth = Math.floor(waveformScrollContainerRef.clientWidth) || 0; waveformCanvasHeight = (waveformScrollContainerRef.offsetHeight || 80) - TIMESCALE_HEIGHT; } setupResizeObserver(); animationFrameId = requestAnimationFrame(animationLoop); requestRedraw(true); } }); });
	onDestroy(() => { isMounted = false; unsubscribeAudioBuffer && unsubscribeAudioBuffer(); unsubscribeSegments && unsubscribeSegments(); unsubscribePlayer && unsubscribePlayer(); if (resizeObserverInstance) { resizeObserverInstance.disconnect(); resizeObserverInstance = null; isObserverSetup = false; } if (animationFrameId) cancelAnimationFrame(animationFrameId); if (debounceScrollTimer) clearTimeout(debounceScrollTimer); if (autoScrollEnableTimer) clearTimeout(autoScrollEnableTimer); animationFrameId = null; window.removeEventListener('mousemove', handleTrimMouseMove); window.removeEventListener('mouseup', handleTrimMouseUp); window.removeEventListener('mousemove', handleEditMouseMove); window.removeEventListener('mouseup', handleEditMouseUp); lastDrawnTime = -1; lastDrawnScrollOffset = -1; lastDrawnZoomLevel = -1; lastDrawnSegmentIndex = -1; lastDrawnBuffer = null; lastDrawnDuration = -1; lastDrawnIsEditing = false; lastDrawnEditStart = -1; lastDrawnEditEnd = -1; });
	...
	/* -------------------------------------------------- */
	/* Waveform Scroll Handler (Manual Scroll)            */
	/* -------------------------------------------------- */
	...
	/* -------------------------------------------------- */
	function startTrimDrag(handle, event) { if (!isTrimming || !$project.player.duration || !segmentWaveformCanvas || isEditingSegment) return; event.preventDefault(); draggingHandle = handle; window.addEventListener('mousemove', handleTrimMouseMove); window.addEventListener('mouseup', handleTrimMouseUp, { once: true }); }
	function handleTrimMouseMove(event) { if (draggingHandle !== 'trim-left' && draggingHandle !== 'trim-right') return; if (!isTrimming || !segmentWaveformCanvas || visibleCanvasWidth <= 0 || !$project.player.duration) return; event.preventDefault(); const rect = segmentWaveformCanvas.getBoundingClientRect(); const clickX = Math.max(0, Math.min(visibleCanvasWidth, event.clientX - rect.left)); let newTime = pxToTime(clickX, $project.player.duration, totalLogicalWidth, visibleCanvasWidth, scrollOffsetPx); const minDuration = 0.1; let newStartTime = trimStartTime; let newEndTime = trimEndTime; if (draggingHandle === 'trim-left') { newStartTime = Math.max(0, Math.min(newTime, trimEndTime - minDuration)); } else { newEndTime = Math.min($project.player.duration, Math.max(newTime, trimStartTime + minDuration)); } if (newStartTime !== trimStartTime || newEndTime !== trimEndTime) { dispatch('trimupdate', { startTime: newStartTime, endTime: newEndTime }); } }
	...
	/* -------------------------------------------------- */
	/* Segment Edit Dragging Handlers                     */
	/* -------------------------------------------------- */
	function startEditDrag(handle, event) {
        console.log(`[Waveform] startEditDrag called. isEditingSegment: ${isEditingSegment}, handle: ${handle}`); // ADD LOG
        if (!isEditingSegment || !$project.player.duration || !segmentWaveformCanvas || isTrimming) {
            console.log("[Waveform] startEditDrag: conditions not met, returning."); // ADD LOG
            return;
        }
		event.preventDefault();
		draggingHandle = handle;
        console.log(`[Waveform] startEditDrag: Set draggingHandle=${draggingHandle}. Adding listeners.`); // ADD LOG
	...
	function handleEditMouseMove(event) {
		...
		if (!isEditingSegment || !segmentWaveformCanvas || visibleCanvasWidth <= 0 || !$project.player.duration) return;
		event.preventDefault();
		...
		let newTime = pxToTime(clickX, $project.player.duration, totalLogicalWidth, visibleCanvasWidth, scrollOffsetPx);
		const minDuration = 0.05; // Minimum segment length
		let newStartTime = editSegmentStartTime;
		let newEndTime = editSegmentEndTime;
		const lowerBound = 0; // Cannot drag before 0
		const upperBound = $project.player.duration; // Cannot drag beyond duration

		// Adjust based on which handle is dragged
		if (draggingHandle === 'edit-left') {
			newStartTime = Math.max(lowerBound, Math.min(newTime, editSegmentEndTime - minDuration));
		} else { // draggingHandle === 'edit-right'
			newEndTime = Math.min(upperBound, Math.max(newTime, editSegmentStartTime + minDuration));
		}

		// Dispatch update only if times actually changed
		if (newStartTime !== editSegmentStartTime || newEndTime !== editSegmentEndTime) {
            console.log(`[Waveform] handleEditMouseMove: Dispatching segmentupdate. Start=${newStartTime.toFixed(3)}, End=${newEndTime.toFixed(3)}`); // ADD LOG
	 ...
	 function handleEditMouseUp() {
        if (draggingHandle === 'edit-left' || draggingHandle === 'edit-right') {
            console.log(`[Waveform] handleEditMouseUp: Clearing draggingHandle=${draggingHandle}. Removing listener.`); // ADD LOG
			draggingHandle = null;
    ...
    // --- Add reactive log for isEditingSegment ---
    $: if (isMounted) {
        console.log(`[Waveform] Prop 'isEditingSegment' updated: ${isEditingSegment}`);
        // If exiting edit mode while dragging, clean up listeners
        if (!isEditingSegment && (draggingHandle === 'edit-left' || draggingHandle === 'edit-right')) {
             console.log(`[Waveform] Exited edit mode while dragging, cleaning up listeners.`);
             handleEditMouseUp(); // Call mouseup handler to cleanup
        }
    }
...
<div class="waveform-container relative flex items-center space-x-1 bg-gray-100 dark:bg-app-bg-dark h-full">
	...
	>
		...
		<canvas bind:this={segmentWaveformCanvas} class="waveform-canvas {($project.audioBuffer && !isTrimming && !isEditingSegment) ? 'cursor-pointer' : 'cursor-default'}" style="height: {waveformCanvasHeight}px; top: {TIMESCALE_HEIGHT}px;" aria-label="Waveform visualization. Click to seek audio." on:click|self={handleCanvasClick} on:wheel|preventDefault={(e) => { if (e.ctrlKey || e.metaKey) { handleZoom(e.deltaY < 0 ? 'in' : 'out'); } }} />
		{#if !webAudioApiSupported} <div class="waveform-overlay"><p class="waveform-overlay-text text-red-600">Web Audio API not supported.</p></div> {:else if !$project.audioBuffer} <div class="waveform-overlay"><p class="waveform-overlay-text">Load audio/video media to view waveform.</p></div> {/if}

		<!-- Trim Overlay and Handles -->
		{#if isTrimming && visibleCanvasWidth > 0 && $project.player.duration > 0}
			{@const trimStartPx = timeToVisiblePx(trimStartTime, $project.player.duration, totalLogicalWidth, scrollOffsetPx)}
			{@const trimEndPx = timeToVisiblePx(trimEndTime, $project.player.duration, totalLogicalWidth, scrollOffsetPx)}
