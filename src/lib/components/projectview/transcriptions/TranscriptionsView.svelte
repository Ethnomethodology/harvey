    ...
    <div class="flex flex-col flex-grow min-h-0 w-full overflow-hidden">
        ...
        <div class="flex flex-grow min-h-0 w-full overflow-x-hidden">
            {#if isLeftPanelVisible}
                <div
                    class="w-[15%] h-full bg-white dark:bg-surface-3 overflow-y-auto flex-shrink-0 transition-all duration-300 ease-in-out"
            ...
            <div class="{middlePanelWidthClass} h-full flex flex-col transition-all duration-300 ease-in-out border-l border-gray-300 dark:border-border">
                <div class="{isMediaPlayerHidden ? '' : ($transcriptStore.englishSegments && $transcriptStore.englishSegments.length > 0 && $transcriptStore.originalSegments && $transcriptStore.originalSegments.length > 0 ? 'h-[calc(50%-1.75rem)]' : 'h-1/2')} bg-white dark:bg-surface-2 flex flex-col">
                    <MediaPlayer
            ...
            <div class="flex-grow min-h-0 bg-white dark:bg-surface-2 overflow-y-auto border-t border-gray-300 dark:border-border">
                 <EditableTranscript
        ...
        {#if currentWaveformLayout === 'vertical'}
            <div bind:clientWidth={verticalWaveformWidthPx} class="w-[5%] h-full flex-shrink-0 transition-all duration-300 ease-in-out border-l border-gray-300 dark:border-border">
                ...
                {:else if $transcriptStore.selectedMediaFile}
                    <div class="flex items-center justify-center h-full text-xs text-gray-400 dark:text-gray-500 bg-white dark:bg-surface-2 p-1">
        ...
        <div class="{rightPanelWidthClass} h-full bg-white dark:bg-surface-2 overflow-y-auto transition-all duration-300 ease-in-out flex flex-col border-l border-gray-300 dark:border-border">
             <RichTextPreview
