<script lang="ts">
  ...
  $: filteredAvailableOptions = searchTerm
    ? availableOptions.filter((option) => option.toLowerCase().includes(searchTerm.toLowerCase()))
    : availableOptions;
      ...
      if (dropdownElement) {
        ...
        const estimatedItemCount = groupedOptions
          ? searchTerm
            ...
            : availableOptions.length + groupedOptions.length
          : filteredAvailableOptions.length;
...
<div class="relative bg-white dark:bg-gray-900" bind:this={rootElement}>
  ...
  >
    ...
    {#if isEditable && assignedOptions.length > 0}
      <span class="text-xs text-gray-500 dark:text-gray-400 px-2 py-1 flex-grow text-left"
        >Add {itemType}...</span
      >
    {/if}
  </div>
  ...
  {#if showDropdown}
    <div
      ...
      style="top: {dropdownY}px; left: {dropdownX}px; width: {dropdownWidth}px; {placement === 'top'
      ...
      <ul class="pb-1">
        {#if !searchTerm && groupedOptions && groupedOptions.length > 0}
          ...
          {#each groupedOptions as group (group.name)}
            ...
            {#if groupOptions.length > 0}
              <div class="relative mt-4 mb-3 mx-2">
                <span
                  class="absolute -top-2 left-2 px-1 text-[9px] font-bold uppercase tracking-wider bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 z-10"
                >
                  {group.name}
                </span>
          ...
          {#if ungroupedOptions.length > 0}
            <div class="relative mt-4 mb-3 mx-2">
              <span
                class="absolute -top-2 left-2 px-1 text-[9px] font-bold uppercase tracking-wider bg-white dark:bg-gray-900 text-gray-400 dark:text-gray-500 z-10"
              >
                Ungrouped
              </span>
