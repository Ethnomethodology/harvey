...
<script>
	...
	// External Data Source Props (optional)
	...
	// Internal reactive variables for data sourcing
	...
	$: currentMediaDuration = externalDuration ?? $transcriptStore.player.duration;
	$: currentIsPlaying = externalIsPlaying ?? $transcriptStore.player.isPlaying;
	$: currentSegmentsToDisplay = externalSegments ?? $transcriptStore.segments; // Ensure this is always an array
	$: activeSegmentIndexForDisplay = externalCurrentSegmentIndex ?? $transcriptStore.player.currentSegmentIndex;

	let prevExternalAudioBuffer = null; // For detecting changes in externalAudioBuffer

	$: totalLogicalWidth = currentMediaDuration > 0 && visibleCanvasWidth > 0 ? visibleCanvasWidth * zoomLevel : 0;
	...
	function resetZoomAndScrollState(clearBuffer = true) {
		...
		if (clearBuffer) lastDrawnBuffer = null; // Only clear if new buffer is truly different
		lastDrawnDuration = -1;
	...
	onMount(() => {
		...
		unsubscribeAudioBuffer = transcriptStore.subscribe(ts => {
			// This subscription now only resets if we are NOT using an external buffer.
			if (externalAudioBuffer === null) {
				const newAudioBuffer = ts.audioBuffer;
				if (newAudioBuffer !== lastDrawnBuffer) {
					resetZoomAndScrollState();
				}
			}
		});
		// These subscriptions are mostly for triggering re-renders via the animationLoop's checks against lastDrawn values.
		// The actual data is sourced via currentPlayTime, currentSegmentsToDisplay etc.
		unsubscribePlayer = transcriptStore.subscribe(ts => { /* Player state changes will be picked up by animationLoop */ });
		unsubscribeSegments = transcriptStore.subscribe(ts => { /* Segments changes will be picked up by animationLoop */ });

		prevExternalAudioBuffer = externalAudioBuffer; // Initialize for the reactive statement below
	...
	function startEditDrag(handle, event) {
        ...
        if (!isEditingSegment || !currentMediaDuration || !segmentWaveformCanvas || isTrimming) {
	...
	function handleEditMouseMove(event) {
		...
		if (!isEditingSegment || !segmentWaveformCanvas || visibleCanvasWidth <= 0 || !currentMediaDuration) return;
		event.preventDefault();
		...
		let newTime = pxToTime(clickX, currentMediaDuration, totalLogicalWidth, visibleCanvasWidth, scrollOffsetPx);
		...
		const upperBound = currentMediaDuration; // Cannot drag beyond duration
    ...
    // Reactive statement to handle externalAudioBuffer changes
    $: if (isMounted && externalAudioBuffer && externalAudioBuffer !== prevExternalAudioBuffer) {
        console.log('[Waveform] externalAudioBuffer changed, resetting zoom and scroll.');
        resetZoomAndScrollState(false); // Don't clear lastDrawnBuffer immediately, let animation loop handle it
        prevExternalAudioBuffer = externalAudioBuffer;
        requestRedraw(true); // Ensure immediate redraw with new buffer
    } else if (isMounted && !externalAudioBuffer && prevExternalAudioBuffer) {
        // External buffer was removed, might need to reset or switch to store
        console.log('[Waveform] externalAudioBuffer removed.');
        prevExternalAudioBuffer = null;
        // If not falling back to a store buffer that's valid, reset.
        if (!$transcriptStore.audioBuffer) { // Check global store here specifically for this fallback case
            resetZoomAndScrollState(true);
        }
        requestRedraw(true);
    }
</script>
...
<div class="waveform-container relative flex items-center space-x-1 bg-gray-100 dark:bg-app-bg-dark h-full">
	...
	>
		...
		{#if isTrimming && visibleCanvasWidth > 0 && currentMediaDuration > 0}
			{@const trimStartPx = timeToVisiblePx(trimStartTime, currentMediaDuration, totalLogicalWidth, scrollOffsetPx)}
			{@const trimEndPx = timeToVisiblePx(trimEndTime, currentMediaDuration, totalLogicalWidth, scrollOffsetPx)}
