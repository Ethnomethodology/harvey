...
<script>
  ...
  $: totalLogicalWidth =
    actualMediaDuration > 0 && visibleCanvasWidth > 0 ? visibleCanvasWidth * zoomLevel : 0;
  ...
  function timeToLogicalPx(time, duration, logicalWidth) {
    if (!duration || duration <= 0 || !logicalWidth || logicalWidth <= 0) return 0;
  ...
  function updateWaveformCache() {
    ...
    if (reqW > MAX_CANVAS_DIMENSION) {
      console.warn(
        '[InteractiveWaveform] Logical width exceeds canvas limits, cache may be truncated.'
      );
    }
  ...
  function clearWaveformCanvases() {
    ...
    if (segmentWaveformCanvas && visibleCanvasWidth > 0 && waveformCanvasHeight > 0) {
      ...
      if (c) {
        ...
        else if (!currentAudioBuffer && !currentAudioPeaks) message = 'Load media to see waveform.';
  ...
  function drawSegmentWaveformUI() {
    ...
    if (baseCacheCanvas && totalLogicalWidth > 0) {
      ctx.drawImage(
        ...
        scrollOffsetPx * dpr,
        0,
        visibleCanvasWidth * dpr,
        waveformCanvasHeight * dpr,
        0,
        0,
        visibleCanvasWidth,
  ...
  function animationLoop() {
    ...
    let needsDraw =
      ...
      (isEditingSegment &&
        (editSegmentStartTime !== lastDrawnEditStart || editSegmentEndTime !== lastDrawnEditEnd));
    ...
    if (needsDraw && visibleCanvasWidth > 0 && (buf || peaks) && dur > 0 && totalLogicalWidth > 0) {
      ...
      needsDraw &&
      ((!buf && !peaks) || visibleCanvasWidth <= 0 || dur <= 0 || totalLogicalWidth <= 0)
    ) {
      ...
      if (isMounted) {
        if (waveformScrollContainerRef) {
          ...
          if (fixedHeightPx > 0) {
            ...
            waveformCanvasHeight =
              (waveformScrollContainerRef.offsetHeight || 80) - TIMESCALE_HEIGHT;
          }
        }
        setupResizeObserver();
        animationFrameId = requestAnimationFrame(animationLoop);
          ...
          if (entry.target === waveformScrollContainerRef) {
            ...
            if (newWidth > 0 && newWidth !== visibleCanvasWidth) {
              ...
              const currentMaxScroll = Math.max(
                0,
                visibleCanvasWidth * zoomLevel - visibleCanvasWidth
              );
              ...
              if (
                ...
                const centerProportion =
                  oldTotalLogicalWidth > 0 ? scrollCenterLogicalPx / oldTotalLogicalWidth : 0;
                const newTotalLogicalWidthAfterUpdate = visibleCanvasWidth * zoomLevel;
                newScrollOffset =
                  centerProportion * newTotalLogicalWidthAfterUpdate - visibleCanvasWidth / 2;
                newScrollOffset = Math.max(
                  0,
                  Math.min(
                    newScrollOffset,
                    Math.max(0, newTotalLogicalWidthAfterUpdate - visibleCanvasWidth)
                  )
                );
              } else {
  ...
  function handleCanvasClick(e) {
    ...
    const time = pxToTime(clickX, dur, totalLogicalWidth, visibleCanvasWidth, scrollOffsetPx);
  ...
  function handleZoom(direction) {
    ...
    tick().then(() => {
      ...
      let newScrollOffset =
        timeToLogicalPx(timeAtCenter, actualMediaDuration, newTotalLogicalWidthAfterZoom) -
        visibleCanvasWidth / 2;
  ...
  function handlePanEnd(event) {
    ...
    if (segmentWaveformCanvas) {
      segmentWaveformCanvas.style.cursor =
        currentAudioBuffer && !isTrimming && !isEditingSegment ? 'pointer' : 'default';
    }
  ...
  function startTrimDrag(handle, event) {
    if (!isTrimming || !actualMediaDuration || !segmentWaveformCanvas || isEditingSegment) return;
    event.preventDefault();
  ...
  function handleTrimMouseMove(event) {
    ...
    if (!isTrimming || !segmentWaveformCanvas || visibleCanvasWidth <= 0 || !actualMediaDuration)
      return;
    event.preventDefault();
  ...
  function startHandleAutoScroll() {
    ...
    function scrollLoop() {
      ...
      if (scrolled) {
        ...
        const clickXInVisibleArea = Math.max(
          0,
          Math.min(visibleCanvasWidth, edgeClientX - containerRect.left)
        );
        let newTime = pxToTime(
          clickXInVisibleArea,
          actualMediaDuration,
  ...
  function startEditDrag(handle, event) {
    if (!isEditingSegment || !actualMediaDuration || !segmentWaveformCanvas || isTrimming) {
      return;
    }
    event.preventDefault();
    draggingHandle = handle;
...
>
  ...
  >
      ...
      on:wheel|preventDefault={(e) => {
        if (e.ctrlKey || e.metaKey) {
          ...
          scrollOffsetPx = Math.max(
            0,
            Math.min(maxScrollPx, scrollOffsetPx + scrollAmount * scrollFactor)
          );
          requestRedraw(true);
        }
      }}
    ...
    {#if showTrimUI && isTrimming && visibleCanvasWidth > 0 && actualMediaDuration > 0}
      ...
      <div
        ...
        on:mousedown|preventDefault={(e) => startTrimDrag('trim-left', e)}
        role="slider"
      ...
      <div
        ...
        on:mousedown|preventDefault={(e) => startTrimDrag('trim-right', e)}
        role="slider"
    ...
    {#if showTrimUI && isEditingSegment && visibleCanvasWidth > 0 && actualMediaDuration > 0}
      ...
      <div
        ...
        on:mousedown|preventDefault={(e) => startEditDrag('edit-left', e)}
        role="slider"
      ...
      <div
        ...
        on:mousedown|preventDefault={(e) => startEditDrag('edit-right', e)}
        role="slider"
  ...
  {#if showTrimUI && isTrimming && visibleCanvasWidth > 0 && actualMediaDuration > 0}
    {@const trimStartVisPx = timeToVisiblePx(
      trimStartTime,
      actualMediaDuration,
      totalLogicalWidth,
      scrollOffsetPx
    )}
    {@const trimEndVisPx = timeToVisiblePx(
      trimEndTime,
      actualMediaDuration,
      totalLogicalWidth,
      scrollOffsetPx
  ...
  {#if showTrimUI && isEditingSegment && visibleCanvasWidth > 0 && actualMediaDuration > 0}
    {@const editStartVisPx = timeToVisiblePx(
      editSegmentStartTime,
      actualMediaDuration,
      totalLogicalWidth,
      scrollOffsetPx
    )}
    {@const editEndVisPx = timeToVisiblePx(
      editSegmentEndTime,
      actualMediaDuration,
      totalLogicalWidth,
      scrollOffsetPx
