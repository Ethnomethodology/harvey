...
<script>
	...
	import { project } from '$lib/stores/projectStore.js'; // Keep for project-level things if any non-transcript state is used.
	...
	$: totalLogicalWidth = $transcriptStore.player?.duration > 0 && visibleCanvasWidth > 0 ? visibleCanvasWidth * zoomLevel : 0;
	...
	onMount(() => {
		...
		// These subscriptions are simplified as their state is read directly in the animationLoop from transcriptStore
		unsubscribePlayer = transcriptStore.subscribe(ts => { /* State read in loop: ts.player */ });
		unsubscribeSegments = transcriptStore.subscribe(ts => { /* State read in loop: ts.segments */ });
	...
	/* -------------------------------------------------- */
	function startTrimDrag(handle, event) { if (!isTrimming || !$transcriptStore.player.duration || !segmentWaveformCanvas || isEditingSegment) return; event.preventDefault(); draggingHandle = handle; window.addEventListener('mousemove', handleTrimMouseMove); window.addEventListener('mouseup', handleTrimMouseUp, { once: true }); }
	function handleTrimMouseMove(event) { if (draggingHandle !== 'trim-left' && draggingHandle !== 'trim-right') return; if (!isTrimming || !segmentWaveformCanvas || visibleCanvasWidth <= 0 || !$transcriptStore.player.duration) return; event.preventDefault(); const rect = segmentWaveformCanvas.getBoundingClientRect(); const clickX = Math.max(0, Math.min(visibleCanvasWidth, event.clientX - rect.left)); let newTime = pxToTime(clickX, $transcriptStore.player.duration, totalLogicalWidth, visibleCanvasWidth, scrollOffsetPx); const minDuration = 0.1; let newStartTime = trimStartTime; let newEndTime = trimEndTime; if (draggingHandle === 'trim-left') { newStartTime = Math.max(0, Math.min(newTime, trimEndTime - minDuration)); } else { newEndTime = Math.min($transcriptStore.player.duration, Math.max(newTime, trimStartTime + minDuration)); } if (newStartTime !== trimStartTime || newEndTime !== trimEndTime) { dispatch('trimupdate', { startTime: newStartTime, endTime: newEndTime }); } }
	...
	function startEditDrag(handle, event) {
        ...
        if (!isEditingSegment || !$transcriptStore.player.duration || !segmentWaveformCanvas || isTrimming) {
	...
	function handleEditMouseMove(event) {
		...
		if (!isEditingSegment || !segmentWaveformCanvas || visibleCanvasWidth <= 0 || !$transcriptStore.player.duration) return;
		event.preventDefault();
		...
		let newTime = pxToTime(clickX, $transcriptStore.player.duration, totalLogicalWidth, visibleCanvasWidth, scrollOffsetPx);
		...
		const upperBound = $transcriptStore.player.duration; // Cannot drag beyond duration
...
<div class="waveform-container relative flex items-center space-x-1 bg-gray-100 dark:bg-app-bg-dark h-full">
	...
	>
		...
		<canvas bind:this={segmentWaveformCanvas} class="waveform-canvas {($transcriptStore.audioBuffer && !isTrimming && !isEditingSegment) ? 'cursor-pointer' : 'cursor-default'}" style="height: {waveformCanvasHeight}px; top: {TIMESCALE_HEIGHT}px;" aria-label="Waveform visualization. Click to seek audio." on:click|self={handleCanvasClick} on:wheel|preventDefault={(e) => { if (e.ctrlKey || e.metaKey) { handleZoom(e.deltaY < 0 ? 'in' : 'out'); } }} />
		{#if !webAudioApiSupported} <div class="waveform-overlay"><p class="waveform-overlay-text text-red-600">Web Audio API not supported.</p></div> {:else if !$transcriptStore.audioBuffer} <div class="waveform-overlay"><p class="waveform-overlay-text">Load audio/video media to view waveform.</p></div> {/if}

		<!-- Trim Overlay and Handles -->
		{#if isTrimming && visibleCanvasWidth > 0 && $transcriptStore.player.duration > 0}
			{@const trimStartPx = timeToVisiblePx(trimStartTime, $transcriptStore.player.duration, totalLogicalWidth, scrollOffsetPx)}
			{@const trimEndPx = timeToVisiblePx(trimEndTime, $transcriptStore.player.duration, totalLogicalWidth, scrollOffsetPx)}
