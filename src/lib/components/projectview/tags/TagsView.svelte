...
<script>
    ...
    if (tagStore.tagInfo && tableContainer) {
      ...
      if (!tabulatorInstance) {
        ...
        if (currentTableMode !== (isGroupView ? 'group' : 'tag')) {
          ...
          // Just update data
          tabulatorInstance
            .replaceData(processedHighlights)
            .then(() => {
    ...
    let columns = [
      {
        ...
        formatter: (cell, formatterParams, onRendered) => {
          ...
          iconContainer.className =
            'w-8 h-8 rounded-full flex items-center justify-center p-1 flex-shrink-0 aspect-square mt-0.5';
    ...
    columns.push({
      ...
      formatter: (cell, formatterParams, onRendered) => {
        ...
        commentsBtn.className =
          'relative p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600';
        mount(MessageCircle, { target: commentsBtn, props: { size: 16 } });
        if (commentCount > 0) {
          const pill = document.createElement('span');
          pill.className =
            'absolute -top-1 -right-1 bg-blue-500 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center';
  ...
  async function handleCommentAction(event) {
    ...
    try {
      ...
      manageCommentInHighlightLocal(
        ...
        commentId,
        newText,
        highlight.source.file_type,
        highlight.source.file_path
      );
      ...
      if (action === 'add') {
        ...
        highlight.comments = (highlight.comments || []).filter(
          (c) => c.id !== commentId && c.parentId !== commentId
        );
      } else if (action === 'update') {
        highlight.comments = (highlight.comments || []).map((c) =>
  ...
  async function handleInspect(highlight) {
    ...
    let viewType =
      highlight.source.file_type === 'audio' || highlight.source.file_type === 'video'
        ? 'media'
        : highlight.source.file_type === 'csv'
          ? 'table'
          : highlight.source.file_type === 'image'
            ? 'image'
            : highlight.source.file_type === 'standalone_transcript'
              ? 'transcript'
              : 'document';
    ...
    const isAttachment =
      originalDocType === 'document-attachment' ||
      originalDocType === 'transcript-attachment' ||
      highlight.source.file_path.includes('/attachments/');
...
<div class="flex flex-col h-full w-full bg-gray-100 dark:bg-gray-950 overflow-hidden">
  <div class="flex h-full w-full divide-x divide-gray-300 dark:divide-border">
    ...
    {#if !panelStateStore.tagsLeftPanelCollapsed}
      ...
      >
        <!-- Header -->
        <h2
          class="relative flex items-center justify-between text-sm font-semibold text-gray-700 dark:text-gray-400 px-1 h-9 border-b border-gray-200 dark:border-gray-800 mb-3"
        >
        ...
        <div class="flex-1 overflow-y-auto p-2">
          ...
          <div class="flex items-center justify-between mb-2 px-2">
            <h3
              class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide m-0"
            >
              Tag Groups
            </h3>
            <button
              class="p-0.5 text-gray-400 dark:text-gray-500 hover:text-blue-500 dark:hover:text-blue-400 transition-colors"
              on:click={() => (isAddGroupModalOpen = true)}
          ...
          {#if groups.length === 0}
            <p class="text-xs text-gray-400 dark:text-gray-500 italic px-2 py-1">
              No tag group found.
            </p>
          {/if}
          {#each groups as group (group.id)}
            <div
              class="mb-2 rounded border border-transparent hover:border-gray-200 dark:hover:border-gray-600"
            >
              <!-- Group Header -->
              <div
          ...
          <div class="mt-2">
            ...
            <h3
              class="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2 uppercase px-2 tracking-wide"
            >
              Ungrouped Tags
            </h3>
            <div class="px-2 mb-2">
              <button
                class="w-full flex items-center justify-center gap-1 py-1.5 px-2 text-xs font-medium text-gray-600 bg-gray-50 border border-gray-200 rounded hover:bg-gray-100 hover:text-blue-600 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-blue-400 transition-colors"
                on:click={() => (isAddTagModalOpen = true)}
                title="Create Tag"
              >
      ...
      {#if tagStore.selectedTag || tagStore.selectedTagGroup}
        ...
        {:else if tagStore.tagInfo}
          ...
          <div class="flex-shrink-0">
            ...
            <div class="mt-2">
              {#if description}
                <p class="text-sm text-gray-600 dark:text-white whitespace-pre-wrap">
                  {description}
                </p>
              {:else}
          ...
          <div class="flex flex-col flex-1 min-h-0">
            <div class="flex justify-between items-center mb-2 flex-shrink-0">
              ...
              <input
                ...
                class="border rounded px-2 py-1 text-sm dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200"
                autocomplete="off"
              />
            </div>
            <div
              class="flex-grow relative border border-gray-300 dark:border-gray-700 rounded-md min-h-0"
            >
  ...
  {#if isCommentsPanelOpen}
    <div
      ...
      on:click={() => (isCommentsPanelOpen = false)}
    ></div>
    <div
