        ...
        {#each comments.filter(c => !c.parentId) as comment}
            <div class="p-3 rounded-lg bg-gray-100 dark:bg-gray-700 relative group">
                {#if editingCommentId === comment.id}
                    <textarea bind:value={editingText} class="w-full p-2 border rounded-md bg-white dark:bg-gray-800 text-black dark:text-white border-gray-300 dark:border-gray-600" rows="3" autocomplete="off" autocorrect="off"></textarea>
            ...
            {#each comments.filter(r => r.parentId === comment.id) as reply}
                <div class="ml-8 p-3 rounded-lg bg-gray-200 dark:bg-gray-600 relative group">
                    {#if editingCommentId === reply.id}
                        <textarea bind:value={editingText} class="w-full p-2 border rounded-md bg-white dark:bg-gray-700 text-black dark:text-white border-gray-300 dark:border-gray-600" rows="2" autocomplete="off" autocorrect="off"></textarea>
                        <div class="mt-2 flex justify-end gap-2">
