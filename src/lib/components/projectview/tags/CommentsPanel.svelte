<script lang="ts">
    ...
    import { X, MoreVertical, MessageCircle, Reply, Pencil, Trash2, Clock, UserRound } from '@lucide/svelte';
...
<div class="flex flex-col h-full">
    ...
    <div class="space-y-4 flex-grow overflow-y-auto pr-1 custom-scrollbar">
        {#each comments.filter(c => !c.parentId) as comment}
            <div class="space-y-2">
                <!-- Parent comment -->
                <div class="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 relative group">
                    {#if editingCommentId === comment.id}
                        ...
                        <div class="mt-2 flex justify-end gap-2">
                            ...
                            <button on:click={handleSaveEdit} class="px-3 py-1 rounded-lg text-xs font-medium bg-blue-600 text-white hover:bg-blue-700 transition-colors">Save</button>
                        </div>
                    {:else}
                        <!-- Avatar + timestamp header -->
                        <div class="flex justify-between items-start mb-2">
                            <div class="flex items-center gap-2">
                                <div class="w-6 h-6 rounded-full bg-gray-100 dark:bg-gray-600 border border-gray-300 dark:border-gray-500 flex items-center justify-center shrink-0">
                                    <UserRound class="w-3.5 h-3.5 text-gray-500 dark:text-gray-300" />
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-sm font-semibold text-gray-900 dark:text-white leading-none">User</span>
                                    <span class="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-0.5 mt-0.5">
                            ...
                            <div class="relative">
                                ...
                                {#if activeMenuId === comment.id}
                                    <div class="absolute right-0 mt-1 w-32 bg-white dark:bg-gray-900 rounded-lg shadow-lg z-20 border border-gray-200 dark:border-gray-700 py-1">
                                        <button on:click={() => handleAction('reply', comment)} class="flex items-center gap-2 w-full text-left px-3 py-1.5 text-xs text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800">
                                            <Reply class="w-3 h-3" /> Reply
                                        </button>
                                        <button on:click={() => handleAction('edit', comment)} class="flex items-center gap-2 w-full text-left px-3 py-1.5 text-xs text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800">
                                            <Pencil class="w-3 h-3" /> Edit
                                        </button>
                                        <button on:click={() => handleAction('delete', comment)} class="flex items-center gap-2 w-full text-left px-3 py-1.5 text-xs text-red-600 dark:text-red-400 hover:bg-gray-50 dark:hover:bg-gray-800">
                ...
                {#each comments.filter(r => r.parentId === comment.id) as reply}
                    <div class="ml-6 p-3 rounded-xl bg-gray-100/50 dark:bg-gray-800/30 border border-gray-200/50 dark:border-gray-700/50 relative group">
                            ...
                            <div class="flex justify-between items-start mb-2">
                                ...
                                <div class="relative">
                                    ...
                                    {#if activeMenuId === reply.id}
                                        <div class="absolute right-0 mt-1 w-32 bg-white dark:bg-gray-900 rounded-lg shadow-lg z-20 border border-gray-200 dark:border-gray-700 py-1">
                                            <button on:click={() => handleAction('edit', reply)} class="flex items-center gap-2 w-full text-left px-3 py-1.5 text-xs text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800">
                                                <Pencil class="w-3 h-3" /> Edit
                                            </button>
                                            <button on:click={() => handleAction('delete', reply)} class="flex items-center gap-2 w-full text-left px-3 py-1.5 text-xs text-red-600 dark:text-red-400 hover:bg-gray-50 dark:hover:bg-gray-800">
                                                <Trash2 class="w-3 h-3" /> Delete
                                            </button>
                                        </div>
                                    {/if}
    ...
    <div class="mt-4 pt-4 border-t dark:border-gray-600">
        {#if replyingToCommentId}
            <div class="flex items-center justify-between mb-2 px-2 py-1.5 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800/40">
                ...
                <button on:click={cancelReply} class="p-0.5 hover:bg-blue-100 dark:hover:bg-blue-800 rounded transition-colors" aria-label="Cancel reply">
