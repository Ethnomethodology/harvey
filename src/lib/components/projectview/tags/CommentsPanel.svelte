...
<div class="flex flex-col h-full p-2">
    <div class="flex justify-between items-center mb-4">
        ...
        <button on:click={() => dispatch('close')} class="p-1 hover:bg-gray-200 dark:hover:bg-gray-600">
    ...
    <div class="space-y-4 flex-grow overflow-y-auto pr-2">
        {#each comments.filter(c => !c.parentId) as comment}
            <div class="p-3 bg-gray-100 dark:bg-gray-700 relative group">
                {#if editingCommentId === comment.id}
                    <textarea bind:value={editingText} class="w-full p-2 border bg-white dark:bg-gray-800 text-black dark:text-white border-gray-300 dark:border-gray-600" rows="3"></textarea>
                    <div class="mt-2 flex justify-end gap-2">
                        <button on:click={handleCancelEdit} class="px-3 py-1 text-xs bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500">Cancel</button>
                        <button on:click={handleSaveEdit} class="px-3 py-1 text-xs bg-blue-600 text-white hover:bg-blue-700">Save</button>
                    </div>
                {:else}
                    <div class="flex justify-between items-start">
                        <p class="text-sm text-gray-800 dark:text-gray-200 flex-grow pr-8">{comment.text}</p>
                        <div class="absolute top-1 right-1">
                            <button on:click={() => toggleMenu(comment.id)} class="p-1 hover:bg-gray-300 dark:hover:bg-gray-600 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-three-dots-vertical" viewBox="0 0 16 16">
                                    <path d="M9.5 13a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0m0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0m0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0"/>
                                </svg>
                            </button>
                            {#if activeMenuId === comment.id}
                                <div class="absolute right-0 mt-2 w-32 bg-white dark:bg-gray-900 shadow-lg z-10 border border-gray-200 dark:border-gray-700">
                                    {#if !comment.parentId}
            ...
            {#each comments.filter(r => r.parentId === comment.id) as reply}
                <div class="ml-8 p-3 bg-gray-200 dark:bg-gray-600 relative group">
                    {#if editingCommentId === reply.id}
                        <textarea bind:value={editingText} class="w-full p-2 border bg-white dark:bg-gray-700 text-black dark:text-white border-gray-300 dark:border-gray-600" rows="2"></textarea>
                        <div class="mt-2 flex justify-end gap-2">
                            <button on:click={handleCancelEdit} class="px-3 py-1 text-xs bg-gray-300 dark:bg-gray-500 hover:bg-gray-400 dark:hover:bg-gray-400">Cancel</button>
                            <button on:click={handleSaveEdit} class="px-3 py-1 text-xs bg-blue-600 text-white hover:bg-blue-700">Save</button>
                        </div>
                    {:else}
                        <div class="flex justify-between items-start">
    ...
    <div class="mt-6">
        ...
        <textarea
            ...
            class="w-full p-2 border bg-white dark:bg-gray-700 text-black dark:text-white border-gray-300 dark:border-gray-600"
            placeholder={replyingToCommentId ? 'Add a reply...' : 'Add a comment...'}
            rows="3"
        ></textarea>
        <div class="mt-2 flex justify-end gap-2">
            <button on:click={handleAddComment} class="px-4 py-2 text-sm font-medium bg-blue-600 text-white hover:bg-blue-700">
