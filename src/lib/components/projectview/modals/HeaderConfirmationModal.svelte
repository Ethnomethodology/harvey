  ...
  function handleConfirm() {
    ...
    fields.forEach((f) => {
      schema[f.name] = {
        ...
        subType: f.subType,
        options:
          f.subType === 'Selectbox' || f.subType === 'Multiselect'
            ? f.options
                .split(',')
                .map((o) => o.trim())
                .filter((o) => o !== '')
      ...
      if (f.subType === 'Currency') {
        schema[f.name].currency =
          f.currency === 'OTHER'
            ? (f.customCurrency || 'XXX').toUpperCase().substring(0, 3)
            : f.currency;
      }
    });
...
<Modal
  ...
  dismissable={false}
  class="w-full p-0 overflow-hidden flex flex-col max-h-[90vh] z-[130]"
>
  ...
  <div class="flex-1 overflow-auto -m-6 p-6">
    {#if step === 1}
      ...
      <div
        class="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden mb-6 bg-white dark:bg-gray-800 shadow-sm"
      >
        <Table hoverable={true} shadow={false}>
          <TableHead class="bg-gray-50 dark:bg-gray-900/50">
            {#each availableFields as f, i (i)}
              <TableHeadCell>
          ...
          <TableBody>
            {#each hasHeaders ? previewData.data.slice(1, 4) : previewData.data.slice(0, 3) as row, rowIndex (rowIndex)}
              <TableBodyRow>
                {#each availableFields as header (header)}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <!-- Option 1: Yes -->
        <div
          class="p-4 border rounded-xl cursor-pointer transition-all {hasHeaders
            ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20'
            : 'border-gray-200 dark:border-gray-700'}"
          on:click={() => (hasHeaders = true)}
        >
          <div class="flex">
            ...
            <div class="ms-2 text-sm select-none">
              <label for="headers-yes" class="font-medium text-heading mb-1"
                >Yes, the first row is the header.</label
              >
              <p class="text-xs font-normal text-body">
                Use values from the first row as field names.
              </p>
            </div>
          </div>
        </div>
        ...
        >
          <div class="flex">
            ...
            <div class="ms-2 text-sm select-none">
              <label for="headers-no" class="font-medium text-heading mb-1"
                >No, treat the first row as data.</label
              >
              <p class="text-xs font-normal text-body">
                Generate generic field names (Field 1, Field 2, etc.).
              </p>
            </div>
          </div>
        </div>
    ...
    {:else}
      <div
        class="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden bg-white dark:bg-gray-800 shadow-sm h-full"
      >
              ...
              <TableBodyRow>
                ...
                <TableBodyCell>
                  <Select
                    ...
                    items={Object.keys(FIELD_TYPES).map((t) => ({ name: t, value: t }))}
                    on:change={() => handleTypeChange(i)}
                  />
                </TableBodyCell>
                <TableBodyCell>
                  <Select
                    size="sm"
                    bind:value={field.subType}
                    items={FIELD_TYPES[field.type].map((st) => ({ name: st, value: st }))}
                    on:change={() => handleSubTypeChange(i)}
                  />
                </TableBodyCell>
                ...
                <TableBodyCell>
                  ...
                  {:else if field.type === 'DateTime'}
                    <Select
                      ...
                      items={(DATETIME_FORMATS[field.subType] || DATETIME_FORMATS['Date']).map(
                        (fmt) => ({ name: fmt, value: fmt })
                      )}
                    />
                  {/if}
                </TableBodyCell>
