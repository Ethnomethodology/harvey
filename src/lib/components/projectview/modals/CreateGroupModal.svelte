<script>
  ...
  async function handleSave() {
    ...
    if (fileToAdd) {
      ...
      console.log(
        '[CreateGroupModal] fileToAdd.relative_path (snake_case):',
        fileToAdd.relative_path
      );
      console.log(
        '[CreateGroupModal] fileToAdd.asset_relative_path:',
        fileToAdd.asset_relative_path
      );
    ...
    try {
      ...
      if (newGroup) {
        // If group creation was successful (backend returns GroupData)
        if (projectUuid) {
          // Ensure projectUuid is valid before updating list
...
>
  ...
  <div class="space-y-4">
    <div>
      <Label for="groupName" class="mb-1 text-sm font-medium text-gray-700 dark:text-gray-300"
