<script>
  ...
  $: if (showModal && nameInputRef) {
    setTimeout(() => {
      // Ensure element is in DOM and rendered
      nameInputRef.focus();
      nameInputRef.select();
    }, 50);
  }
...
>
  <div slot="header" class="flex items-center gap-2">
    ...
    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Rename Group</h3>
  </div>

  <div class="space-y-4">
    <div>
      <Label for="groupRenameName" class="mb-1 text-sm font-medium text-gray-700 dark:text-gray-300"
