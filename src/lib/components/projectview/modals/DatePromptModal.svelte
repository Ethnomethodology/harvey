...
<script>
  ...
  import { Modal, Button, Label, Toggle, Select, Dropdown, Input } from 'flowbite-svelte';
  import { CalendarDays, Clock, Settings2, X, CheckCircle2, Trash2 } from '@lucide/svelte';
  ...
  function formatDate(d, fmt, st, tfmt) {
    ...
    let result = '';
    if (fmt === 'YYYY-MM-DD')
      result = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;
    else if (fmt === 'DD/MM/YYYY')
      result = `${pad(d.getDate())}/${pad(d.getMonth() + 1)}/${d.getFullYear()}`;
    else if (fmt === 'MM/DD/YYYY')
      result = `${pad(d.getMonth() + 1)}/${pad(d.getDate())}/${d.getFullYear()}`;
    else if (fmt === 'MMMM DD, YYYY')
      result = `${months[d.getMonth()]} ${pad(d.getDate())}, ${d.getFullYear()}`;
    else if (fmt === 'MMM DD, YYYY')
      result = `${shortMonths[d.getMonth()]} ${pad(d.getDate())}, ${d.getFullYear()}`;
...
>
  ...
  <div class="space-y-5 py-2">
        ...
        <div class="grid grid-cols-2 gap-3 items-end">
          <div class="space-y-2">
            ...
            <div class="relative">
              ...
              <Dropdown
                triggeredBy="#time_picker_input"
                class="w-48 p-0 z-[10002]"
                strategy="fixed"
              >
                <div
                  class="flex h-48 border dark:border-gray-700 rounded-lg shadow-xl overflow-hidden bg-white dark:bg-gray-800"
                >
                  <div class="flex-1 overflow-y-auto scrollbar-hide">
                    {#each hours as h (h)}
                      <button
                        class="w-full py-1.5 text-xs transition-colors hover:bg-blue-50 dark:hover:bg-blue-900/20 {pad(
                          dateValue.getHours()
                        ) === h
                          ? 'bg-blue-500 text-white font-bold'
                          : ''}"
                        on:click={() => selectTimePart('h', h)}>{h}</button
                      >
                    {/each}
                  </div>
                    ...
                    {#each minutes as m (m)}
                      <button
                        class="w-full py-1.5 text-xs transition-colors hover:bg-blue-50 dark:hover:bg-blue-900/20 {pad(
                          ...
                          ? 'bg-blue-500 text-white font-bold'
                          : ''}"
                        on:click={() => selectTimePart('m', m)}>{m}</button
                      >
                    {/each}
                  </div>
                      ...
                      {#each seconds as s (s)}
                        <button
                          class="w-full py-1.5 text-xs transition-colors hover:bg-blue-50 dark:hover:bg-blue-900/20 {pad(
                            ...
                            ? 'bg-blue-500 text-white font-bold'
                            : ''}"
                          on:click={() => selectTimePart('s', s)}>{s}</button
                        >
                      {/each}
                    </div>
                  {/if}
    ...
    <!-- Preview Result -->
    <div
      class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800/50"
    >
      <div class="flex items-center justify-between mb-2">
        <span class="text-[10px] font-extrabold uppercase tracking-widest text-blue-600/60"
          >Preview</span
        >
