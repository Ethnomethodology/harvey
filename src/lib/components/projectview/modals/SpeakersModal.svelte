...
<script>
  ...
  function updateLocalSecondNames(count, currentSecondNames) {
    ...
    const updated = Array.from({ length: count }, (_, i) => (names[i] != null ? names[i] : ''));
  ...
  function initializeState() {
    if (currentSpeakers) {
      ...
      updateLocalNames(
        renderedCount,
        Array.isArray(currentSpeakers.names) ? [...currentSpeakers.names] : []
      );

      // Initialize second-language names if present
      if (
        Array.isArray(currentSpeakers.translatedNames) &&
        currentSpeakers.translatedNames.some((name) => name && name.trim() !== '')
      ) {
        addSecondNames = true;
  ...
  $: {
    if (hasInitialized) {
      // Prevent running on initial mount before state is set
  ...
  function confirm() {
    ...
    const finalNames = localNames.map((name, i) =>
      name && name.trim() !== '' ? name.trim() : `Speaker ${i + 1}`
    );
    ...
    dispatch('confirm', {
      count: renderedCount,
      names: finalNames,
      translatedNames: finalSecondNames
    });
    closeModal();
  }
...
>
  <div slot="header" class="flex items-center gap-2">
    ...
    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Configure Speakers</h3>
  </div>
    ...
    <div class="space-y-4 pt-2">
      {#if renderedCount > 0}
        <div class="flex justify-between items-center">
          ...
          <Checkbox bind:checked={addSecondNames}>Add names in 2nd language</Checkbox>
        </div>

        <div class="space-y-3 max-h-72 overflow-y-auto pr-2 custom-scrollbar">
          {#each { length: renderedCount } as _, i (i)}
            <div
              class="flex items-center space-x-3 bg-gray-50 dark:bg-gray-800/40 p-3 rounded-lg border border-gray-100 dark:border-gray-800"
            >
      ...
      {:else if listGenerated && renderedCount === 0}
        <div
          class="text-center py-8 bg-gray-50 dark:bg-gray-800/40 rounded-xl border border-dashed border-gray-200 dark:border-gray-700"
        >
      ...
      {:else}
        ...
        >
          <p class="text-sm text-gray-500 dark:text-gray-400">
            Enter speaker count and click "Add" to configure.
          </p>
        </div>
      {/if}
    </div>
  ...
  <svelte:fragment slot="footer">
    <Button color="alternative" on:click={cancel} title="Cancel">Cancel</Button>
    <Button
      color="blue"
      on:click={confirm}
