...
<script>
  ...
  import { Modal, Button, Label, Select, Badge, Alert } from 'flowbite-svelte';
  ...
  $: transcriptOptions = ($transcriptStore.selectedMediaFile?.associated_transcripts || []).map(
    (t) => {
  ...
  function initializeSelections() {
    ...
    console.log(
      '[DualTranscriptModal] Initializing selections. currentPath from store:',
      currentPath
    );
    ...
    if (currentPath && transcriptOptions.length > 0) {
      ...
      if (match) {
        ...
        console.log(
          '[DualTranscriptModal] No match found for currentPath. Defaulting to first option.'
        );
    ...
    if (!secondaryPath || secondaryPath === primaryPath) {
      ...
      const sMatch = sPath
        ? transcriptOptions.find((o) => o.value === sPath || o.relPath === sPath)
        : null;
    ...
    console.log(
      '[DualTranscriptModal] Final selections -> Primary:',
      primaryPath,
      'Secondary:',
      secondaryPath
    );
  ...
  async function updateSegmentCounts(pPath, sPath) {
    ...
    const [pCount, sCount] = await Promise.all([getSegmentCount(pPath), getSegmentCount(sPath)]);
  ...
  async function handleView() {
    if (primaryPath === secondaryPath) {
      message('Please select two different transcripts.', {
  ...
  $: hasMismatch =
    ...
    primarySegmentCount !== secondarySegmentCount;
...
>
  <div slot="header" class="flex items-center gap-2">
    ...
    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Compare Transcripts</h3>
  </div>
  ...
  <div class="space-y-5">
    ...
    >
      ...
      <p class="text-xs text-blue-800 dark:text-blue-300 leading-relaxed">
        Compare two transcripts in interleaved mode. Transcripts must have an identical number of
        segments to be compatible.
      </p>
    </div>
        ...
        <div class="flex justify-between items-center">
          ...
          {#if secondarySegmentCount > 0}
            <Badge color="indigo" size="xs" class="font-mono"
              >{secondarySegmentCount} segments</Badge
            >
          {/if}
        </div>
        <Select
    ...
    {:else if isSame && primaryPath}
      <div class="text-center p-2">
        <p class="text-xs text-orange-500 font-medium italic">
          Please select two different transcripts.
        </p>
      </div>
    {/if}
  </div>
