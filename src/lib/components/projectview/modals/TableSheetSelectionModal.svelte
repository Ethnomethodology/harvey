...
<Modal
  title={`Import Sheets from ${filename}`}
  bind:open={showModal}
  size="md"
  outsideclose={false}
  dismissable={false}
...
>
  ...
  <div class="space-y-4">
    <p class="text-sm text-gray-500 dark:text-gray-400">
      The file <strong>{filename}</strong> contains multiple sheets. Please select which sheets you would
      like to import. Each selected sheet will be imported as a separate table.
    </p>

    <div class="flex items-center mb-2">
      <Checkbox
        checked={selectedSheets.length === sheets.length && sheets.length > 0}
        indeterminate={selectedSheets.length > 0 && selectedSheets.length < sheets.length}
        on:change={toggleAll}
      >
    ...
    <div
      class="border rounded-lg max-h-60 overflow-y-auto p-4 space-y-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700"
    >
      {#each sheets as sheet (sheet)}
        <div class="flex">
          ...
          <div class="ms-2 text-sm select-none">
            ...
            <p id={`helper-text-${sheet}`} class="text-xs font-normal text-body">
              Will be imported as a separate table
            </p>
          </div>
        </div>
      {/each}
  ...
  <svelte:fragment slot="footer">
    ...
    <Button color="blue" on:click={handleConfirm} disabled={selectedSheets.length === 0}
      >Import {selectedSheets.length} {selectedSheets.length === 1 ? 'Sheet' : 'Sheets'}</Button
    >
  </svelte:fragment>
</Modal>
