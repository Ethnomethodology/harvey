...
<script>
  ...
  import { Modal, Button, Label, Select, Helper } from 'flowbite-svelte';
  ...
  $: if (availableTranscripts.length > 0) {
    ...
    const currentSelectionIsValid = availableTranscripts.some(
      (t) => t.relativePath === selectedTranscript
    );
  ...
  function handleConfirm() {
    const selectedTranscriptObject = availableTranscripts.find(
      (t) => t.relativePath === selectedTranscript
    );
