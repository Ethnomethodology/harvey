...
<script>
  ...
  import { Modal, Input, Label, Select, Button, Helper, Checkbox } from 'flowbite-svelte';
  ...
  async function initializeModalState() {
    // Keep async in case other async ops are added later
    ...
    selectedDocxLayout =
      get(activeLayout) ||
      (DOCX_LAYOUT_OPTIONS.length > 0 ? DOCX_LAYOUT_OPTIONS[0].rustLayoutKey : 'Layout1');
  }
  ...
  function handleConfirm() {
    ...
    dispatch('confirm', {
      ...
      format: exportFormat,
      layoutChoice:
        exportFormat === 'docx' || exportFormat === 'md' ? selectedDocxLayout : undefined,
      excludeSpeakerNames:
        exportFormat === 'srt' || exportFormat === 'vtt' || exportFormat === 'ass'
          ? excludeSpeakerNames
          : false
    });
    closeModal();
...
>
  ...
  <div class="p-6 space-y-5 overflow-y-auto max-h-[70vh] custom-scrollbar">
    ...
    <div class="space-y-2">
      ...
      <Select id="export-format" items={exportFormats} bind:value={exportFormat} />
      {#if exportFormat === 'md'}
        <Helper class="italic"
          >Markdown export supports basic styling (bold, italic). Other rich text formatting will be
          converted to plain text.</Helper
        >
      {:else if exportFormat === 'ass'}
        <Helper class="italic"
          >ASS export provides standard subtitles with styling support (bold, italic, underline,
          strikethrough, color).</Helper
        >
    ...
    {#if exportFormat === 'srt' || exportFormat === 'vtt' || exportFormat === 'ass'}
      <div class="pt-1">
        <Checkbox bind:checked={excludeSpeakerNames}>Exclude speaker names from subtitles</Checkbox>
      </div>
    {/if}
    ...
    {#if exportFormat === 'docx' || exportFormat === 'md'}
      <div class="pt-2 space-y-3">
        <Label id="layout-label" class="font-semibold text-sm"
          >{exportFormat.toUpperCase()} Layout</Label
        >
        <div
        ...
        >
          {#each DOCX_LAYOUT_OPTIONS as layout (layout.id)}
            <button
              type="button"
              class="text-left p-3 border rounded-xl transition-all relative {selectedDocxLayout ===
            ...
            >
              <div
                class="font-bold text-[11px] mb-2 {selectedDocxLayout === layout.rustLayoutKey
              ...
              <div
                class="{layout.previewClasses} min-h-[20px] opacity-80 rounded overflow-hidden border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-800"
              >
                {#each layout.columnStyles as style (style.content)}
                  <div
                    class="{style.class} !p-1 !text-[9px] leading-tight flex items-center justify-center text-center"
                  >
                    {style.content}
                  </div>
                {/each}
