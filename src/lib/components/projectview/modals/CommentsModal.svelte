<script lang="ts">
  ...
  import { Modal, Button, Textarea, Dropdown, DropdownItem, Avatar } from 'flowbite-svelte';
...
>
  ...
  <div class="space-y-4 max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar">
    {#each comments.filter((c) => !c.parentId) as comment (comment.id)}
      <div class="space-y-3">
        <div
          class="p-4 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 relative group"
        >
        ...
        {#each comments.filter((r) => r.parentId === comment.id) as reply (reply.id)}
          <div
            class="ml-8 p-4 rounded-xl bg-gray-100/50 dark:bg-gray-800/30 border border-gray-200/50 dark:border-gray-700/50 relative group"
          >
  ...
  <div class="mt-4 pt-4 border-t dark:border-gray-700">
    {#if replyingToCommentId}
      <div
        class="flex items-center justify-between mb-2 px-2 py-1 bg-blue-50 dark:bg-blue-900/20 rounded-md"
      >
  ...
  <svelte:fragment slot="footer">
    ...
    <Button color="blue" on:click={handleAddComment} disabled={!newCommentText.trim()}>
