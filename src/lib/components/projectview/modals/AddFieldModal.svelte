...
<script>
  ...
  import { Modal, Button, Label, Input, Select, Radio, Helper, Alert } from 'flowbite-svelte';
  import {
  ...
  async function handleAdd() {
    ...
    if (!finalFieldKey) {
      await message(
        'Field Key cannot be generated from the Field Name. Please ensure it contains alphanumeric characters.',
        { title: 'Validation Error', type: 'error' }
      );
      return;
    }
    if (
      !/^[a-z0-9_]+$/.test(finalFieldKey) ||
      finalFieldKey.startsWith('_') ||
      finalFieldKey.endsWith('_')
    ) {
      await message(
        'Generated Field Key is invalid (must be alphanumeric with underscores, not starting/ending with underscore). Please adjust Field Name.',
        { title: 'Validation Error', type: 'error' }
      );
      return;
    }
...
>
  ...
  <div class="p-6 space-y-6">
    ...
    <div class="space-y-2">
      ...
      <Input
        ...
        autocomplete="off"
      />
      <Helper class="text-xs text-gray-500">Field names should be unique and descriptive.</Helper>
    </div>

    <div
      class="space-y-4 p-4 bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 rounded-lg"
    >
          ...
          <div class="flex items-center h-5">
            <Radio bind:group={selectedScope} value="project" id="scopeProject" name="scope">
              ...
              <p class="text-xs font-normal text-gray-500 dark:text-gray-400">
                Available for all project items, including documents, media, and tables.
              </p>
            </Radio>
          </div>
        </div>
          ...
          <div class="flex items-center h-5">
            ...
            >
              <span class="font-medium text-gray-900 dark:text-white"
                >Type-specific ({displayType})</span
              >
              <div class="text-xs font-normal text-gray-500 dark:text-gray-400">
                Only applicable to {displayType}.
                {#if !currentItemType || currentItemType === 'project'}
                  <span class="text-amber-600 dark:text-amber-400 block font-medium mt-0.5"
                    >Select an asset to enable this scope.</span
                  >
                {/if}
              </div>
            </Radio>
    ...
    <div class="space-y-2">
      ...
      <Select id="fieldTypeSelect" items={fieldTypeOptions} bind:value={fieldType} />
    </div>
  </div>
  ...
  <div slot="footer" class="flex justify-end gap-3 w-full">
    ...
    <Button color="blue" on:click={handleAdd} title="Create the new custom field definition">
