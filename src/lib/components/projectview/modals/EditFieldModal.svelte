...
<script>
  ...
  let isOtherCurrency =
    editedSchema.subType === 'Currency' &&
    !currencyOptions.find((o) => o.value === editedSchema.currency && o.value !== 'OTHER');
  ...
  const types = ['Text', 'Numeric', 'DateTime', 'Contact', 'Misc'].map((t) => ({
    name: t,
    value: t
  }));
  const subTypes = {
  ...
  function handleSave() {
    ...
    if (finalSchema.subType === 'Selectbox' || finalSchema.subType === 'Multiselect') {
      finalSchema.options = optionsText
        .split(',')
        .map((o) => o.trim())
        .filter((o) => o !== '');
    } else {
    ...
    if (finalSchema.subType === 'Currency') {
      finalSchema.currency =
        selectedCurrency === 'OTHER'
          ? customCurrencyCode.toUpperCase().substring(0, 3)
          : selectedCurrency;
    }
  ...
  $: isPrimaryDisabled =
    currentPrimaryField && currentPrimaryField !== fieldName && !editedSchema.primary;
</script>
...
>
  <div slot="header" class="flex items-center gap-2">
    <svelte:component
      this={getIcon(editedSchema.type, editedSchema.subType)}
      class="w-5 h-5 text-blue-500"
    />
    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit Field Settings</h3>
  </div>

  <div class="space-y-5 overflow-y-auto max-h-[65vh] custom-scrollbar pr-2">
    <!-- Field Name -->
    <div class="space-y-1">
      <Label for="field-name" class="text-xs font-bold uppercase tracking-wider text-gray-500"
        >Field Name</Label
      >
      <Input
        id="field-name"
    ...
    <div class="grid grid-cols-2 gap-4">
      <!-- Type Selection -->
      <div class="space-y-1">
        <Label for="field-type" class="text-xs font-bold uppercase tracking-wider text-gray-500"
          >Data Type</Label
        >
        <Select
          id="field-type"
      ...
      <!-- SubType Selection -->
      <div class="space-y-1">
        <Label for="field-subtype" class="text-xs font-bold uppercase tracking-wider text-gray-500"
          >Sub-Type</Label
        >
        <Select
          id="field-subtype"
    ...
    <div class="grid grid-cols-2 gap-4">
      <!-- Primary Toggle -->
      <div
        class="flex items-center space-x-3 bg-gray-50 dark:bg-gray-800/30 p-3 rounded-xl border border-gray-100 dark:border-gray-800"
    ...
    <div class="space-y-4 pt-2">
      {#if editedSchema.type === 'Numeric'}
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-1">
            <Label for="field-min" class="text-xs font-bold uppercase tracking-wider text-gray-500"
              >{editedSchema.subType === 'Rating' ? 'Min Stars' : 'Min Value'}</Label
            >
            <Input
              id="field-min"
              type="number"
          ...
          <div class="space-y-1">
            <Label for="field-max" class="text-xs font-bold uppercase tracking-wider text-gray-500"
              >{editedSchema.subType === 'Rating' ? 'Max Stars' : 'Max Value'}</Label
            >
            <Input
              id="field-max"
              type="number"
        ...
        {#if editedSchema.subType === 'Currency'}
          <div class="space-y-1">
            ...
            >
            <div class="space-y-2">
              <Select id="field-currency" items={currencyOptions} bind:value={selectedCurrency} />
      ...
      {:else if editedSchema.type === 'DateTime'}
        <div class="space-y-1">
          <Label for="field-format" class="text-xs font-bold uppercase tracking-wider text-gray-500"
      ...
      <!-- Description -->
      <div class="space-y-1">
        <Label for="field-desc" class="text-xs font-bold uppercase tracking-wider text-gray-500"
          >Description</Label
        >
        <Textarea
          id="field-desc"
  ...
  <svelte:fragment slot="footer">
    ...
    <Button color="blue" on:click={handleSave} title="Save field settings">Save Settings</Button>
  </svelte:fragment>
</Modal>
