...
<script>
  ...
  function incrementDuration() {
    ...
    if (mediaDuration > 0) {
      manualSegmentCount = Math.min(
        100,
        Math.max(1, Math.round(mediaDuration / manualSegmentDuration))
      );
    }
  }
  ...
  $: manualSpeakerOptions = [
    { value: 'unassigned', label: 'Unassigned' },
    {
      ...
      disabled: (modalSpeakersConfig?.names?.length || 0) < 2
    }
  ];
  ...
  $: if (showModal && !isTranscribing && jobStatus === null && !isInitialized) {
    ...
    if (
      ...
      (!modalSelectedModel || !downloadedModelsList.some((m) => m.name === modalSelectedModel))
    ) {
    ...
    modalSpeakersConfig = JSON.parse(
      JSON.stringify(speakers || { count: 0, names: [], translatedNames: [] })
    );
...
{#if showModal}
  ...
  >
    ...
    >
      <!-- Header -->
      <div
        class="px-6 py-5 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800/50"
      >
      ...
      <div class="p-6 overflow-y-auto max-h-[70vh]">
        {#if !isTranscribing && jobStatus === null}
          ...
          <div class="space-y-4">
            <div
              class="bg-gray-50 dark:bg-gray-800/40 p-3 rounded-lg border border-gray-100 dark:border-gray-800"
            >
              <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Target File</p>
              <p class="text-sm font-mono text-gray-900 dark:text-gray-200 break-all">
                {fileName || 'N/A'}
              </p>
            </div>
              ...
              >
                <li class="me-2">
                  <button
                    ...
                    class="inline-flex items-center justify-center p-4 border-b-2 rounded-t-lg group transition-all {modalTab ===
                    'automatic'
            ...
            <div class="mt-6">
              {#if modalTab === 'automatic'}
                <div class="space-y-4">
                  {#if hasCriticalConfigIssues}
                    <div
                      class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-4 rounded-xl text-center space-y-3"
                    >
                  ...
                  {:else if downloadedModelsList.length === 0}
                    <div
                      class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 p-4 rounded-xl text-center space-y-3"
                    >
                  ...
                  {:else}
                    <div class="space-y-2">
                      <div class="flex items-center justify-between">
                        <Label for="modalModelSelect" color={!modalSelectedModel ? 'red' : 'gray'}
                          >Transcription Model</Label
                        >
                        {#if modalSelectedModel}
                          {@const selectedModelObj = downloadedModelsList.find(
                            (m) => m.name === modalSelectedModel
                          )}
                          {#if selectedModelObj?.info_url}
                            <button
                              class="text-[10px] text-blue-600 dark:text-blue-400 hover:underline flex items-center space-x-1"
                              on:click|stopPropagation={() =>
                                openExternal(selectedModelObj.info_url)}
                      ...
                      <Select
                        id="modalModelSelect"
                        items={downloadedModelsList.map((m) => ({
                          value: m.name,
                          name: `${m.name} (${m.family || 'whisper.cpp'})`
                        }))}
                      ...
                      {#if !modalSelectedModel}
                        <div
                          class="flex items-center space-x-2 text-red-600 dark:text-red-400 mt-1 animate-pulse"
                        >
                          <AlertTriangle size={14} />
                          <Helper color="red" class="italic text-[11px] font-medium"
                            >Please select a model from the list above to proceed.</Helper
                          >
                        </div>
                      {/if}
                      {#if modalSelectedModel}
                    ...
                    <div class="pt-4 border-t border-gray-100 dark:border-gray-800 space-y-4">
                      <div
                        class="flex justify-between items-center bg-gray-50 dark:bg-gray-800/40 p-3 rounded-lg border border-gray-100 dark:border-gray-800"
                      >
                        <div>
                          ...
                          <p class="text-sm font-bold text-gray-900 dark:text-white">
                            {modalSpeakersConfig?.count > 0 ? modalSpeakersConfig.count : '0'}
                          </p>
                        </div>
                        <Button
                          color="alternative"
                      ...
                      {#if $configStatus.diarization_model_downloaded}
                        <div class="flex items-start space-x-3 p-1">
                          ...
                          <div class="space-y-1 w-full">
                            <Label for="diarize-check" class="cursor-pointer"
                              >Identify different speakers (diarize)</Label
                            >
                            {#if modalEnableDiarization}
                              <Alert
                                color="yellow"
                                class="mt-2 py-2 px-3 text-[11px] border border-yellow-200 dark:border-yellow-900/50 bg-yellow-50/50 dark:bg-yellow-900/20"
                                rounded={false}
                              >
                                <div class="flex items-center gap-2">
                                  <AlertTriangle
                                    size={14}
                                    class="flex-shrink-0 text-yellow-600 dark:text-yellow-400"
                                  />
                                  <span class="text-yellow-800 dark:text-yellow-300"
                                    >Note: This significantly increases processing time.</span
                                  >
                                </div>
                              </Alert>
                            {/if}
                      ...
                      {:else}
                        <div
                          class="bg-gray-50 dark:bg-gray-800/40 p-3 rounded-lg border border-dashed border-gray-200 dark:border-gray-700 flex justify-between items-center"
                        >
                          <div class="space-y-1">
                            ...
                            <p class="text-[10px] text-gray-400 italic">
                              Download diarization model to enable.
                            </p>
                          </div>
                          <Button
                            color="alternative"
                    ...
                    <div class="pt-2">
                      <Button
                        ...
                        class="w-full"
                        size="sm"
                        on:click={() => (showAdditionalParamsModal = true)}
              ...
              {:else}
                <div class="space-y-4">
                  ...
                  <div
                    class="bg-gray-50 dark:bg-gray-800/40 p-3 rounded-lg border border-gray-100 dark:border-gray-800 text-xs space-y-2"
                  >
                    <div class="flex justify-between">
                      <span class="text-gray-500">Media Duration:</span>
                      <span class="font-medium text-gray-900 dark:text-white"
                        >{formatDuration(mediaDuration)}</span
                      >
                    </div>
                    <div class="flex justify-between items-center">
                      <span class="text-gray-500">Total Segmented:</span>
                      <span
                        class="text-sm font-bold {isManualDurationValid
                  ...
                  <div class="space-y-3 pt-2">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                      {#each manualSpeakerOptions as option (option.value)}
                        <Radio
                    ...
                    {#if modalSpeakersConfig.names.length < 2}
                      <Helper class="italic text-[11px]"
                        >Note: 'Alternate Speakers' requires at least 2 speakers.</Helper
                      >
                    {/if}
                  </div>

                  <div
                    class="flex justify-between items-center bg-gray-50 dark:bg-gray-800/40 p-3 rounded-lg border border-gray-100 dark:border-gray-800 mt-2"
                  >
                    <div>
                    ...
                    <button
                      ...
                      on:click={() => (showNestedSpeakersModal = true)}
                      title="Edit Speakers"
                      class="text-xs flex items-center px-2.5 py-1.5 font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
                    >
                      <UserPen size={14} class="mr-1.5" />
                      Edit
                    </button>
        ...
        {:else if isTranscribing && (jobStatus === 'running' || jobStatus === 'initiating')}
          ...
          <div class="flex flex-col items-center py-8 space-y-6 text-center">
            ...
            <div class="space-y-2 w-full px-4">
              ...
              <p
                class="text-sm text-gray-500 dark:text-gray-400 h-10 flex items-center justify-center"
              >
              ...
              {#if transcriptionOutputFileName}
                <div class="flex flex-col items-center">
                  <p
                    ...
                    title={transcriptionOutputFileName}
                  >
      ...
      <!-- Footer -->
      <div
        class="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex justify-between gap-3 bg-gray-50/80 dark:bg-gray-800/80 backdrop-blur-md"
      >
        {#if !isTranscribing && jobStatus === null}
          <button
            on:click={() => (showManageModelsModal = true)}
          ...
          <div class="flex gap-3">
            ...
            <Button
              ...
              disabled={(modalTab === 'automatic' &&
                (!modalSelectedModel || !modalSelectedLanguage)) ||
...
{#if showAdditionalParamsModal}
  <AdditionalParametersModal
    ...
    currentEngine={downloadedModelsList.find((m) => m.name === modalSelectedModel)?.family ||
      'whisper-cpp'}
    initialPrompt={modalInitialPrompt}
    hotwords={modalHotwords}
    on:confirm={async (e) => {
