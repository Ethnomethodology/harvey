...
<script>
  ...
  import { Modal, Input, Label, Select, Button, Helper, Badge } from 'flowbite-svelte';
      ...
      if (getExportData) {
        ...
        if (formattedDataObj) {
          ...
          if (exportFormat === 'xlsx') {
            await invoke('export_formatted_table_to_xlsx', {
              data,
              headers,
              styles,
            ...
            await invoke('export_formatted_table_to_csv', {
              data,
              headers,
              outputPathStr: fullExportPath
            });
          ...
          if (exportFormat === 'xlsx') {
            await invoke('export_table_to_xlsx', {
              tablePathStr: tablePath,
              outputPathStr: fullExportPath
            });
          } else if (exportFormat === 'csv') {
            await invoke('export_table_to_csv', {
              tablePathStr: tablePath,
              outputPathStr: fullExportPath
            });
          }
...
>
  ...
  <div class="space-y-5">
    ...
    <div class="space-y-2">
      ...
      <Select id="table-export-format" items={exportFormats} bind:value={exportFormat} />
      <Helper class="italic">
        {#if exportFormat === 'xlsx'}
  ...
  <svelte:fragment slot="footer">
    <Button color="alternative" on:click={closeModal} disabled={isExporting}>Cancel</Button>
    <Button
      color="blue"
