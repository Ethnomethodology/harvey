...
>
  ...
  <div class="space-y-3">
    <p class="text-sm text-gray-700 dark:text-gray-300">
      You have unsaved changes in the {itemType} "<span
        class="font-semibold text-gray-900 dark:text-white">{itemName}</span
      >".
    </p>
  ...
  <svelte:fragment slot="footer">
    <Button color="alternative" on:click={handleCancel} title="Cancel and stay">Cancel</Button>
    <Button color="red" outline on:click={handleDiscard} title="Discard all changes">
      Discard
    </Button>
    <Button color="blue" on:click={handleSave} title="Save changes and close">Save</Button>
  </svelte:fragment>
</Modal>
