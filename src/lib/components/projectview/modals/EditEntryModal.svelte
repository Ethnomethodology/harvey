...
<script>
  ...
  for (const field in schema) {
    if (
      schema[field].type === 'Misc' &&
      schema[field].subType === 'Multiselect' &&
      typeof editedData[field] === 'string'
    ) {
      editedData[field] = editedData[field]
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
    }
  ...
  function validateField(field, value) {
    ...
    if (!isBlank) {
      if (type === 'Numeric') {
        ...
        if (colSchema.min !== null && colSchema.min !== undefined && num < colSchema.min)
          return `Value must be at least ${colSchema.min}`;
        if (colSchema.max !== null && colSchema.max !== undefined && num > colSchema.max)
          return `Value must be at most ${colSchema.max}`;
        ...
        if (subType === 'Time') {
          if (colSchema.format === 'HH:mm' && !/^([01]\d|2[0-3]):([0-5]\d)$/.test(value))
            return 'Invalid time format (HH:mm)';
          if (
            colSchema.format === 'HH:mm:ss' &&
            !/^([01]\d|2[0-3]):([0-5]\d):([0-5]\d)$/.test(value)
          )
            return 'Invalid time format (HH:mm:ss)';
          if (
            colSchema.format === 'hh:mm A' &&
            !/^(0[1-9]|1[0-2]):([0-5]\d)\s?(AM|PM)$/i.test(value)
          )
          ...
          if (
            colSchema.format === 'DD/MM/YYYY' &&
            !/^(0[1-9]|[12]\d|3[01])\/(0[1-9]|1[0-2])\/\d{4}$/.test(value)
          )
            return 'Invalid date format (DD/MM/YYYY)';
          if (
            colSchema.format === 'MM/DD/YYYY' &&
            !/^(0[1-9]|1[0-2])\/(0[1-9]|[12]\d|3[01])\/\d{4}$/.test(value)
          )
            return 'Invalid date format (MM/DD/YYYY)';
          if (colSchema.format === 'YYYY' && !/^\d{4}$/.test(value))
            return 'Invalid year format (YYYY)';
          if (
            colSchema.format === 'MMMM' &&
            ![
              'january',
              'february',
              'march',
        ...
        if (subType === 'Selectbox' && Array.isArray(colSchema.options)) {
          if (value && !colSchema.options.includes(value))
            return `Value must be one of: ${colSchema.options.join(', ')}`;
  ...
  function handleDateTimeChange(field, type, val) {
    ...
    if (type === 'date') {
      const dateParts = parseDate(val, {
        ...colSchema,
        subType: 'Date',
        format: colSchema.format?.split(/[T ]/)[0] || 'YYYY-MM-DD'
      });
      if (dateParts) {
        currentDateObj.setFullYear(
      ...
      const timeParts = parseDate(val, {
        ...colSchema,
        subType: 'Time',
        format: colSchema.format?.split(/[T ]/).slice(1).join(' ') || 'HH:mm'
      });
      if (timeParts) {
        currentDateObj.setHours(
          timeParts.getHours(),
          timeParts.getMinutes(),
          timeParts.getSeconds()
        );
      }
    }
  ...
  function flowbiteDatepicker(node, { field, isDateTime = false }) {
    ...
    const datePartFormat = (
      isDateTime ? format.split(/[T ]/)[0] || 'YYYY-MM-DD' : format || 'YYYY-MM-DD'
    ).toLowerCase();
    ...
    const handleChange = (e) => {
      ...
      if (d instanceof Date && !isNaN(d)) {
        dateStr = formatDate(d, {
          ...colSchema,
          subType: 'Date',
          format: isDateTime ? format.split(/[T ]/)[0] : format
        });
      }
      if (isDateTime) {
    ...
    const handleOutsideClick = (event) => {
      ...
      if (event.target instanceof Element) {
        isClickInsidePicker =
          event.target.closest('.datepicker-dropdown') || event.target.closest('.datepicker');
      }
  ...
  function selectTimePart(field, part, val, isDateTime = false) {
    const colSchema = schema[field] || {};
    const format = colSchema.format || '';
    const timePartFormat = isDateTime
      ? format.split(/[T ]/).slice(1).join(' ') || 'HH:mm'
      : format || 'HH:mm';

    let currentTimeStr = isDateTime
      ? formatDate(parseDate(editedData[field], colSchema) || new Date(), {
          ...colSchema,
          subType: 'Time',
          format: timePartFormat
        })
      : editedData[field];
    let d =
      parseDate(currentTimeStr, { ...colSchema, subType: 'Time', format: timePartFormat }) ||
      new Date();
...
>
  ...
  <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar max-h-[70vh]">
    {#each columns as col (col.field || col.id || serializeId(col.field))}
      {#if col.field && col.field !== 'harvey_internal_id'}
        {@const colSchema = schema[col.field] || {}}
        <div class="group space-y-2">
          <Label
            class="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-gray-500"
          >
          ...
          {#if colSchema.type === 'DateTime'}
            <div class="space-y-3">
              {#if colSchema.subType === 'Date'}
                <div class="relative">
                  ...
                  <input
                    ...
                    class="cursor-pointer bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 {errors[
                      col.field
                    ]
              ...
              {:else if colSchema.subType === 'Time'}
                ...
                <div class="relative max-w-[12rem]">
                  ...
                  <input
                    ...
                    placeholder="00:00"
                    class="cursor-pointer bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 {errors[
                      col.field
                    ]
                      ? 'border-red-500'
                      : ''}"
                  ...
                  />
                  <Dropdown
                    triggeredBy="#time_input_{sanitizeId(col.field)}"
                    class="{hasSeconds ? 'w-36' : 'w-24'} p-0 z-[10002]"
                    strategy="fixed"
                  >
                    <div
                      class="flex h-64 border dark:border-gray-700 rounded-lg shadow-2xl overflow-hidden"
                    >
                      <div
                        class="flex-1 overflow-y-auto custom-scrollbar bg-gray-50 dark:bg-gray-800"
                      >
                        {#each hours as h (h)}
                          <button
                            class="w-full py-2 text-sm transition-colors hover:bg-blue-100 dark:hover:bg-blue-900/30 {curH ===
                            h
                              ? 'bg-blue-500 text-white font-bold'
                              : ''}"
                            on:click={() => selectTimePart(col.field, 'h', h)}>{h}</button
                          >
                        {/each}
                      </div>
                      ...
                      >
                        {#each minutes as m (m)}
                          <button
                            class="w-full py-2 text-sm transition-colors hover:bg-blue-100 dark:hover:bg-blue-900/30 {curM ===
                            m
                              ? 'bg-blue-500 text-white font-bold'
                              : ''}"
                            on:click={() => selectTimePart(col.field, 'm', m)}>{m}</button
                          >
                        {/each}
                      </div>
                      {#if hasSeconds}
                        ...
                        >
                          {#each seconds as s (s)}
                            <button
                              class="w-full py-2 text-sm transition-colors hover:bg-blue-100 dark:hover:bg-blue-900/30 {curS ===
                              s
                                ? 'bg-blue-500 text-white font-bold'
                                : ''}"
                              on:click={() => selectTimePart(col.field, 's', s)}>{s}</button
                            >
                          {/each}
                        </div>
                      {/if}
              ...
              {:else}
                ...
                {@const curS = currentD.getSeconds().toString().padStart(2, '0')}
                <div
                  class="grid grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700"
                >
                  <div class="space-y-1.5">
                    <Label
                      class="text-[10px] font-extrabold uppercase tracking-widest text-gray-400"
                      >Date</Label
                    >
                    <div class="relative">
                      <div
                      ...
                      <input
                        ...
                        spellcheck="false"
                        value={formatDate(
                          parseDate(editedData[col.field], colSchema) || new Date(),
                          {
                            ...colSchema,
                            subType: 'Date',
                            format: (colSchema.format || '').split(/[T ]/)[0]
                          }
                        )}
                  ...
                  <div class="space-y-1.5">
                    ...
                    <div class="relative">
                      ...
                      <input
                        ...
                        value={formatDate(
                          parseDate(editedData[col.field], colSchema) || new Date(),
                          {
                            ...colSchema,
                            subType: 'Time',
                            format: (colSchema.format || '').split(/[T ]/).slice(1).join(' ')
                          }
                        )}
                      ...
                      />
                      <Dropdown
                        triggeredBy="#dt_time_input_{sanitizeId(col.field)}"
                        class="{hasSeconds ? 'w-36' : 'w-24'} p-0 z-[10002]"
                        strategy="fixed"
                      >
                        <div
                            ...
                            {#each hours as h (h)}
                              <button
                                ...
                                h
                                  ? 'bg-blue-500 text-white font-bold'
                                  : ''}"
                                on:click={() => selectTimePart(col.field, 'h', h, true)}>{h}</button
                              >
                            {/each}
                          </div>
                          <div class="w-px bg-gray-200 dark:border-gray-700"></div>
                            ...
                            {#each minutes as m (m)}
                              <button
                                ...
                                m
                                  ? 'bg-blue-500 text-white font-bold'
                                  : ''}"
                                on:click={() => selectTimePart(col.field, 'm', m, true)}>{m}</button
                              >
                            {/each}
                          </div>
                          {#if hasSeconds}
          ...
          {:else if colSchema.type === 'Misc'}
            ...
            {:else if colSchema.subType === 'Multiselect'}
              <MultiSelect
                items={(colSchema.options || []).map((o) => ({ name: o, value: o }))}
                bind:value={editedData[col.field]}
            ...
            {:else if colSchema.subType === 'Selectbox'}
              <Select
                items={[
                  { name: '-- None --', value: '' },
                  ...(colSchema.options || []).map((o) => ({ name: o, value: o }))
                ]}
                bind:value={editedData[col.field]}
            ...
            {:else if colSchema.subType === 'Project Link'}
              <Select
                items={[
                  { name: '-- None --', value: '' },
                  ...projectAssets.map((a) => ({ name: a.label, value: a.value }))
                ]}
                bind:value={editedData[col.field]}
          ...
          {:else if colSchema.type === 'Numeric'}
              ...
              <div class="flex items-center gap-3 h-10 relative">
                <div class="relative flex-grow h-full flex items-center group">
                  ...
                  >
                    {val}
                    <div
                      class="absolute w-2 h-2 bg-gray-900 dark:bg-gray-700 rotate-45 -bottom-1 left-1/2 -translate-x-1/2"
                    ></div>
                  </div>
                </div>
                <span
            ...
            {:else if colSchema.subType === 'Rating'}
              <div class="flex items-center gap-1 h-10">
                {#each Array(colSchema.max || 5) as _, i (i)}
                  <button
                    type="button"
                  ...
                  >
                    <Star
                      class="w-6 h-6 {(editedData[col.field] || 0) > i
                ...
                {#if colSchema.subType === 'Currency'}
                  ...
                  >
                    <span class="text-gray-500 dark:text-gray-400 font-bold"
                      >{getCurrencySymbol(colSchema.currency)}</span
                    >
                  </div>
                {/if}
                <input
                  ...
                  id="field-{col.field}"
                  bind:value={editedData[col.field]}
                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 {colSchema.subType ===
                  'Currency'
                    ? 'ps-10'
                    : ''} {colSchema.subType === 'Percent' ? 'pe-10' : ''} {errors[col.field]
                    ? 'border-red-500'
                    : ''}"
                />
          ...
          {:else if colSchema.type === 'Contact'}
            <Input
              type={colSchema.subType === 'Email'
                ? 'email'
                : colSchema.subType === 'Phone'
                  ? 'tel'
                  : 'url'}
              id="field-{col.field}"
              bind:value={editedData[col.field]}
  ...
  <svelte:fragment slot="footer">
    ...
    <Button color="blue" on:click={handleSave} title="Save entry changes">Save Changes</Button>
  </svelte:fragment>
</Modal>
