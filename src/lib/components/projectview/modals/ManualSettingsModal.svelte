...
<script>
  ...
  import { Modal, Button, Label, Input, Radio, Helper } from 'flowbite-svelte';
  ...
  <div class="p-6 space-y-5">
    ...
    <div class="space-y-2">
      ...
      <div class="flex items-center gap-3">
        ...
        <div
          class="bg-gray-100 dark:bg-gray-800 px-3 py-2 rounded-lg font-mono text-xs text-gray-600 dark:text-gray-400 min-w-[4.5rem] text-center border border-gray-200 dark:border-gray-700"
        >
    ...
    <div class="space-y-3 pt-2">
      ...
      <div class="grid grid-cols-1 gap-2">
        {#each speakerOptions as option (option.value)}
          <Radio
            name="speakerMode"
            value={option.value}
      ...
      {#if speakerMode === 'alternate' && speakerList.length < 2}
        <Helper color="yellow" class="italic text-[11px]"
          >Add at least 2 speakers to enable alternation.</Helper
        >
      {/if}
    </div>
  </div>
