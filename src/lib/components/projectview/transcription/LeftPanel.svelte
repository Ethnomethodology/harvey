...
<script>
  ...
  $: uniqueProjectFileTree = (() => {
    ...
    const filtered = projectFileTree.filter((node) => {
      ...
      const isMediaFileOrDirectory =
        ...
        node.file_type === 'audio_transcript' ||
        node.file_type === 'video_transcript';
      const isWithinAudioPath =
        normalizedNodePath && normalizedNodePath.startsWith(audioPathPrefix);
      const isWithinVideoPath =
        normalizedNodePath && normalizedNodePath.startsWith(videoPathPrefix);
      ...
      const isWithinLegacyMediaPath =
        normalizedNodePath && normalizedNodePath.startsWith(mediaPathPrefix);
  ...
  async function handleItemClick(event) {
    ...
    if (item.is_directory) {
      ...
      if (mediaNode) {
        console.log('[LeftPanel] Directory clicked, resolved media node:', mediaNode);
        dispatch('requestLoadItem', mediaNode);
      } else {
        console.log('[LeftPanel] Directory clicked, but no media file found inside.');
      }
      return;
    }
  ...
  function handleContextMenu(event) {
    ...
    if (item.is_directory && item.file_type !== 'directory_media_stem') return; // Only allow on files or media stem folders
  ...
  async function handleMenuAction(action) {
    ...
    switch (action) {
      case 'Load':
        const mediaNode =
          item.file_type === 'directory_media_stem' ? findMediaFileInStem(item) : item;
        if (mediaNode && mediaNode.file_type === 'media') selectMedia(mediaNode);
        else console.warn("[LeftPanel] 'Load' action failed. No media file resolved for:", item);
        break;
      case 'OpenData':
      ...
      case 'Rename':
        ...
        if (renameTarget) {
          ...
          console.warn('[LeftPanel] Rename failed: could not resolve media file for:', item);
        }
        break;
      case 'Delete': {
        let confirmMsg = '';

        if (item.file_type === 'media' || item.file_type === 'directory_media_stem') {
          const stemName =
        ...
        try {
          ...
          await message(`An error occurred during the deletion process: ${e.message || e}`, {
            title: 'Delete Error',
            type: 'error'
          });
        }
  ...
  async function handleRenameConfirm(event) {
    ...
    if (!item || !newName || newName.trim() === '') {
      console.error('[LeftPanel] Rename confirmation failed: Missing item or new name.');
    ...
    if (item.file_type === 'media') {
      ...
      const primaryTranscriptName = mediaStem ? `${mediaStem}.json` : null;
  ...
  {#if filesOpen}
    ...
    >
      ...
      {:else if !$project.files || projectFileTree.length === 0}
        <p class="text-xs text-gray-500 dark:text-gray-400 italic px-2 py-2">
    ...
    >
      {#if !contextMenuItem.is_directory || contextMenuItem.file_type === 'directory_media_stem'}
        {#if contextMenuItem.file_type === 'media' || contextMenuItem.file_type === 'directory_media_stem' || contextMenuItem.file_type === 'audio' || contextMenuItem.file_type === 'video'}
          <button
            on:click|stopPropagation={(e) => handleMenuAction('Load')}
            class="block w-full text-left px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200"
            aria-label="Load selected media">Load Media</button
          >
          <hr class="my-1 border-gray-200 dark:border-gray-600" />
        {/if}
        {#if contextMenuItem.file_type === 'note'}
          <button
            on:click|stopPropagation={(e) => handleMenuAction('OpenNote')}
            class="block w-full text-left px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200"
            aria-label="Open selected note">Open Note</button
          >
          <hr class="my-1 border-gray-200 dark:border-gray-600" />
        {/if}
        {#if ['media', 'audio', 'video', 'audio_transcript', 'video_transcript', 'transcript', 'data', 'directory_media_stem', 'other'].includes(contextMenuItem.file_type)}
          <button
            on:click|stopPropagation={(e) => handleMenuAction('Rename')}
            class="block w-full text-left px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200"
            aria-label="Rename selected item">Rename…</button
          >
          <button
            on:click|stopPropagation={(e) => handleMenuAction('Delete')}
            class="block w-full text-left px-3 py-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/50 dark:text-red-500"
            aria-label="Delete selected item">Delete…</button
          >
        {/if}
      {:else}
