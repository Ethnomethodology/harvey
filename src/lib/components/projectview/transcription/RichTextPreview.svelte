  ...
  function handleClickOutsideSearch(event) {
    if (showSearchBox) {
      const isClickInsideSearchUi =
        searchUiContainerElement && searchUiContainerElement.contains(event.target);
      const isClickOnSearchToggleButton =
        searchToggleButtonElement && searchToggleButtonElement.contains(event.target);
    ...
    tick().then(() => {
      ...
      if (shouldScroll) {
        ...
        const isScrollingDown =
          activeSegmentIndex > karaokeScrollIndex && karaokeScrollIndex !== -1;
        ...
        const isItemOutsideSafeZone =
          actualItemTop < safeZoneTop || actualItemBottom > safeZoneBottom;
        ...
        if (forceCentering) {
          let targetDomScrollTop =
            actualItemTop - currentContainerHeight / 2 + actualItemHeight / 2;
...
>
  ...
  >
    <div class="flex items-center">
        ...
        <div class="flex items-center">
          ...
          >
            <span class="truncate"
              >{$displayedTranscripts.find(
                (t) => t.path === ($transcriptStore.activeTranscript?.path || '')
          ...
          >
            {#each $displayedTranscripts as t (t.path)}
              <DropdownItem
                class="text-xs {($transcriptStore.activeTranscript?.path || '') === t.path
          ...
          {#if $transcriptStore.isDualModeActive}
            ...
            >
              <span class="truncate"
                >{$secondaryDisplayedTranscripts.find(
                  (t) => t.path === ($transcriptStore.secondaryTranscriptPath || '')
            ...
            <Dropdown
              ...
              class="w-64 overflow-y-auto max-h-60 z-[1010]"
            >
              {#each $secondaryDisplayedTranscripts as t (t.path)}
                <DropdownItem
                  class="text-xs {($transcriptStore.secondaryTranscriptPath || '') === t.path
                    ? 'font-bold bg-blue-50 dark:bg-gray-700'
                    : ''}"
                  on:click={() => {
      ...
      {:else}
        <span class="px-3 py-1 text-xs text-gray-500 dark:text-gray-600 italic"
      ...
      {#each visibleSegments as seg, idx (`${seg.segmentIndex}-${seg.isPrimary}`)}
                  ...
                  {#if showTextCol}
                    <div
                      ...
                      style="white-space: normal; overflow-wrap: break-word; word-break: normal; {$activeLayout ===
                        'Layout3' || $activeLayout === 'Layout5'
                        ? 'margin-left: 0;'
                        : $activeLayout === 'Layout1'
                          ? '0'
                          : ''}"
                      class:pl-0={$activeLayout === 'Layout3' ||
                        $activeLayout === 'Layout5' ||
                        $activeLayout === 'Layout1'}
                      class:custom-layout3-padding={$activeLayout === 'Layout3' &&
        ...
        {#if previewEditMode && (!$transcriptStore.isDualModeActive || !seg.isPrimary)}
          <div class="flex items-center justify-center insert-button-wrapper relative px-4">
            ...
            <button
              class="btn-icon text-green-400 hover:text-green-600 dark:hover:text-green-300 flex items-center justify-center"
              on:click={() => handleInsertNewSegment(seg.segmentIndex + 1)}
              title="Insert New Segment"
              aria-label="Insert New Segment"
            >
              <PlusSquare class="w-5 h-5" />
...
<style lang="postcss">
  ...
  .segment-block:not(.preview-interaction-disabled):not(.segment-active):hover {
