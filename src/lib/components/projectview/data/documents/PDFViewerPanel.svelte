...
<script>
  ...
  function getRangePageInfo(range) {
    if (!range || !viewerElement || !pdfViewer)
  ...
  async function finalizePageView(pv, pageIndex) {
    ...
    if (pv.div?.querySelector('.textLayer')) {
      // Ensure div exists for textLayer
    ...
    if (!pv.pdfPage && pdfDoc) {
      try {
        ...
        console.error(
          `[finalizePageView] Failed to get pdfPage for page ${pageIndex + 1}: ${e.message}`
        );
  ...
  async function ensureTextLayerReady(pageViewFromCaller, pageIndex) {
    ...
    if (!pv) {
      try {
        ...
        /* ignore scroll errors if viewer is busy */
      }
      ...
      if (!pv) {
        console.error(
          `[ensureTextLayerReady] Critical: No PageView object for page ${pageIndex + 1} after attempting scroll.`
        );
    ...
    if (pv) {
      ...
      if (
        freshPv &&
        (freshPv.textLayer?.textLayerDiv || freshPv.div?.querySelector('.textLayer'))
      ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkTextLayerAvailability = async () => {
        ...
        let div =
          currentPolledPv?.textLayer?.textLayerDiv ||
          currentPolledPv?.div?.querySelector('.textLayer');
        ...
        if (div) {
          ...
          if (totalWait >= MAX_WAIT_TEXTLAYER) {
            cleanupInterval();
            console.error(
              `[ensureTextLayerReady Polling] Timeout waiting for textLayer div on page ${pageIndex + 1}.`
            );
            reject(
              new Error(`Timeout waiting for textLayer div on page ${pageIndex + 1} (polling)`)
            );
          } else if (!currentPolledPv) {
            // Check if pageView itself disappeared
            cleanupInterval();
            console.error(
              `[ensureTextLayerReady Polling] PageView for page ${pageIndex + 1} became null.`
            );
  ...
  async function createHighlightDataForStorage(id, range, color) {
    ...
    if (!actualPageElement) {
      console.warn(
        '[createHighlightDataForStorage] Could not obtain pageElement. QuadPoints will be relative to viewport.'
      );
    ...
    switch (action.type) {
      ...
      case 'removeHighlight':
        if (action.payload.dataForStorage) {
          // This object should contain all necessary highlight data including quadPoints
          ...
          console.warn(
            '[PDF Undo] Cannot re-apply highlight (was remove), missing dataForStorage for undo.',
            action.payload
          );
        }
        break;
      case 'changeColor':
        if (action.payload.id && action.payload.oldColor && action.payload.dataForStorage) {
          ...
          dispatch('pdfhighlightevent', {
            type: 'update',
            ...action.payload.dataForStorage,
            color: action.payload.oldColor
          });
        } else {
          console.warn('[PDF Undo] Cannot revert color change, missing data.', action.payload);
        }
        break;
      case 'updateHighlightQuads': // For partial overlaps
        ...
        if (action.payload.id && action.payload.originalHighlightData && action.payload.oldQuads) {
          ...
          console.warn('[PDF Undo] Cannot revert quad update, missing data.', action.payload);
        }
        break;
      case 'removeHighlightFromSelection':
        ...
        if (action.payload.updatedOriginalHighlight) {
          ...
          if (originalHlData) {
            ...
            console.warn(
              `[PDF Undo removeHighlightFromSelection] Original data for highlight ${id} not found for full update. Restoring with old quads only.`
            );
        ...
        if (action.payload.removedOriginalHighlight) {
          dispatch('pdfhighlightevent', {
            type: 'add',
            ...action.payload.removedOriginalHighlight.originalHighlightData
          });
        }
    ...
    switch (action.type) {
      case 'addHighlight':
        ...
        if (action.payload.dataForStorage) {
          ...
          console.warn(
            '[PDF Redo] Cannot re-do "addHighlight", missing dataForStorage.',
            action.payload
          );
        }
        break;
      ...
      case 'changeColor':
        if (action.payload.id && action.payload.newColor && action.payload.dataForStorage) {
          ...
          console.warn('[PDF Redo] Cannot re-apply color change, missing data.', action.payload);
        }
        break;
      case 'updateHighlightQuads': // For partial overlaps
        ...
        if (action.payload.id && action.payload.originalHighlightData && action.payload.newQuads) {
          ...
          console.warn('[PDF Redo] Cannot re-apply quad update, missing data.', action.payload);
        }
        break;
      case 'removeHighlightFromSelection':
        ...
        if (action.payload.updatedOriginalHighlight) {
          ...
          if (originalHlData) {
            ...
            console.warn(
              `[PDF Redo removeHighlightFromSelection] Original data for highlight ${id} not found for full update. Updating with new quads only.`
            );
        ...
        if (action.payload.removedOriginalHighlight) {
          dispatch('pdfhighlightevent', {
            type: 'remove',
            id: action.payload.removedOriginalHighlight.originalHighlightData.id
          });
        }
        if (action.payload.addedSecondPart) {
  ...
  function handleKeydown(e) {
    if (e.metaKey && !e.shiftKey && e.key === 'z') {
      undo();
      e.preventDefault();
    } else if (e.metaKey && (e.key === 'y' || (e.shiftKey && e.key === 'z'))) {
      redo();
      e.preventDefault();
  ...
  onMount(async () => {
    ...
    annotationMatcherWorker = new Worker(
      new URL('$lib/workers/pdfAnnotationMatcher.worker.js', import.meta.url),
      { type: 'module' }
    );
  ...
  onDestroy(() => {
    isMounted = false;
    if (pdfJsStyleElement && pdfJsStyleElement.parentNode) {
      pdfJsStyleElement.parentNode.removeChild(pdfJsStyleElement);
    }
  ...
  async function handleWorkerMessage(event) {
    ...
    if (error) {
      console.warn(
        `[Worker Message] Error for annotation ${annotationId} on page ${pageIndex + 1}: ${error}`
      );
      return;
    }
    ...
    if (startIndex !== -1 && matchLength > 0) {
      ...
      if (!pageView || !layerDiv) {
        console.warn(
          `[Worker Message] Could not find pageView or layerDiv for page ${pageIndex + 1} to apply highlight ${annotationId}.`
        );
        return;
      }
      ...
      if (range) {
        ...
        if (newQuadPoints.length > 0) {
          const highlight = initialHighlights.find(
            (h) => h.id === annotationId && h.pageIndex === pageIndex
          );
          if (highlight) {
            ...
            console.warn(
              `[Worker Message] Highlight ${annotationId} not found in initialHighlights after worker processing.`
            );
          }
        } else {
          console.warn(
            `[Worker Message] Text found by worker for ${annotationId}, but failed to generate quadPoints from range.`
          );
        }
      } else {
        console.warn(
          `[Worker Message] Text found by worker for ${annotationId}, but findRangeInTextLayer failed to create range on main thread.`
        );
      }
    } else {
      // This case should be covered by 'error' from worker, but as a fallback:
      console.warn(
        `[Worker Message] No match found by worker for ${annotationId} on page ${pageIndex + 1}.`
      );
    }
  }
  ...
  function handleClickOutside(event) {
    if (
      ...
      !highlightDropdownRef.contains(event.target) &&
    ...
    if (
      isZoomDropdownOpen &&
      zoomDropdownRef &&
      !zoomDropdownRef.contains(event.target) &&
      !event.target.closest('[role="menuitem"]')
    ) {
      // Ensure not clicking on a menu item of the zoom dropdown itself
    ...
    if (
      ...
      !selectionToolbarElement.contains(event.target)
    ) {
      ...
      const clickedOnExistingHighlight =
        event.target.closest?.('.pdf-highlight') || event.target.closest?.('.overlay-part');
  ...
  async function handleViewerClick(event) {
    ...
    const clickedOnExistingHighlightTarget =
      targetElement.closest?.('.pdf-highlight') || targetElement.closest?.('.overlay-part');
    ...
    if (!highlightSpan) {
      // If not found by target.closest, try elementsFromPoint (for overlays)
    ...
    if (highlightSpan && viewerContainer.contains(highlightSpan)) {
      ...
      if (id !== clickedHighlightId || !showSelectionToolbar) {
        ...
        if (span) {
          ...
          clickRange.selectNodeContents(span);
  ...
  function positionToolbarAtPoint(clientX, clientY) {
    ...
    if (!toolbarHeight || !toolbarWidth) {
      /* console.warn('Toolbar no dimensions'); */ return;
    }
  ...
  function handleToolbarMouseLeave() {
    clearTimeout(hideToolbarTimeoutId);
    hideToolbarTimeoutId = setTimeout(hideSelectionToolbar, 500);
  }
  ...
  async function handleHighlightAction(color) {
    ...
    if (toolbarMode === 'selection') {
      ...
      if (!selectedRange) {
        console.warn('Highlight Action (Selection Mode): No stored selection range found.');
        return;
      }
      ...
      if (color === 'remove') {
        ...
        const { pageIndex: selectionPageIndex, pageElement: selectionPageElement } =
          getRangePageInfo(rangeToUse);
        ...
        const selectionPageRect = actualSelectionPageElement?.getBoundingClientRect() || {
          top: 0,
          left: 0
        };
        ...
        for (const existingHl of highlightsToProcess) {
          if (
            existingHl.pageIndex === selectionPageIndex &&
            existingHl.quadPoints &&
            existingHl.quadPoints.length > 0
          ) {
            ...
            const finalQuadsBefore =
              quadsBeforeSelection.length > 0
                ? _simplifyAndMergeRects(quadsBeforeSelection.map((q) => quadToRect(q))).map((r) =>
                    rectToQuad(r)
                  )
                : [];
            const finalQuadsAfter =
              quadsAfterSelection.length > 0
                ? _simplifyAndMergeRects(quadsAfterSelection.map((q) => quadToRect(q))).map((r) =>
                    rectToQuad(r)
                  )
                : [];
            ...
            if (finalQuadsBefore.length > 0) {
              dispatch('pdfhighlightevent', {
                type: 'update',
                ...existingHl,
                quadPoints: finalQuadsBefore
              });
              undoPayload.updatedOriginalHighlight = {
                id: existingHl.id,
                oldQuads: originalExistingQuads,
                newQuads: JSON.parse(JSON.stringify(finalQuadsBefore))
              };
            } else {
              dispatch('pdfhighlightevent', { type: 'remove', id: existingHl.id });
              undoPayload.removedOriginalHighlight = {
                originalHighlightData: JSON.parse(JSON.stringify(existingHl))
              };
            }
            ...
            if (finalQuadsAfter.length > 0) {
              ...
              undoPayload.addedSecondPart = {
                id: newSplitHighlightId,
                data: newHighlightDataObject
              };
            }
      ...
      const { pageIndex: newSelectionPageIndex, pageElement: newSelectionPageElement } =
        getRangePageInfo(rangeToUse);
      ...
      const newSelectionPageRect = actualNewSelectionPageElement?.getBoundingClientRect() || {
        top: 0,
        left: 0
      };
      const newSelectionProcessedQuads = processAndMergeQuadPoints(
        newSelectionClientRects,
        newSelectionPageRect
      );
      ...
      if (!visualRendered && newSelectionProcessedQuads.length > 0) {
        // Only warn if quads were expected
        console.warn(
          'Visual rendering of new highlight failed, though processed quads were generated. Aborting deferred tasks.'
        );
        hideSelectionToolbar();
        return;
      }
      if (!visualRendered && newSelectionProcessedQuads.length === 0) {
        console.warn(
          'Visual rendering of new highlight failed, no processed quads. Aborting deferred tasks.'
        );
        hideSelectionToolbar();
        return;
      }
      ...
      deferTask(async () => {
        try {
          ...
          const dataForStorage = await createHighlightDataForStorage(
            newHighlightId,
            rangeToUse,
            color
          );
          if (dataForStorage) {
            ...
            recordAction('addHighlight', {
              id: newHighlightId,
              color,
              rangeData: rangeDataForUndo,
              dataForStorage
            });
            ...
            console.warn(
              `[Highlight Action Defer] Failed to create dataForStorage for ${newHighlightId}. Highlight might not be saved correctly.`
            );
            hideSelectionToolbar();
          }
        } catch (e) {
          console.error(
            `[Highlight Action Defer] Error processing new highlight ${newHighlightId}:`,
            e
          );
          hideSelectionToolbar();
        }
      ...
      // MODIFYING OR REMOVING EXISTING HIGHLIGHT
      if (!clickedHighlightId) {
        console.warn('Highlight Action (Click Mode): clickedHighlightId is null!');
        return;
      }
      ...
      if (!originalHighlightData && color !== 'remove') {
        console.warn(
          `[Highlight Action] Original data for highlight ${currentHighlightId} not found. Cannot update color.`
        );
        hideSelectionToolbar();
        return;
      }
      ...
      if (color === 'remove') {
          ...
          try {
            ...
            const dataForStorageUndo = originalHighlightData
              ...
              : {
                  ...
                  text: '',
                  pageIndex: 0
                }; // Basic fallback for undo
            recordAction('removeHighlight', {
              id: currentHighlightId,
              color: originalColor,
              dataForStorage: dataForStorageUndo
            });
            ...
            console.error(
              `[Highlight Action Defer] Error processing highlight removal ${currentHighlightId}:`,
              e
            );
          }
        });
        ...
        deferTask(async () => {
          try {
            ...
            console.error(
              `[Highlight Action Defer] Error processing highlight color change ${currentHighlightId}:`,
              e
            );
          }
        });
  ...
  function handleViewerMouseDown(event) {
    ...
    const looksLikeText = (n) =>
      n?.nodeType === Node.TEXT_NODE || n?.classList?.contains('pdf-highlight');
  ...
  function applyHighlightToSelectionDOM(range, color, overrideId) {
    ...
    if (!actualPageElement) {
      console.warn(
        '[applyHighlightToSelectionDOM] Could not obtain pageElement for quadPoints calculation.'
      );
      return null;
    }
    const clientRects = range.getClientRects();
  ...
  function handleHighlightingForNodeSegmentDOM(textNode, segmentRange, newColor, newId) {
    ...
    if (parent && parent.nodeName === 'SPAN' && parent.classList.contains('pdf-highlight')) {
      ...
      const existingColor = parent.dataset.hlColor;
      const existingId = parent.dataset.hlId;
      if (existingColor === newColor && existingId === newId) {
        return;
      }
      ...
      highlightSpan.dataset.hlId = newId;
      highlightSpan.dataset.hlColor = newColor;
      highlightSpan.style.backgroundColor = newColor;
      try {
        segmentRange.surroundContents(highlightSpan);
        ...
        try {
          const frag = segmentRange.extractContents();
          highlightSpan.appendChild(frag);
          segmentRange.insertNode(highlightSpan);
          // console.log('[handleHighlightingForNodeSegmentDOM] Fallback surround (extract/insert) successful with span:', highlightSpan);
        } catch (e2) {
          console.error(
            '[handleHighlightingForNodeSegmentDOM] Fallback surround FAILED:',
            e2.message
          );
        }
      }
  ...
  function splitSpanAndApplyHighlightDOM(existingSpan, segmentRange, newColor, newId) {
    ...
    if (!textNode || textNode.nodeType !== Node.TEXT_NODE) {
      console.error('splitApplyDOM: Span no text child.', existingSpan);
      return;
    }
    ...
    const endOffset =
      segmentRange.endContainer === textNode &&
      segmentRange.endOffset <= textNode.textContent.length
        ? segmentRange.endOffset
        : textNode.textContent.length;

    const existingColor = existingSpan.dataset.hlColor;
    const existingId = existingSpan.dataset.hlId;
    ...
    if (textBefore.length > 0) {
      ...
      s.className = 'pdf-highlight';
      s.dataset.hlId = existingId;
      s.dataset.hlColor = existingColor;
      s.style.backgroundColor = existingColor;
      s.textContent = textBefore;
      nodesToInsert.push(s);
    }
    if (textMid.length > 0) {
      const s = document.createElement('span');
      s.className = 'pdf-highlight';
      s.dataset.hlId = newId;
      s.dataset.hlColor = newColor;
      s.style.backgroundColor = newColor;
      s.textContent = textMid;
      nodesToInsert.push(s);
    }
    if (textAfter.length > 0) {
      ...
      s.dataset.hlColor = existingColor;
      s.style.backgroundColor = existingColor;
      s.textContent = textAfter;
      nodesToInsert.push(s);
    }

    if (nodesToInsert.length > 0) {
      const f = document.createDocumentFragment();
      nodesToInsert.forEach((n) => f.appendChild(n));
      if (parent.contains(existingSpan)) parent.replaceChild(f, existingSpan);
    } else if (parent.contains(existingSpan)) {
  ...
  function removeHighlightFromSelectionDOM(range) {
    ...
    const commonAncestorTextLayer = (
      range.commonAncestorContainer.nodeType === Node.ELEMENT_NODE
        ? range.commonAncestorContainer
        : range.commonAncestorContainer.parentNode
    )?.closest('.textLayer');
    if (!commonAncestorTextLayer) return;
    const spansToProcess = Array.from(
      commonAncestorTextLayer.querySelectorAll('.pdf-highlight')
    ...
    spansToProcess.forEach((spanNode) => {
      ...
      if (spanFullyWithinSelection) {
        ...
        if (range.compareBoundaryPoints(Range.END_TO_END, nodeRange) < 0) {
          // Selection ends within this node
          segmentToUnwrap.setEnd(range.endContainer, range.endOffset);
        } else {
          // Selection ends after this node
    ...
    try {
      commonAncestorTextLayer.normalize();
    } catch (e) {
      console.warn('Normalize remove failed:', e);
    }
  }

  function removePartialHighlightDOM(range) {
    removeHighlightFromSelectionDOM(range);
  }
  ...
  function splitSpanAndUnwrapSegmentDOM(existingSpan, segmentRangeToUnwrap) {
    ...
    const startOffset =
      segmentRangeToUnwrap.startContainer === textNode ? segmentRangeToUnwrap.startOffset : 0;
    const endOffset =
      segmentRangeToUnwrap.endContainer === textNode &&
      segmentRangeToUnwrap.endOffset <= textNode.textContent.length
        ? segmentRangeToUnwrap.endOffset
        : textNode.textContent.length;
    ...
    if (textMid.length > 0) {
      nodesToInsert.push(document.createTextNode(textMid));
    }
    if (textAfter.length > 0) {
      const s = document.createElement('span');
      s.className = 'pdf-highlight';
  ...
  function unwrapNodeDOM(node) {
    ...
    try {
      ...
      /* console.error("Unwrap Error:", e, node); */
    }
  }
  ...
  function areQuadsSubsumed(newSelectionQuads, existingHighlightQuads) {
    ...
    for (const existingQuad of existingHighlightQuads) {
      ...
      for (const newSelectionQuad of newSelectionQuads) {
        ...
        const nsRect = {
          nx1: newSelectionQuad[0],
          ny1: newSelectionQuad[1],
          nx2: newSelectionQuad[6],
          ny2: newSelectionQuad[7]
        };
        // Check if nsRect completely contains exRect
        if (
          nsRect.nx1 <= exRect.x1 &&
          nsRect.ny1 <= exRect.y1 &&
          nsRect.nx2 >= exRect.x2 &&
          nsRect.ny2 >= exRect.y2
        ) {
  ...
  function doBoundingBoxesIntersect(boxA, boxB) {
    ...
    return !(
      boxA.x1 >= boxB.x2 || // boxA is to the right of boxB
  ...
  function _subtractSingleRect(rectA, rectB) {
    ...
    if (
      rectA.x1 >= rectB.x2 - TOLERANCE ||
      rectA.x2 <= rectB.x1 + TOLERANCE ||
      rectA.y1 >= rectB.y2 - TOLERANCE ||
      rectA.y2 <= rectB.y1 + TOLERANCE
    ) {
    ...
    for (const currentStripQuad of sortedQuads) {
      if (currentBlock.length === 0) {
        ...
        const horizontallyOverlap =
          Math.max(currentLeft, lastLeft) < Math.min(currentRight, lastRight);
        const horizontallyAdjacentOrClose =
          currentLeft < lastRight + 10 && currentRight > lastLeft - 10;

        if (
          (areOnSameLine && horizontallyAdjacentOrClose) ||
          (areOnAdjacentLine && horizontallyAdjacentOrClose)
        ) {
          currentBlock.push(currentStripQuad);
        } else {
  ...
  async function loadPdfAndLibraries(containerElement) {
    ...
    try {
      if (!pdfjsLib) {
        ...
        PDFLinkService = vm.PDFLinkService;
        PDFFindController = vm.PDFFindController;
  ...
  function setupViewerEvents() {
    ...
    eventBus.on('pagechanging', (e) => {
      // Made synchronous again as loadAnnotationsForPageRange is async
    ...
    eventBus.on('scalechanging', (e) => {
      ...
      if (e.presetValue) {
        ...
        const m = PRESET_SCALES.find(
          (p) => !isNaN(parseFloat(p)) && Math.abs(parseFloat(p) - e.scale) < 0.001
        );
        s =
          m && !['auto', 'page-actual', 'page-fit', 'page-width'].includes(m)
            ? m
            : String(parseFloat(e.scale.toFixed(4)));
      }
      currentScaleValue = s;
      pageRendering = true;
      hideSelectionToolbar();
    ...
    eventBus.on('documentloaded', async () => {
      if (pdfViewer?.pdfDocument) {
        ...
        pageRendering = false;
        console.log(
          '[PDFViewerPanel] event: documentloaded - PDF processing complete. Attempting initial highlights.'
        );
    ...
    eventBus.on('updatefindcontrolstate', ({ state }) => {
      currentFindState = state;
    });
  }

  // --- Apply Initial Highlights (More Robust) ---
  async function applyInitialHighlights() {
    if (initialHighlightsApplied || !pdfDoc || !pdfViewer || !viewerElement || numPages === 0)
      return;
    if (!initialHighlights || initialHighlights.length === 0) {
      initialHighlightsApplied = true;
    ...
    try {
      ...
      console.error(
        '[PDFViewerPanel applyInitialHighlights] Error during loadAnnotationsForPageRange:',
        e
      );
      ...
      console.log(
        '[PDFViewerPanel applyInitialHighlights] Finished applying initial batch of highlights.'
      );
    }
  }
  ...
  async function renderAnnotationsForPage(pageIndex) {
    ...
    if (overlayContainer) {
      const existingDomHighlightParts = overlayContainer.querySelectorAll(
        '.overlay-part[data-hl-id]'
      );
      const domHighlightIdsOnPage = new Set();
      existingDomHighlightParts.forEach((part) => {
        if (part.dataset.hlId) {
          // Ensure hlId exists
      ...
      for (const domId of domHighlightIdsOnPage) {
        if (!storeHighlightIdsOnPage.has(domId)) {
          ...
          console.log(
            `[renderAnnotationsForPage] Clean-up: Removing stale DOM highlight ${domId} from page ${pageIndex + 1}`
          );
    ...
    if (!pageHighlightsFromStore.length) {
      // Check after cleanup, using the filtered list
    ...
    try {
      ...
      console.warn(
        `[renderAnnotationsForPage] Failed to ensure page ${pageIndex + 1} ready. Error: ${e.message}`
      );
      if (!pageView) return;
    }
    ...
    if (!pageView.div) {
      console.warn(
        `[renderAnnotationsForPage] No pageView.div for page ${pageIndex + 1}. Cannot apply highlights.`
      );
      return;
    }
    if (!pdfPage && pageHighlights.some((hl) => !hl.quadPoints && hl.text)) {
      console.warn(
        `[renderAnnotationsForPage] No pdfPage for page ${pageIndex + 1}, text-based fallbacks will fail.`
      );
    }
    ...
    for (const hl of pageHighlightsFromStore) {
      ...
      if (hl.quadPoints && hl.quadPoints.length > 0) {
        ...
        if (!layerDiv || !pdfPage) {
          console.warn(
            `[renderAnnotationsForPage] Text layer/pdfPage for page ${pageIndex + 1} not ready. Cannot text-match ID ${hl.id}. PV State: ${pageView?.textLayer?.renderingDone}`
          );
        ...
        try {
          ...
          console.error(
            `[renderAnnotationsForPage] Error getting textContent or posting to worker for ${hl.id}:`,
            e
          );
  ...
  async function applyHighlightsForPage(pageIndex) {
    ...
    if (
      !loadedPagesWithAnnotations.has(pageIndex) &&
      initialHighlights.some((hl) => hl.pageIndex === pageIndex)
    ) {
  ...
  function removeHighlightOverlay(id) {
    document
      .querySelectorAll(`.highlight-overlay .overlay-part[data-hl-id="${id}"]`)
  ...
  function mapNormalizedOffsetInNodeToRawOffset(rawTextNodeContent, targetNormalizedOffsetInNode) {
    ...
    for (let r = 0; r < rawTextNodeContent.length; r++) {
      // r is the raw character index
      const normLenBeforeR =
        r > 0 ? normalizeTextForMatching(rawTextNodeContent.substring(0, r)).length : 0;
      const normLenIncludingR = normalizeTextForMatching(
        rawTextNodeContent.substring(0, r + 1)
      ).length;

      if (
        normLenIncludingR > targetNormalizedOffsetInNode &&
        normLenBeforeR <= targetNormalizedOffsetInNode
      ) {
        return r;
      }
    }
  ...
  function findRangeInTextLayer(
    textLayerDiv,
    normalizedOverallCharStart,
    normalizedOverallLength,
    ...
    while ((currentNode = walker.nextNode())) {
      ...
      if (
        !foundStart &&
        normalizedOverallCharStart >= accumulatedNormalizedCharsBeforeNode &&
        normalizedOverallCharStart < accumulatedNormalizedCharsBeforeNode + nodeNormalizedLength
      ) {
        startNode = currentNode;
        const normalizedStartInNode =
          normalizedOverallCharStart - accumulatedNormalizedCharsBeforeNode;
      ...
      if (foundStart) {
        ...
        if (
          targetEndNormalizedGlobal <=
          accumulatedNormalizedCharsBeforeNode + nodeNormalizedLength
        ) {
          endNode = currentNode;
          const normalizedEndInNode =
            targetEndNormalizedGlobal - accumulatedNormalizedCharsBeforeNode;
    ...
    if (startNode && endNode && startOffsetRaw !== -1 && endOffsetRaw !== -1) {
      try {
        ...
        if (
          actualNormalizedTextFromRange !== normalizedExpectedText &&
          normalizedExpectedText.length > 0
        ) {
          // Only warn if expected text was provided
          console.warn(
        ...
        console.error('[findRangeInTextLayer] Error setting/verifying range:', e, {
          startNode,
          startOffsetRaw,
          endNode,
          endOffsetRaw
  ...
  function goToPrevPage() {
    if (pdfViewer && currentPageNum > 1) pdfViewer.previousPage();
  }
  function goToNextPage() {
    if (pdfViewer && currentPageNum < numPages) pdfViewer.nextPage();
  }
  function zoomOut() {
    if (pdfViewer) pdfViewer.decreaseScale();
  }
  function zoomIn() {
    if (pdfViewer) pdfViewer.increaseScale();
  }
  ...
  function handlePageInputBlur(e) {
    if (e.target.value !== String(currentPageNum)) e.target.value = currentPageNum;
  }
  function runSearch({ findPrevious = false } = {}) {
    if (!searchQuery.trim() || !eventBus) return;
    if (currentFindState === 3 && lastSearched === searchQuery.trim()) {
      return;
    }
    const query = searchQuery.trim();
    const isNewQuery = query !== lastSearched;
    lastSearched = query;
    eventBus.dispatch('find', {
      source: pdfViewer,
      type: isNewQuery ? '' : 'again',
      query,
      phraseSearch: true,
      caseSensitive: false,
      entireWord: false,
  ...
  function handleSearchKeydown(e) {
    if (e.key === 'Enter') {
      e.preventDefault();
      runSearch({ findPrevious: e.shiftKey });
    }
  }
  ...
  let currentZoomLabel = $derived((() => {
    ...
    if (!isNaN(numScale)) {
      ...
      const presetMatch = zoomOptions.find(
        (opt) =>
          opt.value &&
          !isNaN(parseFloat(opt.value)) &&
          Math.abs(parseFloat(opt.value) - numScale) < 0.001
      );
...
>
  ...
  >
    <button
      ...
      title="Previous page"
      disabled={currentPageNum <= 1 || loading || !pdfDoc}><ChevronLeft class="w-4 h-4" /></button
    >
    <input
      type="number"
      class="mini-toolbar-input w-12 text-center"
      bind:value={pageNumInput}
      min="1"
      max={numPages || 1}
    ...
    <button
      ...
      title="Next page"
      disabled={currentPageNum >= numPages || loading || !pdfDoc}
    ...
    <button
      ...
      on:click={zoomOut}
      title="Zoom out"
      disabled={loading || !pdfDoc}><Minus class="w-4 h-4" /></button
    >
    ...
    <div class="relative" bind:this={zoomDropdownRef}>
      ...
      {#if isZoomDropdownOpen}
        <div
          class="absolute top-full mt-1 left-0 z-30 w-48 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 shadow-lg overflow-y-auto max-h-60 py-1"
        >
    ...
    <button
      ...
      on:click={zoomIn}
      title="Zoom in"
      disabled={loading || !pdfDoc}><Plus class="w-4 h-4" /></button
    >
    <div class="separator"></div>
    <div class="flex items-center space-x-0.5">
      <input
        type="search"
        class="mini-toolbar-input w-40 text-xs"
        placeholder="Search"
      ...
      <button
        ...
        on:click={() => runSearch({ findPrevious: true })}
        disabled={loading || !pdfDoc || !searchQuery}><ChevronLeft class="w-3 h-3" /></button
      >
      <button
        class="mini-toolbar-button"
        title="Next match"
        on:click={() => runSearch({ findPrevious: false })}
        disabled={loading || !pdfDoc || !searchQuery}><ChevronRight class="w-3 h-3" /></button
      >
    </div>
    <div class="separator"></div>
    <div class="relative" bind:this={highlightDropdownRef}>
      <button
        type="button"
        class="mini-toolbar-button flex items-center focus:outline-none focus:ring-0 outline-none"
      ...
      {#if isHighlightDropdownOpen}
        <div
          class="absolute top-full mt-1 right-0 z-30 w-32 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 shadow-lg py-1"
        >
          {#each highlightOptions as option (option.value || option.label)}
            <div
            ...
            >
              <span
                ...
                style="background-color: {option.value === 'rgba(255, 255, 255, 1)'
                  ? '#fff'
                  : option.value};"
              ></span>
              <span>{option.label}</span>
  ...
  <div
    bind:this={pdfViewerWrapperElement}
    class="flex-grow overflow-hidden bg-gray-200 dark:bg-gray-950 relative pdf-viewer-wrapper"
  >
    {#if error}
      <div class="absolute inset-0 flex items-center justify-center p-4 z-40 pointer-events-none">
        <div
          class="text-red-700 dark:text-red-300 p-4 bg-red-100 dark:bg-red-900/80 rounded border border-red-400 dark:border-red-600 max-w-lg text-center shadow-lg"
        >
          <p class="font-semibold mb-2">Error:</p>
          <p class="text-sm break-words">{@html error}</p>
        </div>
      </div>
    {:else if loading || isLoadingInitialAnnotations}
      <div
        class="absolute inset-0 flex flex-col items-center justify-center z-50 bg-gray-900/75 dark:bg-black/75 pointer-events-auto"
      >
    ...
    {#if showSelectionToolbar}
      <div
        class="absolute z-[100] pointer-events-auto transition-opacity duration-100"
      ...
      >
        ...
        >
          {#each highlightOptions.filter((opt) => opt.label !== 'None') as opt (opt.value || opt.label)}
            <Button
              color="none"
              class="p-1 rounded-full hover:scale-110 transition-transform duration-100"
              on:click={() => {
                handleHighlightAction(opt.value);
              }}
            >
              <span
          ...
          {#if toolbarMode === 'click'}
            ...
            <Dropdown
              bind:open={isTagDropdownOpen}
              triggeredBy="#pdf-tag-btn"
            ...
            >
              ...
              {#each filteredTagGroups as group (group.id)}
                ...
                <Dropdown
                  ...
                  trigger="hover"
                  class="w-48 p-2 space-y-1 z-[100002]"
                >
                  {#if (groupedTagsMap[group.id] || []).length > 0}
                    {#each (groupedTagsMap[group.id] || []) as tag (tag.id)}
                      <li class="rounded hover:bg-gray-100 dark:hover:bg-gray-600 list-none">
                        <Checkbox
                          checked={(
                            $project.currentPdfAnnotations.find((h) => h.id === clickedHighlightId)
                              ?.tags || []
                          ).includes(tag.name)}
                          on:change={() =>
                            toggleTagInHighlightLocal(clickedHighlightId, tag.name, 'pdf', pdfPath)}
          ...
          {:else}
            <div class="relative">
              <button
                ...
                class="p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 group relative focus:outline-none focus:ring-0 outline-none"
                on:click={async () => {
                  await handleHighlightAction('rgba(255, 242, 117, 0.5)');
                  setTimeout(() => {
    ...
    <div
      ...
      class="pdfViewerContainer absolute inset-0 overflow-auto"
      id="viewerContainer"
    >
...
<style lang="postcss">
  ...
  .mini-toolbar-input[type='number']::-webkit-inner-spin-button,
  .mini-toolbar-input[type='number']::-webkit-outer-spin-button {
    -webkit-appearance: none;
    margin: 0;
  }
  .mini-toolbar-input[type='number'] {
  ...
  .pdfViewerContainer {
    @apply p-4 bg-gray-300 dark:bg-gray-900;
    height: 100%;
    width: 100%;
    overflow: auto;
    position: absolute;
  ...
  :global(#viewer .canvasWrapper) {
    z-index: 1;
  }
  :global(#viewer .textLayer) {
    opacity: 1;
    user-select: text;
    cursor: text;
    pointer-events: auto;
  ...
  :global(#viewer .annotationLayer) {
    z-index: 30;
    pointer-events: none;
  } /* PDF.js native annotations */
  :global(#viewer .annotationLayer *) {
    pointer-events: auto;
  }
  :global(#viewer .textLayer ::selection) {
    background: theme('colors.blue.500' / 30%);
  }
  :global(#viewer .textLayer ::-moz-selection) {
    background: theme('colors.blue.500' / 30%);
  }
