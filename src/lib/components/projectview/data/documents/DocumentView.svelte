...
<script>
  ...
  export function handleDeletedAttachment(path) {
    if (
      documentEditorPanelRef &&
      typeof documentEditorPanelRef.handleDeletedAttachment === 'function'
    ) {
  ...
  function handlePdfHighlight(event) {
    ...
    if (currentSelectedPathInStore === itemPath) {
      ...
      console.warn(
        `[DocumentView] PDF Highlight event for path ${itemPath} but store selected path is ${currentSelectedPathInStore}. Event ignored.`
      );
    }
  }
  ...
  onMount(() => {
    console.log(
      '[DocumentView] Component container mounted. Document path:',
      itemPath,
      'Is PDF:',
      isPdf
    );
    ...
    {#key itemPath}
      {#if itemPath}
        {#if isPdf}
          ...
          />
        {:else if isJsonDoc}
          <!-- Assuming .json documents are handled by DocumentEditorPanel -->
