<script>
    ...
    $: if (normalizedTablePath && $project.id) {
...
<div class="relative inline-block text-left">
    <Button id="table-views-dropdown-btn" color="alternative" size="xs" class="min-w-[120px] max-w-[200px] justify-between px-3 py-1.5 focus:ring-0 shadow-sm ml-2">
        <span class="truncate pr-2">{buttonLabel}</span>
        <ChevronDown class="w-3.5 h-3.5 text-gray-500 shrink-0" />
    </Button>
    <Dropdown bind:open={dropdownOpen} triggeredBy="#table-views-dropdown-btn" class="w-56 z-[1001] max-h-96 overflow-y-auto">
        <DropdownItem class={activeSubItemType === null || activeSubItemType === undefined ? 'font-bold bg-blue-50 dark:bg-gray-700' : ''} on:click={selectBaseTable}>
            Base Table
        </DropdownItem>
        {#if views.length > 0}
        ...
        {#each views as view, index}
            {#if view.isSurvey}
                <DropdownItem id="survey-menu-item-{index}" class="flex items-center justify-between {activeSubItemType === 'view' && activeSubItemPath?.view_name === view.view_name ? 'font-bold bg-blue-50 dark:bg-gray-700' : ''}">
                    <span class="truncate">{view.view_name}</span>
                    <ChevronRight class="w-4 h-4 ml-2 text-gray-400 shrink-0" />
                </DropdownItem>
                <Dropdown placement="right-start" triggeredBy="#survey-menu-item-{index}" trigger="hover" class="w-56 z-[1002] max-h-80 overflow-y-auto shadow-xl border border-gray-200 dark:border-gray-700">
                    <DropdownItem class="font-bold border-b border-gray-200 dark:border-gray-600 truncate {activeSubItemType === 'view' && activeSubItemPath?.view_name === view.view_name ? 'bg-blue-50 dark:bg-gray-700' : ''}" on:click={() => selectView(view)}>
                        {view.view_name} (Dataset)
                    </DropdownItem>
                    {#each view.children as child}
                        <DropdownItem class="truncate {activeSubItemType === 'doc' && activeSubItemPath === child.path ? 'font-bold bg-blue-50 dark:bg-gray-700' : ''}" on:click={() => selectDocument(child.path)}>
                            {child.name}
                        </DropdownItem>
                    {/each}
                </Dropdown>
            {:else}
                <DropdownItem class="truncate {activeSubItemType === 'view' && activeSubItemPath?.view_name === view.view_name ? 'font-bold bg-blue-50 dark:bg-gray-700' : ''}" on:click={() => selectView(view)}>
                    {view.view_name}
                </DropdownItem>
            {/if}
        {/each}
