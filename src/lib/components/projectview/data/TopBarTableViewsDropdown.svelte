<script>
  ...
  $: normalizedTablePath = (() => {
    ...
    let relative = tablePath.startsWith($project.baseDirectory)
      ? tablePath.substring($project.baseDirectory.length)
      : tablePath;
...
<div class="relative inline-block text-left">
  <Button
    id="table-views-dropdown-btn"
    color="alternative"
    size="xs"
    class="min-w-[140px] max-w-[220px] justify-between px-3 !py-1.5 focus:ring-0 shadow-sm ml-2"
  ...
  <Dropdown
    ...
    triggeredBy="#table-views-dropdown-btn"
    class="w-60 z-[1001] max-h-96 overflow-y-auto"
  >
    <DropdownItem
      class="text-xs flex items-center {activeSubItemType === null ||
      activeSubItemType === undefined
        ? 'font-bold bg-blue-50 dark:bg-gray-700'
        : ''}"
      on:click={selectBaseTable}
    >
    ...
    {/if}
    {#each views as view, index (view.view_name)}
      {#if view.isSurvey}
        <DropdownItem
          id="survey-menu-item-{index}"
          class="text-xs flex items-center justify-between {activeSubItemType === 'view' &&
          activeSubItemPath?.view_name === view.view_name
            ? 'font-bold bg-blue-50 dark:bg-gray-700'
            : ''}"
        >
        ...
        <Dropdown
          ...
          triggeredBy="#survey-menu-item-{index}"
          trigger="hover"
          class="w-60 z-[1002] max-h-80 overflow-y-auto shadow-xl border border-gray-200 dark:border-gray-700"
        >
          <DropdownItem
            class="text-xs flex items-center font-bold border-b border-gray-200 dark:border-gray-600 {activeSubItemType ===
              'view' && activeSubItemPath?.view_name === view.view_name
          ...
          {#each view.children as child (child.path)}
            <DropdownItem
              class="text-xs flex items-center {activeSubItemType === 'doc' &&
              activeSubItemPath === child.path
                ? 'font-bold bg-blue-50 dark:bg-gray-700'
                : ''}"
              on:click={() => selectDocument(child.path)}
            >
      ...
      {:else}
        <DropdownItem
          class="text-xs flex items-center {activeSubItemType === 'view' &&
          activeSubItemPath?.view_name === view.view_name
            ? 'font-bold bg-blue-50 dark:bg-gray-700'
            : ''}"
          on:click={() => selectView(view)}
