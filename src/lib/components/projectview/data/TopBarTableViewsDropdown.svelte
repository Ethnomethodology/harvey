...
<div class="relative inline-block text-left">
    ...
    <Dropdown bind:open={dropdownOpen} triggeredBy="#table-views-dropdown-btn" class="w-60 z-[1001] max-h-96 overflow-y-auto">
        <DropdownItem class="flex items-center {activeSubItemType === null || activeSubItemType === undefined ? 'font-bold bg-blue-50 dark:bg-gray-700' : ''}" on:click={selectBaseTable}>
        ...
        {#each views as view, index}
            {#if view.isSurvey}
                ...
                <Dropdown placement="right-start" triggeredBy="#survey-menu-item-{index}" trigger="hover" class="w-60 z-[1002] max-h-80 overflow-y-auto shadow-xl border border-gray-200 dark:border-gray-700">
                    <DropdownItem class="flex items-center font-bold border-b border-gray-200 dark:border-gray-600 {activeSubItemType === 'view' && activeSubItemPath?.view_name === view.view_name ? 'bg-blue-50 dark:bg-gray-700' : ''}" on:click={() => selectView(view)}>
                        <FolderClosed class="w-4 h-4 mr-2.5 text-gray-400 shrink-0" />
                        <span class="truncate">{view.view_name} (Dataset)</span>
                    </DropdownItem>
                    {#each view.children as child}
                        <DropdownItem class="flex items-center {activeSubItemType === 'doc' && activeSubItemPath === child.path ? 'font-bold bg-blue-50 dark:bg-gray-700' : ''}" on:click={() => selectDocument(child.path)}>
            ...
            {:else}
                <DropdownItem class="flex items-center {activeSubItemType === 'view' && activeSubItemPath?.view_name === view.view_name ? 'font-bold bg-blue-50 dark:bg-gray-700' : ''}" on:click={() => selectView(view)}>
