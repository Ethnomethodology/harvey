...
<script>
  ...
  async function handleDeleteCustomField(fieldKey) {
    ...
    if (confirmed) {
      try {
        ...
        await message(`Failed to delete custom field: ${error.message || error}`, {
          title: 'Error',
          type: 'error'
        });
      }
  ...
  async function getOriginalAssetDetails(selectedPath, projectStoreState) {
    if (!selectedPath || !projectStoreState || !projectStoreState.baseDirectory) {
      ...
      return {
        ...
        originalType: fallbackName.includes('.')
          ? fallbackName.substring(fallbackName.lastIndexOf('.') + 1)
          : 'unknown',
    ...
    let originalRelativePath = selectedPath.startsWith(projectStoreState.baseDirectory + pathSep)
      ...
      : selectedPath.startsWith(projectStoreState.baseDirectory)
        ? selectedPath.substring(projectStoreState.baseDirectory.length)
        : selectedPath;
    ...
    let originalType = await getFileExtname(originalFileName).then((ext) =>
      ext ? ext.substring(1).toLowerCase() : 'unknown'
    ); // remove dot
    let isView = false;

    const selectedFileExt = await getFileExtname(selectedPath).then((ext) =>
          ...
          for (const docFile of projectStoreState.documentFiles) {
            if (docFile.name === potentialOriginalFileName) {
              ...
              if (currentJsonStem === originalDocStem) {
                ...
                originalAbsolutePath = await resolve(
                  projectStoreState.baseDirectory,
                  docFile.relativePath
                );
                originalFileName = docFile.name;
                originalType = await getFileExtname(originalFileName).then((ext) =>
                  ext ? ext.substring(1).toLowerCase() : 'unknown'
                );
                isView = true;
                break;
  ...
  async function fetchAllProjectGroups() {
    ...
    if (currentProjectId) {
      try {
        allProjectGroupsForPanel = await invoke('get_project_groups', {
  ...
  onMount(async () => {
    try {
      ...
      message(`Error loading initial data for InfoPanel: ${error.message || error}`, {
        title: 'Error',
        type: 'error'
      });
    }
  ...
  async function loadMetadata(assetRelativePathToLoad) {
    ...
    try {
      ...
      const baseName =
        currentOriginalAssetDetails?.originalFileName || (await basename(assetRelativePathToLoad));
      const absPath =
        currentOriginalAssetDetails?.originalAbsolutePath ||
        (projectStoreState.baseDirectory
          ? `${projectStoreState.baseDirectory}${getPathSep()}${assetRelativePathToLoad}`
          : assetRelativePathToLoad);
      ...
      const baseNameOnError =
        currentOriginalAssetDetails?.originalFileName ||
        (await basename(assetRelativePathToLoad || 'Unknown.file').catch(() => 'Unknown.file'));
      const absPathOnError =
        currentOriginalAssetDetails?.originalAbsolutePath ||
        (projectStoreState.baseDirectory && assetRelativePathToLoad
          ? `${projectStoreState.baseDirectory}${getPathSep()}${assetRelativePathToLoad}`
          : assetRelativePathToLoad || '');
      currentFileMetadata = {
        // Set default structure on error
        file_name: baseNameOnError,
        file_path: assetRelativePathToLoad,
  ...
  async function saveFileName() {
    ...
    const currentFileExtension = await getFileExtname(currentFullFileName).then((ext) =>
      ext ? ext.substring(1).toLowerCase() : ''
    );
    const currentFileNameWithoutExtension = currentFileExtension
      ? currentFullFileName.substring(0, currentFullFileName.lastIndexOf('.'))
      : currentFullFileName;
    ...
    if (editedFileNameStem !== currentFileNameWithoutExtension) {
      if (!editedFileNameStem) {
        await message('File name (stem) cannot be empty.', {
      ...
      if (itemType === 'media_data' || itemType === 'audio' || itemType === 'video') {
        ...
        nameToSend = editedFileNameStem.endsWith('.json')
          ? editedFileNameStem
          : `${editedFileNameStem}.json`;
      } else {
        nameToSend = currentFileExtension
          ? `${editedFileNameStem}.${currentFileExtension}`
          : editedFileNameStem;
      }

      let effectiveItemTypeForRename =
        itemType === 'media_data' || itemType === 'audio' || itemType === 'video'
          ? 'media'
      ...
      try {
        await renameProjectItem(
          currentFileMetadata.db_absolute_file_path,
          nameToSend,
          effectiveItemTypeForRename
        );
        // Rely on itemPath prop change from ProjectView/NotesView to trigger full reload if name change affects path
      } catch (err) {
        await message(`Error renaming item: ${err.message || err}`, {
          title: 'Rename Failed',
          type: 'error'
        });
      }
  ...
  async function saveTextMetadata() {
    ...
    if (!currentFileMetadata || !currentFileMetadata.file_path || !itemType) {
      await message('Cannot save: Missing file path or item type.', {
        title: 'Save Error',
        type: 'error'
      });
      return;
    ...
    try {
      ...
      try {
        ...
        project.update((p) => ({
          ...
          isMediaNoteMetadataDirty: false
        }));
      } catch (err) {
      ...
      await message(`An unexpected error occurred: ${err.message || err}.`, {
        title: 'Error',
        type: 'error'
      });
    }
    ...
    for (const name of removedGroupNames) {
      ...
      if (group) {
        try {
          ...
          message(`Failed to remove group '${name}': ${err}`, {
            title: 'Group Error',
            type: 'error'
          });
        }
  ...
  $: {
    (async () => {
      ...
      if (itemPath && itemType && currentProjectStoreState?.baseDirectory) {
        const newOriginalDetails = await getOriginalAssetDetails(
          itemPath,
        ...
        if (newDerivedRelativePath && newDerivedRelativePath !== previousProcessedItemPath) {
          ...
          if (currentProjectStoreState.id && currentAssetRelativePathForGroups) {
            await fetchFileAssignedGroups(
              currentProjectStoreState.id,
              currentAssetRelativePathForGroups
            );
          }
        ...
        fileAssignedGroups = [];
        if (itemPath === null) {
          // Only clear previous if itemPath is definitively null
  ...
  $: {
    if (currentFileMetadata && $customFieldDefinitionsStore) {
      ...
      for (const def of $customFieldDefinitionsStore) {
        ...
        if (typeof def.scope === 'string') {
          ...
          def.scope &&
          typeof def.scope === 'object' &&
          typeof def.scope.AssetType === 'string'
        ) {
          const assetTypeScopeValue = def.scope.AssetType.toLowerCase();
          if (assetTypeScopeValue === itemType) isApplicable = true;
          else if (
            assetTypeScopeValue === 'media' &&
            (itemType === 'audio' || itemType === 'video' || itemType === 'media_data')
          )
            isApplicable = true;
        }
        if (isApplicable) {
    ...
    if (currentFileMetadata) {
      if (currentFileMetadata.file_name) {
        const ext = currentFileMetadata.file_name.includes('.')
          ? currentFileMetadata.file_name.substring(
              currentFileMetadata.file_name.lastIndexOf('.') + 1
            )
          : '';
        const nameWithoutExt = ext
          ? currentFileMetadata.file_name.substring(
              0,
              currentFileMetadata.file_name.length - (ext.length + (ext ? 1 : 0))
            )
          : currentFileMetadata.file_name;
  ...
  $: {
    ...
    if (
      ...
      if (currentProjectState.isDocumentMetadataDirty)
        project.update((p) => ({ ...p, isDocumentMetadataDirty: false }));
...
<div class="h-full bg-white dark:bg-gray-900 flex flex-col overflow-hidden">
  <div
    class="text-sm font-semibold border-b px-1 h-9 border-gray-300 dark:border-gray-800 text-gray-700 dark:text-gray-300 flex-shrink-0 flex items-center justify-between mb-2"
  >
    ...
    {#if currentFileMetadata}
      <div class="space-y-3 mt-1 pb-4">
        <div>
          <Label
            for="fileNameInput"
            class="mb-1 text-xs text-gray-500 font-semibold uppercase tracking-wide"
            >File Name</Label
          >
          <Input
            type="text"
            id="fileNameInput"
            bind:value={editableMetadata.file_name}
            class="text-xs !py-1.5"
            size="sm"
            placeholder="Enter name without extension"
            autocorrect="off"
            autocomplete="off"
            on:blur={handleFileNameBlur}
            on:keydown={handleFileNameKeydown}
          />
          {#if currentFileMetadata.file_name && currentFileMetadata.file_name.includes('.')}
            <p class="mt-1 text-[10px] text-gray-500 dark:text-gray-400">
              Extension: {currentFileMetadata.file_name.substring(
                currentFileMetadata.file_name.lastIndexOf('.')
              )}
            </p>
          {/if}
        </div>
        ...
        <div>
          <Label
            for="filePathAbsolute"
            class="mb-1 text-xs text-gray-500 font-semibold uppercase tracking-wide"
            >File Path</Label
          >
          <div
            id="filePathAbsolute"
            class="text-xs text-gray-900 dark:text-gray-100 break-all px-2 py-1.5 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md min-h-[30px]"
          >
            {currentFileMetadata.db_absolute_file_path || ''}
          </div>
        </div>
        ...
        <div>
          ...
          <div
            id="fileTypeDisplay"
            class="text-xs text-gray-900 dark:text-gray-100 break-words px-2 py-1.5 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md min-h-[30px]"
          >
            {currentFileMetadata.file_type || 'N/A'}
          </div>
        </div>
        ...
        {#if currentFileMetadata.creation_time}
          <div>
            ...
            <div
              id="createdAt"
              class="text-xs text-gray-900 dark:text-gray-100 px-2 py-1.5 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md min-h-[30px]"
            >
              {new Date(currentFileMetadata.creation_time).toLocaleString()}
            </div>
          </div>
        {/if}
        ...
        {#if currentFileMetadata.last_modified}
          <div>
            ...
            <div
              ...
              class="text-xs text-gray-900 dark:text-gray-100 px-2 py-1.5 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md min-h-[30px]"
            >
              {new Date(currentFileMetadata.last_modified).toLocaleString()}
            </div>
          </div>
        {/if}
        ...
        <div>
          <Label
            for="titleInput"
            class="mb-1 text-xs text-gray-500 font-semibold uppercase tracking-wide">Title</Label
          >
          <Input
            type="text"
            id="titleInput"
            bind:value={editableMetadata.title}
        ...
        <div>
          ...
          <Textarea
            id="descriptionInput"
            bind:value={editableMetadata.description}
            on:input={debouncedSaveMetadata}
            rows="3"
            class="text-xs !py-1.5"
            autocorrect="off"
        ...
        <div>
          <Label
            for="summaryInput"
            class="mb-1 text-xs text-gray-500 font-semibold uppercase tracking-wide">Summary</Label
          >
          <Textarea
            id="summaryInput"
            bind:value={editableMetadata.summary}
            on:input={debouncedSaveMetadata}
            rows="2"
            class="text-xs !py-1.5"
            autocorrect="off"
        ...
        {#if (itemType === 'doc' || itemType === 'standalone_transcript') && currentFileMetadata?.customFields && Array.isArray(currentFileMetadata.customFields)}
          {@const attachmentsField = currentFileMetadata.customFields.find(
            (f) => f.key === 'attachments'
          )}
          {#if attachmentsField && attachmentsField.value}
            ...
            {#if Array.isArray(attachments) && attachments.length > 0}
              ...
              <div class="mb-3">
                <h3
                  class="text-xs font-semibold text-gray-500 dark:text-gray-400 tracking-wider mb-2"
                >
                  Attachments
                </h3>
        ...
        {#if (currentFileMetadata.duration_seconds || currentFileMetadata.width || currentFileMetadata.video_codec || currentFileMetadata.audio_codec || currentFileMetadata.bit_rate) && !(Array.isArray(currentFileMetadata.customFields) && currentFileMetadata.customFields.some((cf) => cf.key === '_isScreenshot' && cf.value === true))}
          <hr class="my-4 border-gray-300 dark:border-gray-700" />
          <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 tracking-wider mb-2">
            Technical Details
          </h3>
          {#if currentFileMetadata.duration_seconds}
            <div class="mb-3">
              <label
                for="durationInput"
                class="font-semibold text-gray-600 dark:text-gray-400 block mb-1">Duration:</label
              >
              <span
                id="durationInput"
                class="text-gray-900 dark:text-gray-100 block w-full border border-gray-300 dark:border-gray-800 px-1.5 py-1 bg-gray-50 dark:bg-gray-800 min-h-[30px] break-words"
                >{formatDuration(currentFileMetadata.duration_seconds)}</span
              >
            </div>
          {/if}
          {#if currentFileMetadata.width && currentFileMetadata.height}
            <div class="mb-3">
              <label
                for="dimensionsInput"
                class="font-semibold text-gray-600 dark:text-gray-400 block mb-1">Dimensions:</label
              >
              <span
                id="dimensionsInput"
                class="text-gray-900 dark:text-gray-100 block w-full border border-gray-300 dark:border-gray-800 px-1.5 py-1 bg-gray-50 dark:bg-gray-800 min-h-[30px] break-words"
                >{currentFileMetadata.width} x {currentFileMetadata.height}</span
              >
            </div>
          {/if}
          {#if currentFileMetadata.frame_rate}
            <div class="mb-3">
              <label
                for="frameRateInput"
                class="font-semibold text-gray-600 dark:text-gray-400 block mb-1">Frame Rate:</label
              >
              <span
                id="frameRateInput"
                class="text-gray-900 dark:text-gray-100 block w-full border border-gray-300 dark:border-gray-800 px-1.5 py-1 bg-gray-50 dark:bg-gray-800 min-h-[30px] break-words"
                >{currentFileMetadata.frame_rate.toFixed(2)} fps</span
              >
            </div>
          {/if}
          {#if currentFileMetadata.bit_rate}
            <div class="mb-3">
              <label
                for="bitRateInput"
                class="font-semibold text-gray-600 dark:text-gray-400 block mb-1">Bit Rate:</label
              >
              <span
                id="bitRateInput"
                class="text-gray-900 dark:text-gray-100 block w-full border border-gray-300 dark:border-gray-800 px-1.5 py-1 bg-gray-50 dark:bg-gray-800 min-h-[30px] break-words"
                >{formatBitrate(currentFileMetadata.bit_rate)}</span
              >
            </div>
          {/if}
          {#if currentFileMetadata.video_codec}
            <div class="mb-3">
              <label
                for="videoCodecInput"
                class="font-semibold text-gray-600 dark:text-gray-400 block mb-1"
                >Video Codec:</label
              >
              <span
                id="videoCodecInput"
                class="text-gray-900 dark:text-gray-100 block w-full border border-gray-300 dark:border-gray-800 px-1.5 py-1 bg-gray-50 dark:bg-gray-800 min-h-[30px] break-words"
                >{currentFileMetadata.video_codec || ''}</span
              >
            </div>
          {/if}
          {#if currentFileMetadata.audio_codec}
            <div class="mb-3">
              ...
              <span
                id="audioCodecInput"
                class="text-gray-900 dark:text-gray-100 block w-full border border-gray-300 dark:border-gray-800 px-1.5 py-1 bg-gray-50 dark:bg-gray-800 min-h-[30px] break-words"
                >{currentFileMetadata.audio_codec || ''}</span
              >
            </div>
          {/if}
        {/if}
        ...
        {#if currentAssetRelativePathForGroups && get(project).id}
          <hr class="my-4 border-gray-300 dark:border-gray-700" />
          <div class="mb-2">
            <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 tracking-wider mb-1">
              Groups
            </h3>
        ...
        {#if $customFieldDefinitionsStore}
          ...
          <div class="flex justify-between items-center mb-2">
            <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 tracking-wider">
              Custom Fields
            </h3>
            <button
          ...
          {#if editableMetadata.customFields.length === 0 && $customFieldDefinitionsStore && $customFieldDefinitionsStore.filter( (def) => {
                const scope = def.scope;
                let applicable = false;
                if (typeof scope === 'string') {
                  if (scope.toLowerCase() === 'project') applicable = true;
                } else if (scope && typeof scope === 'object' && typeof scope.AssetType === 'string') {
                  const assetTypeScope = scope.AssetType.toLowerCase();
                  if (assetTypeScope === itemType) applicable = true;
                  else if (assetTypeScope === 'media' && (itemType === 'audio' || itemType === 'video' || itemType === 'media_note')) applicable = true;
                }
                return applicable;
              } ).length > 0}
            <p class="text-xs text-gray-500 dark:text-gray-400 italic">
              No custom fields have values for this item. Add a definition above.
            </p>
          {/if}
        {/if}
      </div>
...
<AddFieldModal
  bind:showModal={showAddFieldModal}
  currentItemType={itemType}
  on:close={() => (showAddFieldModal = false)}
/>
...
{#if isCreateGroupModalOpen && get(project).id}
  <CreateGroupModal
    ...
    fileToAdd={createGroupModalFileToAssign
      ? { relativePath: createGroupModalFileToAssign, name: '' }
      : null}
    ...
    on:groupCreatedAndFileAdded={async (event) => {
      ...
      if (
        event.detail.file &&
        event.detail.file.relativePath === currentAssetRelativePathForGroups
      ) {
        await fetchFileAssignedGroups(get(project).id, currentAssetRelativePathForGroups);
      }
      createGroupModalFileToAssign = null;
