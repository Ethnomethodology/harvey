<script>
  ...
  function getFileName(path) {
    ...
    if (typeof path === 'object' && path.is_transcript)
      return path.displayLabel || path.name || path.path.split(/[\/\\]/).pop();
  ...
  function openRenameFolderModal(folderName) {
    ...
    const viewName = folderName.endsWith('_participants')
      ? folderName.slice(0, -13)
      : folderName.endsWith('_questions')
  ...
  async function handleDeleteTranscript(transcript) {
    ...
    const confirmed = await ask(
      `Are you sure you want to delete the transcript file "${transcript.name}"?\n\nThis will remove it from the project.\n\nThis action cannot be undone.`,
      { title: 'Delete Transcript', type: 'warning' }
    );
    if (!confirmed) return;
    ...
    try {
      ...
      if (isActive) {
        ...
        const activeMediaFile = findFileInTree(
          currentFiles,
          projectStoreState.selectedMediaNotePath
        );

        if (
          activeMediaFile &&
          activeMediaFile.associated_transcripts &&
          activeMediaFile.associated_transcripts.length > 0
        ) {
  ...
  async function handleDeleteChart(chart) {
    ...
    const { ask } = await import('@tauri-apps/plugin-dialog');
    const confirmed = await ask(`Are you sure you want to delete ${chart.chart_name}?`, {
      title: 'Delete Chart',
      type: 'warning'
    });
    if (!confirmed) return;
  ...
  async function handleDeleteDocument(documentPath) {
    ...
    const confirmed = await ask(
      ...
      { title: 'Delete Attachment', type: 'warning' }
    );
    if (!confirmed) return;
  ...
  async function getOriginalAssetDetails(selectedPath, projectStoreState) {
    ...
    let originalRelativePath = selectedPath.startsWith(projectStoreState.baseDirectory)
    ...
    try {
      ...
      if (itemType === 'table') {
        ...
        if (activeMediaFile && activeMediaFile.associated_transcripts) {
          ...
          const mappedTranscripts = activeMediaFile.associated_transcripts
            .map((t) => {
      ...
      try {
        ...
        console.error(
          `[AttachmentsPanel] Could not load raw asset_metadata attachments:`,
          metaError
        );
      }
      ...
      console.error(
        `[AttachmentsPanel] Error loading metadata for ${assetRelativePathToLoad}:`,
        error
      );
    } finally {
  ...
  $: {
    ...
    (async () => {
      ...
      const isSupportedType =
        ...
        itemType === 'table' ||
        currentProjectStoreState.selectedMediaNotePath;
      const currentPathToUse =
        itemType === 'doc' && currentProjectStoreState.selectedMediaNotePath
          ? currentProjectStoreState.selectedMediaNotePath
          : itemPath;
...
<div class="h-full bg-white dark:bg-gray-900 flex flex-col overflow-hidden">
  <div
    class="text-sm font-semibold border-b pb-1 px-1 border-gray-300 dark:border-gray-800 text-gray-700 dark:text-gray-300 flex-shrink-0 flex items-center justify-between h-9 mb-2"
  >
  ...
  <div class="flex-grow overflow-y-auto min-h-0">
    ...
    {:else if attachments.length > 0}
      <ul class="divide-y divide-gray-200 dark:divide-gray-800">
        ...
        {#each Object.keys(groupedAttachments.folders) as folderName (folderName)}
          <li class="flex flex-col border-b border-gray-100 dark:border-gray-800 last:border-0">
            ...
            >
              ...
              <div class="flex items-center gap-2 shrink-0">
                <span
                  class="text-xs text-gray-400 dark:text-gray-500 group-hover:hidden transition-opacity"
                  >{groupedAttachments.folders[folderName].length} items</span
                >
                <div class="opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity">
                  <button
                    class="text-gray-500 dark:text-gray-400 p-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded"
                    title="Folder Options"
                    id="folder-options-{folderName.replace(/[^a-zA-Z0-9]/g, '-')}"
                    on:click|stopPropagation
                  >
                    <MoreVertical class="w-4 h-4" />
                  </button>
                  <Dropdown
                    triggeredBy="#folder-options-{folderName.replace(/[^a-zA-Z0-9]/g, '-')}"
                    class="w-36 z-50"
                ...
                {#each groupedAttachments.folders[folderName] as { attachment, originalIndex } (attachment)}
                  <li
                    ...
                    on:click={() => {
                      ...
                      dispatch('requestOpenLexicalDocument', { docPath: attachment });
                    }}
                  >
                    <div class="flex items-center space-x-2 truncate">
                    ...
                    <div
                      class="opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity flex items-center justify-center shrink-0"
                    >
                      <button
                        class="text-gray-500 dark:text-gray-400 p-0.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded"
                        title="Document Options"
                        id="doc-options-folder-{originalIndex}"
                        on:click|stopPropagation
                      >
                        <MoreVertical class="w-3.5 h-3.5" />
                      </button>
                      <Dropdown
                        triggeredBy="#doc-options-folder-{originalIndex}"
                        class="w-36 z-50"
                        on:click={(e) => e.stopPropagation()}
                      >
                        <DropdownItem
                        ...
                        <DropdownItem
                          ...
                          on:click={(e) => {
                            e.stopPropagation();
                            handleDeleteDocument(attachment);
                          }}
                        >
                          <Trash2 class="w-4 h-4" /> Delete
                        </DropdownItem>
        ...
        {#each groupedAttachments.root as { attachment, originalIndex } (attachment)}
          <li
            ...
            class:bg-blue-100={currentTrackIndex === originalIndex ||
              (typeof attachment === 'object' &&
                attachment.is_transcript &&
                attachment.path === $project.activeTranscriptPathInDataTab)}
            class:dark:bg-blue-800={currentTrackIndex === originalIndex ||
              (typeof attachment === 'object' &&
                attachment.is_transcript &&
                attachment.path === $project.activeTranscriptPathInDataTab)}
          ...
          >
            <div class="flex items-center space-x-3 truncate">
              {#if typeof attachment === 'object' && attachment.chart_name}
                {#if attachment.chart_type === 'bar'}<ChartBar
                    class="w-4 h-4 text-gray-400 shrink-0"
                  />{/if}
                {#if attachment.chart_type === 'column'}<ChartColumn
                    class="w-4 h-4 text-gray-400 shrink-0"
                  />{/if}
                {#if attachment.chart_type === 'line'}<LineChart
                    class="w-4 h-4 text-gray-400 shrink-0"
                  />{/if}
                {#if attachment.chart_type === 'scatter'}<ScatterChart
                    class="w-4 h-4 text-gray-400 shrink-0"
                  />{/if}
                {#if attachment.chart_type === 'pie'}<PieChart
                    class="w-4 h-4 text-gray-400 shrink-0"
                  />{/if}
                {#if attachment.chart_type === 'gantt'}<SquareChartGantt
                    class="w-4 h-4 text-gray-400 shrink-0"
                  />{/if}
              {:else if typeof attachment === 'object' && attachment.view_name}
                {#if attachment.view_type === 'partial'}<Table2
                    class="w-4 h-4 text-gray-400 shrink-0"
                  />{/if}
                {#if attachment.view_type === 'pivot'}<LayoutGrid
                    class="w-4 h-4 text-gray-400 shrink-0"
                  />{/if}
              {:else if typeof attachment === 'object' && attachment.is_transcript}
              ...
              <span
                ...
                title={typeof attachment === 'object'
                  ? attachment.chart_name ||
                    attachment.view_name ||
                    (attachment.is_transcript ? attachment.displayLabel : '')
                  : attachment}
              >
                {getFileName(attachment)}
              </span>
            ...
            {#if typeof attachment === 'object' && attachment.chart_name}
              <div class="opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity">
                <button
                  class="text-gray-500 dark:text-gray-400 p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded"
                  title="Chart Options"
                  id="chart-options-{originalIndex}"
                  on:click|stopPropagation
                >
                  <MoreVertical class="w-4 h-4" />
                </button>
                <Dropdown
                  triggeredBy="#chart-options-{originalIndex}"
                  class="w-36 z-50"
                  on:click={(e) => e.stopPropagation()}
                >
                  <DropdownItem
            ...
            {:else if typeof attachment === 'object' && attachment.view_name}
              <div class="opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity">
                <button
                  class="text-gray-500 dark:text-gray-400 p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded"
                  title="View Options"
                  id="view-options-{originalIndex}"
                  on:click|stopPropagation
                >
                  <MoreVertical class="w-4 h-4" />
                </button>
                <Dropdown
                  triggeredBy="#view-options-{originalIndex}"
                  class="w-36 z-50"
                  on:click={(e) => e.stopPropagation()}
                >
                  <DropdownItem
                  ...
                  <DropdownItem
                    ...
                    on:click={(e) => {
                      ...
                      dispatch('requestConfigureView', { view: attachment });
                    }}
                  >
            ...
            {:else if typeof attachment === 'object' && attachment.is_transcript}
              <div class="opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity">
                <button
                  ...
                  id="transcript-options-{originalIndex}"
                  on:click|stopPropagation
                >
                  <MoreVertical class="w-4 h-4" />
                </button>
                <Dropdown
                  triggeredBy="#transcript-options-{originalIndex}"
                  class="w-36 z-50"
                  on:click={(e) => e.stopPropagation()}
                >
                  <DropdownItem
                  ...
                  <DropdownItem
                    ...
                    on:click={(e) => {
                      e.stopPropagation();
                      handleDeleteTranscript(attachment);
                    }}
                  >
                    <Trash2 class="w-4 h-4" /> Delete
                  </DropdownItem>
            ...
            {:else if typeof attachment === 'string' && attachment.endsWith('.json')}
              <div
                class="opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity flex items-center justify-center"
              >
                <button
                  class="text-gray-500 dark:text-gray-400 p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded"
                  title="Document Options"
                  id="doc-options-{originalIndex}"
                  on:click|stopPropagation
                >
                  <MoreVertical class="w-4 h-4" />
                </button>
                <Dropdown
                  triggeredBy="#doc-options-{originalIndex}"
                  class="w-36 z-50"
                  on:click={(e) => e.stopPropagation()}
                >
                  <DropdownItem
            ...
            {:else if typeof attachment === 'string' && /\.(png|jpe?g|gif|webp|svg)$/i.test(attachment)}
              ...
              >
                <button
                  class="text-gray-500 dark:text-gray-400 p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded"
                  title="Image Options"
                  id="image-options-{originalIndex}"
                  on:click|stopPropagation
                >
                  <MoreVertical class="w-4 h-4" />
                </button>
                <Dropdown
                  triggeredBy="#image-options-{originalIndex}"
                  class="w-36 z-50"
                  on:click={(e) => e.stopPropagation()}
                >
                  <DropdownItem
            ...
            {:else}
              ...
              >
                <button
                  class="text-gray-500 dark:text-gray-400 p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded"
                  title="Media Options"
                  id="media-options-{originalIndex}"
                  on:click|stopPropagation
                >
                  <MoreVertical class="w-4 h-4" />
                </button>
                <Dropdown
                  triggeredBy="#media-options-{originalIndex}"
                  class="w-36 z-50"
                  on:click={(e) => e.stopPropagation()}
                >
                  <DropdownItem
    ...
    {:else}
      <p class="text-xs text-gray-500 dark:text-gray-400 italic px-2 py-4">No attachments found.</p>
    {/if}
  </div>
</div>
