...
<script>
  ...
  function processHighlights(highlights, type) {
    ...
    if (type === 'image') {
      return highlights
        ...
        .map((annotation) => {
          const titleBody = annotation.body.find(
            (b) => b.purpose === 'commenting' && b.type === 'Title'
          );
          const descriptionBody = annotation.body.find(
            (b) => b.purpose === 'commenting' && b.type === 'Description'
          );
      ...
      // Handles 'doc', 'pdf', 'standalone_transcript'
  ...
  async function handleHighlightsUpdate(newHighlights) {
    if (effectiveType === 'standalone_transcript') {
      ...
      const isSubDocument =
        (itemType === 'doc' || get(project).selectedDocumentType === 'tables') &&
        itemPath &&
...
<div class="h-full bg-white dark:bg-gray-900 flex flex-col overflow-hidden">
  ...
  <div class="flex-grow overflow-y-auto overflow-x-hidden min-h-0 text-xs relative px-2 pb-4">
    {#if processedHighlights.length > 0}
      <ul class="space-y-3">
        {#each processedHighlights as highlight (highlight.id)}
          <li
            class="border border-gray-200 dark:border-gray-800 rounded-md shadow-sm hover:shadow-md transition-shadow relative focus-within:z-10"
            style="border-left-color: {highlight.color}; border-left-width: 4px;"
          >
            <div
            ...
            >
              <div class="flex justify-between items-start gap-2">
                <div class="flex-grow">
                  {#if effectiveType === 'image'}
                    <p class="font-semibold text-black dark:text-white leading-snug">
                      {highlight.title || 'No Title'}
                    </p>
                    <p class="text-gray-600 dark:text-gray-300 mt-1 line-clamp-3">
                      {highlight.description || 'No Description'}
                    </p>
                  {:else}
                    {#if highlight.pageIndex !== undefined && highlight.pageIndex !== null}
                      <p
                        class="text-[10px] uppercase tracking-wider font-bold text-blue-600 dark:text-blue-400 mb-1"
                      >
                        Page {highlight.pageIndex + 1}
                      </p>
                    {/if}
                    <p
                      class="font-medium text-gray-900 dark:text-gray-100 leading-relaxed break-all"
                    >
                      {highlight.text}
                    </p>
                  {/if}
            ...
            <div
              class="border-t border-gray-100 dark:border-gray-800 px-3 py-2 rounded-b-md flex flex-col bg-gray-50/30 dark:bg-gray-900/50"
            >
              ...
              <div class="flex justify-end w-full mt-2">
                ...
                >
                  ...
                  {#if highlight.comments && highlight.comments.length > 0}
                    <span
                      class="absolute -top-1.5 -right-1.5 bg-blue-600 text-white text-[9px] font-bold rounded-full h-4 w-4 flex items-center justify-center border-2 border-white dark:border-gray-800"
                    >
    ...
    {:else if effectiveType === 'doc' || effectiveType === 'media' || effectiveType === 'standalone_transcript' || effectiveType === 'image' || effectiveType === 'table' || effectiveType === 'pdf'}
      <p class="text-gray-500 dark:text-gray-400 italic px-1 py-2">No highlights for this item.</p>
    {:else}
