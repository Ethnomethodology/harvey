<script>
  ...
  function getCategoryInfo(file) {
    switch (file.file_type) {
      ...
      case 'document':
        return CATEGORY_ORDER.find((c) => c.key === 'documents');
      case 'image':
        return CATEGORY_ORDER.find((c) => c.key === 'images');
      case 'table':
        return CATEGORY_ORDER.find((c) => c.key === 'tables');
      case 'standalone_transcript':
        return CATEGORY_ORDER.find((c) => c.key === 'standalone_transcripts');
      default:
        return CATEGORY_ORDER.find((c) => c.key === 'others');
    }
  }
      ...
      (files || []).forEach((file) => {
        ...
        switch (file.file_type) {
          ...
          case 'media':
            ...
            break;
          case 'document':
            newCategorizedFiles.documents.push(file);
            break;
          case 'image':
            newCategorizedFiles.images.push(file);
            break;
          case 'table':
            newCategorizedFiles.tables.push(file);
            break;
          case 'standalone_transcript':
            newCategorizedFiles.standalone_transcripts.push(file);
            break;
          default:
            newCategorizedFiles.others.push(file);
            break;
  ...
  async function handleGroupDetailsUpdated(event) {
    ...
    if (updatedGroup.project_id) {
      ...
      console.warn(
        '[GroupDetailView] project_id not available in updatedGroup, cannot refresh project groups list.'
      );
    }
  }
  ...
  async function handleContextMenuRemoveFromGroup(event) {
    ...
    const confirmed = await confirm(
      `Are you sure you want to remove "${item.name}" from the group "${groupData.name}"?`,
      {
    ...
    try {
      ...
      updateProjectStoreState({
        statusMessage: `File "${item.name}" removed from group "${groupData.name}".`
      });
  ...
  async function handleContextMenuReveal(event) {
    ...
    try {
      ...
      await message(`Could not reveal file: ${err.message || err}`, {
        title: 'Error',
        type: 'error'
      });
    }
  ...
  async function handleRenameModalConfirm(event) {
    ...
    const originalExtension = originalFullName.includes('.')
    ...
    console.log(
      `[GroupDetailView] Rename Confirm: Original='${originalFullName}', FromModal='${newNameFromModal}', Final='${finalNewName}'`
    );
    ...
    try {
      ...
      await renameProjectItem(
        itemToRename.full_path,
        finalNewName,
        itemType,
        currentProj.xmlPath,
        currentProj.baseDirectory
      );
      updateProjectStoreState({
        statusMessage: `Item "${originalFullName}" renamed to "${finalNewName}" successfully.`
      });
      await fetchGroupContents(); // Refresh this group's view
    } catch (error) {
      console.error('Error renaming item:', error);
      await message(`Failed to rename item: ${error.message || error}`, {
        title: 'Error',
        type: 'error'
      });
    } finally {
  ...
  async function handleContextMenuDelete(event) {
    ...
    try {
      ...
      await message(`Failed to delete item: ${error.message || error}`, {
        title: 'Error',
        type: 'error'
      });
    }
  ...
  async function handleContextMenuTranscribe(event) {
    ...
    function findMediaByPathRecursiveLocal(nodes, path) {
      ...
      for (const node of nodes) {
        ...
        if (
          ...
          !node.is_directory &&
          nodePathNormalized === targetPathNormalized
        ) {
          return node;
        }
    ...
    if (canonicalFileEntry) {
      console.log(
        '[GroupDetailView] Found canonical FileEntry, dispatching requestmediaselection with its path:',
        canonicalFileEntry.path
      );
      pathForDispatch = canonicalFileEntry.path;
      // DO NOT call selectMedia here. Let ProjectView.handleRequestMediaSelection do it.
    } else {
      console.error(
        `[GroupDetailView] Canonical FileEntry not found for path: ${mediaPathToFind}. Falling back to itemFromGroup.full_path for dispatch.`
      );
  ...
  function handleContextMenuAddToGroup(event) {
    // Renamed from original placeholder to avoid conflict
    const item = event.detail.item;
    if (!item) return;
  ...
  async function fetchProjectGroupsForMenu(forceRefresh = false) {
    ...
    projectGroupsForMenu = [...get(currentProjectGroupsList)].sort((a, b) =>
      a.name.localeCompare(b.name)
    ); // Read from store after update
  }
  ...
  async function openAddToGroupSubMenu(x_pos, y_pos) {
    ...
    setTimeout(() => {
      if (showAddToGroupSubMenu)
        document.addEventListener('click', closeAddToGroupSubMenuListener, {
          capture: true,
          once: true
        });
    }, 0);
  ...
  async function handleAddFileToExistingGroupInGroupView(group) {
    ...
    const currentProject = get(projectStore);
    if (!currentProject || !currentProject.id) {
      await message('Project context is missing. Cannot add to group.', {
        title: 'Error',
        type: 'error'
      });
      return;
    ...
    try {
      ...
      updateProjectStoreState({
        statusMessage: `File "${itemForAddToGroup.name}" added to group "${group.name}".`
      });
      if (group.id === groupData.id) {
        // 'group' is the target group, 'groupData' is the currently viewed group
  ...
  function handleModalGroupCreated() {
    // When group is created, but file might not have been added if itemForAddToGroup was null
    fetchProjectGroupsForMenu(true); // Refresh group list
    showCreateGroupModalFromGroupView = false;
    itemForAddToGroup = null; // Reset
  }

  function handleModalGroupCreatedAndFileAdded(event) {
    // When group is created AND current file added
    fetchProjectGroupsForMenu(true); // Refresh group list
    showCreateGroupModalFromGroupView = false;
    updateProjectStoreState({
      statusMessage: `File "${itemForAddToGroup?.name}" added to new group "${event.detail.group?.name}".`
    });

    const newGroupData = event.detail.group;
    if (newGroupData && itemForAddToGroup) {
      // Ensure itemForAddToGroup was processed
  ...
  {#if groupData}
    ...
    <div class="mb-4 pb-2 border-b border-gray-300 dark:border-gray-700">
      ...
      {#if groupData.description && groupData.description.trim() !== ''}
        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1 max-h-20 overflow-y-auto pr-2">
          {groupData.description}
        </p>
      {:else}
        <p class="text-sm text-gray-400 dark:text-gray-500 mt-1 italic h-5">
      ...
      <div class="mb-4 flex justify-between items-center">
        <div class="relative inline-block text-left">
          <Button
            color="alternative"
            size="sm"
            class="flex items-center space-x-1 px-3 py-1.5 focus:ring-0 group"
          >
            <div
          ...
          <Dropdown class="w-48 p-3 space-y-2">
            {#each columns as col (col.key)}
              <li>
                <Checkbox bind:checked={col.visible} disabled={col.disabled} class="cursor-pointer"
                  >{col.label}</Checkbox
                >
              </li>
            {/each}
                ...
                >
                  {#each filesInCategory as file (file.relative_path)}
                    <div
                      ...
                      on:keydown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') handleFileDoubleClick(file);
                      }}
                    ...
                    >
                      ...
                      <div
                        class="aspect-square w-full relative bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 rounded-xl overflow-hidden transition-all duration-300 group-hover:border-blue-200 dark:group-hover:border-blue-900 group-hover:shadow-md"
                      >
                        <!-- Preview Area -->
                        <div class="absolute inset-0 pb-8">
                          ...
                          {:else if (file.file_type === 'document' || file.file_type.includes('transcript')) && file.full_path}
                            ...
                            {:else if file.full_path.endsWith('.json')}
                              <DocumentThumbnail
                                {file}
                                isTranscript={file.file_type.includes('transcript')}
                              />
                            {:else}
                              <div
                                class="absolute inset-0 flex items-center justify-center transition-transform duration-300 group-hover:scale-110 text-gray-400 dark:text-gray-500"
                              >
                          ...
                          {:else if file.file_type === 'table' && file.full_path}
                            <TableThumbnail {file} />
                          {:else if file.file_type === 'audio' || (file.file_type === 'media' && AUDIO_EXTENSIONS.has(file.name
                                  .split('.')
                                  .pop()
                                  ?.toLowerCase() ?? ''))}
                            <AudioThumbnail {file} />
                        ...
                        <!-- Bottom Band -->
                        <div
                          class="absolute bottom-0 left-0 right-0 h-8 bg-gray-100 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between px-2.5 gap-2 transition-colors duration-300 group-hover:bg-blue-50/50 dark:group-hover:bg-blue-900/20"
                        >
                          <span
                            class="text-[10px] font-semibold text-gray-600 dark:text-gray-300 truncate leading-tight uppercase tracking-tight"
                            title={file.name}
                          >
                            {file.name}
                          </span>
                ...
                {#each columns as col (col.key)}
                  {#if col.visible}
                    <TableHeadCell
                      ...
                      class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 select-none"
                    >
                  ...
                  >
                    {#if columns.find((c) => c.key === 'type').visible}
                      <TableBodyCell
                        ...
                        title={getCategoryInfo(file)?.singularName || 'Unknown'}
                      >
                        <div class="flex items-center space-x-2 text-gray-600 dark:text-gray-300">
                          <svelte:component
                            this={getCategoryInfo(file)?.icon || File}
                            class="w-4 h-4"
                          />
                    ...
                    {#if columns.find((c) => c.key === 'title').visible}
                      <TableBodyCell
                        class="whitespace-nowrap text-gray-500 dark:text-gray-400 truncate max-w-[150px]"
                        title={file.title || ''}
                      >
                        {file.title || ''}
                      </TableBodyCell>
                    ...
                    {#if columns.find((c) => c.key === 'description').visible}
                      <TableBodyCell
                        class="whitespace-nowrap text-gray-500 dark:text-gray-400 truncate max-w-[200px]"
...
{#if showAddToGroupSubMenu && itemForAddToGroup}
  ...
  >
    <button
      on:click|stopPropagation={handleNewGroupClickInGroupView}
    ...
    {#if projectGroupsForMenu.length > 0}
      ...
      {#each projectGroupsForMenu as group (group.id)}
        <button
          on:click|stopPropagation={() => handleAddFileToExistingGroupInGroupView(group)}
          class="block w-full text-left px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-800 dark:text-gray-200 truncate"
          title={group.name}
        >
          {group.name}
        </button>
      {/each}
    {:else}
      <span class="block w-full text-left px-3 py-1.5 text-gray-400 dark:text-gray-500 italic"
        >No other groups</span
      >
    {/if}
    <!-- This closes the #if projectGroupsForMenu.length > 0 -->
  </div>
{/if}
<!-- This closes the #if showAddToGroupSubMenu && itemForAddToGroup -->
