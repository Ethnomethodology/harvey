<script>
    ...
    function drawWaveform() {
        ...
        try {
            ...
            const isFloatArray = !Number.isInteger(firstVal) && Math.abs(firstVal) < 2.0 && firstVal !== 0;
            ...
            if (isFloatArray) {
                ...
                if (uint8.byteOffset % 4 === 0) {
                    ...
                    rawPeaks = new Float32Array(uint8.buffer.slice(uint8.byteOffset, uint8.byteOffset + uint8.byteLength));
                }
            }
            ...
            console.debug(`[AudioThumbnail] ${file.name}: maxPeak=${maxObservedPeak.toFixed(4)}, boost=${normalizationFactor.toFixed(2)}x, visibleBars=${visibleBars}`);
...
<div class="w-full h-full relative bg-gray-50 dark:bg-gray-950 overflow-hidden flex items-center justify-center">
    ...
    {:else}
        ...
        {#if isLoading}
            <div class="absolute inset-0 flex items-center justify-center bg-gray-50/50 dark:bg-gray-950/50">
