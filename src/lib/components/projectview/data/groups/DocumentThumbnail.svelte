    ...
    if (node.type === 'table') {
      ...
      for (const rowNode of rowNodes) {
        if (rowNode.type === 'tablerow' && rowNode.children) {
          let cells = [];
          for (const cellNode of rowNode.children.slice(0, 10)) {
            // Up to 10 cols
            let cellSpans = [];
      ...
      const isBlock = ['paragraph', 'heading', 'listitem', 'quote', 'code', 'image'].includes(
        node.type
      );
      if (isBlock) {
        let spans = [];
        collectSpans(node, spans);
        if (spans.length > 0 || node.type === 'image') {
          // Image blocks might not have spans
          result.push({
            type: node.type,
            tag: node.tag || 'p',
...
<div
  class="w-full h-full p-2.5 overflow-hidden relative group/thumb transition-colors duration-200"
>
  {#if isLoading}
  ...
  {:else if error || blocks.length === 0}
    <div class="absolute inset-0 flex items-center justify-center text-gray-300 dark:text-gray-700">
      <svelte:component
        this={isTranscript ? MessageSquareText : FileText}
        size={40}
        strokeWidth={1}
      />
    </div>
  {:else}
    <div class="flex flex-col gap-1 select-none pointer-events-none">
      {#each blocks as block, i (i)}
        {#if block.type === 'table'}
          <div
            class="flex flex-col border-[0.5px] border-gray-200 dark:border-gray-800 rounded-sm mb-1 overflow-hidden"
          >
            {#each block.rows as row, j (j)}
              <div class="flex border-b-[0.5px] border-gray-100 dark:border-gray-900 last:border-0">
                {#each row.cells as cell, k (k)}
                  <div
                    class="flex-1 p-0.5 border-r-[0.5px] border-gray-100 dark:border-gray-900 last:border-0 min-w-0 overflow-hidden"
                  >
                    <div class="text-[6px] text-gray-400 dark:text-gray-500 truncate leading-tight">
                      {#each cell.spans as span, l (l)}
                        {#if span.type === 'date'}
                          <span
                            class="inline-flex items-center gap-0.5 bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200 px-1 py-0.5 rounded-full text-[6px] font-medium leading-none"
                          >
        ...
        {:else if block.type === 'image'}
          <div
            class="flex items-center gap-1 text-[7px] text-gray-400 dark:text-gray-600 bg-gray-50 dark:bg-gray-900 px-1 py-0.5 rounded-sm w-fit"
          >
        ...
        {:else}
          <div
            class="break-words leading-tight {block.type === 'heading'
              ? 'font-bold text-gray-700 dark:text-gray-300 text-[9px]'
              : 'text-gray-500 dark:text-gray-400 text-[8px]'} {block.type === 'code'
              ? 'font-mono bg-gray-50 dark:bg-gray-900 px-1 py-0.5 rounded-sm opacity-80'
              : ''}"
            style={block.type === 'quote'
              ? 'border-left: 2px solid rgba(156, 163, 175, 0.4); padding-left: 5px; font-style: italic;'
              : ''}
          >
            {#if block.type === 'listitem'}
              <span class="mr-1 opacity-40 inline-block">•</span>
            {/if}
            {#each block.spans as span, m (m)}
              {#if span.type === 'date'}
                <span
                  class="inline-flex items-center gap-0.5 bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200 px-1 py-0.5 rounded-full text-[6px] font-medium leading-none"
                >
    ...
    <div
      class="absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-white dark:from-gray-950 via-white/80 dark:via-gray-950/80 to-transparent pointer-events-none"
    ></div>
  {/if}
</div>
