...
<script>
  import {
    ...
    setSelectedGroup,
    currentProjectGroupsList,
    updateProjectGroupsList,
    HARVEY_FILES_DIR,
    MEDIA_DIR_NAME,
    AUDIOS_DIR_NAME,
    VIDEOS_DIR_NAME,
  ...
  async function handleImportTranscriptConfirm(event) {
    ...
    if (sourceType === 'msWord') {
      ...
      await message(`Import from "${sourceType}" is not supported.`, {
        title: 'Import Error',
        type: 'error'
      });
    }
    ...
    if (action === 'Open') {
      ...
      if (group && group.id) {
        const confirmed = await confirm(
          `Are you sure you want to delete the group "${group.name}"? This action cannot be undone, but the files within the group will not be deleted.`,
          {
        ...
        if (confirmed) {
          ...
          if (!currentProjectId) {
            message('Project ID is missing. Cannot delete group.', {
              title: 'Error',
              type: 'error'
            });
            return;
          ...
          try {
            ...
            await message(`Failed to delete group "${group.name}": ${err}`, {
              title: 'Delete Error',
              type: 'error'
            });
            project.update((p) => ({
              ...p,
              statusMessage: `Failed to delete group "${group.name}".`
            }));
          }
        }
      } else {
  ...
  async function handleContextMenuAction(action) {
    ...
    if (!item) {
      console.error('[DataLeftPanel] Context item null.');
      closeContextMenu();
      return;
    }
    ...
    if (action !== 'AddToGroup') {
      // Assuming 'AddToGroup' would be a specific action string if passed here
      closeContextMenu();
    }
    ...
    if (itemType === 'media') {
      switch (action) {
        ...
        case 'Rename':
          itemToRename = {
            path: item.path,
            name: item.media_xml_identifier,
            file_type: 'media',
        ...
        case 'Delete':
          const stemName =
            ...
            (item.name.includes('.')
              ...
              : item.name);
          const confirmMsg = `Delete media "${stemName}"? This deletes the entire folder (media, transcripts, data). Cannot be undone.`;
          const options = {
          ...
          try {
            const confirmed = await confirm(confirmMsg, options);
            if (confirmed) {
              project.update((p) => ({ ...p, statusMessage: `Deleting ${stemName}...` }));
              try {
                await deleteProjectItem(itemPathForClosure);
              } catch (err) {
                console.error(`[DataLeftPanel] Delete failed for ${stemName}:`, err);
              }
            } else {
              project.update((p) => ({ ...p, statusMessage: 'Deletion cancelled.' }));
            }
          } catch (e) {
            console.error('[DataLeftPanel] Error confirm/delete:', e);
            await message(`Error deleting: ${e}`, { title: 'Delete Error', type: 'error' });
          }
          break;

        default:
          console.warn(`[DataLeftPanel] Unknown action for media: ${action}`);
      }
      ...
      switch (action) {
        case 'Open':
          if (isPdf) {
            try {
              await openerPlugin.openPath(item.path);
            } catch (e) {
              await message(`Could not open PDF externally: ${e}`, {
                title: 'Open Error',
                type: 'error'
              });
            }
          } else {
            dispatch('requestviewchange', { viewType: 'documents', itemPath: item.path });
          }
          break;
        case 'Rename':
          itemToRename = {
            ...
            name: item.name,
            file_type: 'doc',
            media_xml_identifier: null
          };
          showRenameModal = true;
          break;
        case 'Delete':
          const confirmDocMsg = `Delete document "${item.name}"? Cannot be undone.`;
          const docOptions = {
          ...
          try {
            const confirmed = await confirm(confirmDocMsg, docOptions);
            if (confirmed) {
              project.update((p) => ({ ...p, statusMessage: `Deleting ${item.name}...` }));
              try {
                await deleteProjectItem(itemPathForClosure);
              } catch (err) {
                console.error(`[DataLeftPanel] Delete failed for ${item.name}:`, err);
              }
            } else {
              project.update((p) => ({ ...p, statusMessage: 'Deletion cancelled.' }));
            }
          ...
          break;
        default:
          console.warn(`[DataLeftPanel] Unknown action for document: ${action}`);
      }
    } else if (itemType === 'table') {
      switch (action) {
        case 'Open':
          dispatch('requestviewchange', { viewType: 'tables', itemPath: item.path });
          break;
        case 'Rename':
          itemToRename = {
            path: item.path,
          ...
          break;
        case 'Delete':
          const confirmTableMsg = `Delete table "${item.name}"? This cannot be undone.`;
          const tableOptions = {
          ...
          try {
            const confirmed = await confirm(confirmTableMsg, tableOptions);
            if (confirmed) {
              project.update((p) => ({ ...p, statusMessage: `Deleting ${item.name}...` }));
              try {
                await deleteProjectItem(itemPathForClosure);
              } catch (err) {
                await message(`Error deleting table: ${err}`, {
                  title: 'Delete Error',
                  type: 'error'
                });
              }
            } else {
              project.update((p) => ({ ...p, statusMessage: 'Table deletion cancelled.' }));
            }
          } catch (e) {
            await message(`Error deleting table: ${e}`, { title: 'Delete Error', type: 'error' });
          }
          break;
        default:
          console.warn(`[DataLeftPanel] Unknown action for table: ${action}`);
      }
    } else if (itemType === 'image') {
      switch (action) {
        case 'Open':
          dispatch('requestviewchange', { viewType: 'images', itemPath: item.path });
          break;
        case 'Rename':
          itemToRename = {
            path: item.path,
          ...
          break;
        case 'Delete':
          const confirmImageMsg = `Delete image "${item.name}"? This cannot be undone.`;
          const imageOptions = {
          ...
          try {
            const confirmed = await confirm(confirmImageMsg, imageOptions);
            if (confirmed) {
              project.update((p) => ({ ...p, statusMessage: `Deleting ${item.name}...` }));
              try {
                await deleteProjectItem(itemPathForClosure);
              } catch (err) {
                await message(`Error deleting image: ${err}`, {
                  title: 'Delete Error',
                  type: 'error'
                });
              }
            } else {
              project.update((p) => ({ ...p, statusMessage: 'Image deletion cancelled.' }));
            }
          } catch (e) {
            await message(`Error deleting image: ${e}`, { title: 'Delete Error', type: 'error' });
          }
          break;
        default:
          console.warn(`[DataLeftPanel] Unknown action for image: ${action}`);
      }
      ...
      switch (action) {
        ...
        case 'Rename':
          ...
          itemToRename = {
            path: item.path,
            name: nameWithoutExt,
            file_type: 'standalone_transcript',
            media_xml_identifier: null
          };
          showRenameModal = true;
          break;
        case 'Delete':
          const confirmTranscriptMsg =
            'Are you sure you want to delete this transcript? This cannot be undone.';
          const transcriptOptions = {
            title: 'Confirm Transcript Deletion',
            type: 'warning',
            okLabel: 'Delete',
            cancelLabel: 'Cancel'
          };
          try {
            ...
            if (confirmed) {
              try {
                await deleteStandaloneTranscript(item.path);
                project.update((p) => ({ ...p, currentStandaloneTranscriptPath: null }));
              } catch (err) {
                await message(`Error deleting transcript: ${err}`, {
                  title: 'Delete Error',
                  type: 'error'
                });
              }
            ...
            await message(`Error deleting transcript: ${e}`, {
              title: 'Delete Error',
              type: 'error'
            });
          }
          break;
        default:
          console.warn(`[DataLeftPanel] Unknown action for imported transcript: ${action}`);
      }
    }
  }
  ...
  async function handleImportClick(categoryType) {
    ...
    if (!categoryInfo || !categoryInfo.importEnabled) {
      message(`Import for ${categoryInfo?.name || 'this category'} not available.`, {
    ...
    if (categoryType === 'video' || categoryType === 'audio') {
      ...
      try {
        ...
        console.error(`[DataLeftPanel] Error importDocumentFile:`, e);
      }
      ...
      try {
        ...
        console.error(`[DataLeftPanel] Error importImageFile:`, e);
      }
    } else if (categoryType === 'standalone_transcript') {
      showImportTranscriptModal = true;
    } else {
      message(`Specific import for ${categoryInfo.name} not implemented.`, {
        title: 'Coming Soon',
        type: 'info'
      });
    }
    ...
    if (pendingSheetNamesToImport.length > 0) {
      ...
      try {
        ...
        if (isActivelyImportingTable) {
          ...
          message(`Error extracting sheet ${nextSheet}: ${errorMessage}`, {
            title: 'Import Error',
            type: 'error'
          });
  ...
  async function handleHeaderConfirmation(event) {
    ...
    console.log(
      `[DataLeftPanel] handleHeaderConfirmation: hasHeaders = ${hasHeaders}, schema =`,
      schema
    );
    ...
    try {
      ...
      message(`Error finalising table import: ${error.message || error}`, {
        title: 'Import Error',
        type: 'error'
      });
  ...
  async function triggerHeaderConfirmationCancel() {
    ...
    try {
      ...
      if (
        headerConfirmationData.tablePath &&
        !pathsToDelete.includes(headerConfirmationData.tablePath)
      ) {
      ...
      if (pathsToDelete.length > 0) {
        ...
        for (const path of pathsToDelete) {
          ...
          try {
            ...
            if (projectStoreVal.xmlPath) {
              await invoke('delete_project_item', {
                itemPath: path,
                projectXmlPath: projectStoreVal.xmlPath
              });
            }
          } catch (e) {
        ...
        await refreshProjectFiles();
        message('Table import cancelled. All imported files have been reverted.', {
  ...
  async function handleRenameConfirm(event) {
    ...
    if (!item || !newName || newName.trim() === '') {
      console.error('[DataLeftPanel] Rename failed: Invalid input.');
    ...
    if (item.file_type === 'media') {
      ...
      const confirmRename = await confirm(
        `Rename media '${item.media_xml_identifier}' to '${finalNewStemName}'? Renames folder & primary transcript.`,
        { title: 'Confirm Media Rename', type: 'warning', okLabel: 'Rename', cancelLabel: 'Cancel' }
      );
      if (!confirmRename) {
        itemToRename = null;
        return;
      ...
      try {
        await renameProjectItem(item.path, finalNewStemName, item.file_type);
      } catch (err) {
        console.error(`[DataLeftPanel] Rename failed for ${item.media_xml_identifier}:`, err);
      } finally {
        itemToRename = null;
      }
    } else if (item.file_type === 'doc') {
      const stemNameFromModal = finalNewNameFromModal;
      const originalExtension = item.name.includes('.')
        ? item.name.substring(item.name.lastIndexOf('.'))
        : '';
      if (!originalExtension) {
        await message(
          `Error: Original file '${item.name}' appears to have no extension. Cannot rename.`,
          { title: 'Rename Error', type: 'error' }
        );
        itemToRename = null;
        return;
      }
      const allowedExts = ['.json', '.pdf', '.md', '.txt'];
      if (!allowedExts.includes(originalExtension.toLowerCase())) {
        await message(
          `Error: Original file type '${originalExtension}' cannot be renamed via this interface.`,
          { title: 'Rename Error', type: 'error' }
        );
        itemToRename = null;
        return;
      ...
      try {
        await renameProjectItem(item.path, newNameWithOriginalExt, item.file_type);
      } catch (err) {
        console.error(`[DataLeftPanel] Rename failed for ${item.name}:`, err);
      } finally {
        itemToRename = null;
      }
      ...
      if (!originalExtension) {
        await message(
          `Error: Original table file '${item.name}' appears to have no extension. Cannot rename.`,
          { title: 'Rename Error', type: 'error' }
        );
        itemToRename = null;
        return;
      ...
      if (!allowedTableExts.includes(originalExtension.toLowerCase())) {
        await message(
          `Error: Original table file type '${originalExtension}' cannot be renamed like this.`,
          { title: 'Rename Error', type: 'error' }
        );
        itemToRename = null;
        return;
      ...
      try {
        ...
        console.error(`[DataLeftPanel] Rename failed for table ${item.name}:`, err);
      } finally {
        itemToRename = null;
      }
      ...
      if (!originalExtension) {
        await message(
          `Error: Original image file '${item.name}' appears to have no extension. Cannot rename.`,
          { title: 'Rename Error', type: 'error' }
        );
        itemToRename = null;
        return;
      ...
      if (!allowedImageExts.includes(originalExtension.toLowerCase())) {
        await message(
          `Error: Original image file type '${originalExtension}' cannot be renamed like this.`,
          { title: 'Rename Error', type: 'error' }
        );
        itemToRename = null;
        return;
      ...
      try {
        ...
        console.error(`[DataLeftPanel] Rename failed for image ${item.name}:`, err);
      } finally {
        itemToRename = null;
      }
    } else if (item.file_type === 'standalone_transcript') {
      const nameForBackend = finalNewNameFromModal.endsWith('.json')
        ? finalNewNameFromModal
        : `${finalNewNameFromModal}.json`;
      try {
        await renameProjectItem(item.path, nameForBackend, item.file_type);
      } catch (err) {
        console.error(`[DataLeftPanel] Rename failed for imported transcript ${item.name}:`, err);
      } finally {
        itemToRename = null;
      }
    } else {
  ...
  onMount(async () => {
    ...
    const listener = (event) => {
      ...
      if (
        contextMenuVisible &&
        mainContextMenuElement &&
        !mainContextMenuElement.contains(event.target)
      ) {
        closeContextMenu();
      }
      ...
      const groupItemContextMenuElement = document.getElementById(
        'notes-left-panel-group-item-context-menu'
      );
      if (
        groupContextMenuVisible &&
        groupItemContextMenuElement &&
        !groupItemContextMenuElement.contains(event.target)
      ) {
        closeGroupContextMenu();
      }
    };
  ...
  function handleNewGroupClick() {
    ...
    if (
      ...
      String($project.id) === 'null'
    ) {
      // Changed to $project.id
      message(
        'Project ID is not available from $project.id. Cannot create a new group. Please ensure the project is fully loaded.',
  ...
  async function handleAddFileToExistingGroup(group) {
    if (!groupSubMenuItem || !$project || !$project.id) {
      // Changed from project_uuid to id
      await message('Cannot add to group: Missing item or project context (project ID).', {
        title: 'Error',
        type: 'error'
      }); // Updated message
      closeGroupSubMenu();
    ...
    if (!relativePath) {
      await message('Cannot add to group: Item relative path is missing.', {
        title: 'Error',
        type: 'error'
      });
      closeGroupSubMenu();
    ...
    try {
      ...
      project.update((p) => ({
        ...p,
        statusMessage: `File ${groupSubMenuItem.name} added to group ${group.name}.`
      }));
  ...
  const LS_KEY_DATA_PANEL_STATE = 'harveyDataPanelCategoryState';
  ...
  onMount(() => {
    const defaultState = {};
    CATEGORIES_BASE.forEach((cat) => {
      defaultState[cat.type] = true;
    });
    try {
      const savedState = localStorage.getItem(LS_KEY_DATA_PANEL_STATE);
      if (savedState) {
        const parsedState = JSON.parse(savedState);
        categoryOpenState = { ...defaultState, ...parsedState };
      } else {
        categoryOpenState = defaultState;
      }
    } catch (e) {
      console.error('[DataLeftPanel] Failed load category state:', e);
      categoryOpenState = defaultState;
    }
  });
  function toggleCategory(categoryType) {
    if (categoryOpenState.hasOwnProperty(categoryType)) {
      categoryOpenState[categoryType] = !categoryOpenState[categoryType];
      categoryOpenState = categoryOpenState;
    } else {
      console.warn(`[DataLeftPanel] Toggle unknown category: ${categoryType}`);
    }
  }
  $: if (Object.keys(categoryOpenState).length > 0) {
    try {
      localStorage.setItem(LS_KEY_DATA_PANEL_STATE, JSON.stringify(categoryOpenState));
    } catch (e) {
      console.error('[DataLeftPanel] Failed save category state:', e);
    }
  }
  ...
  $: displayCategories = (() => {
    ...
    let documents = projectDocumentFiles
      .map((docXml) => {
        const fullPath = $project.baseDirectory
          ? normalizePath(`${$project.baseDirectory}/${docXml.relativePath}`)
          : docXml.relativePath;
        return {
          name: docXml.name,
          path: fullPath,
          relativePath: docXml.relativePath,
          file_type: 'doc'
        };
      })
      .sort((a, b) => a.name.localeCompare(b.name));

    let tables = projectTableFiles
      .map((tableXml) => {
        const fullPath = $project.baseDirectory
          ? normalizePath(`${$project.baseDirectory}/${tableXml.relativePath}`)
          : tableXml.relativePath;
        return {
          name: tableXml.name,
          path: fullPath,
          relativePath: tableXml.relativePath,
          file_type: 'table'
        };
      })
      .sort((a, b) => a.name.localeCompare(b.name));

    let images = projectImageFiles
      .map((imageXml) => {
        const fullPath = $project.baseDirectory
          ? normalizePath(`${$project.baseDirectory}/${imageXml.relativePath}`)
          : imageXml.relativePath;
        const assetUrl = fullPath ? convertFileSrc(fullPath) : null;
        return {
          name: imageXml.name,
          path: fullPath,
          relativePath: imageXml.relativePath,
          file_type: 'image',
          assetUrl
        };
      })
    ...
    let standaloneTranscripts = projectStandaloneTranscriptFiles
      .map((tsXml) => {
        const fullPath = $project.baseDirectory
          ? normalizePath(`${$project.baseDirectory}/${tsXml.relativePath}`)
          : tsXml.relativePath;
        return {
          name: tsXml.name,
          path: fullPath,
          relativePath: tsXml.relativePath,
          file_type: 'standalone_transcript'
        };
      })
      .sort((a, b) => a.name.localeCompare(b.name));
    ...
    function findMediaFilesRecursive(nodes) {
      ...
      for (const node of nodes) {
        if (node.file_type === 'media' && !node.is_directory && node.path) {
          const relativePathOriginal =
            node.relativePath ||
            ($project.baseDirectory && node.path.startsWith($project.baseDirectory)
              ? node.path.substring($project.baseDirectory.length + 1)
              : node.path);
    ...
    return CATEGORIES_BASE.map((cat) => {
      if (cat.type === 'video') {
        return { ...cat, files: videos };
      } else if (cat.type === 'audio') {
        return { ...cat, files: audios };
      } else if (cat.type === 'document') {
        return { ...cat, files: documents };
      } else if (cat.type === 'table') {
        return { ...cat, files: tables };
      } else if (cat.type === 'image') {
        return { ...cat, files: images };
      } else if (cat.type === 'standalone_transcript') {
        return { ...cat, files: standaloneTranscripts };
      } else {
        return { ...cat, files: [] };
      }
    });
  ...
  $: {
    ...
    if (autoPath && autoPath !== prevAutoOpenPath) {
      ...
      if (AUDIO_EXTENSIONS.has(extension) || VIDEO_EXTENSIONS.has(extension)) {
        ...
        $project.standaloneTranscriptFiles?.some(
          (f) =>
            f.relativePath &&
            normalizePath(`${$project.baseDirectory}/${f.relativePath}`) === autoPath
        )
      ) {
        itemCategoryType = 'standalone_transcript';
      } else if (
        lowerPath.endsWith('.pdf') ||
        (lowerPath.endsWith('.json') && !itemCategoryType) ||
      ...
      if (itemCategoryType) {
        ...
        if (itemCategory) {
          ...
          console.warn(
            `[DataLeftPanel] Auto open path ${autoPath} (type ${itemCategoryType}) NOT FOUND in current displayCategories.`
          );
        }
      }
    }
      ...
      if (selectedItemPathInStore && $project.baseDirectory) {
        ...
        if (AUDIO_EXTENSIONS.has(extension)) determinedItemType = 'audio';
          ...
          const projectFileListsForOthers = [
            {
              files: $project.standaloneTranscriptFiles,
              type: 'standalone_transcript',
              isRelative: true,
              exts: TRANSCRIPT_EXTENSIONS
            },
            ...
            {
              files: $project.documentFiles,
              type: 'document',
              isRelative: true,
              exts: new Set(['pdf', 'txt', 'md', ...DOC_JSON_EXTENSIONS])
            }
          ];
          for (const listInfo of projectFileListsForOthers) {
            if (
              listInfo.files?.some((f) => {
                const filePathToCheck = listInfo.isRelative
                  ? normalizePath(`${$project.baseDirectory}/${f.relativePath}`)
                  : f.path;
                return (
                  filePathToCheck === path && (listInfo.exts ? listInfo.exts.has(extension) : true)
                );
              })
            ) {
...
<div class="h-full bg-white dark:bg-gray-900 flex flex-col overflow-hidden">
  {#if !panelState.dataLeftPanelCollapsed}
    ...
    <div class="flex flex-col flex-grow overflow-hidden" bind:this={panelContainer}>
      ...
      <div
        class="flex-grow overflow-y-auto min-h-0 px-2"
        style="flex-basis: {categoriesHeightPercent}%;"
      >
        <ul class="space-y-2 text-xs">
          {#each filteredCategories as category (category.type)}
            <li>
              <div
                class="flex items-center justify-between group mb-1 pr-1 py-1 cursor-pointer select-none hover:bg-gray-100 dark:hover:bg-gray-800 ${categoryContextMenuVisible &&
                categoryContextMenuType === category.type
                  ? 'bg-gray-100 dark:bg-gray-800'
                  : ''}"
                on:click={() => toggleCategory(category.type)}
                role="button"
                aria-expanded={categoryOpenState[category.type] ?? true}
                aria-controls={`category-content-${category.type}`}
                tabindex="0"
                on:keydown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') toggleCategory(category.type);
                }}
              >
                <div class="flex items-center space-x-1.5 text-gray-600 dark:text-gray-600">
                  <span class="flex-shrink-0 w-4 h-4 flex items-center justify-center">
                    {@html categoryOpenState[category.type] ? CHEVRON_DOWN_SVG : CHEVRON_RIGHT_SVG}
                  </span>
                  <span class="flex-shrink-0"
                    ><svelte:component this={category.iconComponent} class="w-4 h-4" /></span
                  >
                  <span class="font-medium text-gray-700 dark:text-gray-400">{category.name}</span>
                </div>
                <button
                  type="button"
                  class="ml-2 flex-shrink-0 text-gray-400 dark:text-gray-700 hover:text-gray-600 dark:hover:text-gray-400 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity ${categoryContextMenuVisible &&
                  categoryContextMenuType === category.type
                    ? 'opacity-100'
                    : ''}"
                  title="Options"
              ...
              {#if categoryOpenState[category.type]}
                <div id={`category-content-${category.type}`} role="region">
                      ...
                      {#each category.files as fileItem (fileItem.path || fileItem.relativePath)}
                        <li class="group">
                          <div
                            ...
                            on:keydown={(e) => {
                              if (e.key === 'Enter' || e.key === ' ') handleItemClick(fileItem);
                            }}
                          >
                            <span
                              class="flex items-center space-x-1 text-gray-800 dark:text-gray-200 truncate"
                            ...
                            <button
                              ...
                              title="Options for {fileItem.name}"
                              on:click|stopPropagation={(e) => handleItemContextMenu(e, fileItem)}
                            >
                              {@html CONTEXT_MENU_ICON_SVG}
                            </button>
                          </div>
                  ...
                  {:else if category.type === 'video' || category.type === 'audio' || category.type === 'document' || category.type === 'table' || category.type === 'image' || category.type === 'standalone_transcript'}
                    <p class="ml-9 text-xs text-gray-400 dark:text-gray-700 italic py-1">
        ...
        {#if $project.isLoading}
          <p class="text-xs text-gray-500 dark:text-gray-600 italic px-1 py-2">
      ...
      <div
        class="flex-grow overflow-y-auto min-h-0 px-2 pt-2"
        style="flex-basis: {100 - categoriesHeightPercent}%;"
      >
        <h3
          class="flex items-center justify-between text-xs font-semibold text-gray-500 dark:text-gray-600 px-1 mb-1.5 group"
        >
        ...
        <ul class="ml-2 space-y-0.5 border-l border-gray-200 dark:border-gray-700 text-xs">
          {#if $currentProjectGroupsList && $currentProjectGroupsList.length > 0}
            {#each $currentProjectGroupsList as group (group.id)}
              <li class="group">
                <div
                  ...
                  tabindex="0"
                  on:keydown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') handleGroupSelected(group);
                  }}
                  title={group.name}
                >
                  <span
                    class="flex items-center text-gray-800 dark:text-gray-200 truncate"
            ...
            {/each}
          {:else}
            <p class="ml-2.5 text-xs text-gray-400 dark:text-gray-700 italic py-1">
  ...
  {:else}
    ...
    <div
      class="flex flex-col items-center space-y-2 pt-2 flex-grow overflow-y-auto min-h-0 w-full px-1"
    >
    ...
    {#each CATEGORIES_BASE as category (category.type)}
      <Dropdown
        triggeredBy="#collapsed-category-{category.type}"
        trigger="hover"
        placement="right-start"
        class="w-64 z-[1001] shadow-xl border border-gray-200 dark:border-gray-700"
      >
        <div
          class="px-4 py-2 font-bold border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-200 flex justify-between items-center"
        >
          <span>{category.name}</span>
          {#if category.type === 'document' || category.type === 'table'}
            <button
              id="add-btn-{category.type}"
              class="p-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors"
              title="Add New"><Plus class="w-4 h-4 text-blue-600 dark:text-blue-400" /></button
            >
            <Dropdown
              triggeredBy="#add-btn-{category.type}"
              placement="right-start"
              class="w-36 z-[1002]"
            ...
            >
              <DropdownItem
                on:click={(e) => {
                  ...
                  if (category.type === 'document') {
                    const p = get(project);
                    if (p?.xmlPath) createNewDocument(p.xmlPath);
                  } else {
              ...
              <DropdownItem
                on:click={(e) => {
                  e.stopPropagation();
                  handleImportClick(category.type);
                }}
              >
                Import...
              </DropdownItem>
          ...
          {:else}
            <button
              ...
              on:click={(e) => {
                e.stopPropagation();
                handleImportClick(category.type);
              }}><Plus class="w-4 h-4 text-blue-600 dark:text-blue-400" /></button
            >
          {/if}
        </div>
        <div class="max-h-96 overflow-y-auto">
          {#if (filteredCategories.find((fc) => fc.type === category.type)?.files?.length || 0) > 0}
            {#each filteredCategories.find((fc) => fc.type === category.type)?.files || [] as file (file.path || file.name)}
              <DropdownItem
                class="truncate text-sm flex items-center py-1.5 {file.path ===
                selectedItemPathInStore
                  ? 'bg-blue-50 dark:bg-gray-700 font-semibold text-blue-700 dark:text-blue-400'
                  : ''}"
                on:click={() => handleItemClick(file)}
                title={file.name}
              >
                <svelte:component
                  this={category.iconComponent}
                  class="w-3.5 h-3.5 mr-2 text-gray-400 dark:text-gray-500 shrink-0"
                />
    ...
    <!-- Dropdown for Groups -->
    <Dropdown
      triggeredBy="#collapsed-category-groups"
      trigger="hover"
      placement="right-start"
      class="w-64 z-[1001] shadow-xl border border-gray-200 dark:border-gray-700"
    >
      ...
      >
        ...
        <button
          ...
          on:click={(e) => {
            e.stopPropagation();
            handleNewGroupClick();
          }}><Plus class="w-4 h-4 text-black dark:text-white" /></button
        >
      </div>
      <div class="max-h-96 overflow-y-auto">
        {#if $currentProjectGroupsList && $currentProjectGroupsList.length > 0}
          {#each $currentProjectGroupsList as group (group.id)}
            <DropdownItem
              class="truncate text-sm flex items-center py-1.5 {$project.selectedGroupId ===
              group.id
                ? 'bg-blue-50 dark:bg-gray-700 font-semibold text-blue-700 dark:text-blue-400'
                : ''}"
              on:click={() => handleGroupSelected(group)}
  ...
  {#if contextMenuVisible && contextMenuItem && !panelState.dataLeftPanelCollapsed}
    ...
    >
      {#if contextMenuItem.file_type === 'media'}
        ...
        <button
          on:mouseenter={(e) => {
            handleShowAddToGroupSubMenu(e, contextMenuItem);
          }}
        ...
        >
          ...
          <svg
            ...
            height="16"
            fill="currentColor"
            class="bi bi-chevron-right w-3 h-3"
            viewBox="0 0 16 16"
            ><path
              fill-rule="evenodd"
        ...
        <button
          on:click|stopPropagation={() => {
            handleContextMenuAction('Reveal');
          }}
          class="block w-full text-left px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-800 dark:text-gray-200"
      ...
      {:else if contextMenuItem.file_type === 'standalone_transcript'}
        ...
        >
      {:else}
        <span class="block w-full text-left px-3 py-1.5 text-gray-400 dark:text-gray-700 italic"
    ...
    >
      ...
      {#if localProjectGroupsForSubMenu && localProjectGroupsForSubMenu.length > 0}
        {#each localProjectGroupsForSubMenu as group (group.id)}
          <button
            on:click|stopPropagation={() => {
              handleAddFileToExistingGroup(group);
            }}
            class="flex items-center w-full text-left px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-800 dark:text-gray-200 truncate"
            title="{group.name} {group.description ? '(' + group.description + ')' : ''}"
          >
            <!-- Removed folder icon from here -->
            <span class="ml-1">{group.name}</span>
            <!-- Added ml-1 for slight indent if needed, or adjust as per design -->
          </button>
        {/each}
      {:else}
        <span class="block w-full text-left px-3 py-1.5 text-gray-400 dark:text-gray-700 italic"
          >No existing groups</span
        >
      {/if}
    </div>
  {/if}
  ...
  {#if categoryContextMenuVisible && !panelState.dataLeftPanelCollapsed}
    ...
    >
      ...
      {:else}
            ...
            on:click|stopPropagation={() => {
              if (categoryContextMenuType === 'document') {
                ...
                if (currentProject && currentProject.xmlPath) {
                  ...
                  message('Could not create document: Project path is not available.', {
                    title: 'Error',
                    type: 'error'
                  });
                }
        ...
        <button
          on:click|stopPropagation={() => {
            handleImportClick(categoryContextMenuType);
            closeCategoryContextMenu();
          }}
          class="block w-full text-left px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-800 dark:text-gray-200"
...
<FileRenameModal
  ...
  isMediaRename={itemToRename?.file_type === 'media'}
...
<ImportTranscriptSourceModal
  bind:showModal={showImportTranscriptModal}
  on:confirm={(event) => handleImportTranscriptConfirm(event)}
  on:close={() => (showImportTranscriptModal = false)}
/>
...
<GroupRenameModal
  ...
  on:save={async (event) => {
    ...
    try {
      ...
      await message(`Failed to rename group '${oldGroupName}': ${err}`, {
        title: 'Rename Error',
        type: 'error'
      });
...
{#if groupContextMenuVisible && groupContextMenuItem}
  ...
  >
    <button
      on:click={() => handleGroupContextMenuAction('Open')}
      class="block w-full text-left px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-800 dark:text-gray-200"
      >Open</button
    >
    <button
      on:click={() => handleGroupContextMenuAction('Rename')}
      class="block w-full text-left px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-800 dark:text-gray-200"
      >Rename...</button
    >
    <hr class="my-1 border-gray-200 dark:border-gray-800" />
    <button
      on:click={() => handleGroupContextMenuAction('Delete')}
      class="block w-full text-left px-3 py-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/50 dark:text-red-500"
      >Delete...</button
    >
  </div>
...
<style lang="postcss">
  ...
  .overflow-y-auto::-webkit-scrollbar-thumb {
    @apply rounded bg-gray-400 bg-opacity-50 dark:bg-gray-700 dark:bg-opacity-50;
  }
  .overflow-y-auto::-webkit-scrollbar-thumb:hover {
    @apply bg-gray-500 bg-opacity-70 dark:bg-gray-600 dark:bg-opacity-70;
  }
  .overflow-y-auto {
