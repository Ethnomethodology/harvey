<script>
  ...
  function attemptSetupSync() {
    console.log(
      '[StandaloneTranscriptView] attemptSetupSync called. splitPartnerPath:',
      splitPartnerPath
    );
    cleanupSync();
    let attempts = 0;
  ...
  onMount(() => {
    console.log(
      '[StandaloneTranscriptView] Component container mounted. Transcript path:',
      itemPath
    );
...
>
  ...
  <div class="flex-grow min-h-0 overflow-hidden {mediaPath && !isVideoHidden ? 'h-1/2' : 'h-full'}">
    {#if splitPartnerPath}
      <div
        class="flex h-full w-full divide-gray-300 dark:divide-gray-600 {orientation === 'horizontal'
          ? 'flex-row divide-x'
          : 'flex-col divide-y'}"
      >
        <div
