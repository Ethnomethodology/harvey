...
* **Format Conversion:** If a standalone transcript is loaded and its JSON structure is just an array of segment objects (legacy format), `TranscriptEditorPanel` uses `@lexical/headless` to parse the HTML strings and programmatically generate a valid `@lexical/table` structure *before* presenting it to the user.
