...
<script>
  ...
  export function openLexicalDocument(docPath) {
    if (tableViewerPanelRef && typeof tableViewerPanelRef.openLexicalDocument === 'function') {
      ...
      console.warn(
        '[TableView] tableViewerPanelRef or openLexicalDocument method not available yet.'
      );
      return false;
    }
  }
...
<div class="h-full flex-grow min-w-0 bg-white dark:bg-gray-800">
  {#key itemPath}
    ...
    {:else}
      <div
        class="h-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-gray-500"
      >
