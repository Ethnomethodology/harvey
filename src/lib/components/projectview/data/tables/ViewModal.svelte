<script>
  ...
  $: normalizedTablePath = (() => {
    ...
    let relative = tablePath.startsWith(projectStoreState.baseDirectory)
      ? tablePath.substring(projectStoreState.baseDirectory.length)
      : tablePath;
  ...
  const viewTypes = [
    {
      value: 'partial',
      name: 'Partial Table View',
      description: 'Select specific columns and apply filters',
      icon: 'Table2'
    },
    {
      value: 'pivot',
    ...
    {
      value: 'survey',
      name: 'Survey Data Table',
      description: 'Organise survey responses into documents',
      icon: 'FileText'
    }
  ];
    ...
    if (dataSource && dataSource !== 'Base Table') {
      ...
      if (selectedView) {
        try {
          ...
          const { transformedData, transformedColumns, transformedSchema } = applyViewConfigToData(
            tableData,
            columns,
            schema,
            config,
  ...
  $: requiredOrPrimaryColumns = allColumns
    .filter((c) => {
  ...
  $: if (selectedViewType === 'partial' && partialSelectedColumns) {
    const missingRequired = requiredOrPrimaryColumns.filter(
      (f) => !partialSelectedColumns.includes(f)
    );
    if (missingRequired.length > 0) {
      // Re-assign to trigger reactivity and keep required columns selected
      partialSelectedColumns = [
        ...new Set([...partialSelectedColumns, ...requiredOrPrimaryColumns])
      ];
    }
  }
  ...
  function resetForm() {
    ...
    const isActiveViewPartial = views.some(
      (v) => v.view_name === activeViewName && v.view_type === 'partial'
    );
    ...
    try {
      ...
      if (selectedViewType === 'partial') {
        ...
        if (config.valueFields && config.valueFields.length > 0) {
          ...
          pivotValueFields = [
            { field: config.valueField, aggregation: config.aggregation || 'Sum' }
          ];
        } else {
          pivotValueFields = [{ field: '', aggregation: 'Sum' }];
        }
        ...
        surveyParticipantIncludedFields =
          config.surveyParticipantIncludedFields || allColumns.map((c) => c.value);
        ...
        if (selectedViewType === 'survey') {
          ...
          try {
            ...
            notificationStore.add(
              'Survey documents successfully generated. They are now available under the Attachments Panel.',
              'success'
            );
          } catch (genError) {
  ...
  function generatePivotData(
    ...
    if (!rowFields || rowFields.length === 0 || actualValueFields.length === 0)
    ...
    setTimeout(() => {
      ...
      if (!previewTabulatorInstance) {
        ...
        if (partialFilterField && partialFilterValue) {
          previewTabulatorInstance.setFilter(
            partialFilterField,
            partialFilterOperator || 'like',
            partialFilterValue
          );
        }
      }
  ...
  $: if (open && isEditingExisting && selectedViewType === 'pivot') {
    ...
    if (pivotRowFields && pivotRowFields.length > 0 && validValueFields.length > 0) {
      generatedPivotResult = generatePivotData(
        activeData,
        pivotRowFields,
        pivotColFields,
        validValueFields[0].field,
        validValueFields[0].aggregation,
        validValueFields
      );
    } else {
...
>
  ...
  <div
    class="flex-1 flex overflow-hidden -m-6 h-full border-t border-gray-200 dark:border-gray-700"
  >
    <!-- Left Sidebar: Create / Open Existing -->
    <div
      class="border-r border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden bg-gray-50 dark:bg-gray-800 transition-all duration-300 w-80"
      style={activeTab === 'create' && isEditingExisting && selectedViewType === 'survey'
        ? 'display: none;'
        : ''}
    >
      {#if !(activeTab === 'create' && isEditingExisting)}
        <div class="flex border-b border-gray-200 dark:border-gray-700">
          <button
            class="flex-1 py-3 text-sm font-medium border-b-2 {activeTab === 'create'
              ? 'border-blue-600 text-blue-600 dark:border-blue-500 dark:text-blue-500 bg-white dark:bg-gray-900'
          ...
          <button
            class="flex-1 py-3 text-sm font-medium border-b-2 {activeTab === 'existing'
              ? 'border-blue-600 text-blue-600 dark:border-blue-500 dark:text-blue-500 bg-white dark:bg-gray-900'
              : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 hover:dark:text-gray-300'}"
            on:click={() => handleTabChange('existing')}
          >
            <div class="flex items-center justify-center gap-2">
              <FolderOpen size={16} /> Existing
            </div>
          </button>
        </div>
      {/if}
        ...
        {#if activeTab === 'create'}
          <div class="space-y-4">
            {#if !isEditingExisting}
              ...
              <div
                class="text-sm text-gray-500 dark:text-gray-400 italic pt-4 border-t border-gray-200 dark:border-gray-700"
              >
              ...
              <Accordion flush>
                <AccordionItem>
                  <span slot="header" class="flex items-center"
                    ><Table2 class="w-4 h-4 mr-2" />General Details</span
                  >
                  <div class="space-y-4">
                    <div>
                      <Label for="viewName" class="mb-2">View Name</Label>
                ...
                {#if selectedViewType === 'partial'}
                  <AccordionItem open>
                    <span slot="header" class="flex items-center"
                      ><Table2 class="w-4 h-4 mr-2" />Data Mapping</span
                    >
                    <div class="space-y-4">
                      <div>
                        ...
                        <Select id="dataSource" bind:value={dataSource}>
                          ...
                          {#each views as view (view.view_name)}
                      ...
                      <div>
                        ...
                        <MultiSelect
                          items={allColumns}
                          bind:value={partialSelectedColumns}
                          placeholder="Select fields to display"
                        />
                        <Helper class="mt-2"
                          >Columns not selected will be hidden in this view. <span
                            class="font-semibold text-blue-600 dark:text-blue-400"
                            >Primary and required fields cannot be hidden.</span
                          ></Helper
                        >
                      </div>
                    </div>
                  ...
                  <AccordionItem>
                    <span slot="header" class="flex items-center"
                      ><Filter class="w-4 h-4 mr-2" />Default Filter</span
                    >
                    <div class="space-y-4">
                      <div>
                        ...
                        <Select
                          id="filterField"
                          items={[{ value: '', name: '-- No Filter --' }, ...allColumns]}
                          bind:value={partialFilterField}
                        />
                      </div>
                      {#if partialFilterField}
                        <div>
                          ...
                          <Select
                            ...
                            bind:value={partialFilterOperator}
                          />
                        </div>
                        <div>
                ...
                {:else if selectedViewType === 'pivot'}
                  <AccordionItem open>
                    <span slot="header" class="flex items-center"
                      ><LayoutGrid class="w-4 h-4 mr-2" />Data Mapping</span
                    >
                    <div class="space-y-4">
                      <div>
                        <Label for="dataSource" class="mb-2">Data Source</Label>
                      ...
                      <div>
                        ...
                        <MultiSelect
                          id="pivotCol"
                          items={allColumns}
                          bind:value={pivotColFields}
                          placeholder="Select column fields"
                        />
                      </div>
                      <div>
                        <Label class="mb-2">Values to Aggregate</Label>
                        {#each pivotValueFields as vf, index (index)}
                          <div class="flex items-center gap-2 mb-2">
                            <Select
                              class="flex-1"
                              items={pivotValueOptions}
                              bind:value={vf.field}
                            ...
                            <Button
                              ...
                              class="p-2"
                              on:click={() =>
                                (pivotValueFields = pivotValueFields.filter((_, i) => i !== index))}
                              disabled={pivotValueFields.length === 1}
                            >
        ...
        {:else if activeTab === 'existing'}
          {#if existingViews.length === 0}
            <div class="text-sm text-gray-500 dark:text-gray-400 italic">
              No existing views found for this table.
            </div>
          {:else}
            <ul class="space-y-2">
              {#each existingViews as view (view.view_name)}
                <li>
                  <div
                    class="w-full flex items-center justify-between p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors group"
                  >
                    <button
                      class="flex-1 text-left flex flex-col items-start pr-2 overflow-hidden"
                      on:click={() => selectExistingView(view)}
                    >
                      <div class="font-medium text-gray-900 dark:text-white truncate w-full">
                        {view.view_name}
                      </div>
                      <div
                        class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mt-1"
                      >
                        {view.view_type} View
                      </div>
                    </button>
                    <div class="flex flex-col items-end gap-1 flex-shrink-0">
                      <Button
                        size="xs"
                        color="light"
                        class="p-1 border-0 shadow-none opacity-0 group-hover:opacity-100 transition-opacity bg-transparent hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/30"
                        on:click={() => deleteView(view.view_name)}
                        title="Delete"
                      >
    ...
    <div class="flex-1 flex flex-col overflow-hidden bg-white dark:bg-gray-900 relative">
      {#if activeTab === 'create' && !isEditingExisting}
        <div class="p-8 h-full overflow-y-auto bg-gray-50/30 dark:bg-gray-900/30 flex flex-col">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            {#each viewTypes as type (type.value)}
              <button
              ...
              >
                <div
                  class="w-16 h-16 bg-blue-50 dark:bg-blue-900/30 rounded-xl flex items-center justify-center text-blue-600 dark:text-blue-400 flex-shrink-0"
                >
                ...
                <div class="flex flex-col pt-1">
                  <span class="font-bold text-lg text-gray-900 dark:text-white mb-1"
                    >{type.name}</span
                  >
      ...
      {:else if activeTab === 'create' && isEditingExisting}
        {#if selectedViewType === 'partial'}
          <div
            class="flex-1 w-full h-full p-4 bg-gray-50/50 dark:bg-gray-900/50 border-l border-gray-100 dark:border-gray-800 flex flex-col"
          >
            <div
              class="mb-2 text-sm font-semibold text-gray-600 dark:text-gray-400 flex items-center gap-2"
            >
              <Table2 size={16} /> Live Preview: {viewName}
            </div>
            <div
              class="flex-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-sm overflow-hidden"
        ...
        {:else if selectedViewType === 'pivot'}
          ...
          >
            ...
            <div
              class="flex-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-sm overflow-auto"
            >
              <table
                    ...
                    {#each generatedPivotResult.colHeaders as headerRow, levelIndex (levelIndex)}
                      <tr>
                        ...
                        {#if levelIndex === generatedPivotResult.colHeaders.length - 1}
                          {#each pivotRowFields as rowField (rowField)}
                            <th
                              scope="col"
        ...
        {:else if selectedViewType === 'survey'}
          <div
            class="flex-1 w-full h-full bg-white dark:bg-gray-900 overflow-y-auto custom-scrollbar p-8"
          >
            <div class="max-w-3xl mx-auto space-y-8">
              <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
                  This tool will generate individual documents from your survey data table,
                  organizing the responses based on your configuration below.
                </p>
              </div>

              <div
                class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700"
              >
                <div class="col-span-1 md:col-span-2">
                  ...
                  <Input
                    id="surveyViewName"
                    bind:value={viewName}
                    placeholder="E.g., Survey_2026"
                  />
                </div>
                <div class="col-span-1 md:col-span-2">
                  <Label for="surveyViewDescription" class="mb-2 font-semibold"
                    >Description (Optional)</Label
                  >
                  <Textarea
                    id="surveyViewDescription"
                    bind:value={viewDescription}
                    placeholder="What is this survey data view for?"
                    rows="2"
                  />
                </div>
              </div>
              ...
              <div class="space-y-6">
                <div>
                  <Label class="mb-2 text-lg font-bold"
                    >How would you like to organize your survey data?</Label
                  >
                  <div class="flex gap-4">
                    <button
                      class="flex-1 p-4 border rounded-lg text-left transition-all {surveyGroupByType ===
                      'Participants'
                    ...
                    >
                      ...
                      <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        One document will be created for each participant response.
                      </div>
                    </button>
                    <button
                      class="flex-1 p-4 border rounded-lg text-left transition-all {surveyGroupByType ===
                    ...
                    >
                      ...
                      <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        One document will be created for each selected question.
                      </div>
                    </button>
                  </div>
                </div>
                ...
                <div
                  class="bg-white dark:bg-gray-900 p-6 rounded-lg border border-gray-200 dark:border-gray-700 space-y-6 shadow-sm"
                >
                  <div>
                    <Label for="uniqueIdField" class="mb-2 font-semibold"
                      >Unique Identifier of the Participant</Label
                    >
                    <Select
                      id="uniqueIdField"
                      items={allColumns}
                      bind:value={surveyUniqueIdentifierField}
                      placeholder="Select a field that uniquely identifies each participant"
                    />
                  </div>
                  ...
                  {#if surveyGroupByType === 'Participants'}
                    <div>
                      ...
                      <div
                        class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 max-h-60 overflow-y-auto bg-gray-50 dark:bg-gray-800"
                      >
                            ...
                            <Checkbox
                              ...
                              on:change={(e) => {
                                if (e.target.checked) {
                                  ...
                                  surveyParticipantIncludedFields =
                                    surveyParticipantIncludedFields.filter((f) => f !== col.value);
                                }
                              }}
                            >
                              {col.name}
                      ...
                      <Helper class="mt-2 text-blue-600 dark:text-blue-400 font-medium">
                        This will create {activeData.length} document{activeData.length === 1
                          ? ''
                          : 's'} (one for each participant), containing the selected fields.
                      </Helper>
                    </div>
                  {:else if surveyGroupByType === 'Questions'}
                    <div>
                      ...
                      <MultiSelect
                        items={allColumns}
                        bind:value={surveySelectedQuestions}
                        placeholder="Select the columns that represent questions"
                      />
                    </div>
                    <div>
                      ...
                      <MultiSelect
                        items={allColumns}
                        bind:value={surveyIncludedOtherFields}
                        placeholder="Select other fields to include besides the questions and identifier"
                      />
                      <Helper class="mt-2 text-blue-600 dark:text-blue-400 font-medium">
                        This will create {surveySelectedQuestions.length} document{surveySelectedQuestions.length ===
                        1
                          ? ''
                          : 's'} (one for each question selected). Each document will list all participant
                        responses.
                      </Helper>
                    </div>
                  {/if}
                ...
                <div class="flex justify-end pt-4">
                  <Button
                    ...
                    disabled={!viewName ||
                      !surveyUniqueIdentifierField ||
                      (surveyGroupByType === 'Participants'
                        ? surveyParticipantIncludedFields.length === 0
                        : surveySelectedQuestions.length === 0)}
                  >
