<script>
    ...
    .filter((c) => {
      ...
      return activeData.some(
        (row) =>
          ...
          row[c.field] !== '' &&
          !isNaN(parseFloat(row[c.field])) &&
          isFinite(row[c.field])
      );
    })
    .map((c) => ({ value: c.field, name: c.title }));

  $: dateColumns = activeColumns
    .map((c) => {
      const fieldName = typeof c.getField === 'function' ? c.getField() : c.field;
      const title = fieldName; // Ignore c.title because it is an HTML element in TableViewerPanel
      return { field: fieldName, title };
  ...
  $: categoricalColumns = activeColumns
    .map((c) => {
      const fieldName = typeof c.getField === 'function' ? c.getField() : c.field;
      const title = fieldName; // Ignore c.title because it is an HTML element in TableViewerPanel
      return { field: fieldName, title };
    ...
    .filter((c) => {
      ...
      if (
        colSchema.type === 'Misc' &&
        (colSchema.subType === 'Selectbox' || colSchema.subType === 'Multiselect')
      )
        return true;
      return false;
    })
  ...
  function selectExistingChart(chart) {
    ...
    try {
      ...
      ganttShowGridLines =
        config.ganttShowGridLines !== undefined ? config.ganttShowGridLines : true;
  ...
  async function deleteChart(name) {
    ...
    const confirmed = await ask(`Are you sure you want to delete ${targetName}?`, {
      title: 'Delete Chart',
      type: 'warning'
    });
    if (!confirmed) return;
  ...
  function renderChart() {
    ...
    try {
      if (
        ...
        let validData = activeData.filter(
          (row) => row[xAxisCol] !== null && row[xAxisCol] !== undefined && row[xAxisCol] !== ''
        );
        if (aggregationType !== 'Count') {
          validData = validData.filter(
            (row) => row[yAxisCol] !== null && row[yAxisCol] !== undefined && row[yAxisCol] !== ''
          );
        }
        ...
        const seriesArray = seriesNames.map((sName) => {
          ...
          if (
            valType === 'Stacked' ||
            valType === '100% Stacked' ||
            valType === 'Stacked Line' ||
            valType === '100% Stacked Line'
          ) {
            sConfig.stack = 'total';
          }
              ...
              color:
                selectedChartType === 'line'
                  ...
                  : valueLabelPosition === 'Inside End'
                    ...
                    : '#000',
              formatter:
                valType === '100% Stacked' || valType === '100% Stacked Line' ? '{c}%' : '{c}'
            };
          }
        ...
        if (showLegend) {
          ...
          if (legendPosition === 'Top')
            option.grid.top = Math.max(
              option.grid.top,
              titlePosition === 'Top' ? (hasSubtext ? 90 : 70) : 40
            );
          if (legendPosition === 'Bottom')
            option.grid.bottom = Math.max(
              option.grid.bottom,
              titlePosition === 'Bottom' ? (hasSubtext ? 90 : 70) : 40
            );
        }
        ...
        if (selectedChartType === 'bar') {
          ...
          if (
            (selectedChartType === 'column' && barType === '100% Stacked') ||
            (selectedChartType === 'line' && lineType === '100% Stacked Line')
          ) {
        ...
        const validData = activeData.filter(
          (row) =>
            row[xAxisCol] !== null &&
            row[xAxisCol] !== undefined &&
            row[xAxisCol] !== '' &&
            row[yAxisCol] !== null &&
            row[yAxisCol] !== undefined &&
            row[yAxisCol] !== ''
        );
        ...
        if (showLegend) {
          ...
          if (legendPosition === 'Top')
            seriesConfig.center[1] = titlePosition === 'Top' ? '60%' : '55%';
          if (legendPosition === 'Bottom')
            seriesConfig.center[1] = titlePosition === 'Bottom' ? '40%' : '45%';
        }

        option.series = [seriesConfig];
      } else if (selectedChartType === 'gantt') {
        if (!yAxisCol || !taskCol || !startDateCol || !endDateCol) {
          chartInstance.clear();
          return;
        }
        ...
        const validData = activeData.filter(
          (row) =>
            ...
            row[yAxisCol] !== '' &&
            row[taskCol] !== null &&
            row[taskCol] !== undefined &&
            row[taskCol] !== '' &&
            row[startDateCol] !== null &&
            row[startDateCol] !== undefined &&
            row[startDateCol] !== '' &&
            row[endDateCol] !== null &&
            row[endDateCol] !== undefined &&
            row[endDateCol] !== ''
        );
        ...
        option.tooltip = {
          ...
          formatter: function (params) {
            ...
            return (
              `<div class="font-bold mb-1">${lane}</div>` +
...
>
  ...
  >
    ...
    <div
      class="w-80 border-r border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden bg-gray-50 dark:bg-gray-800"
    >
      {#if !(activeTab === 'create' && isEditingExisting)}
                ...
                <Accordion flush>
                  <AccordionItem open>
                    <span slot="header" class="flex items-center"
                      ><Database class="w-4 h-4 mr-2" />Data Mapping</span
                    >
                    <div class="space-y-4">
                      <div>
                      ...
                      {:else}
                        <div>
                          <Label for="xAxisCol" class="mb-2"
                            >{selectedChartType === 'scatter'
                              ? 'X-Axis Values'
                              : 'Categories (' +
                                (selectedChartType === 'bar' ? 'Y' : 'X') +
                                '-Axis)'}</Label
                          >
                          <Select
                            id="xAxisCol"
                            items={selectedChartType === 'scatter'
                              ? numericColumns
                              : categoricalColumns}
                            bind:value={xAxisCol}
                          />
                        ...
                        <div>
                          <Label for="yAxisCol" class="mb-2"
                            >{selectedChartType === 'scatter'
                              ? 'Y-Axis Values'
                              : 'Values (' +
                                (selectedChartType === 'bar' ? 'X' : 'Y') +
                                '-Axis)'}</Label
                          >
                          <div class="flex gap-2">
                            <Select
                        ...
                        <div>
                          ...
                          <Select
                            id="breakdownCol"
                            items={[{ value: '', name: '-- None --' }, ...categoricalColumns]}
                            bind:value={breakdownCol}
                          />
                        </div>
                      {/if}
                    </div>
                  ...
                  <AccordionItem>
                    <span slot="header" class="flex items-center"
                      ><Palette class="w-4 h-4 mr-2" />Appearance</span
                    >
                    <div class="space-y-4">
                      {#if selectedChartType === 'bar' || selectedChartType === 'column'}
                        <div>
                          ...
                          <Select
                            id="barType"
                            items={[
                              { value: 'Clustered', name: 'Clustered' },
                              { value: 'Stacked', name: 'Stacked' },
                              { value: '100% Stacked', name: '100% Stacked' }
                            ]}
                            bind:value={barType}
                          />
                        </div>
                        <div>
                          ...
                          <Select
                            ...
                            items={[
                              ...
                              { value: 'Highest', name: 'Highest values first (Leaderboard)' },
                              { value: 'Lowest', name: 'Lowest values first' },
                              { value: 'A-Z', name: 'Alphabetical (A-Z)' }
                            ]}
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            items={[
                              ...
                              { value: 'Stacked Line', name: 'Stacked Line' },
                              { value: '100% Stacked Line', name: '100% Stacked Line' }
                            ]}
                            bind:value={lineType}
                          />
                        </div>
                      ...
                      {:else if selectedChartType === 'scatter'}
                        <div>
                          ...
                          <Select
                            id="scatterConnectPoints"
                            items={[
                              { value: 'None', name: 'None' },
                              { value: 'Line', name: 'Line' },
                              { value: 'Line with Markers', name: 'Line with Markers' }
                            ]}
                            bind:value={scatterConnectPoints}
                          />
                        </div>
                        <div>
                          ...
                          <Select
                            ...
                            items={[
                              { value: 'None', name: 'None' },
                              { value: 'Linear Regression', name: 'Linear Regression' }
                            ]}
                            bind:value={scatterTrendline}
                          />
                        </div>
                      {:else if selectedChartType === 'pie'}
                        <div>
                      ...
                      {:else if selectedChartType === 'gantt'}
                        <div>
                          ...
                          <Select
                            id="ganttTaskDisplay"
                            items={[
                              { value: 'Stacked', name: 'Stacked (Shared Lanes)' },
                              { value: 'Clustered', name: 'Clustered (Separate Rows)' }
                            ]}
                            bind:value={ganttTaskDisplay}
                          />
                        </div>
                        <div>
                          <Label for="ganttXAxisInterval" class="mb-2">X-Axis Interval</Label>
                          <Select
                            id="ganttXAxisInterval"
                            items={[
                            ...
                            bind:value={ganttXAxisInterval}
                          />
                        </div>
                        <div class="pt-2">
                          <Checkbox bind:checked={ganttShowGridLines}
                            >Show Vertical Grid Lines</Checkbox
                          >
                        </div>
                        <div class="pt-2">
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          items={[
                            ...
                            { value: 'Soft Pastels', name: 'Soft Pastels' },
                            { value: 'Warm Pastels', name: 'Warm Pastels' },
                            { value: 'Warm Sunset', name: 'Warm Sunset' },
                            { value: 'Ocean Blues', name: 'Ocean Blues' }
                          ]}
                          bind:value={colorPalette}
                        />
                        <div class="flex flex-wrap justify-center gap-1 mt-2">
                          {#each palettes[colorPalette] || palettes['Modern'] as color (color)}
                            <div
                  ...
                  <AccordionItem>
                    <span slot="header" class="flex items-center"
                      ><Type class="w-4 h-4 mr-2" />Labels</span
                    >
                    <div class="space-y-4">
                      <div>
                      ...
                      {#if selectedChartType === 'bar' || selectedChartType === 'column' || selectedChartType === 'gantt'}
                        <div>
                          <Label for="yAxisWidth" class="mb-2"
                            >{selectedChartType === 'bar' || selectedChartType === 'gantt'
                              ? 'Y'
                              : 'X'}-Axis Label Width: {yAxisWidthLimit}px</Label
                          >
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            items={[
                              ...
                              { value: 'Wrap', name: 'Wrap to next line' }
                            ]}
                            bind:value={longTextHandling}
                          />
                        </div>
                      {/if}
                      <div class="pt-2">
                      ...
                      {#if showValueLabels}
                        <div>
                          ...
                          <Select
                            ...
                            items={[
                              { value: 'Inside End', name: 'Inside End (White text)' },
                              { value: 'Outside End', name: 'Outside End (Dark text)' }
                            ]}
                            bind:value={valueLabelPosition}
                          />
                        </div>
                      {/if}
                    </div>
        ...
        {:else if activeTab === 'existing'}
          {#if existingCharts.length === 0}
            <div class="text-sm text-gray-500 dark:text-gray-400 italic">
              No existing charts found for this table.
            </div>
          {:else}
            <ul class="space-y-2">
              {#each existingCharts as chart (chart.chart_name)}
                <li>
                  <div
                  ...
                  >
                    ...
                    <div class="flex flex-col items-end gap-1 flex-shrink-0">
                      ...
                      <Button
                        ...
                        on:click={() => deleteChart(chart.chart_name)}
                        title="Delete"
                      >
        ...
        <div class="p-8 h-full overflow-y-auto bg-gray-50/30 dark:bg-gray-900/30 flex flex-col">
          ...
          <div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {#each chartTypes as type (type.value)}
              ...
              >
                <div
                  class="absolute inset-0 bg-blue-50/0 group-hover:bg-blue-50/50 dark:group-hover:bg-blue-900/10 transition-colors z-0"
                ></div>
                <div
                  class="w-14 h-14 bg-blue-50 dark:bg-blue-900/30 rounded-full flex items-center justify-center mb-3 text-blue-600 dark:text-blue-400 group-hover:scale-110 transition-transform shadow-sm z-10"
                >
                  ...
                  {#if type.icon === 'SquareChartGantt'}<SquareChartGantt
                      size={24}
                      strokeWidth={2}
                    />{/if}
                </div>
                <span class="font-medium text-sm text-gray-900 dark:text-white z-10"
                  >{type.name}</span
                >
              </button>
            {/each}
