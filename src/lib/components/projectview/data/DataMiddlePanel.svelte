...
<script>
  ...
  const highlightOptions = [
    ...
    { value: '#AEEFFA', label: 'Light Blue' },
    { value: '#FFC0CB', label: 'Pink' },
    { value: '#D3D3D3', label: 'Gray' },
    { value: '#90EE90', label: 'Light Green' }
  ];

  onMount(() => {
    // (onMount logic unchanged)
    ...
    editor = createEditor({
      ...
      onError: (error) => {
        console.error('[DataMiddlePanel Minimal Editor] Error:', error);
      },
    ...
    if (editorContainer) {
      ...
      unregisterMinimalListeners = mergeRegister(
        editor.registerUpdateListener(({ editorState }) => {
          editorState.read(() => {
            updateMinimalToolbarState();
          });
        }),
        editor.registerCommand(
          SELECTION_CHANGE_COMMAND,
          () => {
            if (isReady) updateMinimalToolbarState();
            return false;
          },
          COMMAND_PRIORITY_LOW
        ),
      ...
      tick().then(() => {
        if (editor) {
          ...
          const isNowEditable = editor.isEditable();
          console.log(
            `[DataMiddlePanel Minimal Editor] editor.isEditable() returned: ${isNowEditable}`
          );
          if (isNowEditable) {
            editor.focus(() => {
              console.log('[DataMiddlePanel Minimal Editor] Focus callback.');
            });
            isReady = true;
            editor.getEditorState().read(updateMinimalToolbarState);
          } else {
            console.error('[DataMiddlePanel Minimal Editor] FAILED TO SET EDITABLE.');
          }
        }
      });
    } else {
      console.error('[DataMiddlePanel Minimal Editor] editorContainer not found!');
    }
    return () => {
      if (editor) {
        console.log('[DataMiddlePanel Minimal Editor] Destroying editor.');
        unregisterMinimalListeners();
        editor = null;
      }
      isReady = false;
    };
  ...
  function updateMinimalToolbarState() {
    ...
    if (_isRangeSelection(selection)) {
      ...
      selectedTextColor = _getSelectionStyleValueForProperty(selection, 'color', '#000000');
      selectedHighlightColor = _getSelectionStyleValueForProperty(
        selection,
        'background-color',
        'transparent'
      );
      ...
      let element =
        _findMatchingParent(
          anchorNode,
          (node) => _isElementNode(node) && node.getType() !== 'root'
        ) || anchorNode;
      if (element) {
        ...
        if (_isElementNode(element)) {
          selectedAlignment = element.getFormatType() || 'left';
        }
      }
    }
    isBold = isBold;
  ...
  function applyStyle(styleName, value) {
    ...
    editor.focus(() => {
      editor.update(() => {
        const selection = _getSelection();
        if (_isRangeSelection(selection)) {
          _patchStyleText(selection, { [styleName]: value });
        }
      });
    });
  }
  function applyTextColor(event) {
    const color = event.target.value === 'null' ? null : event.target.value;
    ...
    editor.focus(() => {
      if (type === 'paragraph' || type === 'h1' || type === 'h2' || type === 'h3') {
        ...
        editor.update(() => {
          const selection = _getSelection();
          if (_isRangeSelection(selection)) {
            _setBlocksType(selection, formatFunction);
          }
        });
      } else if (type === 'ul') {
      ...
      editor.update(() => {
        ...
        if (_isRangeSelection(selection)) {
          try {
            ...
            console.log(
              ' -> Cleared inline formats/styles and attempted to set block to paragraph.'
            );
          } catch (error) {
            // Log error if getNodes or subsequent operations fail
            console.error(
              '[DataMiddlePanel Minimal Editor] Error during manual clearFormatting:',
              error
            );
          }
        }
...
<div class="h-full flex flex-col bg-white dark:bg-gray-900 rounded-md shadow p-2 space-y-2">
  ...
  <div
    class="toolbar flex items-center flex-wrap gap-x-1 gap-y-1 border-b border-gray-300 dark:border-gray-700 pb-1 mb-1 flex-shrink-0"
  >
    <select
      class="mini-toolbar-select"
      value={blockType}
    ...
    >
    <button
      class="mini-toolbar-button line-through"
    ...
    <select
      ...
      on:change={(e) => alignElement(e.target.value)}
      title="Alignment"
    >
    ...
    <div class="separator"></div>
    <button class="mini-toolbar-button" on:click={outdentContent} title="Outdent">OD</button>
    <button class="mini-toolbar-button" on:click={indentContent} title="Indent">ID</button>
    <div class="separator"></div>
    <select
      class="mini-toolbar-select"
      ...
      title="Text Color"
    >
      {#each colorOptions as option (option.value ?? 'null')}
        <option value={option.value ?? 'null'} style="color: {option.value ?? 'inherit'};"
          >{option.label}</option
        >
      {/each}
    </select>
    ...
    >
      {#each highlightOptions as option (option.value ?? 'null')}
        <option
          value={option.value ?? 'null'}
          style="background-color: {option.value ?? 'inherit'}; color: {option.value
            ? 'black'
            : 'inherit'};">{option.label}</option
        >
      {/each}
    </select>
    <div class="separator"></div>
    <button class="mini-toolbar-button" on:click={clearFormatting} title="Clear Formatting"
      >Clear</button
    >
  </div>
  <!-- Wrapper Div (Unchanged) -->
  <div
    class="lexical-wrapper flex-grow min-h-[100px] overflow-y-auto border border-dashed border-green-400 p-1"
    bind:this={editorWrapper}
  >
    <div
      bind:this={editorContainer}
      class="lexical-minimal-content focus:outline-none min-h-full"
...
<style lang="postcss">
  ...
  .toolbar select.mini-toolbar-select {
    @apply px-2 py-0.5 border border-gray-300 dark:border-gray-700 rounded bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-xs disabled:opacity-50 focus:outline-none focus:ring-1 focus:ring-blue-500;
    margin-right: 2px;
  }
  .toolbar select.mini-toolbar-select {
    height: 26px;
  ...
  .lexical-minimal-content {
    outline: none;
    caret-color: currentcolor;
    height: 100%;
  }
  .lexical-minimal-content p {
    @apply mb-1;
  }
  .lexical-minimal-content h1 {
    @apply text-2xl font-bold mb-1 mt-2;
  }
  .lexical-minimal-content h2 {
    @apply text-xl font-semibold mb-1 mt-1;
  }
  .lexical-minimal-content h3 {
    @apply text-lg font-semibold mb-1;
  }
  .lexical-minimal-content ul {
    @apply list-disc list-inside mb-1 pl-4;
  }
  .lexical-minimal-content ol {
    @apply list-decimal list-inside mb-1 pl-4;
  }
  .lexical-minimal-content li {
    @apply mb-0.5 pl-1;
  }
  .lexical-minimal-content [style*='text-decoration-line: underline line-through'],
  .lexical-minimal-content [style*='text-decoration: underline line-through'] {
    text-decoration: underline line-through;
  }
  .lexical-minimal-content .text-left {
    text-align: left;
  }
  .lexical-minimal-content .text-center {
    text-align: center;
  }
  .lexical-minimal-content .text-right {
    text-align: right;
  }
  .lexical-minimal-content .text-justify {
    text-align: justify;
  }
  .lexical-wrapper::-webkit-scrollbar {
    @apply w-[8px] h-[8px];
  }
  .lexical-wrapper::-webkit-scrollbar-track {
    @apply bg-gray-100 dark:bg-gray-900 rounded-lg;
  }
  .lexical-wrapper::-webkit-scrollbar-thumb {
    @apply bg-gray-400 dark:bg-gray-700 rounded-lg border-2 border-solid border-gray-100 dark:border-gray-900;
  }
  .lexical-wrapper::-webkit-scrollbar-thumb:hover {
