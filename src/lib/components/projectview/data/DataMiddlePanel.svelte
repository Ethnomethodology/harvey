...
<!-- Main container -->
<div class="h-full flex flex-col bg-white dark:bg-surface-2 rounded-md shadow p-2 space-y-2">
    <div class="text-xs text-gray-500 dark:text-d-gray-400">Minimal Test Editor (Dropdowns):</div>
...
<style lang="postcss">
    /* Styling (Unchanged) */
    .toolbar button.mini-toolbar-button, .toolbar select.mini-toolbar-select { @apply px-2 py-0.5 border border-gray-300 dark:border-border rounded bg-gray-100 dark:bg-d-gray-700 hover:bg-gray-200 dark:hover:bg-d-gray-600 text-xs disabled:opacity-50 focus:outline-none focus:ring-1 focus:ring-blue-500; margin-right: 2px; } .toolbar select.mini-toolbar-select { height: 26px; } .toolbar button.mini-toolbar-button.active { @apply bg-blue-100 dark:bg-blue-800 border-blue-300 dark:border-blue-600 text-blue-800 dark:text-blue-200; } .lexical-wrapper { position: relative; } .lexical-minimal-content { outline: none; caret-color: currentcolor; height: 100%; }
	...
	.lexical-wrapper::-webkit-scrollbar { @apply w-[8px] h-[8px]; } .lexical-wrapper::-webkit-scrollbar-track { @apply bg-gray-100 dark:bg-d-gray-800 rounded-lg; } .lexical-wrapper::-webkit-scrollbar-thumb { @apply bg-gray-400 dark:bg-d-gray-500 rounded-lg border-2 border-solid border-gray-100 dark:border-d-gray-800; } .lexical-wrapper::-webkit-scrollbar-thumb:hover { @apply bg-gray-500 dark:bg-d-gray-400; } .lexical-wrapper { scrollbar-width: thin; scrollbar-color: theme('colors.gray.400') theme('colors.gray.100'); } html.dark .lexical-wrapper { scrollbar-color: theme('colors.d-gray.500') theme('colors.d-gray.800'); }
</style>