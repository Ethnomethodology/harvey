...
{#if showDialog}
<div
    class="annotation-dialog absolute z-[1001] bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-xl p-3 flex flex-col space-y-2"
    style={popupStyle}>
    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Add Annotation Details</h3>

    <div class="flex flex-col">
        <label for="annotation-title" class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Title (Optional)</label>
        <input
            type="text"
            id="annotation-title"
            bind:value={title}
            class="w-full px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-blue-500 focus:border-blue-500"
            placeholder="Enter title"
        />
    </div>

    <div class="flex flex-col">
        <label for="annotation-description" class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description (Optional)</label>
        <textarea
            id="annotation-description"
            bind:value={description}
            class="w-full px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-blue-500 focus:border-blue-500 h-16 resize-y"
            placeholder="Enter description"
        ></textarea>
    </div>

    <div class="flex flex-col">
        <span class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Color (Mandatory)</span>
