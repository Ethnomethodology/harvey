<script>
    ...
    try {
      ...
      if (includeAnnotations) {
        ...
        for (const annotation of annotations) {
          ...
          const borderColorBody = body.find(
            (b) => b.purpose === 'rendering' && b.type === 'BorderColor'
          );
          const borderSizeBody = body.find(
            (b) => b.purpose === 'rendering' && b.type === 'BorderSize'
          );

          const fillColor = colorBody ? colorBody.value : 'rgba(255, 242, 117, 0.5)';
          const strokeColor = borderColorBody
            ? borderColorBody.value
            : fillColor.includes('255, 255, 255')
              ? 'rgba(156, 163, 175, 1)'
              : adjustOpacity(fillColor, 1);
          ...
          if (s === 'censored' || s === 'censored-circle') {
            if (s === 'censored') {
              pixelateRegion(
                shapeData.x * S * sx,
                shapeData.y * S * sy,
                shapeData.width * S * sx,
                shapeData.height * S * sy
              );
            } else {
              // Circular censorship
        ...
        for (const annotation of annotations) {
          ...
          const textColorBody = body.find(
            (b) => b.purpose === 'rendering' && b.type === 'TextColor'
          );
          ...
          if ((textBody && textBody.value) || htmlBody) {
            ...
            const renderRichText = (
              ...
              segments.forEach((seg) => {
                ...
                parts.forEach((part) => {
                  if (part === '\n') {
                    ...
                    const fontFamily = seg.fontFamily
                      ? `${seg.fontFamily}, sans-serif`
                      : 'sans-serif';
                    ...
                    words.forEach((word) => {
                      ...
                      if (wordWidth > effectiveAvailableW && word.trim() !== '') {
                        ...
                        if (
                          currentX + wordWidth > effectiveAvailableW &&
                          currentX > 0 &&
                          !isSpace
                        ) {
                          lines.push([]);
              ...
              const lineHeights = lines.map((line) => {
                ...
                return (
                  Math.max(...line.map((s) => s.fontSize), baseFontSize) * LINE_HEIGHT_MULTIPLIER
                );
              });
              ...
              lines.forEach((line, i) => {
                ...
                while (
                  ...
                  /^\s*$/.test(visibleLine[visibleLine.length - 1].text)
                )
                  visibleLine.pop();
                ...
                const lineBaseline =
                  currentY + (h - maxFontSizeInLine) / 2 + maxFontSizeInLine * 0.8;
                ...
                line.forEach((seg) => {
                  ...
                  if (renderMode !== 'text') {
                    if (seg.highlight && seg.highlight !== 'transparent') {
                      ...
                      const inkTop =
                        lineBaseline -
                        (m.actualBoundingBoxAscent || seg.fontSize * 0.8) -
                        safetyPadding;
                      const inkBottom =
                        lineBaseline +
                        (m.actualBoundingBoxDescent || seg.fontSize * 0.2) +
                        safetyPadding;
            ...
            const padding =
              s === 'rectangle' || s === 'speech-bubble-rect' || s === 'text-area' ? 4 : 8;
  ...
  function adjustOpacity(rgbaColor, newOpacity) {
    if (!rgbaColor || typeof rgbaColor !== 'string' || !rgbaColor.startsWith('rgba(')) {
      return rgbaColor;
    }
  ...
  function getBubbleTailInfo(shapeData, isCircle, S = 1000) {
    ...
    if (isCircle) {
      ...
      const baseHalfWidth =
        tailWidth !== undefined ? (tailWidth * S) / 2 : Math.max(sizeParam * 0.1, 0.01 * S);
      ...
      const arc = (p, r, sweep = 1) =>
        r > 0 ? `A ${r} ${r} 0 0 ${sweep} ${p.x} ${p.y}` : `L ${p.x} ${p.y}`;

      if (side === 'top') {
        bx = Math.max(
          TL.x + R + baseHalfWidth,
          Math.min(TR.x - R - baseHalfWidth, center.x + dir.x * t)
        );
        ...
        const curvePart =
          tailStyle === 'curved'
            ? (() => {
                ...
                const cp = {
                  x: bc.x + spineV.x * 0.5 + perpV.x,
                  y: bc.y + spineV.y * 0.5 + perpV.y
                };
        ...
        by = Math.max(
          TR.y + R + baseHalfWidth,
          Math.min(BR.y - R - baseHalfWidth, center.y + dir.y * t)
        );
        ...
        bx = Math.max(
          BL.x + R + baseHalfWidth,
          Math.min(BR.x - R - baseHalfWidth, center.x + dir.x * t)
        );
        ...
        // left
        by = Math.max(
          TL.y + R + baseHalfWidth,
          Math.min(BL.y - R - baseHalfWidth, center.y + dir.y * t)
        );
  ...
  async function initializeViewer(pathForImage) {
    ...
    if (!pathForImage || !osdViewerElement) {
      console.warn('[ImageViewerPanel initializeViewer] Skipping: no path or osdViewerElement.', {
        pathForImage,
        osdViewerElementExists: !!osdViewerElement
      });
      isLoading = false;
      error = 'Viewer element not ready or path missing.';
      return;
    }

    console.log(
      `[ImageViewerPanel initializeViewer] Proceeding with initialization for: ${pathForImage}`
    );
    ...
    if (osdViewer) {
      try {
        osdViewer.destroy();
        console.log('Previous OpenSeadragon instance destroyed.');
      } catch (e) {
        console.warn('Error destroying previous OpenSeadragon instance:', e);
      }
      osdViewer = null;
    }
    if (osdViewerElement) {
    ...
    try {
      ...
      console.error(
        `[ImageViewerPanel initializeViewer] Top-level error for path ${pathForImage}:`,
        err
      );
  ...
  function setupDrawingEvents() {
    if (!osdViewer) {
      console.warn('setupDrawingEvents: OpenSeadragon viewer not available.');
      return;
    }
  ...
  function onMouseDown(event) {
    ...
    if (activeDrawingTool === 'polygon') {
      ...
      if (
        ...
        activeDrawingTool === 'speech-bubble-rect' ||
        activeDrawingTool === 'text-area' ||
        activeDrawingTool === 'censored'
      ) {
          ...
          return {
            ...
            target: {
              ...
              selector: {
                ...
                value: {
                  ...selector,
                  tail: { x: currentViewportPoint.x, y: currentViewportPoint.y }
                }
              }
            }
          };
    ...
    if (isDraggingTailWidth && draggedAnnotationId) {
      const updatedAnnotations = $currentAnnotations.map((a) => {
        if (a.id === draggedAnnotationId) {
          ...
          if (info) {
            if (isCircle) {
              ...
              const tailAngle = Math.atan2(
                selector.tail.y - selector.cy,
                selector.tail.x - selector.cx
              );
          ...
          return {
            ...a,
            target: { ...a.target, selector: { ...a.target.selector, value: selector } }
          };
        }
        return a;
      });
      ...
      const updatedAnnotations = $currentAnnotations.map((a) => {
        if (a.id === draggedAnnotationId) {
          ...
          if (
            ...
            selector.shape === 'speech-bubble-rect' ||
            selector.shape === 'text-area' ||
            selector.shape === 'censored'
          ) {
            selector.x += dx;
            selector.y += dy;
          } else if (
            selector.shape === 'circle' ||
            selector.shape === 'speech-bubble-circle' ||
            selector.shape === 'censored-circle' ||
            selector.shape === 'text-area-circle'
          ) {
            selector.cx += dx;
            selector.cy += dy;
      ...
      const updatedAnnotations = $currentAnnotations.map((a) => {
        if (a.id === draggedAnnotationId) {
          ...
          if (
            ...
            if (!isNaN(idx)) {
              selector.points[idx] = {
                x: selector.points[idx].x + dx,
                y: selector.points[idx].y + dy
              };
            }
          }
          return {
      ...
      if (viewportRect.width > 0.001 && viewportRect.height > 0.001) {
        ...
        if (isSpeech) {
          ...
          shapeData.tail = {
            ...
            y: viewportRect.y + viewportRect.height + 0.05
          };
          ...
          body: [
            {
              ...
              value: isCensored
                ? 'url(#censoredPattern)'
                : getSelectedColor(isSpeech || isTextArea),
    ...
    const newAnnotation = {
      ...
      target: {
        selector: {
          ...
          value: { shape: 'polygon', points: currentPolygon.points }
        }
      },
  ...
  async function handleAnnotationDialogUpdate(event) {
    ...
    if (shape && shape !== updatedSelector.shape) {
      ...
      if (
        shape === 'circle' &&
        (oldShape === 'rectangle' || oldShape === 'text-area' || oldShape === 'censored')
      ) {
        ...
        shape === 'rectangle' &&
        (oldShape === 'circle' ||
          oldShape === 'speech-bubble-circle' ||
          oldShape.endsWith('-circle'))
      ) {
        ...
        if (oldShape === 'censored-circle' || oldShape === 'censored')
          updatedSelector.shape = 'censored';
        else if (oldShape === 'text-area-circle' || oldShape === 'text-area')
          updatedSelector.shape = 'text-area';
    ...
    const newBody = [{ type: 'Color', value: color, purpose: 'highlighting' }];
    ...
    if (borderColor)
      newBody.push({ type: 'BorderColor', value: borderColor, purpose: 'rendering' });
  ...
  function startTailWidthDrag(event, annotationId) {
    ...
    const mousePoint = new OpenSeadragon.Point(
  ...
  async function handleAnnotationDialogCancel() {
    if (!isEditingExisting && annotationBeingEdited) {
      const annotationsWithoutNew = $currentAnnotations.filter(
        (a) => a.id !== annotationBeingEdited.id
      );
  ...
  async function handleAnnotationDialogDelete() {
    if (annotationBeingEdited) {
      const annotationsWithoutDeleted = $currentAnnotations.filter(
        (a) => a.id !== annotationBeingEdited.id
      );
  ...
  onMount(() => {
    ...
    if (imagePath && osdViewerElement) {
      initializeViewer(imagePath);
    } else {
      isLoading = false;
      console.log('[ImageViewerPanel onMount] No imagePath or osdViewerElement, not initializing.');
    }
  });
...
<div class="flex flex-col h-full w-full bg-white dark:bg-gray-900 shadow overflow-hidden">
  ...
  >
    <div id="image-annotation-toolbar-container" class="flex items-center">
      <!-- Highlights Group -->
      <div
        class="flex items-center space-x-1 border-r border-gray-300 dark:border-gray-600 pr-2 mr-2"
      >
        <span
          class="text-[10px] font-semibold text-gray-500 dark:text-gray-400 mr-1 uppercase tracking-wider hidden lg:inline-block"
          >Highlight</span
        >
        <button
          ...
          on:click={() =>
            (activeDrawingTool = activeDrawingTool === 'rectangle' ? null : 'rectangle')}
        ...
        {#each baseColors as color, i (i)}
          <button
            ...
            style="background: {color.rgb === 'transparent'
              ? 'linear-gradient(45deg, rgba(255,255,255,1) 45%, rgba(255,0,0,1) 45%, rgba(255,0,0,1) 55%, rgba(255,255,255,1) 55%)'
              : `rgba(${color.rgb}, 1)`};"
  ...
  <div class="flex-grow overflow-hidden min-h-0 relative">
    {#if isLoading && !error}
      <div
        class="absolute inset-0 flex items-center justify-center text-gray-500 dark:text-gray-600 z-10 bg-white/50 dark:bg-gray-900/50"
      >
        Loading image viewer...
      </div>
    {:else if error}
      <div
        class="absolute inset-0 flex items-center justify-center text-red-600 dark:text-red-400 p-4 text-center z-10 bg-white/80 dark:bg-gray-900/80"
      >
        {error}
      </div>
    {/if}
    <div
      bind:this={osdViewerElement}
      class="w-full h-full osd-viewer-container"
    ...
    <svg
      ...
      class="pointer-events-none z-20 absolute inset-0"
    ...
    >
      ...
      {#each $currentAnnotations as annotation (annotation.id)}
        ...
        {@const colorBody = annotation.body.find(
          (b) => b.purpose === 'highlighting' && b.type === 'Color'
        )}
        {@const borderColorBody = annotation.body.find(
          (b) => b.purpose === 'rendering' && b.type === 'BorderColor'
        )}
        {@const borderSizeBody = annotation.body.find(
          (b) => b.purpose === 'rendering' && b.type === 'BorderSize'
        )}
        ...
        {#if shapeData.shape === 'rectangle'}
          ...
          {#if selectedAnnotationId === annotation.id}
            ...
            <circle
              ...
              stroke="blue"
              stroke-width="1"
              class="pointer-events-auto cursor-nw-resize"
              on:pointerdown={(e) => startResizeDrag(e, annotation.id, 'nw')}
            />
            <circle
              cx={(shapeData.x + shapeData.width) * S}
              cy={shapeData.y * S}
              r={handleRadius * S}
              fill="white"
              stroke="blue"
              stroke-width="1"
              class="pointer-events-auto cursor-ne-resize"
              on:pointerdown={(e) => startResizeDrag(e, annotation.id, 'ne')}
            />
            <circle
              cx={shapeData.x * S}
              cy={(shapeData.y + shapeData.height) * S}
              r={handleRadius * S}
              fill="white"
              stroke="blue"
              stroke-width="1"
              class="pointer-events-auto cursor-sw-resize"
              on:pointerdown={(e) => startResizeDrag(e, annotation.id, 'sw')}
            />
            <circle
              cx={(shapeData.x + shapeData.width) * S}
              cy={(shapeData.y + shapeData.height) * S}
              ...
              stroke="blue"
              stroke-width="1"
              class="pointer-events-auto cursor-se-resize"
              on:pointerdown={(e) => startResizeDrag(e, annotation.id, 'se')}
            />
            <circle
              cx={(shapeData.x + shapeData.width / 2) * S}
              cy={shapeData.y * S}
              r={handleRadius * S}
              fill="white"
              stroke="blue"
              stroke-width="1"
              class="pointer-events-auto cursor-n-resize"
              on:pointerdown={(e) => startResizeDrag(e, annotation.id, 'n')}
            />
            <circle
              cx={(shapeData.x + shapeData.width / 2) * S}
              cy={(shapeData.y + shapeData.height) * S}
              ...
              stroke="blue"
              stroke-width="1"
              class="pointer-events-auto cursor-s-resize"
              on:pointerdown={(e) => startResizeDrag(e, annotation.id, 's')}
            />
            <circle
              cx={shapeData.x * S}
              cy={(shapeData.y + shapeData.height / 2) * S}
              r={handleRadius * S}
              fill="white"
              stroke="blue"
              stroke-width="1"
              class="pointer-events-auto cursor-w-resize"
              on:pointerdown={(e) => startResizeDrag(e, annotation.id, 'w')}
            />
            <circle
              cx={(shapeData.x + shapeData.width) * S}
              cy={(shapeData.y + shapeData.height / 2) * S}
              ...
              stroke="blue"
              stroke-width="1"
              class="pointer-events-auto cursor-e-resize"
              on:pointerdown={(e) => startResizeDrag(e, annotation.id, 'e')}
            />
          {/if}
        ...
        {:else if shapeData.shape === 'censored-circle'}
          ...
          {#if selectedAnnotationId === annotation.id}
            <circle
              cx={(shapeData.cx + shapeData.r) * S}
              cy={shapeData.cy * S}
              r={handleRadius * S}
              fill="white"
              stroke="blue"
              stroke-width="1"
              class="pointer-events-auto cursor-ew-resize"
              on:pointerdown={(e) => startResizeDrag(e, annotation.id, 'r')}
            />
            <circle
              cx={(shapeData.cx - shapeData.r) * S}
              cy={shapeData.cy * S}
          ...
          {#if selectedAnnotationId === annotation.id}
            {#if tailInfo}
              ...
              <circle
                ...
                on:pointerdown={(e) => startTailWidthDrag(e, annotation.id)}
              />
              <circle
                cx={tailInfo.b2.x}
                cy={tailInfo.b2.y}
        ...
        {:else if shapeData.shape === 'polygon'}
          ...
          />
          {#if selectedAnnotationId === annotation.id}
            {#each shapeData.points as point, i (i)}
              <circle
                cx={point.x * S}
                cy={point.y * S}
                r={handleRadius * S}
                ...
                stroke-width="1"
                class="pointer-events-auto cursor-move"
                on:pointerdown={(e) => startResizeDrag(e, annotation.id, i.toString())}
              />
            {/each}
          {/if}
        {/if}
      ...
      {#each $currentAnnotations as annotation (annotation.id)}
        ...
        {@const textBody = annotation.body.find(
          (b) => b.purpose === 'content' && b.type === 'TextualBody'
        )}
        {@const htmlBody = annotation.body.find(
          (b) => b.purpose === 'rendering' && b.type === 'HtmlBody'
        )}
        {@const textColorBody = annotation.body.find(
          (b) => b.purpose === 'rendering' && b.type === 'TextColor'
        )}
        {@const fontSizeBody = annotation.body.find(
          (b) => b.purpose === 'rendering' && b.type === 'FontSize'
        )}
        ...
        {#if textBody || htmlBody}
          {#if shapeData.shape === 'rectangle' || shapeData.shape === 'speech-bubble-rect' || shapeData.shape === 'text-area'}
            ...
            >
              <div
                style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; box-sizing: border-box; padding: 4px; overflow: hidden;"
              >
                <div
                  class="annotation-text"
                  style="margin: 0; padding: 0; text-align: center; color: {textColor}; font-family: sans-serif; font-size: {fontSize}; line-height: 1.5; word-break: break-word; width: 100%; white-space: pre-wrap;"
                >
                  {#if htmlBody}
                    {@html fixHtmlForDisplay(htmlBody.value)}
                  {:else if textBody && textBody.value && !(textBody.value
                        .trim()
                        .startsWith('{') && textBody.value.includes('"root":'))}
          ...
          {:else if shapeData.shape === 'circle' || shapeData.shape === 'speech-bubble-circle' || shapeData.shape === 'text-area-circle'}
            ...
            <foreignObject
              data-annotation-id={annotation.id}
              x={shapeData.cx * S - side / 2}
              y={shapeData.cy * S - side / 2}
              width={side}
              height={side}
              class="pointer-events-none"
            >
    ...
    {#if showAnnotationCreationDialog}
      {#key annotationBeingEdited?.id}
        <AnnotationCreationDialog
          x={dialogX}
          y={dialogY}
          initialText={annotationBeingEdited?.body?.find(
            (b) => b.type === 'TextualBody' && b.purpose === 'content'
          )?.value ||
            (annotationBeingEdited?.target?.selector?.value?.shape?.startsWith('speech-bubble') ||
            annotationBeingEdited?.target?.selector?.value?.shape?.startsWith('text-area')
              ? ''
              : null)}
          initialHtml={annotationBeingEdited?.body?.find(
            (b) => b.type === 'HtmlBody' && b.purpose === 'rendering'
          )?.value || null}
          initialTextColor={annotationBeingEdited?.body?.find(
            (b) => b.type === 'TextColor' && b.purpose === 'rendering'
          )?.value || 'black'}
          initialFontSize={annotationBeingEdited?.body?.find(
            (b) => b.type === 'FontSize' && b.purpose === 'rendering'
          )?.value || 14}
          initialBorderColor={annotationBeingEdited?.body?.find(
            (b) => b.type === 'BorderColor' && b.purpose === 'rendering'
          )?.value || null}
          initialBorderSize={annotationBeingEdited?.body?.find(
            (b) => b.type === 'BorderSize' && b.purpose === 'rendering'
          )?.value || 1}
          ...
          initialDescription={annotationBeingEdited?.body?.find((b) => b.type === 'Description')
            ?.value || ''}
          initialColor={annotationBeingEdited?.body?.find((b) => b.type === 'Color')?.value ||
            'rgba(255, 242, 117, 0.5)'}
          initialTags={annotationBeingEdited?.tags || []}
          isEditing={isEditingExisting}
          useSolidColors={annotationBeingEdited?.target?.selector?.value?.shape.startsWith(
            'speech-bubble'
          ) || annotationBeingEdited?.target?.selector?.value?.shape.startsWith('text-area')}
          isCensoredMode={annotationBeingEdited?.target?.selector?.value?.shape.startsWith(
            'censored'
          )}
