...
* **`FloatingModifyHighlightToolbar.svelte`:** Renders a floating toolbar for an *existing* highlight. Includes tag assignment (Flowbite Dropdown/Checkbox), color alteration, and highlight deletion. Interfaces deeply with `$projectStore` and `$tagStore`.
