  ...
  <div class="highlight-options">
    {#each highlightOptions as option}
      <button
        ...
        on:click|stopPropagation={() => handleHighlight(option.value)}
        title={option.label}
      ></button>
    {/each}
  </div>
  <button class="remove-highlight border-l border-gray-300 dark:border-gray-700 pl-1 ml-1" on:click|stopPropagation={handleRemove} title="Remove Highlight">
