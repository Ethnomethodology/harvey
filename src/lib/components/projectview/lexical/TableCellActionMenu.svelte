<script>
    ...
    $: if (isOpen && cellNodeKey && editor) { // Check editor exists
        try {
        ...
        try {
            ...
            if (_isTableCellNode(node) && node.isAttached()) {
                ...
                success = true;
                 ...
                 console.warn("TableCellActionMenu: Target cell node is not attached or invalid, cannot select.");
            }
        } catch (readError) {
    ...
    function handleInsertRow(above) {
        if (!editor) return;
        editor.update(() => {
            if (!selectCell()) { // Call selectCell inside update if it modifies selection
    ...
    const menuStructure = [
        { type: 'submenu', label: 'Styles', items: [
            { type: 'cellColors' },
            { type: 'rowColors' },
            { type: 'columnColors' }
        ] },
...
{#if isOpen && cellNodeKey}
    <div
        class="absolute z-40 min-w-[180px] bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg py-1 text-sm"
            ...
            {:else if item.type === 'submenu'}
              <div class="relative" on:mouseleave={closeSubmenu}>
                ...
                {#if openSubmenuLabel === item.label}
                  <div
                    class="fixed z-50 min-w-[180px] bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg py-1 text-sm"
