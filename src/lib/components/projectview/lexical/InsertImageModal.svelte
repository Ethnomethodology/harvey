    ...
    try {
      ...
      if (result && result.custom_fields_json) {
        ...
        if (attachmentsField && attachmentsField.value) {
          ...
          localAttachments = fileAttachments.filter(
            (a) => typeof a === 'string' && /\.(png|jpe?g|gif|webp|svg)$/i.test(a)
          );
        }
      }
    } catch (error) {
      ...
      {#if activeTab === 'attachments'}
        <div class="h-72 overflow-y-auto">
          ...
          {:else if localAttachments.length === 0}
            <p class="text-sm text-gray-500 italic flex justify-center items-center h-full">
              No images attached to this document.
            </p>
          {:else}
            <div class="grid grid-cols-2 gap-2">
              {#each localAttachments as attachment (attachment)}
                <button
                ...
                >
                  <div
                    class="w-full h-24 bg-gray-100 dark:bg-gray-800 flex items-center justify-center overflow-hidden"
                  >
                    <img
                  ...
                  <p class="px-2 py-1 truncate text-xs text-gray-700 dark:text-gray-300">
      ...
      {:else if activeTab === 'local'}
        <div class="h-72 flex flex-col justify-center items-center">
          ...
          <p class="mb-4 text-sm text-gray-600 dark:text-gray-400 text-center px-4">
            Select an image from your computer. It will be copied into the project's attachments
            folder.
          </p>
