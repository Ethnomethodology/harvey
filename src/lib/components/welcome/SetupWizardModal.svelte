<script>
  ...
  onMount(async () => {
    ...
    const unlistenTranscriptionComplete = await listen(
  ...
  async function startModelDownloads() {
    ...
    if (transcriptionEngines.fasterWhisper) {
      ...
      for (const model of modelsToDownload) {
        ...
        installLogs.push({
          id: installLogs.length,
          message: `Downloading ${model.name} (${installProgress.current}/${installProgress.total})...`
        });
        ...
        try {
          ...
          installLogs.push({
            id: installLogs.length,
            message: `Error downloading ${model.name}: ${e.message || e}`
          });
        }
      }
  ...
  async function verifyHfToken() {
    ...
    try {
      ...
      if (hasAccess === true) {
        ...
        diarizationError =
          'Access to the diarization model was not granted. Please accept the license on Hugging Face.';
      }
    } catch (e) {
      console.error('Token verification failed:', e);
      diarizationError =
        typeof e === 'string' ? e : e.message || 'An error occurred while verifying the token.';
      diarizationAccessGranted = false;
    } finally {
      isVerifyingToken = false;
    }
  ...
  <div class="flex-grow overflow-y-auto p-8">
      ...
      <div class="h-full flex flex-col items-center justify-center space-y-4" in:fade>
        ...
        <div class="text-center">
          ...
          <p class="text-sm text-gray-500">
            Removing partial installation to prevent environment corruption.
          </p>
        </div>
      </div>
    {:else if currentStep === 1}
      <div in:fade>
        ...
        <p class="text-gray-600 dark:text-gray-400 mb-8">
          Select the AI engines and tools you'd like to use. We'll set up the required libraries
          next.
        </p>
        <div class="space-y-6">
          <div
            class="flex items-start p-4 rounded-xl border-2 border-blue-200 dark:border-blue-900/50 bg-blue-50/30"
          >
            ...
            <div class="flex-grow">
              ...
              <div class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                ...
                <button
                  ...
                  class="text-[10px] font-bold text-blue-600 mt-2 flex items-center"
                >
                  {showMoreInfo ? 'Show less' : 'More info'}
                  <ChevronRight
                    class="w-3 h-3 ml-0.5 transition-transform {showMoreInfo ? 'rotate-90' : ''}"
                  />
                </button>
                {#if showMoreInfo}
                  <div class="mt-3 space-y-2 border-t pt-3" transition:fade>
                    <div class="text-xs">
                      <strong>
                        <button
                          ...
                          on:click={() => openLink('https://www.python.org/')}>Python</button
                        >
                        &
                        <button
                          class="text-blue-600 hover:underline"
                    ...
                    <div class="text-xs">
                      <strong>
                        <button
                          class="text-blue-600 hover:underline"
                          on:click={() => openLink('https://ffmpeg.org/')}>FFmpeg</button
                        >:
                    ...
                    <div class="text-xs">
                      <strong>
                        <button
                          class="text-blue-600 hover:underline"
                          on:click={() => openLink('https://pytorch.org/')}>PyTorch</button
                        >
                        &
                        <button
                          class="text-blue-600 hover:underline"
                          on:click={() =>
                            openLink('https://huggingface.co/docs/transformers/index')}
                          >Transformers</button
                        >:
                    ...
                    <div class="text-xs">
                      <strong>
                        <button
                          class="text-blue-600 hover:underline"
                          on:click={() => openLink('https://github.com/pyannote/pyannote-audio')}
                          >pyannote.audio</button
                        >:
                    ...
                    <div class="text-xs">
                      <strong>
                        <button
                          class="text-blue-600 hover:underline"
                          on:click={() => openLink('https://pandoc.org/')}>Pandoc</button
                        >:
          ...
          <div class="space-y-3">
            <h4 class="text-sm font-bold text-gray-500 uppercase flex items-center">
              <MessageSquareText class="w-4 h-4 mr-2" /> Transcription Engines
            </h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div
                ...
                tabindex="0"
                on:click={() =>
                  (transcriptionEngines.whisperCpp = !transcriptionEngines.whisperCpp)}
                on:keydown={(e) =>
                  e.key === 'Enter' &&
                  (transcriptionEngines.whisperCpp = !transcriptionEngines.whisperCpp)}
                class="flex flex-col p-4 rounded-xl border-2 text-left cursor-pointer transition-all {transcriptionEngines.whisperCpp
                ...
                <div class="flex justify-between items-start">
                  ...
                  <button
                    ...
                    on:click|stopPropagation={() =>
                      openLink('https://github.com/ggerganov/whisper.cpp')}
                  >
              ...
              <div
                ...
                tabindex="0"
                on:click={() =>
                  (transcriptionEngines.fasterWhisper = !transcriptionEngines.fasterWhisper)}
                on:keydown={(e) =>
                  e.key === 'Enter' &&
                  (transcriptionEngines.fasterWhisper = !transcriptionEngines.fasterWhisper)}
                class="flex flex-col p-4 rounded-xl border-2 text-left cursor-pointer transition-all {transcriptionEngines.fasterWhisper
                  ? 'border-blue-600 bg-blue-50/30'
                  : 'border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700'}"
              >
                <div class="flex justify-between items-start">
                  ...
                  <button
                    class="p-1 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-md transition-colors text-blue-600"
                    on:click|stopPropagation={() =>
                      openLink('https://github.com/SYSTRAN/faster-whisper')}
                  >
                    <ExternalLink class="w-3.5 h-3.5" />
                  </button>
                </div>
          ...
          <div class="space-y-3">
            <h4 class="text-sm font-bold text-gray-500 uppercase flex items-center">
              <Languages class="w-4 h-4 mr-2" /> Translation Engines
            </h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div
                role="button"
                ...
                on:keydown={(e) =>
                  e.key === 'Enter' && (translationEngines.helsinki = !translationEngines.helsinki)}
                class="flex flex-col p-4 rounded-xl border-2 text-left cursor-pointer transition-all {translationEngines.helsinki
                  ? 'border-blue-600 bg-blue-50/30'
                  : 'border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700'}"
              >
                <div class="flex justify-between items-start">
                  ...
                  <button
                    class="p-1 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-md transition-colors text-blue-600"
                    on:click|stopPropagation={() => openLink('https://huggingface.co/Helsinki-NLP')}
                  >
                    <ExternalLink class="w-3.5 h-3.5" />
                  </button>
                </div>
              ...
              <div
                ...
                on:keydown={(e) =>
                  e.key === 'Enter' && (translationEngines.nllb = !translationEngines.nllb)}
                class="flex flex-col p-4 rounded-xl border-2 text-left cursor-pointer transition-all {translationEngines.nllb
                  ? 'border-blue-600 bg-blue-50/30'
                  : 'border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700'}"
              >
                <div class="flex justify-between items-start">
                  ...
                  <button
                    class="p-1 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-md transition-colors text-blue-600"
                    on:click|stopPropagation={() =>
                      openLink('https://huggingface.co/facebook/nllb-200-distilled-600M')}
                  >
                    <ExternalLink class="w-3.5 h-3.5" />
                  </button>
                </div>
        ...
        {#if installProgress.phase === 'idle'}
          <div class="mb-8 space-y-4">
            <p class="text-gray-600 dark:text-gray-400 text-sm">
              Harvey uses <strong>micromamba</strong> to install and manage required AI libraries locally
              on your device.
            </p>
            ...
            >
              ...
              <div class="flex flex-wrap gap-2">
                <span
                  class="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-full text-xs font-medium"
                  >Core Libraries</span
                >
                {#if transcriptionEngines.whisperCpp}
                  <span
                    class="px-3 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full text-xs font-medium"
                    >whisper.cpp</span
                  >
                {/if}
                {#if transcriptionEngines.fasterWhisper}
                ...
                {#if translationEngines.helsinki}
                  <span
                    class="px-3 py-1 bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 rounded-full text-xs font-medium"
                    >Helsinki-NLP</span
                  >
                {/if}
            ...
            <p class="text-[11px] text-gray-500 italic">
              Once downloaded, everything runs offline on your device to ensure privacy. This
              process may take a few minutes.
            </p>
          </div>

          <button
            on:click={startCoreInstallation}
            class="px-8 py-3 bg-blue-600 text-white rounded-xl font-bold flex items-center space-x-2 mx-auto shadow-lg hover:bg-blue-700 transition-all"
          >
          ...
          <div class="space-y-6">
            ...
            >
              <div class="flex justify-between items-center">
                ...
                {#if installProgress.phase !== 'complete'}<Loader2
                    class="w-5 h-5 animate-spin text-blue-600"
                  />{:else}<Check class="w-6 h-6 text-green-500" />{/if}
              </div>
            </div>
            <div
              class="bg-gray-900 rounded-lg p-4 font-mono text-[11px] text-gray-300 h-64 overflow-y-auto scrollbar-hide"
            >
              {#each installLogs as log, i (i)}<div class="mb-1">{log.message}</div>{/each}
    ...
    {:else if currentStep === 3}
      <div in:fade>
        ...
        <p class="text-gray-600 dark:text-gray-400 text-sm mb-6">
          Select models to download from HuggingFace.
        </p>
        <div class="grid grid-cols-1 gap-2">
          {#each availableWhisperCppModels as model (model.name)}
            <div
              class="flex items-center p-3 rounded-lg border transition-all {selectedWhisperCppModels.includes(
                model.name
              )
            ...
            >
              <button
                on:click={() =>
                  (selectedWhisperCppModels = selectedWhisperCppModels.includes(model.name)
                    ? selectedWhisperCppModels.filter((m) => m !== model.name)
                    : [...selectedWhisperCppModels, model.name])}
                class="flex-grow flex items-start space-x-3 text-left focus:outline-none"
              >
                <div
                  class="w-5 h-5 border rounded mt-0.5 flex flex-shrink-0 items-center justify-center bg-white dark:bg-gray-800"
                >
                  {#if selectedWhisperCppModels.includes(model.name)}<Check
                ...
                <div class="flex flex-col min-w-0 pr-2">
                  <div class="flex items-center space-x-2">
                    ...
                    >
                    <span
                      class="text-[10px] font-mono text-gray-500 bg-gray-100 dark:bg-gray-900 px-1.5 py-0.5 rounded border border-gray-200 dark:border-gray-700"
                      >{model.size}</span
                    >
                    {#if model.name === 'ggml-base'}
                      <span
                        class="text-[9px] font-bold text-blue-600 bg-blue-100 dark:bg-blue-900/40 px-1.5 py-0.5 rounded-full uppercase tracking-wider"
                        >Starter</span
                      >
                    {/if}
                  </div>
                  <p class="text-[11px] text-gray-500 line-clamp-1 italic mt-0.5">
                    {model.description}
                  </p>
                </div>
              </button>
    ...
    {:else if currentStep === 4}
      <div in:fade>
        ...
        <div class="grid grid-cols-1 gap-2 overflow-y-auto max-h-[400px]">
          {#each availableFasterWhisperModels as model (model.name)}
            <div
              class="flex items-center p-3 rounded-lg border transition-all {selectedFasterWhisperModels.includes(
                model.name
              )
                ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20'
                : 'border-gray-200 dark:border-gray-800'}"
            >
              <button
                on:click={() =>
                  (selectedFasterWhisperModels = selectedFasterWhisperModels.includes(model.name)
                    ? selectedFasterWhisperModels.filter((m) => m !== model.name)
                    : [...selectedFasterWhisperModels, model.name])}
                class="flex-grow flex items-start space-x-3 text-left focus:outline-none"
              >
                <div
                  class="w-5 h-5 border rounded mt-0.5 flex flex-shrink-0 items-center justify-center bg-white dark:bg-gray-800"
                >
                  {#if selectedFasterWhisperModels.includes(model.name)}<Check
                      class="w-4 h-4 text-blue-600"
                    />{/if}
                </div>
                <div class="flex flex-col min-w-0 pr-2">
    ...
    {:else if currentStep === 6}
      <div in:fade>
        ...
        <div class="grid grid-cols-1 gap-2 overflow-y-auto max-h-[350px]">
          {#if isFetchingHelsinki}<div class="py-8 text-center">
              <Loader2 class="w-6 h-6 animate-spin mx-auto mb-2" />Fetching...
            </div>
          {:else}
            {#each filteredHelsinki as model (model.id)}
              <div
                class="flex items-center p-3 rounded-lg border transition-all {helsinkiModels.includes(
                  model.id
                )
                  ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20'
                  : 'border-gray-200 dark:border-gray-800'}"
              >
                <button
                  on:click={() =>
                    (helsinkiModels = helsinkiModels.includes(model.id)
                      ? helsinkiModels.filter((m) => m !== model.id)
                      : [...helsinkiModels, model.id])}
                  class="flex-grow flex items-start space-x-3 text-left focus:outline-none"
                >
                  <div
                    class="w-5 h-5 border rounded mt-0.5 flex flex-shrink-0 items-center justify-center bg-white dark:bg-gray-800"
                  >
                    {#if helsinkiModels.includes(model.id)}<Check
                        class="w-4 h-4 text-blue-600"
                      />{/if}
                  </div>
                  <div class="flex flex-col min-w-0 pr-2">
    ...
    {:else if currentStep === 7}
      <div in:fade>
        ...
        <p class="text-[11px] text-gray-500 mb-4 italic">
          One model supporting 200+ languages. Great for rare languages, but very heavy and requires
        ...
        <div class="grid grid-cols-1 gap-2 overflow-y-auto max-h-[350px]">
          {#if isFetchingNLLB}<div class="py-8 text-center">
              <Loader2 class="w-6 h-6 animate-spin mx-auto mb-2" />Fetching...
            </div>
          {:else}
            {#each filteredNLLB as model (model.id)}
              <div
                class="flex items-center p-3 rounded-lg border transition-all {nllbModels.includes(
                  model.id
                )
                  ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20'
                  : 'border-gray-200 dark:border-gray-800'}"
              >
                <button
                  on:click={() =>
                    (nllbModels = nllbModels.includes(model.id)
                      ? nllbModels.filter((m) => m !== model.id)
                      : [...nllbModels, model.id])}
                  class="flex-grow flex items-start space-x-3 text-left focus:outline-none"
                >
                  <div
    ...
    {:else if currentStep === 9}
      <div in:fade>
        ...
        {#if installProgress.phase !== 'models' && installProgress.phase !== 'complete'}
          <p class="text-gray-600 dark:text-gray-400 text-sm mb-6">
            Review your selections. We'll download these models to your local device.
          </p>

          <div class="space-y-4 mb-8">
            <div
              class="bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-300"
            >
              <div
                class="px-4 py-3 bg-gray-100/50 dark:bg-gray-800 flex justify-between items-center"
              >
                <div class="flex items-center space-x-2">
                  <span class="text-sm font-bold text-gray-700 dark:text-gray-200"
                    >{selectedModelsSummary.count} Models to download</span
                  >
                  {#if selectedModelsSummary.count > 0}
                    <span class="text-gray-400">&bull;</span>
                    <span class="text-sm font-medium text-blue-600 dark:text-blue-400"
                      >~{selectedModelsSummary.totalGB} GB Total</span
                    >
                  {/if}
                </div>
                {#if selectedModelsSummary.count > 0}
              ...
              {#if showModelDetails && selectedModelsSummary.count > 0}
                <div
                  class="p-2 space-y-1 border-t border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-900/30"
                  in:fade={{ duration: 200 }}
                >
                  {#each selectedModelsSummary.models as model (model.name)}
                    <div
                      class="flex items-center justify-between p-2 rounded-lg hover:bg-white dark:hover:bg-gray-800 transition-colors"
                    >
                      <div class="flex flex-col">
                        <span
                          class="text-sm font-bold text-gray-800 dark:text-gray-200 line-clamp-1"
                          >{model.name}</span
                        >
                        <span class="text-[10px] text-gray-500 uppercase font-mono"
                          >{model.type}</span
                        >
                      </div>
                      <span
                        class="text-xs font-mono text-gray-500 bg-gray-100 dark:bg-gray-900 px-2 py-0.5 rounded border border-gray-200 dark:border-gray-700"
                        >{model.size}</span
                      >
                    </div>
                  {/each}
              ...
              {#if selectedModelsSummary.count === 0}
                <div
                  class="p-6 text-center text-sm text-gray-500 italic border-t border-gray-200 dark:border-gray-700"
                >
          ...
          <div class="flex flex-col items-center">
            <button
              on:click={startModelDownloads}
              class="px-10 py-3 bg-blue-600 text-white rounded-xl font-bold flex items-center space-x-2 shadow-lg hover:bg-blue-700 transition-all active:scale-95"
            >
              <span
                >{selectedModelsSummary.count === 0
                  ? 'Skip to Final Step'
                  : 'Start Downloads'}</span
              >
              <ChevronRight class="w-5 h-5" />
            </button>
          </div>
        {:else}
          <div class="space-y-6">
            ...
            >
              <div class="flex justify-between items-center mb-4">
                <div>
                  <h4 class="font-bold text-gray-900 dark:text-gray-100">
                    {installProgress.phase === 'complete'
                      ? 'Finished!'
                      : `Downloading (${installProgress.current}/${installProgress.total})`}
                  </h4>
                  <p class="text-xs text-gray-500 font-mono mt-1">{installProgress.currentItem}</p>
                </div>
                {#if installProgress.phase !== 'complete'}<Loader2
              ...
              {#if installProgress.phase === 'models'}<div
            ...
            {#if installProgress.phase === 'models' && downloadProgressData[installProgress.currentItem]}
              ...
              <div class="px-2">
                <div
                  class="flex justify-between text-[10px] text-gray-500 mb-1 font-mono uppercase"
                >
                  <span>{installProgress.currentItem}</span><span
                    >{(dp.downloaded_bytes / 1048576).toFixed(1)} / {dp.total_bytes
                      ? (dp.total_bytes / 1048576).toFixed(1)
                      : '?'} MB</span
                  >
                </div>
                <div class="w-full h-1 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style="width: {dp.total_bytes
                      ? (dp.downloaded_bytes / dp.total_bytes) * 100
                      : 0}%"
                  ></div>
                </div>
              </div>
            {/if}
            <div
              class="bg-gray-900 rounded-lg p-4 font-mono text-[11px] text-gray-300 h-48 overflow-y-auto scrollbar-hide border border-gray-800"
            >
              {#each installLogs as log, i (i)}<div class="mb-1">{log.message}</div>{/each}
            </div>
          </div>
      ...
      <div in:fade={{ duration: 200 }}>
        ...
        {:else}
          <p class="text-sm text-gray-600 dark:text-gray-400 mb-8">
            Speaker diarization automatically identifies and separates different speakers in an
            audio file. Harvey uses the gated <button
              on:click|preventDefault={() =>
                openLink('https://huggingface.co/pyannote/speaker-diarization-3.1')}
              class="text-blue-600 dark:text-blue-400 hover:underline font-medium"
              >pyannote/speaker-diarization-3.1</button
            > model. Follow the steps below to authenticate and download the model.
          </p>

          <ol
            class="relative text-gray-700 dark:text-gray-300 border-s border-gray-200 dark:border-gray-700 ml-3.5"
          >
            <!-- Step 1: Create Account -->
            <li class="mb-10 ms-8">
              <span
                class="absolute flex items-center justify-center w-8 h-8 rounded-full -start-4 ring-4 ring-white dark:ring-gray-900 {diarizationAccessGranted ||
                diarizationDownloaded
                  ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400'
                  : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'}"
              >
                <span class="font-medium text-sm">1</span>
              </span>
              <h3 class="font-medium leading-tight text-gray-900 dark:text-white mb-2">
                Create HuggingFace Account
              </h3>
              <p class="text-xs text-gray-600 dark:text-gray-400">
                If you don't have one, create a free HuggingFace account on their <button
                  on:click|preventDefault={() => openLink('https://huggingface.co/join')}
            ...
            <li class="mb-10 ms-8">
              ...
              <h3 class="font-medium leading-tight text-gray-900 dark:text-white mb-2">
                Accept Diarization Agreement
              </h3>
              <p class="text-xs text-gray-600 dark:text-gray-400 mb-3">
                Accept the user agreement on the Pyannote HuggingFace page to unlock access to the
                model.
              </p>
              <button
                on:click={() => openLink('https://huggingface.co/pyannote/speaker-diarization-3.1')}
                class="text-xs font-bold text-blue-600 hover:text-blue-700 dark:text-blue-400 flex items-center"
              >
            ...
            <li class="mb-10 ms-8">
              ...
              <p class="text-xs text-gray-600 dark:text-gray-400">
                Generate an access token (Read access) from your HuggingFace account <button
                  on:click|preventDefault={() => openLink('https://huggingface.co/settings/tokens')}
                  class="text-blue-600 hover:underline dark:text-blue-400 font-medium"
                  >settings</button
                >.
              </p>
            ...
            <li class="mb-10 ms-8">
              ...
              {#if diarizationError && !isDownloadingDiarization}
                <p
                  class="text-[10px] text-red-600 dark:text-red-400 font-medium mt-2 flex items-center"
                >
            ...
            <li class="ms-8">
              <span
                class="absolute flex items-center justify-center w-8 h-8 rounded-full -start-4 ring-4 ring-white dark:ring-gray-900 {diarizationDownloaded
                  ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400'
                  : diarizationAccessGranted
                    ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400'
                    : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'}"
              ...
              {#if diarizationDownloaded}
                <div
                  class="flex items-center space-x-2 text-green-600 dark:text-green-400 text-sm font-bold"
                  in:fade
                >
                  <Check class="w-4 h-4" />
              ...
              {:else}
                <p class="text-xs text-gray-600 dark:text-gray-400 mb-3">
                  Once authenticated and approved, download the model to your machine.
                </p>
                <button
                ...
                >
                  {#if isDownloadingDiarization}<Loader2
                      class="w-4 h-4 animate-spin mr-2"
                    />{:else}<Download class="w-4 h-4" />{/if}
              ...
              {#if diarizationLogs.length > 0}
                <div
                  class="mt-4 bg-gray-900 rounded-lg p-4 font-mono text-[10px] text-gray-300 h-40 overflow-y-auto scrollbar-hide border border-gray-800"
                >
                  {#each diarizationLogs as log, i (i)}
  ...
  <svelte:fragment slot="footer">
    <button
      ...
      disabled={currentStep === 1 ||
        ...
        isCleaningUp ||
        currentStep === 3 ||
        (currentStep === 2 && installProgress.phase === 'complete')}
      class="px-4 py-2 text-sm font-bold text-gray-600 dark:text-gray-400 disabled:opacity-30 flex items-center transition-colors hover:text-gray-900 dark:hover:text-gray-200"
    >
    ...
    <div class="flex space-x-3">
      {#if currentStep === 2}
        <button
          ...
          class="px-6 py-2 bg-blue-600 text-white rounded-lg font-bold disabled:opacity-50 flex items-center hover:bg-blue-700 transition-colors"
        >
      ...
      {:else if currentStep < 7}
        <button
          ...
          class="px-6 py-2 bg-blue-600 text-white rounded-lg font-bold flex items-center hover:bg-blue-700 transition-colors"
        >
          Next <ChevronRight class="w-4 h-4 ml-1" />
        </button>
      {:else if currentStep === 9}
      ...
      {:else}
        <button
          on:click={close}
          disabled={isDownloadingDiarization || isCleaningUp}
          class="px-8 py-2 bg-green-600 text-white rounded-lg font-bold disabled:opacity-50 hover:bg-green-700 transition-colors"
        >
