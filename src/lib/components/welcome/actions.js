...
export async function openProjectWindow(project) {
  ...
  try {
    ...
    console.error(
      `[ProjectWindow] ACTION_ERROR: Failed during openProjectWindow for ${project?.name}:`,
      error
    );
  ...
  try {
    ...
    try {
      ...
      if (!initialOverwrite && isDirExistsError(initialError)) {
        ...
        if (userConfirmation) {
          console.log(
            '[ProjectActions] User confirmed overwrite. Attempting creation with overwrite=true.'
          );
    ...
    await loadProjects({ setRecentProjects, setIsLoading }).catch((e) =>
      console.error('Error reloading projects after create error:', e)
    );
  }
...
export async function handleOpenProject({ setRecentProjects, setIsLoading }) {
  try {
    ...
    if (selected && typeof selected === 'string' && !Array.isArray(selected)) {
      ...
      if (project && project.path && project.name) {
        console.log(`Project "${project.name}" imported/found. Opening...`);
        console.log('Project imported/found via dialog:', project);
        await openProjectWindow(project);
      } else {
        console.error('Import process returned invalid project data:', project);
        setIsLoading(false);
        await showWelcomeScreen();
        await loadProjects({ setRecentProjects, setIsLoading });
      }
    } else if (Array.isArray(selected)) {
      console.log('Cannot open multiple projects at once.');
    } else {
...
export async function handleOpenRecent(project) {
  console.log(
    'handleOpenRecent called for project:',
    project ? `${project.name} (${project.path})` : 'undefined'
  );
  if (!project || !project.path || !project.name) {
    console.error('handleOpenRecent received invalid project data:', project);
    return;
  }
  await openProjectWindow(project);
}
...
export async function handleMenuAction(
  action,
  project,
  { setRecentProjects, setOpenMenu, setProjectToRename, setIsRenameModalOpen, setIsLoading }
) {
  setOpenMenu(null);
  try {
    console.log(
      `handleMenuAction: Action='${action}', Project='${project.name}' Path='${project.path}'`
    );
    switch (action) {
      case 'Open':
        await openProjectWindow(project);
        break;
      case 'Locate':
        console.log(`Locating project: ${project.name}...`);
        setIsLoading(true);
        try {
          await invoke('locate_in_finder', { projectXmlPath: project.path });
          console.log(`Located project: ${project.name}`);
        } catch (locateError) {
          console.error(
            `ACTION_ERROR: Failed during locate_in_finder for ${project?.path}:`,
            locateError
          );
        } finally {
          setIsLoading(false);
        ...
        break;
      case 'Rename':
        console.log('Opening rename modal for project:', project);
        setProjectToRename(project);
        setIsRenameModalOpen(true);
        break;
      case 'Remove': {
        console.log(`Preparing to remove ${project.name}...`);
        const confirmationRemove = await ask(
          `Are you sure you want to remove "${project.name}" from the list? This will not delete the project files.`,
          { title: 'Confirm Removal', type: 'warning', okLabel: 'Remove', cancelLabel: 'Cancel' }
        );
        if (confirmationRemove) {
          console.log(`Removing "${project.name}" from list...`);
          setIsLoading(true);
          try {
            await invoke('remove_project_from_list', { projectXmlPath: project.path });
            console.log(`"${project.name}" removed from list.`);
          } catch (removeError) {
            console.error(
              `ACTION_ERROR: Failed during remove_project_from_list for ${project?.path}:`,
              removeError
            );
          } finally {
            await loadProjects({ setRecentProjects, setIsLoading });
      ...
      case 'Delete': {
        console.log(`Preparing to delete ${project.name}...`);
        const confirmationDelete = await ask(
          `Are you sure you want to permanently delete the project "${project.name}"? This action cannot be undone and will delete the project folder and its contents (or just the XML file if the folder name doesn't match).`,
          {
        ...
        if (confirmationDelete) {
          console.log(`Deleting project "${project.name}" from disk...`);
          setIsLoading(true);
          try {
            await invoke('delete_project', { projectXmlPath: project.path });
            console.log(`Project "${project.name}" deleted from disk.`);
          } catch (deleteError) {
            console.error(
              `ACTION_ERROR: Failed during delete_project invoke for ${project?.path}:`,
              deleteError
            );
          } finally {
            await loadProjects({ setRecentProjects, setIsLoading });
...
export async function handleRenameConfirm(event, { setRecentProjects, setIsLoading }) {
  ...
  if (
    !projectXmlPath ||
    !newName ||
    typeof projectXmlPath !== 'string' ||
    typeof newName !== 'string' ||
    newName.trim() === ''
  ) {
    console.error('Rename confirmation missing valid path or new name:', event.detail);
    return;
  }
