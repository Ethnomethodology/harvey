...
<script>
  ...
  const setRecentProjects = (projects) => {
    recentProjects = projects;
  };
  const setIsLoading = (bool) => {
    isLoading = bool;
  };
  const setOpenMenu = (path) => {
    openMenuProjectPath = path;
  };
  // This function is now passed to handleMenuAction to update the local state
  const updateProjectToRename = (project) => {
    projectToRename = project;
  };
  // This function is now passed to handleMenuAction to update the local state
  const updateIsRenameModalOpen = (bool) => {
    isRenameModalOpen = bool;
  };
  ...
  onMount(async () => {
    ...
    await invoke('set_menu_context', { context: 'welcome' }).catch((err) =>
...
<div class="flex h-screen bg-gray-100 dark:bg-gray-950 font-sans text-sm">
  ...
  >
    <div class="w-fit mb-8">
      ...
      <p class="text-xs text-gray-500 dark:text-gray-400 relative left-[3px]">
        v{appVersion || '0.1'}
      </p>
    </div>
    <nav class="flex flex-col space-y-1">
      <a
        href="#"
        class="px-3 py-2 rounded-md {activeTab === 'projects'
          ? 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-100 font-medium'
          : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50 hover:text-gray-800 dark:hover:text-gray-200'} text-sm"
      ...
      <a
        href="#"
        class="px-3 py-2 rounded-md {activeTab === 'about'
          ? 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-100 font-medium'
          : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50 hover:text-gray-800 dark:hover:text-gray-200'} text-sm"
  ...
  <div class="w-3/4 p-8 flex flex-col overflow-hidden">
    {#if activeTab === 'projects'}
      {#if hasCriticalConfigIssues || hasNonCriticalConfigIssues}
        <div
          class="mb-6 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-5 flex items-start space-x-4 flex-shrink-0 shadow-sm"
        >
          ...
          <div class="flex-grow">
            <h3
              class="text-base font-bold text-yellow-900 dark:text-yellow-100 uppercase tracking-tight"
            >
              Configuration Required
            </h3>
            <p class="text-sm text-yellow-800 dark:text-yellow-300 mt-1 leading-relaxed">
              Your installation is almost complete! Some required libraries or AI models are
              missing. Please complete the setup to enable full transcription and translation
              features.
            </p>
      ...
      <div
        class="flex-grow overflow-y-auto bg-white dark:bg-gray-900 p-6 rounded-lg border border-gray-200 dark:border-gray-700 shadow-inner"
      >
        <h2
          class="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-200 border-b border-gray-200 dark:border-gray-700 pb-2"
        >
          Recent Projects
        </h2>
        {#if isLoading}
        ...
        {:else if recentProjects.length === 0}
          ...
          <p class="text-center text-gray-400 dark:text-gray-500 text-xs py-1">
        ...
        {:else}
          <ProjectList
            ...
            on:toggleMenu={({ detail: path }) =>
              (openMenuProjectPath = openMenuProjectPath === path ? null : path)}
          />
        {/if}
      </div>
