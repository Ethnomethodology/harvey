  ...
  >
    ...
    <div class="flex items-center">
      ...
      {:else if isAuthenticated}
        <span class="text-sm font-medium text-green-600 dark:text-green-400 mr-2"
          >Authenticated</span
        >
      {:else}
...
{#if isPanelOpen}
  <div
    class="p-4 bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 text-sm text-gray-700 dark:text-gray-300"
  >
    <p class="mb-4">
    ...
    <p class="mb-4">
      While most models are public, an access token is required to download <strong
        >gated models</strong
      > (like those for speaker identification) and helps avoid download rate limits.
    </p>

    <h4 class="font-semibold mb-2 dark:text-gray-200">Setup Instructions</h4>
    <ol class="list-decimal list-inside mb-4 space-y-1">
      <li>
        If you don't have one, create a HuggingFace account on their <a
          href="https://huggingface.co/join"
          on:click|preventDefault={() => openLink('https://huggingface.co/join')}
          class="text-blue-600 hover:underline dark:text-blue-400">website</a
        >.
      </li>
      <li>
        Generate an access token from your HuggingFace account settings. You can find it under <a
          ...
          class="text-blue-600 hover:underline dark:text-blue-400">Access Tokens</a
        >.
      </li>
