<script>
  ...
  $: whisperCppDownloadedCount = Array.isArray(downloadedModels)
    ? downloadedModels.filter(
        (m) => m.family === 'whisper-cpp' || (!m.family && !m.name.includes('/'))
      ).length
    : 0;
    ...
    return baseList
      .map((m) => {
        ...
        return {
          ...
          family:
            local?.family ||
            (selectedEngine === 'faster-whisper' ? 'faster-whisper' : 'whisper-cpp'),
          isInstalled: !!local
        };
      })
      .sort((a, b) => {
  ...
  onMount(async () => {
    ...
    try {
      ...
      unlistenFinished = await listen('transcription-download-finished', async () => {
        console.log(
          'Frontend: Received transcription-download-finished event. Setting isDownloading to false.'
        );
        isDownloading = false;
        isInstallingDependencies = false;
        isChecking = true;
  ...
  async function handleInstallFWDependencies() {
    ...
    showLogModal = true;
    modalLogs = [
      { id: uuidv4(), message: 'Starting installation of Faster-Whisper dependencies...' }
    ];
    dependencyErrors = [];
  ...
  async function handleInstallWCDependencies() {
    ...
    const unlistenLog = await listen('installation-log', (event) => {
      modalLogs = [
        ...
        {
          id: uuidv4(),
          message: event.payload.log_line || event.payload.message || event.payload.status
        }
      ];
    });
...
<div class="flex flex-col h-full overflow-y-auto p-1">
  <div class="flex justify-between items-center mb-2 px-1">
    ...
    <div class="flex items-center">
      ...
      {:else if selectedEngine === 'faster-whisper' && !$configStatus.faster_whisper_dependencies_installed}
        <span class="text-sm font-medium text-red-600 dark:text-red-400 uppercase"
          >FASTER-WHISPER LIBRARIES MISSING</span
        >
  ...
  {#if $configStatus.isInitialized && selectedEngine === 'faster-whisper' && $configStatus.python_libraries_installed && !$configStatus.faster_whisper_dependencies_installed}
    <div
      class="mb-4 flex flex-col bg-orange-100 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800 p-3 rounded-md shadow-sm"
    >
      <div class="flex items-center justify-between mb-2">
        <div class="flex items-center">
          <svg
            class="w-4 h-4 text-orange-600 dark:text-orange-400 mr-2 flex-shrink-0"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
          ...
          <span class="text-xs text-orange-800 dark:text-orange-300 font-medium"
            >Faster-Whisper libraries are missing.</span
          >
        </div>
        <button
          class="bg-orange-600 hover:bg-orange-700 text-white px-3 py-1.5 rounded-md text-[11px] font-semibold transition-colors shadow-sm"
          on:click={handleInstallFWDependencies}
      ...
      {#if dependencyErrors.length > 0}
        <div
          class="mt-1 text-[10px] text-orange-700/80 dark:text-orange-400/80 font-mono bg-orange-50/50 dark:bg-orange-950/30 p-2 rounded border border-orange-200/50 dark:border-orange-800/50 max-h-32 overflow-y-auto"
        >
        {#each dependencyErrors as err, i (i)}
  ...
  >
    <div class="flex items-center justify-between mb-2">
      <div class="flex items-center">
        ...
        <span class="text-xs text-orange-800 dark:text-orange-300 font-medium"
          >Whisper.cpp library is missing.</span
        >
      </div>
      {#if $configStatus.python_libraries_installed}
        <button
          class="bg-orange-600 hover:bg-orange-700 text-white px-3 py-1.5 rounded-md text-[11px] font-semibold transition-colors shadow-sm"
          on:click={handleInstallWCDependencies}
          disabled={isInstallingDependencies}
        >
          Install Now
        </button>
      {:else}
        <span class="text-[10px] text-orange-700/80 dark:text-orange-400/80 italic"
          >Install Python libraries in General Settings first</span
        >
      {/if}
    </div>
  </div>
...
>
  <div class="flex items-center justify-between mb-2">
    <span class="text-sm font-semibold text-blue-800 dark:text-blue-300"
      >Select Transcription Engine</span
    >
            ...
            <div class="flex flex-col min-w-0 pr-4">
              ...
              >
                <span class="flex items-center" title="Engine Family">
                  {#if model.family}
                    <span
                      >{model.family === 'faster-whisper' ? 'Faster-Whisper' : 'Whisper.cpp'}</span
                    >
                  {/if}
                </span>
                ...
                <div class="flex flex-col items-end">
                  ...
                  >
                    {#if status === 'cancelling'}Cancelling...{:else}{display.progressText ||
                        (model.family === 'faster-whisper' ? 'Downloading...' : 'Starting...')}{/if}
                  </span>
                  <button
                    class="btn-cancel"
