...
<script>
	...
	onMount(async () => {
		...
		try {
			...
			unlistenFinished = await listen('transcription-download-finished', () => {
...
<InstallLogModal bind:showModal={showLogModal} logs={modalLogs} isInstalling={isDownloading || isInstallingDependencies} title={isInstallingDependencies ? "Installing Dependencies" : "Downloading Transcription Model"} inProgressText={isInstallingDependencies ? "Installing..." : "Downloading..."} />
