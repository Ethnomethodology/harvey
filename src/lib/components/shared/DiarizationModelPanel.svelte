...
<script>
  ...
  async function handleDeleteModel() {
    const confirmed = await ask(
      'Are you sure you want to delete the diarization model? This will remove it from your disk.',
  ...
  async function handleDownload() {
    ...
    try {
      ...
      installLogs = [
        ...installLogs,
        { id: installLogs.length, message: `Error: ${e.message || e}` }
      ];
...
<div class="flex flex-col space-y-4 pt-2 pb-8">
  ...
  <div class="text-sm text-gray-700 dark:text-gray-300">
    <p class="mb-8 text-gray-600 dark:text-gray-400">
      Speaker diarization automatically identifies and separates different speakers in an audio
      file. Harvey uses the gated <a
        ...
        >pyannote/speaker-diarization-3.1</a
      > model for this purpose. Follow the steps below to authenticate and download the model.
    </p>
    ...
    >
      ...
      <li class="mb-10 ms-8">
        <span
          class="absolute flex items-center justify-center w-8 h-8 rounded-full -start-4 ring-4 ring-white dark:ring-gray-900 {isAuthenticated
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          If you don't have one, create a free HuggingFace account on their <a
            href="https://huggingface.co/join"
            on:click|preventDefault={() => openLink('https://huggingface.co/join')}
            class="text-blue-600 hover:underline dark:text-blue-400 font-medium">website</a
          >.
        </p>
      </li>
      ...
      <li class="mb-10 ms-8">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          Generate an access token from your HuggingFace account settings. You can find it under <a
            href="https://huggingface.co/settings/tokens"
            on:click|preventDefault={() => openLink('https://huggingface.co/settings/tokens')}
            class="text-blue-600 hover:underline dark:text-blue-400 font-medium">Access Tokens</a
          >.
        </p>
      </li>
      ...
      <li class="mb-10 ms-8">
        <span
          class="absolute flex items-center justify-center w-8 h-8 rounded-full -start-4 ring-4 ring-white dark:ring-gray-900 {hasAccess
            ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400'
            : isAuthenticated
              ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400'
              : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'}"
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">
          Accept the user agreement on the Pyannote HuggingFace page to unlock access to the model.
        </p>
        <Button
          color="alternative"
          size="xs"
      ...
      <li class="ms-8">
        <span
          class="absolute flex items-center justify-center w-8 h-8 rounded-full -start-4 ring-4 ring-white dark:ring-gray-900 {hasAccess
            ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400'
            : isAuthenticated && !$configStatus.python_libraries_installed
        ...
        {#if !$configStatus.python_libraries_installed}
          <p class="text-sm text-orange-600 dark:text-orange-400 mb-3">
            Python libraries are missing. Please install them in the General Settings before
            downloading.
          </p>
        {:else if hasAccess && cachePath}
          <p class="text-sm text-green-600 dark:text-green-400 mb-3">
            Model is successfully downloaded.
          </p>
          <div class="flex items-center gap-4">
            <code
              class="text-xs bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded px-2 py-1 flex-grow truncate"
              >{cachePath}</code
            >
            <Button
              color="red"
...
<InstallLogModal
  ...
  {isChecking}
  title="Diarization Model Download"
  inProgressText="Download in progress..."
  buttonInProgressText="Downloading..."
/>
