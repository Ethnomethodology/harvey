  ...
  onMount(async () => {
    try {
      ...
      if (savedTranslationConfig) {
        if (savedTranslationConfig.helsinki_batch_size !== undefined)
          config.helsinki_batch_size = savedTranslationConfig.helsinki_batch_size;
        if (savedTranslationConfig.nllb_batch_size !== undefined)
          config.nllb_batch_size = savedTranslationConfig.nllb_batch_size;
        if (savedTranslationConfig.num_threads !== undefined)
          config.num_threads = savedTranslationConfig.num_threads;
        if (savedTranslationConfig.device_preference !== undefined)
          config.device_preference = savedTranslationConfig.device_preference;
        if (savedTranslationConfig.diarization_device !== undefined)
          config.diarization_device = savedTranslationConfig.diarization_device;
        if (savedTranslationConfig.diarization_threads !== undefined)
          config.diarization_threads = savedTranslationConfig.diarization_threads;
        if (
          savedTranslationConfig.quantization_preference !== undefined &&
          savedTranslationConfig.quantization_preference !== null
        ) {
      ...
      if (savedTranscriptionConfig) {
        if (
          savedTranscriptionConfig.faster_whisper_compute_type !== undefined &&
          savedTranscriptionConfig.faster_whisper_compute_type !== null
        ) {
          config.faster_whisper_compute_type = savedTranscriptionConfig.faster_whisper_compute_type;
        }
        if (savedTranscriptionConfig.faster_whisper_beam_size !== undefined)
          config.faster_whisper_beam_size = savedTranscriptionConfig.faster_whisper_beam_size;
        if (savedTranscriptionConfig.num_threads !== undefined)
          config.transcription_num_threads = savedTranscriptionConfig.num_threads;
        if (
          savedTranscriptionConfig.device_preference !== undefined &&
          savedTranscriptionConfig.device_preference !== null
        )
          config.transcription_device_preference = savedTranscriptionConfig.device_preference;
      }
...
<div class="h-full overflow-y-auto">
  <div
    class="mb-6 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md"
  >
  ...
  {#if statusMessage && statusType !== 'info'}
    <div
      class="mb-4 flex items-center p-2 rounded-md"
      ...
      class:dark:text-green-300={statusType === 'success'}
      class:dark:bg-red-900={statusType === 'error'}
      class:dark:text-red-300={statusType === 'error'}
    >
  ...
  <Accordion class="w-full space-y-4 bg-transparent border-0" flush multiple={false}>
    <!-- Transcription Panel -->
    <div
      class="bg-white dark:bg-gray-800/60 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden"
    >
      <AccordionItem
        open
        defaultClass="w-full flex items-center justify-between bg-gray-100 dark:bg-gray-800 px-4 py-3 border-b dark:border-gray-700 focus:outline-none hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
      >
        <span
          slot="header"
          class="flex items-center text-base font-medium text-gray-700 dark:text-gray-200"
        >
        ...
        <div class="p-4 space-y-4 bg-white dark:bg-gray-900">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              <p class="text-[10px] text-gray-500">
                Auto will use GPU if available. 'CPU' bypasses GPU for both engines.
              </p>
            </div>
            <div class="space-y-1">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300"
              ...
              <input
                type="number"
                bind:value={config.transcription_num_threads}
                min="1"
                max="32"
                class="input w-full"
                autocomplete="off"
              ...
              />
              <p class="text-[10px] text-gray-500">
                Threads for inference (Faster-Whisper & Whisper.cpp).
              </p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-1">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300"
                >Compute Type (Quantization)</label
              >
              <Select items={computeTypeOptions} bind:value={config.faster_whisper_compute_type} />
              <p class="text-[10px] text-gray-500">
                Precision of model weights. Int8 is fastest on CPU.
              </p>
            </div>
            <div class="space-y-1">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300"
              ...
              <input
                type="number"
                bind:value={config.faster_whisper_beam_size}
                min="1"
                max="10"
                class="input w-full"
                autocomplete="off"
              ...
              />
              <p class="text-[10px] text-gray-500">
                Number of paths to search. 1 is greedy (fastest), 5 is standard.
              </p>
            </div>
          </div>
          ...
          <!-- Panel Actions -->
          <div
            class="pt-4 flex justify-end space-x-3 border-t border-gray-100 dark:border-gray-800 mt-4"
          >
            <button class="btn-secondary" on:click={resetTranscription} disabled={isBusy}
        ...
        <div class="p-4 space-y-4 bg-white dark:bg-gray-900">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="space-y-1">
              ...
              <p class="text-[10px] text-gray-500">
                Force specific hardware. 'Auto' selects best available.
              </p>
            </div>
            ...
            <div class="space-y-1">
              ...
              <input
                type="number"
                bind:value={config.diarization_threads}
                min="1"
                max="32"
                class="input w-full"
                autocomplete="off"
          ...
          <div
            class="pt-4 flex justify-end space-x-3 border-t border-gray-100 dark:border-gray-800 mt-4"
          >
            <button class="btn-secondary" on:click={resetDiarization} disabled={isBusy}
              >Reset Defaults</button
            >
            <button class="btn-primary" on:click={handleSave} disabled={isBusy}>Save</button>
          </div>
      ...
      >
        ...
        <div class="p-4 space-y-4 bg-white dark:bg-gray-900">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="space-y-1 md:col-span-2 border-t pt-2 mt-2 dark:border-gray-800">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Model Optimization (Quantization)</label
              >
              ...
              {#if config.quantization_preference !== originalQuantization}
                <div
                  class="mt-2 p-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded text-xs text-yellow-800 dark:text-yellow-200 flex items-start"
                >
                  <AlertTriangle class="w-3 h-3 mr-1 mt-0.5 flex-shrink-0" />
                  <span
                    >Changing quantization requires re-downloading translation models to apply the
                    new optimization format.</span
                  >
                </div>
              {/if}
            </div>
            ...
            <div class="space-y-1">
              <div class="flex justify-between items-center">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300"
                  >Batch Size for NLLB</label
                >
                <label class="flex items-center space-x-1 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={nllbAuto}
                    on:change={toggleNllbAuto}
              ...
              {#if nllbAuto}
                <input
                  type="text"
                  value="Dynamic (Hardware Optimized)"
                  class="input w-full italic text-gray-500 bg-gray-100 dark:bg-gray-800"
                  disabled
                />
              {:else}
                <input
                  type="number"
                  bind:value={config.nllb_batch_size}
                  min="1"
                  max="32"
                  class="input w-full"
                />
              {/if}
              <p class="text-[10px] text-gray-500">
                Higher = Faster on GPU. Auto uses safe defaults.
              </p>
            </div>
            <div class="space-y-1">
              <div class="flex justify-between items-center">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300"
                  >Batch Size for Helsinki</label
                >
                <label class="flex items-center space-x-1 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={helsinkiAuto}
                    on:change={toggleHelsinkiAuto}
                    class="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-3 h-3"
                  />
                  <span class="text-xs text-gray-600 dark:text-gray-400">Auto</span>
                </label>
              ...
              {:else}
                <input
                  type="number"
                  bind:value={config.helsinki_batch_size}
                  min="1"
                  max="64"
                  class="input w-full"
                />
          ...
          <div
            class="pt-4 flex justify-end space-x-3 border-t border-gray-100 dark:border-gray-800 mt-4"
          >
            <button class="btn-secondary" on:click={resetTranslation} disabled={isBusy}
              >Reset Defaults</button
            >
            <button class="btn-primary" on:click={handleSave} disabled={isBusy}>Save</button>
          </div>
