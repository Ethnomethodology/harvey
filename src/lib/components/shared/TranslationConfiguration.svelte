<script>
  ...
  $: helsinkiDownloadedCount = Array.isArray(downloadedModels)
    ? downloadedModels.filter((m) => m.family === 'helsinki' || !m.family).length
    : 0;
  $: nllbDownloadedCount = Array.isArray(downloadedModels)
    ? downloadedModels.filter((m) => m.family === 'nllb').length
    : 0;
      ...
      for (const id in downloadStatus) {
        if (downloadStatus[id] === 'downloading' && !baseList.some((m) => m.id === id)) {
          baseList.push({
            ...
            tgt: null,
            downloads: 0,
            family: id.toLowerCase().includes('nllb') ? 'nllb' : 'helsinki'
          });
        }
      }
    } else {
  ...
  onMount(async () => {
    ...
    try {
      unlistenStart = await listen('translation-download-start', (event) => {
        ...
        modalLogs = [
          ...modalLogs,
          { id: uuidv4(), message: `Starting download for ${modelName}...` }
        ];
        isDownloading = true;
      ...
      unlistenLog = await listen('translation-download-log', (event) => {
        ...
        if (
          log_line.includes('CTranslate2') ||
          log_line.includes('micromamba') ||
          log_line.includes('Optimizing')
        ) {
          isInstallingDependencies = true;
        }

        if (
          downloadStatus[model_name] === 'downloading' ||
      ...
      unlistenComplete = await listen('translation-download-complete', async (event) => {
        ...
        modalLogs = [
          ...modalLogs,
          { id: uuidv4(), message: `Download complete for ${downloadedModelName}.` }
        ];
        isDownloading = false;
        isInstallingDependencies = false;
      ...
      unlistenError = await listen('translation-download-error', (event) => {
        ...
        modalLogs = [
          ...modalLogs,
          { id: uuidv4(), message: `Error downloading ${model_name}: ${error_message}` }
        ];
        isDownloading = false;
        isInstallingDependencies = false;
      });
      ...
      unlistenFinished = await listen('translation-download-finished', async () => {
        console.log(
          'Frontend: Received translation-download-finished event. Setting isDownloading to false.'
        );
        isDownloading = false;
        isInstallingDependencies = false;
        isChecking = true;
  ...
  async function handleDownload(targetModelId) {
    ...
    const family = knownModel
      ? knownModel.family
      : modelToDownload.toLowerCase().includes('nllb')
        ? 'nllb'
        : 'helsinki';
  ...
  async function handleDelete(model) {
    ...
    if (!modelNameForDelete) {
      notificationStore.add('Cannot delete model: Missing name.', 'error');
      return;
    }
    const confirmed = await ask(
      `Are you sure you want to delete the model "${modelNameForDelete}"? This will remove the entire model folder from disk.`,
    ...
    try {
      ...
      notificationStore.add(
        `Failed to delete model ${modelNameForDelete}: ${err.message || err}`,
        'error'
      );
    }
  }
  ...
  async function handleCancel(modelId) {
    ...
    try {
      ...
      notificationStore.add(
        `Failed to cancel download for ${modelId}: ${err.message || err}`,
        'error'
      );
  ...
  async function handleInstallDependencies() {
    ...
    modalLogs = [
      {
        id: uuidv4(),
        message: 'Starting installation of optimized translation backend (CTranslate2)...'
      }
    ];
  ...
  function formatModelDisplayName(modelName) {
    ...
    if (baseName.toLowerCase().includes('nllb')) {
      ...
      if (sizeLabel === '1.3B')
        return isDistilled ? 'NLLB-200 Distilled (Medium)' : 'NLLB-200 (Medium)';
      if (sizeLabel === '3.3B')
        return isDistilled ? 'NLLB-200 Distilled (Large)' : 'NLLB-200 (Large)';
...
<div class="flex flex-col h-full overflow-hidden">
    ...
    <div class="flex items-center">
      ...
      {:else if (selectedEngine === 'helsinki' ? helsinkiDownloadedCount : nllbDownloadedCount) > 0}
        <span class="text-sm font-medium text-green-600 dark:text-green-400 uppercase">
          {selectedEngine === 'helsinki' ? helsinkiDownloadedCount : nllbDownloadedCount}
          {selectedEngine === 'helsinki' ? 'HELSINKI-NLP' : 'NLLB'}
          {(selectedEngine === 'helsinki' ? helsinkiDownloadedCount : nllbDownloadedCount) === 1
            ? 'MODEL'
            : 'MODELS'} DOWNLOADED
        </span>
      {:else}
        <span class="text-sm font-medium text-yellow-600 dark:text-yellow-400 uppercase"
          >NO {selectedEngine === 'helsinki' ? 'HELSINKI-NLP' : 'NLLB'} MODELS DOWNLOADED</span
        >
      {/if}
    </div>
  </div>
  ...
  <InstallLogModal
    ...
    title={isInstallingDependencies ? 'Installing Dependencies' : 'Downloading Translation Model'}
    inProgressText={isInstallingDependencies ? 'Installing...' : 'Downloading...'}
  />

  <div
    class="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800 rounded-md p-3 mb-4 flex-shrink-0"
  >
    <div class="flex items-center justify-between mb-2">
      <span class="text-sm font-semibold text-blue-800 dark:text-blue-300"
        >Select Translation Engine</span
      >
    ...
    {#if selectedEngine === 'helsinki'}
      <div class="text-[11px] text-blue-700/80 dark:text-blue-400/80 leading-relaxed mb-2">
        <p>
          ...
          <button
            ...
            on:click={() => openLink('https://huggingface.co/Helsinki-NLP')}>Helsinki-NLP</button
          >
        </p>
        <p>
          <strong class="text-blue-800 dark:text-blue-300">Pros:</strong> Lightweight, very fast on CPU,
          performs better for common language pairs.
        </p>
        <p>
          <strong class="text-blue-800 dark:text-blue-300">Cons:</strong> Requires separate model for
          every language pair (e.g. ja-en, fr-en).
        </p>
      </div>
      ...
      {#if !ct2Installed && translationModelCount > 0}
        ...
        >
          ...
          <button
            class="bg-orange-600 hover:bg-orange-700 text-white px-2 py-1 rounded text-[10px] transition-colors"
            on:click={handleInstallDependencies}
    ...
    {:else}
      <div class="text-[11px] text-blue-700/80 dark:text-blue-400/80 leading-relaxed">
        <p>
          ...
          <button
            class="hover:underline text-blue-600 dark:text-blue-400 font-medium"
            on:click={() => openLink('https://huggingface.co/facebook/nllb-200-distilled-600M')}
            >NLLB (Meta)</button
          >
        </p>
        <p>
          <strong class="text-blue-800 dark:text-blue-300">Pros:</strong> One model supports 200+ languages.
          Great for rare languages.
        </p>
        <p>
    ...
    <div class="flex flex-col space-y-3 flex-grow overflow-hidden px-1">
      ...
      {#if selectedOption === 'selectLanguages'}
        <div class="flex flex-col space-y-3 h-full overflow-hidden">
          <div class="flex space-x-2 flex-shrink-0 p-0.5">
            <div class="relative flex-grow">
              ...
              <input
                ...
                placeholder={selectedEngine === 'helsinki'
                  ? "Search languages (e.g. 'French', 'en-ja')..."
                  : 'Search NLLB models...'}
                autocomplete="off"
                autocorrect="off"
                autocapitalize="off"
          ...
          <div
            class="border dark:border-gray-700 rounded-md flex-grow overflow-y-auto bg-gray-50 dark:bg-gray-800/50 p-2"
          >
            <div class="space-y-2">
                ...
                >
                  <div class="relative z-10 flex justify-between items-start">
                    <div class="flex flex-col min-w-0 pr-4">
                      <div class="flex items-center space-x-2">
                        <span class="font-semibold text-gray-800 dark:text-gray-200 truncate">
                          {#if model.family === 'helsinki' && model.src && model.tgt}
                            {getLangName(model.src)} <span class="text-gray-400">→</span>
                            {getLangName(model.tgt)}
                          {:else}
                        ...
                        <button
                          ...
                          on:click|stopPropagation={() =>
                            openExternal(`https://huggingface.co/${model.id}`)}
                        >
                          <svg
                      ...
                      <div
                        class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 flex items-center space-x-3"
                      >
                    ...
                    <div class="flex-shrink-0 flex items-center space-x-2 pt-1">
                      ...
                      {:else if status === 'downloading' || status === 'cancelling'}
                        <div class="flex flex-col items-end w-full">
                          <span
                            class="text-[10px] text-blue-700 dark:text-blue-300 font-medium tabular-nums mb-1"
                          >
                            {#if status === 'cancelling'}Cancelling...{:else}Downloading... {progressPercent}%{/if}
                          </span>
                          {#if status === 'downloading' && progressPercent > 0}
                            <div
                              class="w-24 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden mb-1.5"
                            >
                              <div
                      ...
                      {:else}
                        <button
                          ...
                          on:click={() => handleDownload(model.id)}
                          disabled={!isDownloadEnabled}
                          title={!isDownloadEnabled
                            ? 'Configure download location first'
                            : 'Download model'}>Download</button
                        >
                      {/if}
                    </div>
                  </div>
            ...
            {#if !hasFetched && searchQuery.trim() === ''}
              <div class="py-4 flex justify-center">
                <button
                  ...
                  title="Refresh available translation models"
                >
                  {#if isFetchingModels}
        ...
        <div class="flex items-end space-x-2 flex-shrink-0">
          <div class="flex flex-col flex-grow">
            ...
            <input
              ...
              class="input w-full"
              placeholder={selectedEngine === 'helsinki'
                ? 'e.g. Helsinki-NLP/opus-mt-en-jap'
