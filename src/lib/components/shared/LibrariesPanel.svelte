<script>
  ...
  async function handleDelete() {
    const confirmed = await ask(
      'Are you sure you want to delete the local library environment? This will delete all installed libraries and downloaded models. You need to re-install the libraries and download the models again to use AI features.',
...
>
  <div
    class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/30 flex justify-between items-center"
  >
  ...
  <div class="p-6 text-sm text-gray-700 dark:text-gray-300">
    <p class="mb-2">
      Harvey uses <strong
        ><button
          class="text-blue-600 dark:text-blue-400 hover:underline focus:outline-none font-bold"
          on:click={() =>
            openLink('https://mamba.readthedocs.io/en/latest/user_guide/micromamba.html')}
          >micromamba</button
        ></strong
      >
      to install and manage a few required libraries.
      <button
        class="text-blue-600 dark:text-blue-400 hover:underline ml-1 focus:outline-none"
      ...
      <ul class="list-disc list-inside mb-4 pl-2 space-y-1 text-gray-600 dark:text-gray-400">
        ...
        <li>
          <strong>
            ...
            <button
              class="text-blue-600 hover:underline"
              on:click={() => openLink('https://huggingface.co/docs/transformers/index')}
              >Transformers</button
            >:
    ...
    <div class="flex items-center">
      ...
      {:else}
        <p class="text-red-600 dark:text-red-400 mr-4 font-medium">
