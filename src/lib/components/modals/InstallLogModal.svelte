...
>
  ...
  <div
    ...
    class="log-container bg-gray-100 dark:bg-gray-900 p-3 rounded-md text-xs font-mono border border-gray-300 dark:border-gray-700 h-64 overflow-y-auto"
  >
        ...
        <div class="mt-4 px-1">
          <div class="flex justify-between items-end mb-1.5">
            <div class="flex flex-col">
              <span
                class="text-[10px] font-semibold text-blue-700 dark:text-blue-400 uppercase tracking-wider leading-none mb-1"
                >{inProgressText}</span
              >
              {#if currentFile}
                <span
                  class="text-[11px] font-medium text-gray-700 dark:text-gray-300 truncate max-w-[200px]"
                  title={currentFile}>{currentFile}</span
                >
              {/if}
            </div>
            <span
              class="text-[10px] font-mono font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded"
              >{progress}%</span
            >
          </div>
          <div
            class="w-full bg-gray-200 dark:bg-gray-800 rounded-full h-1.5 overflow-hidden border border-gray-300/30 dark:border-gray-700/30"
          >
            <div
              class="bg-blue-500 dark:bg-blue-400 h-full rounded-full transition-all duration-300 ease-out shadow-[0_0_8px_rgba(59,130,246,0.5)]"
