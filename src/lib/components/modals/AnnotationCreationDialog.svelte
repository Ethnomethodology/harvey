<script>
  ...
  let selectedBorderColor =
    selectedColor === 'url(#censoredPattern)'
      ? 'black'
      : initialBorderColor ||
        (initialColor.includes('255, 255, 255')
          ? 'rgba(156, 163, 175, 1)'
          : initialColor.replace(', 0.5', ', 1'));
  ...
  $: highlightOptions = isCensoredMode
    ? censoredColors
    : useSolidColors
      ? solidColors
      : transparentColors;
  ...
  $: if (dialogElement && panelBounds) {
    ...
    const actualDialogHeight =
      currentDialogRect.height > 0 ? currentDialogRect.height : dialogHeight;
...
>
      ...
      <div class="flex space-x-2">
        <button
          ...
          class:bg-blue-600={selectedShape === 'rectangle' ||
            selectedShape === 'text-area' ||
            selectedShape === 'censored' ||
            selectedShape === 'speech-bubble-rect'}
        ...
        <button
          ...
          class:bg-blue-600={selectedShape === 'circle' ||
            selectedShape === 'text-area-circle' ||
            selectedShape === 'censored-circle' ||
            selectedShape === 'speech-bubble-circle'}
  ...
  <div class="mb-4">
    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"
      >{isCensoredMode ? 'Anonymise Style' : 'Background Color'}</label
    >
    <Dropdown
  ...
  {#if isCensoredMode}
    <div
      class="mb-4 p-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded text-[10px] text-amber-800 dark:text-amber-200 leading-tight"
    >
      <div class="flex items-start space-x-1.5">
        <Info class="w-3 h-3 mt-0.5 flex-shrink-0" />
        <span
          >Anonymization is only permanent when the image is <strong
            >exported with annotations</strong
          >.</span
        >
      </div>
    </div>
