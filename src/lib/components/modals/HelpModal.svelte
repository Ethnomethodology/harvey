...
>
  ...
  <svelte:fragment slot="header">
    <div class="flex items-center justify-between flex-grow mr-8">
      <div class="flex items-center space-x-3">
        ...
        <h3 id="help-modal-title" class="text-lg font-bold text-gray-900 dark:text-white truncate">
          Help Center
        </h3>
      </div>
      ...
      <div class="flex items-center space-x-4">
        ...
        <div class="relative w-64 help-search-container">
          ...
          {#if showSearchResults && filteredResults.length > 0}
            <div
              class="absolute top-full mt-1 left-0 right-0 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-[160] py-1 max-h-64 overflow-y-auto"
            >
              {#each filteredResults as result (result.id)}
                ...
                >
                  ...
                  <span
                    class="text-[10px] text-gray-400 group-hover:text-blue-500 uppercase tracking-wider"
                    >{result.sidebarId}</span
                  >
                </button>
              {/each}
            </div>
          {:else if showSearchResults && searchQuery.trim() !== ''}
            <div
              class="absolute top-full mt-1 left-0 right-0 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-[160] p-4 text-center"
            >
  ...
  <div class="flex flex-grow overflow-hidden h-full">
    <!-- Sidebar -->
    <div
      class="w-64 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 p-4 flex flex-col flex-shrink-0 overflow-y-auto font-sans transition-all duration-300"
    >
      <nav class="flex flex-col space-y-1">
        {#each sidebarTabs as tab (tab.id)}
          <button
            class="px-4 py-2 text-sm rounded-md text-left transition-all duration-200 {currentPage.sidebarId ===
            tab.id
    ...
    <div class="flex-grow flex flex-col overflow-hidden bg-white dark:bg-gray-900 font-sans">
      ...
      <div
        class="flex-shrink-0 px-8 py-3 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-900 h-12"
      >
        <button
          ...
          {#if childPages && childPages.length > 0}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8 not-prose">
              {#each childPages as child (child.id)}
                ...
                >
                  <span
                    class="text-xl font-bold mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 text-gray-800 dark:text-gray-100"
                    >{child.label}</span
                  >
      ...
      <div
        class="flex-shrink-0 px-8 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900 h-14"
      >
        <button
