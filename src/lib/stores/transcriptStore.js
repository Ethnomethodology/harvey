...
export function undoTranscriptChange() {
  ...
  transcriptStore.update((ts) => {
    ...
    return {
      ...
      secondaryTranscriptSegments:
        prevState.secondarySegments !== null
          ? prevState.secondarySegments
          : ts.secondaryTranscriptSegments,
...
export function redoTranscriptChange() {
  ...
  transcriptStore.update((ts) => {
    ...
    return {
      ...
      secondaryTranscriptSegments:
        nextState.secondarySegments !== null
          ? nextState.secondarySegments
          : ts.secondaryTranscriptSegments,
      transcriptUndoStack: newUndoStack,
      transcriptRedoStack: newRedoStack,
      transcriptDirty: true,
...
export function clearTranscriptState() {
  transcriptStore.update((ts) => {
    if (
      ts.activeTranscript ||
      ts.segments.length > 0 ||
      ts.transcriptDirty ||
      ts.isTranscriptLoading ||
...
export async function selectMedia(fileEntry, transcriptPathToPrioritize = null) {
  ...
  if (fileEntry && currentSelectedPath === fileEntry.path) {
    const transcriptsChanged =
      JSON.stringify(currentSelectedMedia?.associated_transcripts) !==
      JSON.stringify(fileEntry?.associated_transcripts);
    if (transcriptsChanged) {
      transcriptStore.update((ts) => ({
        ...ts,
        selectedMediaFile: {
          ...ts.selectedMediaFile,
          associated_transcripts: fileEntry.associated_transcripts
        }
      }));
      await loadInitialTranscript(fileEntry, transcriptPathToPrioritize);
    } else if (
      transcriptPathToPrioritize &&
      get(transcriptStore).currentTranscriptPath !== transcriptPathToPrioritize
    ) {
    ...
    const shouldUpdateSelection =
      (!fileEntry && currentSelectedPath !== null) ||
      (fileEntry && currentSelectedPath !== fileEntry.path) ||
      transcriptsChanged;

    let speakersToLoad = { count: 0, names: [], translatedNames: [] };
    if (
      fileEntry &&
      fileEntry.file_type === 'media' &&
      !fileEntry.is_directory &&
      fileEntry.speakers &&
      typeof fileEntry.speakers === 'object'
    ) {
      const loadedCount = Number(fileEntry.speakers['@count']) || 0;
      const loadedNamesRaw = fileEntry.speakers.name;
      const loadedNames = Array.isArray(loadedNamesRaw)
        ? loadedNamesRaw
        : loadedNamesRaw
          ? [loadedNamesRaw]
          : [];

      let loadedTranslatedNamesRaw =
        fileEntry.speakers.translatedNames ||
        fileEntry.speakers.translated_names ||
        fileEntry.speakers.second_names;
      let loadedTranslatedNames = [];

      if (Array.isArray(loadedTranslatedNamesRaw)) {
        loadedTranslatedNames = loadedTranslatedNamesRaw.map((name) =>
      ...
      if (speakersToLoad.count !== speakersToLoad.names.length) {
        console.warn(
          `[TranscriptStore selectMedia] Discrepancy count/names for ${fileEntry.name}. Adjusting.`
        );
    ...
    if (shouldUpdateSelection || speakersChanged) {
      const newSelectedMedia =
        fileEntry && !fileEntry.is_directory && fileEntry.file_type === 'media'
          ? { ...fileEntry }
          : null;
      if (newSelectedMedia && (!newSelectedMedia.name || !newSelectedMedia.path)) {
        console.error(
          '[TranscriptStore] CRITICAL: Attempting set selectedMediaFile without name/path!',
          newSelectedMedia
        );
      }
      if (newSelectedMedia && !newSelectedMedia.media_xml_identifier) {
        console.warn(
          '[TranscriptStore] WARNING: Setting selectedMediaFile without media_xml_identifier! Saving might fail.',
          newSelectedMedia
        );
      }
      ...
      if (
        newlySelectedMedia &&
        Array.isArray(newlySelectedMedia.associated_transcripts) &&
        newlySelectedMedia.associated_transcripts.length > 0
      ) {
...
export function setTranscriptData(path, data, inferSpeakers = false) {
  ...
  transcriptStore.update((ts) => {
    ...
    if (projectRootPath && normalizedInputPath.startsWith(projectRootPath)) {
      relativePathToMatch = normalizedInputPath
        .substring(projectRootPath.length)
        .replace(/^[\\/]/, '');
    }
    ...
    if (!transcriptInfo) {
      console.error(
        `[setTranscriptData] Could not find transcript info for path: ${normalizedInputPath}. Current selectedMediaFile:`,
        mediaFile
      );
      console.error(
        `[setTranscriptData] Available associated_transcripts:`,
        mediaFile?.associated_transcripts
      );
    ...
    const langCode =
      transcriptInfo.language_code ||
      (normalizedInputPath.endsWith('.en.json') ? 'en' : 'original');
    ...
    const hasTranslatedNames =
      updatedSpeakers.translatedNames &&
      updatedSpeakers.translatedNames.some((n) => n && n.trim() !== '');
    const speakerNamesToUse =
      isTranslation && hasTranslatedNames ? updatedSpeakers.translatedNames : updatedSpeakers.names;

    const finalSegmentsForDisplay = remapSegmentSpeakerNames(
      [...newSegments],
      updatedSpeakers,
      speakerNamesToUse
    );
    ...
    updateProjectStoreState({
      statusMessage: `Media transcript loaded: ${targetPath.split(/[\\/]/).pop()}`
    });
...
export function insertTranscriptSegment(index, newSegment) {
  ...
  if (
    !newSegment ||
    typeof newSegment.start_time !== 'number' ||
    typeof newSegment.end_time !== 'number'
  ) {
    console.error(
      '[TranscriptStore] insertTranscriptSegment called with invalid segment data:',
      newSegment
    );
    return;
  }
  ...
  if (get(transcriptStore).isDualModeActive) {
    ...
    }; // Create a twin with empty text
    transcriptStore.update((ts) => {
      const newSegments = [...ts.segments.slice(0, index), newSegment, ...ts.segments.slice(index)];
      const newSecondarySegments = [
        ...ts.secondaryTranscriptSegments.slice(0, index),
        newSecondarySegment,
        ...ts.secondaryTranscriptSegments.slice(index)
      ];
      return {
        ...ts,
        segments: newSegments,
...
export function updateSpeakerConfig(newCount, newNames, newTranslatedNames = null) {
  ...
  for (let i = 0; i < count; i++) {
    let proposedName =
      names[i] && typeof names[i] === 'string' && names[i].trim() !== '' ? names[i].trim() : null;
    let finalName;
    if (proposedName && validatedNames.includes(proposedName)) {
      console.warn(
        `[TranscriptStore updateSpeakerConfig] Duplicate primary name: '${proposedName}'. Using default.`
      );
      proposedName = null;
    }
    if (!proposedName) {
  ...
  if (Array.isArray(newTranslatedNames)) {
    for (let i = 0; i < count; i++) {
      const proposedTranslatedName =
        newTranslatedNames[i] && typeof newTranslatedNames[i] === 'string'
          ? newTranslatedNames[i].trim()
          : '';
  ...
  if (!mediaRelativePath) {
    console.error(
      '[TranscriptStore updateSpeakerConfig] Cannot save: Missing Media Relative Path.'
    );
  ...
  const getSpeakerNamesForPath = (path, langCode, config) => {
    const isTranslation =
      (langCode && langCode.includes('-')) || (path && path.endsWith('.en.json'));
    const hasTranslatedNames =
      config.translatedNames && config.translatedNames.some((n) => n && n.trim() !== '');
  ...
  const newSegments = remapSegmentSpeakerNames(
    [...oldSegments],
    newSpeakerConfig,
    primarySpeakerNamesToUse
  );
  ...
  if (currentTs.isDualModeActive && currentTs.secondaryTranscriptSegments) {
    ...
    newSecondarySegments = remapSegmentSpeakerNames(
      [...currentTs.secondaryTranscriptSegments],
      newSpeakerConfig,
      secondarySpeakerNamesToUse
    );
    if (
      JSON.stringify(currentTs.secondaryTranscriptSegments) !== JSON.stringify(newSecondarySegments)
    ) {
  ...
  transcriptStore.update((ts) => ({
    ...
    segments: newSegments,
    secondaryTranscriptSegments: ts.isDualModeActive
      ? newSecondarySegments
      : ts.secondaryTranscriptSegments,
    transcriptDirty:
      ts.transcriptDirty ||
      JSON.stringify(oldSpeakerConfig) !== JSON.stringify(newSpeakerConfig) ||
      segmentsChanged
  }));
  ...
  invoke('save_speaker_config', { payload: innerPayload })
    .then(async () => {
      ...
      try {
        ...
        if (mediaRelativePath && currentProj.xmlPath) {
          ...
          console.debug(
            `[transcriptStore] Synchronized speaker names to DB for: ${mediaRelativePath}`
          );
        }
      } catch (syncError) {
        console.error(
          '[transcriptStore] Failed to synchronize speaker names to database:',
          syncError
        );
      }
      ...
      projectMainStore.update((p) => {
        ...
        function findAndUpdateMediaSpeakers(nodes, targetIdentifier, newSpeakerData) {
          ...
          for (const node of nodes) {
            if (
              node.media_xml_identifier === targetIdentifier &&
              (node.file_type === 'media' || node.file_type === 'directory_media_stem')
            ) {
              node.speakers = {
        ...
        const didUpdate = findAndUpdateMediaSpeakers(
          updatedFiles,
          mediaIdentifier,
          newSpeakerConfig
        );
        if (didUpdate) {
          ...
          console.warn(
            '[TranscriptStore via projectMainStore] Could not find media identifier in project.files tree to update speakers.'
          );
          return p;
        }
      });
    ...
    .catch((error) => {
      const currentTs = get(transcriptStore);
      const mediaId =
        currentTs.selectedMediaFile?.media_xml_identifier || currentTs.selectedMediaFile?.name;
      console.error(
        `[TranscriptStore updateSpeakerConfig] Failed persist config for ${mediaId}:`,
        error
      );
      const errorMessage = error?.message || String(error);
      updateProjectStoreState({
        error: `Failed save speaker config: ${errorMessage}`,
        statusMessage: 'Error saving speaker config.'
      });
      if (typeof message !== 'undefined') {
        message(`Error saving speaker settings: ${errorMessage}`, {
          title: 'Save Error',
          type: 'error'
        });
      }
...
export function setTranscriptionStatus(isTranscribing, jobIdToSet = null, options = {}) {
  console.log(
    `[DEBUG TS setStatus] Called with: isTranscribing=${isTranscribing}, jobIdToSet=${jobIdToSet}, options=`,
    options
  );
  const {
  ...
  transcriptStore.update((ts) => {
    ...
    if (isTranscribing) {
      if (
        jobIdToSet &&
        ts.transcriptionJobId === jobIdToSet &&
        (ts.transcriptionJobStatus === 'done' ||
          ts.transcriptionJobStatus === 'error' ||
          ts.transcriptionJobStatus === 'cancelled')
      ) {
        console.warn(
          `[DEBUG TS setStatus] Attempted to set job ${jobIdToSet} to active, but it's already in terminal state: ${ts.transcriptionJobStatus}. Ignoring.`
        );
        return ts;
      }

      const newActiveMediaDuringStart =
        mediaPath || ts.selectedMediaFile?.path || ts.activeMediaDuringTranscriptionStart;
      const jobStatusToSet = status || (jobIdToSet ? 'running' : 'initiating');
      const messageToSet =
        initialProgressMessage ||
        (jobStatusToSet === 'initiating' ? `Initiating...` : `Processing...`);

      // Set start time if starting fresh, otherwise keep existing
      const startTime =
        !ts.isTranscribing || !ts.transcriptionStartTime ? Date.now() : ts.transcriptionStartTime;
      ...
      updatedState = {
        ...
        transcriptionProgress: {
          percent:
            ...
            ts.transcriptionJobId === jobIdToSet &&
            ts.transcriptionJobId !== null
              ? ts.transcriptionProgress.percent
              : 0,
    ...
    console.log(
      `[DEBUG TS setStatus Updated] Store updated. New jobStatus=${updatedState.transcriptionJobStatus}, new jobId=${updatedState.transcriptionJobId}, progressMsg='${updatedState.transcriptionProgress.message}', showModal=${updatedState.showTranscribeModal}`
    );
    return updatedState;
  });
...
export function updateTranscriptionProgress(progressPayload) {
  transcriptStore.update((ts) => {
    ...
    if (
      ts.isTranscribing &&
      ts.transcriptionJobStatus === 'initiating' &&
      ts.transcriptionJobId === null
    ) {
      return {
        ...ts,
      ...
      ts.isTranscribing &&
      ts.transcriptionJobStatus === 'running' &&
      ts.transcriptionJobId === eventJobId
    ) {
      return {
        ...ts,
        transcriptionProgress: {
...
export function clearTranscriptionStatus(finalStatusMessage = 'Ready', error = null) {
  transcriptStore.update((ts) => {
    console.log(
      `[DEBUG TS clearStatus] Called. Current store before clear: isTranscribing=${ts.isTranscribing}, jobId=${ts.transcriptionJobId}, jobStatus=${ts.transcriptionJobStatus}`
    );
    return {
      ...ts,
      isTranscribing: false,
...
export async function loadInitialTranscript(mediaFileEntry, transcriptPathToPrioritize = null) {
  ...
  console.log(
    '[TranscriptStore loadInitialTranscript] mediaFileEntry.associated_transcripts:',
  ...
  if (transcriptToLoad) {
    try {
      ...
      console.error(
        `[TranscriptStore loadInitialTranscript] Failed to load prioritized transcript ${transcriptToLoad.path}:`,
        e
      );
      // Try to load the next available transcript if the prioritized one fails
      const remainingTranscripts = sortedTranscripts.filter(
        (t) => t.path !== transcriptToLoad.path
      );
      if (remainingTranscripts.length > 0) {
        ...
        console.error(
          '[TranscriptStore loadInitialTranscript] No other transcripts available to load.'
        );
        transcriptStore.update((ts) => ({
          ...ts,
          isTranscriptLoading: false,
          transcriptErrorMessage: `Failed to load any transcript for ${mediaFileEntry.name}.`
        }));
        updateProjectStoreState({
          statusMessage: `Error loading transcripts for ${mediaFileEntry.name}.`,
          error: `Failed to load any transcript.`
        });
      }
    }
  } else {
    console.warn(
      '[TranscriptStore loadInitialTranscript] No transcript found to load, even after prioritization and sorting.'
    );
    transcriptStore.update((ts) => ({ ...ts, isTranscriptLoading: false }));
    updateProjectStoreState({ statusMessage: `No transcripts found for ${mediaFileEntry.name}.` });
  }
...
function remapSegmentSpeakerNames(segmentsToRemap, speakerConfig, targetSpeakerNames = null) {
  const userNames =
    targetSpeakerNames && targetSpeakerNames.length > 0
      ? targetSpeakerNames
      : speakerConfig && Array.isArray(speakerConfig.names)
        ? speakerConfig.names
        : [];
  ...
  return segmentsToRemap.map((seg) => {
    ...
    if (userAssignedIndex >= 0 && userAssignedIndex < userNames.length) {
      ...
      const sourceNames =
        userNames === speakerConfig.translatedNames
          ? speakerConfig.names
          : speakerConfig.translatedNames;
...
listen('media_renamed', (event) => {
  ...
  const {
    old_media_stem,
    new_media_stem,
    new_media_file_relative_path: rawRelativePath,
    new_absolute_path: rawAbsolutePath
  } = event.payload;
...
listen('custom_transcription_job_completed', async (event) => {
  ...
  const {
    status,
    jobFinishedPath: rawJobFinishedPath,
    transcriptFilePath: rawTranscriptFilePath,
    translatedTranscriptFilePath: rawTranslatedTranscriptFilePath,
    errorMessage
  } = event.payload;
  ...
  if (currentStore.isTranscribing && jobFinishedPath === currentStore.mediaPathForLastJob) {
    ...
    switch (status) {
      ...
      default:
        console.warn(
          `[TranscriptStore] Unknown status in custom_transcription_job_completed: ${status}`
        );
        return;
    }

    updatePayload.showTranscribeModal = shouldShowToastNotification ? false : true;
    updatePayload.transcriptionProgress = {
      ...currentStore.transcriptionProgress,
    ...
    if (status === 'done' && currentStore.selectedMediaFile?.path === jobFinishedPath) {
      try {
        ...
        if (service.refreshProjectFiles && currentStore.selectedMediaFile?.path) {
          console.log(
            '[TranscriptStore] Refreshing project files to update transcript associations.'
          );
          ...
          if (updatedMediaFile) {
            ...
            console.warn(
              `[TranscriptStore] Could not find the updated media file in project store after refresh for path: ${mediaPath}`
            );
          }
        }
      } catch (e) {
...
listen('translation_job_completed', async (event) => {
  ...
  const {
    jobId,
    status,
    originalTranscriptPath: rawOriginalTranscriptPath,
    newTranscriptPath: rawNewTranscriptPath,
    errorMessage
  } = event.payload;
  ...
  if (currentStore.isTranslating && jobId === currentStore.translationJobId) {
    ...
    updatePayload.translationProgress = {
      ...currentStore.translationProgress,
      message: finalProgressMessage
    };
      ...
      try {
        ...
        if (service.refreshProjectFiles) {
          ...
          if (mediaPath) {
            ...
            if (updatedMediaFile) {
              console.log(
                '[TranscriptStore] Re-selecting media after translation completion to update associated transcripts.',
                updatedMediaFile
              );
              selectMedia(updatedMediaFile, newTranscriptPath);
            } else {
              console.warn(
                `[TranscriptStore] Could not find the updated media file in project store after translation refresh for path: ${mediaPath}`
              );
            }
          }
        }
        ...
        console.error(
          '[TranscriptStore] Error refreshing project files after translation completion:',
          e
        );
      }
    }
  ...
  if (store.transcriptDirty) {
    ...
    try {
      ...
      const confirmed = await confirm(
        'Failed to save changes. Deactivate Dual Mode anyway? Changes will be lost.',
        { title: 'Save Failed', type: 'warning' }
      );
      if (!confirmed) return;
    }
  }
...
export async function setSecondaryTranscript(path) {
  ...
  if (store.currentTranscriptPath === targetPath) {
    const otherTranscripts =
      store.selectedMediaFile?.associated_transcripts?.filter((t) => t.path !== targetPath) || [];
    if (otherTranscripts.length > 0) {
      await switchTranscript(otherTranscripts[0].path);
    } else {
      console.error(
        '[TranscriptStore] Cannot switch primary away from secondary: no other transcripts available.'
      );
      return;
    }
  }
  ...
  try {
    ...
    if (primarySegments.length !== segments.length) {
      message(
        'The number of segments between the two transcripts are not the same. Please select a different pair or generate again.',
        { title: 'Segment Mismatch', type: 'error' }
      );
...
export function toggleDualMode(active) {
  const store = get(transcriptStore);
  if (store.transcriptDirty) {
    message('Please save your changes before enabling or disabling Compare Transcripts.', {
  ...
  if (active) {
    ...
    const associatedTranscripts =
      get(transcriptStore).selectedMediaFile?.associated_transcripts || [];
...
export function setTranslationStatus(isTranslating, jobIdToSet = null, options = {}) {
  const { status = null, errorMessage = null, sourcePath = null } = options;
  ...
  transcriptStore.update((ts) => {
    ...
    if (isTranslating) {
      ...
      // Set start time if starting fresh, otherwise keep existing
      const startTime =
        !ts.isTranslating || !ts.translationStartTime ? Date.now() : ts.translationStartTime;
...
export function clearTranslationStatus(finalStatusMessage = 'Ready', error = null) {
  transcriptStore.update((ts) => {
    console.log(
      `[DEBUG TS clearTranslationStatus] Called. Current store before clear: isTranslating=${ts.isTranslating}, jobId=${ts.translationJobId}, jobStatus=${ts.translationJobStatus}`
    );
    return {
      ...ts,
      isTranslating: false,
