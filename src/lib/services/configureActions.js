...
export async function getDownloadedModels() {
  try {
    ...
    const transcriptionModels = Array.isArray(models)
      ? models.filter((model) => {
...
export async function downloadModel(model, downloadLocation) {
  ...
  console.log(
    `Attempting to download model: ${model.name} from ${model.download_url} to ${downloadLocation}`
  );
  try {
...
export async function moveModelsAndUpdateLocation(newLocation) {
  ...
  try {
    ...
    console.log(
      `Backend successfully moved models (if any) and updated location to: ${newLocation}`
    );
...
export async function downloadTranslationModel(
  ...
  console.log(
    `Attempting to download translation model: ${model_name} (family: ${family}) to ${downloadLocation}`
  );
  try {
...
export async function deleteTranslationModel(model) {
  console.log(
    `Attempting to delete translation model: ${model.name || model.id} (family: ${model.family})`
  );
  try {
...
export async function exportTranscript(
  ...
  console.log(
    `[ConfigureActions] Attempting export to "${filePath}" (Format: "${format}", Layout: "${layoutChoice || 'default'}", ExcludeSpeakers: ${excludeSpeakerNames})`
  );

  if (format !== 'docx' && (!segments || segments.length === 0)) {
    // Segments not needed upfront for docx if using transcriptJsonPath
  ...
  if (format === 'docx') {
    ...
    try {
      ...
      console.log(
        '[ConfigureActions] Invoking export_transcript_to_markdown with payload:',
        payload
      );
    ...
    try {
        ...
        if (isValidJson(textData)) {
          try {
            ...
            if (parsedJson && parsedJson.root && Array.isArray(parsedJson.root.children)) {
              textConversionEditor.setEditorState(
                textConversionEditor.parseEditorState(parsedJson)
              );
              return textConversionEditor.getEditorState().read(() => $getRoot().getTextContent());
            } else {
              console.warn(
                '[CSV Export] Invalid Lexical JSON structure detected:',
            ...
            console.warn(
              '[CSV Export] Failed to parse JSON, returning original string:',
              e,
              textData.substring(0, 100)
            );
      ...
      console.log(
        `[ConfigureActions] Writing ${csvContent.length} bytes of CSV content to ${filePath} using writeTextFile`
      );
      ...
      console.error(
        `[ConfigureActions] Error during CSV export generation or file writing:`,
        error
      );
