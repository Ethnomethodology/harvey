...
def download_model(token):
    ...
    try:
        ...
        print(f"Error: Failed to download or load pipeline: {e}", file=sys.stderr)
        sys.exit(1)

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python download_diarization_model.py <HUGGING_FACE_TOKEN>", file=sys.stderr)
        sys.exit(1)
