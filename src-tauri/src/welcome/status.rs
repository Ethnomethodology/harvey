...
pub async fn check_config_status<R: Runtime>(app_handle: AppHandle<R>) -> Result<ConfigStatus, CommandError> {
    ...
    let mut verification_status = config.verification_status.clone();
    let mut config_changed = false;

    // 1. Python Libraries
    let mut python_libs_installed = verification_status.python_libraries_verified;
    ...
    if !python_libs_installed {
        if check_python_libraries_installed(app_handle.clone()).await.unwrap_or(false) {
            ...
            if !verification_status.python_libraries_verified {
                verification_status.python_libraries_verified = true;
                config_changed = true;
            }
        }
    }
    ...
    if hf_token_present != verification_status.hf_token_verified {
        verification_status.hf_token_verified = hf_token_present;
        config_changed = true;
    }

    // 3. Transcription Models
    let mut transcription_models_downloaded = verification_status.transcription_models_verified;
    ...
    if !transcription_models_downloaded {
        ...
        if models.iter().any(|m| !m.name.contains("opus-mt")) {
            ...
            if !verification_status.transcription_models_verified {
                verification_status.transcription_models_verified = true;
                config_changed = true;
            }
        }
    }
    ...
    // 4. Diarization Model
    let mut diarization_model_downloaded = verification_status.diarization_model_verified;
    ...
    if !diarization_model_downloaded {
        if check_diarization_model_access(app_handle.clone()).await.unwrap_or(false) {
            ...
            if !verification_status.diarization_model_verified {
                verification_status.diarization_model_verified = true;
                config_changed = true;
            }
        }
    }
    ...
    let mut translation_models_downloaded = verification_status.translation_models_verified;
    ...
    if !translation_models_downloaded {
        if !get_local_translation_models().await?.is_empty() {
            ...
            if !verification_status.translation_models_verified {
                verification_status.translation_models_verified = true;
                config_changed = true;
            }
        }
    }
    ...
    if config_changed {
        config.verification_status = verification_status;
        write_config(&config)?;
    }
