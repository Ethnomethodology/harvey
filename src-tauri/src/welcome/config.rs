...
impl fmt::Display for CommandError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            CommandError::XmlDeserialization(msg) => {
                write!(f, "XML Deserialization Error: {}", msg)
            }
...
impl From<quick_xml::Error> for CommandError {
    fn from(error: quick_xml::Error) -> Self {
        CommandError::XmlProcessing(error.to_string())
    }
}
impl From<quick_xml::DeError> for CommandError {
    fn from(error: quick_xml::DeError) -> Self {
        CommandError::XmlDeserialization(error.to_string())
    }
}
...
impl From<reqwest::Error> for CommandError {
    fn from(error: reqwest::Error) -> Self {
        CommandError::HttpDownload(error.to_string())
    }
}
impl From<zip::result::ZipError> for CommandError {
    fn from(error: zip::result::ZipError) -> Self {
        CommandError::ZipExtraction(error.to_string())
    }
}
impl From<zip_extract::ZipExtractError> for CommandError {
    fn from(error: zip_extract::ZipExtractError) -> Self {
        CommandError::ZipExtraction(error.to_string())
    }
}
impl From<csv::Error> for CommandError {
    fn from(error: csv::Error) -> Self {
        CommandError::CsvProcessing(error.to_string())
    }
}
impl From<serde_json::Error> for CommandError {
    fn from(error: serde_json::Error) -> Self {
        CommandError::JsonProcessing(error.to_string())
    }
}
impl From<tauri_plugin_shell::Error> for CommandError {
    fn from(error: tauri_plugin_shell::Error) -> Self {
        CommandError::ShellCommand(error.to_string())
    }
}
...
impl From<rusqlite::Error> for CommandError {
    fn from(error: rusqlite::Error) -> Self {
        CommandError::RusqliteError(error.to_string())
    }
}
...
impl From<calamine::XlsxError> for CommandError {
    fn from(error: calamine::XlsxError) -> Self {
        CommandError::Message(error.to_string())
    }
}
impl From<crate::utils::PathError> for CommandError {
    fn from(error: crate::utils::PathError) -> Self {
        CommandError::Path(error.to_string())
    }
}
...
pub fn get_config_dir() -> Result<PathBuf, CommandError> {
    UserDirs::new()
        .map(|dirs| dirs.home_dir().join(CONFIG_DIR_NAME))
        .ok_or_else(|| CommandError::Message("Could not find user home directory.".to_string()))
}
pub fn get_config_file_path() -> Result<PathBuf, CommandError> {
    get_config_dir().map(|dir| dir.join(CONFIG_FILE_NAME))
}
pub fn ensure_config_dir_exists() -> Result<PathBuf, CommandError> {
...
pub fn read_config() -> Result<Config, CommandError> {
    ...
    if config_path.exists() {
        println!(
            "Found legacy config file at {:?}. Migrating to SQLite...",
            config_path
        );
        ...
        if let Ok(settings_iter) = stmt.query_map([], |row| {
            ...
            for result in settings_iter.flatten() {
                ...
                match key.as_str() {
                    ...
                    "selected_translation_family" => {
                        config.selected_translation_family = Some(value)
                    }
                    "selected_transcription_engine" => {
                        config.selected_transcription_engine = Some(value)
                    }
    ...
    if let Ok(mut stmt) = conn.prepare(
        ...
        if let Ok(projects_iter) = stmt.query_map([], |row| {
            ...
            for result in projects_iter.flatten() {
                ...
                let last_opened_ts = chrono::DateTime::parse_from_rfc3339(&format!(
                    "{}Z",
                    last_opened_ts.replace(" ", "T")
                ))
                .map(|dt| dt.with_timezone(&Utc))
                .unwrap_or_else(|_| Utc::now());
...
pub fn write_config(config: &Config) -> Result<(), CommandError> {
    ...
    {
        let mut stmt = tx
            .prepare("INSERT OR REPLACE INTO global_settings (key, value) VALUES (?1, ?2)")
            .map_err(|e| CommandError::RusqliteError(e.to_string()))?;
        stmt.execute(params!["download_location", &config.download_location])
            .map_err(|e| CommandError::RusqliteError(e.to_string()))?;
        if let Some(val) = &config.selected_translation_family {
            stmt.execute(params!["selected_translation_family", val])
                .map_err(|e| CommandError::RusqliteError(e.to_string()))?;
        }
        if let Some(val) = &config.selected_transcription_engine {
            stmt.execute(params!["selected_transcription_engine", val])
                .map_err(|e| CommandError::RusqliteError(e.to_string()))?;
        }
        if let Some(val) = &config.theme {
            stmt.execute(params!["themePreference", val])
                .map_err(|e| CommandError::RusqliteError(e.to_string()))?;
        }
        if let Some(val) = &config.ffmpeg_path {
            stmt.execute(params!["ffmpegPath", val])
                .map_err(|e| CommandError::RusqliteError(e.to_string()))?;
        }
        ...
        if let Some(at) = &config.advanced_translation {
            if let Ok(at_json) = serde_json::to_string(at) {
                stmt.execute(params!["advanced_translation", at_json])
                    .map_err(|e| CommandError::RusqliteError(e.to_string()))?;
            }
        }
        ...
        if let Some(at) = &config.advanced_transcription {
            if let Ok(at_json) = serde_json::to_string(at) {
                stmt.execute(params!["advanced_transcription", at_json])
                    .map_err(|e| CommandError::RusqliteError(e.to_string()))?;
            }
        }
    }
...
pub fn add_or_update_project_in_config(project_info: ProjectInfo) -> Result<(), CommandError> {
    ...
    match config
        .projects
        .iter_mut()
        .find(|p| p.path == project_info.path)
    {
        Some(existing_project) => {
            println!("Updating existing project in config: {}", project_info.name);
            existing_project.last_opened_ts = project_info.last_opened_ts;
            existing_project.name = project_info.name;
        }
        None => {
            println!("Adding new project to config: {}", project_info.name);
            config.projects.push(project_info);
        }
    }
    write_config(&config)
}

pub fn rename_project_in_db(
    old_path: &str,
    new_path: &str,
    new_name: &str,
) -> Result<(), CommandError> {
    ...
    println!(
        "Attempting database update for rename: {} -> {}",
        old_path, new_path
    );
    ...
    let rows_affected = stmt
        .execute(params![new_path, new_name, ts_str, ts_str, old_path])
        .map_err(|e| CommandError::RusqliteError(e.to_string()))?;

    if rows_affected == 0 {
        println!(
            "Warning: No project found in database with path '{}' to rename.",
            old_path
        );
    } else {
        println!(
            "Successfully updated database record for renamed project ({} rows affected).",
            rows_affected
        );
    }
...
pub fn get_default_download_location() -> Result<String, CommandError> {
    let user_dirs =
        UserDirs::new().ok_or_else(|| CommandError::from("Cannot determine user directories."))?;
    let default_location: PathBuf = user_dirs.home_dir().join(CONFIG_DIR_NAME).join("models");
    default_location
        .to_str()
        .ok_or_else(|| {
            CommandError::from("Failed to convert default download location path to string.")
        })
        .map(|s| s.to_string())
}
