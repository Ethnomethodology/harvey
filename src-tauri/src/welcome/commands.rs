...
use crate::welcome::config::{
    ModelInfo, ProjectInfo, CloudConfigPayload, add_or_update_project_in_config, read_config, write_config, // Keep these config functions
...
#[command] pub async fn get_downloaded_models() -> Result<Vec<ModelInfo>, CommandError> { /* ... */ let config = read_config()?; log::info!("Config: Returning {} downloaded models.", config.downloaded_models.len()); Ok(config.downloaded_models) }
#[command] pub async fn delete_model(model_to_delete: ModelInfo) -> Result<(), CommandError> { /* ... */ log::info!("Config: delete_model: {}", model_to_delete.name); let mut config = read_config()?; let initial_len = config.downloaded_models.len(); let base_location = if !config.download_location.trim().is_empty() { config.download_location.clone() } else { log::warn!("Config/Delete: Download location empty, using default."); get_default_download_location()? }; if base_location.trim().is_empty() { return Err(format!("Cannot determine download location for '{}'.", model_to_delete.name).into()); } let model_path = PathBuf::from(base_location).join(&model_to_delete.name); if model_path.exists() { log::info!("Config: Deleting model path: {:?}", model_path); if model_path.is_dir() { fs::remove_dir_all(&model_path)?; log::info!("Config: Deleted directory."); } else if model_path.is_file() { log::warn!("Expected model path {:?} is file, deleting.", model_path); fs::remove_file(&model_path)?; } else { log::warn!("Model path {:?} not file/dir. Skipping delete.", model_path); } } else { log::info!("Config: Model path {:?} missing. No delete needed.", model_path); } config.downloaded_models.retain(|m| m.name != model_to_delete.name); if config.downloaded_models.len() < initial_len { write_config(&config)?; log::info!("Config: Deleted entry '{}' from config.", model_to_delete.name); } else { log::warn!("Model '{}' not in config list.", model_to_delete.name); } Ok(()) }
...
#[command] pub async fn cancel_download_command( cancellation_state: State<'_, DownloadCancellationState>, model_name: String, ) -> Result<(), CommandError> { /* ... */ log::info!("CMD: cancel_download: {}", model_name); if let Some(flag_entry)=cancellation_state.0.get(&model_name){let flag=flag_entry.value(); flag.store(true,Ordering::Relaxed); log::info!("Cancel flag set for {}",model_name);} else {log::warn!("No active download token for '{}'.",model_name);} Ok(()) }
 ...
 // --- Cloud Configuration Commands (Unchanged) ---
 #[command] pub async fn get_cloud_config() -> Result<CloudConfigPayload, CommandError> { /* ... */ log::info!("Config: get_cloud_config called."); let config = read_config()?; let payload = CloudConfigPayload { api_key: config.cloud_api_key, model: config.cloud_model, consent: config.cloud_consent }; log::info!("Config: Returning cloud config: key_present={}, model={:?}, consent={:?}", payload.api_key.is_some(), payload.model, payload.consent); Ok(payload) }
 #[command] pub async fn save_cloud_config(payload: CloudConfigPayload) -> Result<(), CommandError> { /* ... */ log::info!("Config: save_cloud_config called: key_present={}, model={:?}, consent={:?}", payload.api_key.is_some(), payload.model, payload.consent); let api_key_to_save = payload.api_key.filter(|k| !k.trim().is_empty()); let mut config = read_config()?; config.cloud_api_key = api_key_to_save; config.cloud_model = payload.model; config.cloud_consent = payload.consent; write_config(&config)?; log::info!("Config: Cloud configuration saved."); Ok(()) }
