...
fn resolve_model_path(base_location: &str, model: &ModelInfo) -> (PathBuf, bool) {
    ...
    let is_crisper_whisper = family == "crisper-whisper";
    ...
    let folder_name = if (is_translation || is_faster_whisper || is_crisper_whisper) && model.name.contains('/') {
    ...
    } else if is_crisper_whisper {
        PathBuf::from("transcription").join("crisper-whisper")
