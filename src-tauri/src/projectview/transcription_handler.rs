...
fn time_str_to_seconds(time_str: &str) -> Result<f64, CommandError> {
    ...
    let hours = parts[0]
        .parse::<f64>()
        .map_err(|_| CommandError::from(format!("Invalid hours in time string: {}", time_str)))?;
    let minutes = parts[1]
        .parse::<f64>()
        .map_err(|_| CommandError::from(format!("Invalid minutes in time string: {}", time_str)))?;
    let seconds = parts[2]
        .parse::<f64>()
        .map_err(|_| CommandError::from(format!("Invalid seconds in time string: {}", time_str)))?;
...
fn parse_transcript_block(
    transcript_text_content: &str,
    ...
    let re_timestamp_speaker = Regex::new(r"^(\d{2}:\d{2}:\d{2})\s+(Speaker\s*\d+):?\s*$")
        .map_err(|e| {
            CommandError::from(format!(
                "Regex compilation for timestamp/speaker failed: {}",
                e
            ))
        })?;

    // Matches "HH:MM:SS" on a single line
    let re_timestamp_only = Regex::new(r"^(\d{2}:\d{2}:\d{2})\s*$").map_err(|e| {
        CommandError::from(format!(
            "Regex compilation for timestamp_only failed: {}",
            e
        ))
    })?;

    // Matches "Speaker N" on a single line (for split headers)
    let re_speaker_only = Regex::new(r"^(Speaker\s*\d+):?\s*$").map_err(|e| {
        CommandError::from(format!("Regex compilation for speaker_only failed: {}", e))
    })?;

    for line in transcript_text_content.lines() {
        let trimmed_line = line.trim();
        debug!(
            "[parse_transcript_block] Processing line: '{}'",
            trimmed_line
        );
        ...
        if let Some(caps) = re_timestamp_speaker.captures(trimmed_line) {
            ...
            debug!(
                "[parse_transcript_block] Matched timestamp+speaker line: '{}'",
                trimmed_line
            );
            ...
            debug!(
                "[parse_transcript_block] Matched timestamp only line: '{}'",
                trimmed_line
            );
            ...
            debug!(
                "[parse_transcript_block] Matched speaker only line: '{}'",
                trimmed_line
            );
            ...
            if let Some(ref mut seg) = current_segment {
                debug!(
                    "[parse_transcript_block] Appending to segment '{}': '{}'",
                    seg.speaker, trimmed_line
                );
                if !seg.text.is_empty() && !seg.text.ends_with('\n') {
                    seg.text.push(' ');
                }
                ...
                debug!(
                    "[Transcript Parse] Ignoring line (no current segment): {}",
                    trimmed_line
                );
            }
        }
    ...
    if let Some(mut seg) = current_segment.take() {
        ...
        if seg.end_time == 0.0 {
            ...
            info!(
                "[Transcript Parse] Setting end time for final segment ({}) to: {:.2}s",
                seg.speaker, seg.end_time
            );
        }
        if !seg.text.is_empty() {
            segments.push(seg);
            ...
            warn!(
                "[Transcript Parse] Final segment (speaker {}) had no text, discarding.",
                seg.speaker
            );
        }
    }
    ...
    debug!(
        "[parse_transcript_block] Finished parsing. Total segments: {}",
        segments.len()
    );
...
pub async fn import_word_transcript<R: Runtime>(
    ...
    let original_docx_filename = source_docx_path
        .file_name()
        .and_then(|s| s.to_str())
    ...
    let truncated_transcript_base_name =
        truncate_filename_stem(&original_docx_filename, MAX_FILENAME_STEM_LENGTH);
    let transcript_filename_stem_truncated = Path::new(&truncated_transcript_base_name)
        .file_stem() // Get stem from "truncated.docx"
        .and_then(|s| s.to_str())
        .unwrap_or("standalone_transcript") // Fallback
        .to_string();
    info!(
        "[import_word_transcript] Original DOCX: '{}', Truncated base for transcript: '{}'",
        original_docx_filename, transcript_filename_stem_truncated
    );
    ...
    let transcripts_root = project_base_dir
    ...
    let (_final_transcript_folder, _final_json_filename, transcript_filename_stem) = loop {
        ...
        let name_conflict = project_data
            .standalone_transcript_files
            .files
            .iter()
            .any(|f| f.name == current_stem || f.name.starts_with(&format!("{}.", current_stem)));

        if !candidate_folder.exists() && !name_conflict {
            let file_name = format!("{}.json", current_stem);
            debug!(
                "[import_word_transcript] Found unique transcript folder: {} and filename: {}",
        ...
        if folder_counter > 1000 {
            return Err(CommandError::from(format!(
                "Could not find unique folder for transcript stem '{}' after 1000 attempts.",
                transcript_filename_stem_truncated
            )));
        }
    };

    let temp_html_dir = project_base_dir
        .join(HARVEY_FILES_DIR)
        .join(DOCS_DIR)
        .join(TEMP_SUBDIR_DOCS);
    fs::create_dir_all(&temp_html_dir)
        .map_err(|e| CommandError::from(format!("Failed to create temp dir for HTML: {}", e)))?;
    let unique_id = Uuid::new_v4().to_string();
    // Use current stem for temp file to keep it shorter as well, though not strictly necessary for temp file.
    let temp_html_filename = format!(
        "temp_transcript_html_{}_{}.html",
        transcript_filename_stem, unique_id
    );
    let temp_html_path = temp_html_dir.join(&temp_html_filename);

    let script_path_raw = app_handle
        .path()
        .resolve(
    ...
    let script_path = canonicalize_path(&script_path_raw).unwrap_or(script_path_raw); // Best effort, fallback to raw if fails (e.g. if file doesn't exist yet, though it should)
    ...
    info!(
        "[import_word_transcript] Pandoc CMD: {} {}",
        script_path.display(),
        pandoc_args.join(" ")
    );
        ...
        match event {
            ...
            tauri_plugin_shell::process::CommandEvent::Terminated(payload) => {
                if payload.code == Some(0) {
                    ...
                    error!(
                        "[import_word_transcript] Pandoc failed. Code: {:?}, Stderr: {}",
                        payload.code, pandoc_output_stderr
                    );
                }
                break;
            }
    ...
    for line_stripped_of_tags in full_stripped_text.lines() {
        if transcript_heading_re.is_match(line_stripped_of_tags) {
            debug!(
                "[HTML Parse] Found 'Transcript' heading: '{}'. Clearing previous collected lines.",
                line_stripped_of_tags
            );
    ...
    info!(
        "[import_word_transcript] Collected {} lines (Transcript header found or implicit): {:?}",
        collected_lines_for_block.len(),
        collected_lines_for_block
    );
    ...
    info!(
        "[import_word_transcript] Parsed {} transcript segments.",
        segments.len()
    );

    let target_standalone_transcripts_dir = project_base_dir
        .join(HARVEY_FILES_DIR)
        .join(TRANSCRIPTS_DIR);
    fs::create_dir_all(&target_standalone_transcripts_dir).map_err(|e| {
        CommandError::from(format!(
            "Failed to create standalone transcripts dir: {}",
            e
        ))
    })?;
    ...
    let new_transcript_filename = final_transcript_path
        .file_name()
        .unwrap()
        .to_string_lossy()
        .to_string();
    ...
    fs::write(&final_transcript_path, json_content).map_err(|e| {
        CommandError::from(format!(
            "Failed to save transcript JSON to {}: {}",
            final_transcript_path.display(),
            e
        ))
    })?;
    info!(
        "[import_word_transcript] Saved standalone transcript to: {}",
        final_transcript_path.display()
    );
    ...
    let project_data_for_uuid: ProjectXml = serde_json::from_str(&project_xml_content_for_uuid)
        .map_err(|e| {
            CommandError::XmlDeserialization(format!("Failed to parse project XML for UUID: {}", e))
        })?;
    ...
    if let Ok(assets) = db_handler::get_project_assets_for_link(&project_id_for_db) {
        if let Some(matching_media) = assets.iter().find(|a| {
            let is_media =
                a.file_type.as_deref() == Some("video") || a.file_type.as_deref() == Some("audio");
            if !is_media {
                return false;
            }

            let media_filename = Path::new(&a.name)
                .file_stem()
                .and_then(|s| s.to_str())
                .unwrap_or("");
    ...
    if let Err(e) = db_handler::save_asset_metadata(
        ...
        error!(
            "Failed to save transcript metadata to DB for {}: {} (project_id: {})",
            asset_relative_path_for_db, e, project_id_for_db
        );
        // Consider cleaning up the created JSON file if DB save fails
        return Err(CommandError::from(format!(
            "Failed to save transcript metadata to DB: {}",
            e
        )));
    }
    info!(
        "[import_word_transcript] Saved transcript metadata to DB for: {}",
    ...
    if !project_data
        ...
        .files
        .iter()
        .any(|t| t.relative_path == new_standalone_transcript_entry.relative_path)
    {
        project_data
            .standalone_transcript_files
            .files
            .push(new_standalone_transcript_entry);
        project_data
            .standalone_transcript_files
            .files
...
pub async fn save_standalone_transcript_and_update_xml(
    ...
    info!(
        "[Backend Save Imported Transcript] Target Path: {}",
        target_path
    );
    ...
    if let Some(parent) = target_path_buf.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| CommandError::from(format!("Failed to create parent dir: {}", e)))?;
    }
    ...
    // Update XML
    let mut project_data: ProjectXml =
        serde_json::from_str(&fs::read_to_string(&project_xml_path_buf)?)?;
    ...
    if !project_data
        ...
        .files
        .iter()
        .any(|t| t.relative_path == relative_path)
    {
        project_data
            .standalone_transcript_files
            .files
        ...
        info!(
            "[Backend Save Imported Transcript] Added XML entry: {}",
            relative_path
        );
    } else {
        // Update name if changed
        if let Some(pos) = project_data
            .standalone_transcript_files
            .files
            .iter()
...
mod tests {
    ...
    use crate::projectview::shared_types::{
        FileMetadata, ProjectXml, StandaloneTranscriptEntryXml, TranscriptSegment,
    };
    use chrono::Utc;
    use rusqlite::{Connection, OptionalExtension};
    use serde_json; // For serializing segments in test setup // For in-memory DB

    #[test]
    fn test_metadata_saved_to_db_and_xml_updated_correctly(
        ...
        assert!(
            final_transcript_path.exists(),
            "Transcript file should be created for test setup"
        );
        ...
        // Replicating db_handler::save_asset_metadata's core SQL for the test:
        {
            ...
            conn_test_db.execute(
                ...
                rusqlite::params![
                    ...
                    db_handler::to_sql_optional_str(
                        file_metadata_for_db_obj.audio_codec.as_deref()
                    ),
                    db_handler::to_sql_optional_str(
                        file_metadata_for_db_obj.video_codec.as_deref()
                    ),
                    ...
                    db_handler::to_sql_optional_str(
                        file_metadata_for_db_obj.original_import_path.as_deref()
                    ),
                    db_handler::to_sql_optional_str(None), // speaker_names_json
                    db_handler::to_sql_optional_blob(
                        file_metadata_for_db_obj.waveform_data.as_deref()
                    ),
                    db_handler::to_sql_optional_str(
                        file_metadata_for_db_obj.language_code.as_deref()
                    ),
        ...
        let mut current_project_data: ProjectXml =
            serde_json::from_str(&fs::read_to_string(&project_xml_path)?)?;
        ...
        if !current_project_data
            ...
            .files
            .iter()
            .any(|t| t.relative_path == new_standalone_transcript_entry_obj.relative_path)
        {
            current_project_data
                .standalone_transcript_files
                .files
                .push(new_standalone_transcript_entry_obj.clone());
            current_project_data
                .standalone_transcript_files
                .files
                .sort_by(|a, b| a.name.cmp(&b.name));
        ...
        let loaded_db_meta_opt: Option<db_handler::FileMetadataWithCustomFieldsFromDb> = {
            ...
            stmt_load
                .query_row(
                    rusqlite::params!["test_uuid_db_xml", asset_relative_path_for_db_str],
                    |row| {
        ...
        assert!(
            loaded_db_meta_opt.is_some(),
            "Metadata should be found in DB"
        );
        if let Some(loaded_db_meta_val) = loaded_db_meta_opt {
            assert_eq!(
                loaded_db_meta_val.file_name,
                file_metadata_for_db_obj.file_name
            );
            assert_eq!(
                loaded_db_meta_val.file_path,
                file_metadata_for_db_obj.file_path
            );
            assert_eq!(
                loaded_db_meta_val.title.unwrap_or_default(),
                file_metadata_for_db_obj.title
            );
        ...
        // Verify XML Data
        let updated_project_data_from_xml_check: ProjectXml =
            serde_json::from_str(&fs::read_to_string(&project_xml_path)?)?;
        assert_eq!(
            updated_project_data_from_xml_check
                .standalone_transcript_files
                .files
                .len(),
            1
        ...
        assert_eq!(
            updated_project_data_from_xml_check
                ...
                .files[0]
                .name,
            new_standalone_transcript_entry_obj.name
        );
        assert!(
            updated_project_data_from_xml_check
                ...
                .files
                .is_empty(),
            "Document metadata files list in XML should be empty regarding this transcript."
        );
