...
pub async fn get_project_assets_for_link_command(
    ...
    debug!(
        "[CMD] get_project_assets_for_link_command for project_id: {}",
        project_id
    );
    db_handler::get_project_assets_for_link(&project_id).map_err(|e| e.to_string())
}
...
pub async fn get_asset_metadata_command(
    ...
    match db_handler::load_asset_metadata(&project_id, &asset_relative_path) {
        Ok(Some(mut base_metadata)) => {
            ...
            match db_handler::load_media_transcript_data(&project_id, &asset_relative_path) {
                Ok(Some(media_data)) => {
                    ...
                    info!(
                        "[CMD] Successfully loaded base and media_transcript_data for {} - {}",
                        project_id, asset_relative_path
                    );
                }
                Ok(None) => {
        ...
        Ok(None) => {
            info!(
                "[CMD] No asset_metadata found for {} - {}",
                project_id, asset_relative_path
            );
            Ok(None)
        }
...
pub async fn update_asset_metadata_command(
    ...
    debug!(
        "[CMD] update_asset_metadata_command for project_xml: {}, asset_path: {}, type: {}",
        project_xml_path_str, asset_relative_path, asset_type
    );
    ...
    sanitized_metadata_for_db.file_name = path_obj
        .file_name()
    ...
    let custom_fields_json_string: Option<String> =
        custom_fields_payload.and_then(|json_val| serde_json::to_string(&json_val).ok());
...
use crate::projectview::db_handler::{
    ...
    get_all_custom_field_definitions, // get_custom_field_definition, // Import if individual get needed later
...
pub async fn create_custom_field_definition_command(
    ...
    field_name: String,
    field_type: String,
    scope_str: String, // default_value: Option<String> // Removed from signature
) -> Result<(), String> {
    ...
    match add_custom_field_definition(project_id.as_str(), &definition) {
        Ok(_) => {
            info!(
                "[CMD] Custom field definition created successfully for project_id {}: {}",
        ...
        Err(e) => {
            error!(
                "[CMD] Error creating custom field definition for project_id {}: {}: {}",
                project_id, field_key, e
            );
            Err(format!(
                "Failed to create custom field definition for project_id '{}', key '{}': {}",
                project_id, field_key, e
            ))
        }
    }
...
pub async fn get_all_custom_field_definitions_command(
    ...
    debug!(
        "[CMD] get_all_custom_field_definitions_command called for project_id: {}",
        project_id
    );
    match get_all_custom_field_definitions(project_id.as_str()) {
        Ok(definitions) => {
            info!(
                "[CMD] Retrieved {} custom field definitions for project_id {}.",
                definitions.len(),
                project_id
            );
            Ok(definitions)
        ...
        Err(e) => {
            error!(
                "[CMD] Error retrieving all custom field definitions for project_id {}: {}",
                project_id, e
            );
            Err(format!(
                "Failed to retrieve custom field definitions for project_id {}: {}",
                project_id, e
            ))
        }
    }
