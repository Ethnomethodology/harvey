...
#[tauri::command]
pub async fn get_xlsx_sheets(source_path_str: String) -> Result<Vec<String>, CommandError> {
...
pub async fn import_table_file(
    ...
    if !source_path.exists() || !source_path.is_file() {
        error!(
            "[import_table_file] Source table file not found: {}",
            source_path_str
        );
        return Err(CommandError::from(format!(
    ...
    let project_xml_content_for_uuid = fs::read_to_string(&project_xml_path).map_err(|e| {
        error!(
            "[import_table_file] Failed to read project XML for UUID from {}: {}",
            project_xml_path.display(),
            e
        );
    ...
    let project_data_for_uuid: ProjectXml = serde_json::from_str(&project_xml_content_for_uuid)
        .map_err(|e| {
            error!(
                "[import_table_file] Failed to parse project XML for UUID from {}: {}",
                project_xml_path.display(),
                e
            );
    ...
    if project_id_for_db.is_empty() {
        ...
        return Err(CommandError::Message(format!(
            "Project ID (UUID) is missing in the project file ({}). Table import cannot proceed.",
            project_xml_path.display()
        )));
    }
    debug!(
        "[import_table_file] Project ID for DB: {}",
        project_id_for_db
    );
    ...
    let original_source_filename_with_ext = match source_path.file_name().and_then(|s| s.to_str()) {
        ...
        None => {
            error!(
                "[import_table_file] Could not get original table filename with extension from: {}",
                source_path.display()
            );
            return Err(CommandError::from(
                "Could not get original table filename with extension",
            ));
        }
    };
    debug!(
        "[import_table_file] Original source filename: {}",
        original_source_filename_with_ext
    );

    let original_source_extension = source_path
        .extension()
        .and_then(|s| s.to_str())
        .map(|s| s.to_lowercase())
        .unwrap_or_default();
    debug!(
        "[import_table_file] Original source extension: {}",
        original_source_extension
    );

    if original_source_extension != "csv" && original_source_extension != "xlsx" {
        error!(
            "[import_table_file] Unsupported table file type: .{}",
            original_source_extension
        );
        return Err(CommandError::from(format!(
    ...
    let stem_base =
        if original_source_extension == "xlsx" && sheet_name_opt.is_some() && should_append_sheet {
            let original_stem = Path::new(&original_source_filename_with_ext)
                .file_stem()
                .and_then(|s| s.to_str())
                .unwrap_or("");
    ...
    let table_file_stem_truncated = Path::new(&stem_base)
        .file_stem()
        .and_then(|s| s.to_str())
        .ok_or_else(|| {
            error!(
                "[import_table_file] Could not get stem from truncated table filename: {}",
                stem_base
            );
            CommandError::from(format!(
                "Could not get stem from truncated table filename: {}",
                stem_base
            ))
        })?;
    debug!(
        "[import_table_file] Truncated table file stem: {}",
        table_file_stem_truncated
    );

    info!(
        "[import_table_file] Updating project XML to include table: {}",
        original_source_filename_with_ext
    );
    let xml_content = fs::read_to_string(&project_xml_path).map_err(|e| {
        error!(
            "[import_table_file] Failed to read project XML for update from {}: {}",
            project_xml_path.display(),
            e
        );
        CommandError::Io(format!(
            "Failed to read project XML for update from {}: {}",
            project_xml_path.display(),
            e
        ))
    })?;
    let mut project_data: ProjectXml = serde_json::from_str(&xml_content).map_err(|e| {
        error!(
            "[import_table_file] Failed to parse project XML for update from {}: {}",
            project_xml_path.display(),
            e
        );
        CommandError::XmlDeserialization(format!(
            "Failed to parse project XML for update from {}: {}",
            project_xml_path.display(),
            e
        ))
    })?;
    ...
    let (folder_path, final_table_filename) = loop {
        ...
        if !candidate_folder.exists() && !name_conflict {
            ...
            debug!(
                "[import_table_file] Found unique folder path: {} and filename: {}",
                candidate_folder.display(),
                file_name
            );
    ...
    fs::create_dir_all(&folder_path).map_err(|e| {
        error!(
            "[import_table_file] Failed to create directory {}: {}",
            folder_path.display(),
            e
        );
        CommandError::Io(format!(
    ...
    debug!(
        "[import_table_file] Created unique folder: {}",
        folder_path.display()
    );
    ...
    let relative_path_for_xml = final_table_path
        ...
        .map_err(|e| {
            error!(
                "[import_table_file] Failed to strip prefix {} from {}: {}",
                project_base_dir.display(),
                final_table_path.display(),
                e
            );
        ...
        .replace("\\", "/");
    debug!(
        "[import_table_file] Relative path for XML: {}",
    ...
    if project_data
        ...
        .files
        .iter()
        .any(|f| f.relative_path == new_table_entry.relative_path)
    {
        ...
        if let Some(existing_entry) = project_data
            ...
            .files
            .iter_mut()
            .find(|f| f.relative_path == new_table_entry.relative_path)
        {
    ...
    save_project_xml(&project_xml_path, &project_data).map_err(|e| {
        error!(
            "[import_table_file] Failed to save project XML {}: {}",
            project_xml_path.display(),
            e
        );
        e
    ...
    info!(
        "[import_table_file] Saved table metadata to DB for: {} (project_id: {})",
        relative_path_for_xml, project_id_for_db
    );
    ...
    let target_extension = final_table_path
        .extension()
        .and_then(|e| e.to_str())
        .unwrap_or("");
    let preview_data = match target_extension {
        ...
        _ => {
            error!(
                "[import_table_file] Unsupported table extension for preview: {}",
                target_extension
            );
            return Err(CommandError::from(format!(
                "Unsupported table extension for preview: {}",
                target_extension
            )));
        }
    }?;
...
pub async fn delete_table_column(
    ...
    let mut loaded_value = match extension.as_str() {
        ...
        _ => {
            return Err(CommandError::from(format!(
                "Unsupported table extension for deleting column: {}",
                extension
            )))
        }
    };
    ...
    let original_headers_val = loaded_value
        .get("headers")
    ...
    let mut data_value = loaded_value
        .get("data")
        .ok_or_else(|| CommandError::from("Loaded table data is missing 'data' field"))?
        .clone();

    let new_headers: Vec<String> = original_headers
        .into_iter()
...
pub async fn set_table_headers(
    ...
    if !table_path.exists() {
        error!(
            "[set_table_headers] Table file does not exist at: {}",
            table_path.display()
        );
        return Err(CommandError::from(format!(
    ...
    if !project_xml_path.exists() {
        error!(
            "[set_table_headers] Project XML file does not exist at: {}",
            project_xml_path.display()
        );
        return Err(CommandError::from(format!(
            "Project XML file not found: {}",
            project_xml_path.to_string_lossy()
        )));
    }
    ...
    if !has_headers {
        ...
        let loaded_data = match extension.as_str() {
            ...
            _ => {
                return Err(CommandError::from(format!(
                    "Unsupported table extension for header generation: {}",
                    extension
                )))
            }
        };

        let headers_val = loaded_data
            .get("headers")
            .ok_or_else(|| CommandError::from("Missing 'headers' in loaded data"))?;
        let data_val = loaded_data
            .get("data")
            .ok_or_else(|| CommandError::from("Missing 'data' in loaded data"))?;
    ...
    if let Some(table_entry) = project_data
        .table_files
        .files
        .iter_mut()
        .find(|f| f.relative_path == relative_path_for_xml)
    {
        ...
        return Err(CommandError::from(format!(
            "Table not found in project XML: {}",
            relative_path_for_xml
        )));
    }
...
pub async fn load_table_data(table_path_str: String) -> Result<Value, CommandError> {
    ...
    if !table_path.exists() || !table_path.is_file() {
        error!(
            "[load_table_data] Table file not found or is not a file: {}",
            table_path.display()
        );
        return Err(CommandError::from(format!(
            "Table file not found: {}",
    ...
    let project_xml_path = get_project_xml_path_from_item(&table_path)?;
    debug!(
        "[load_table_data] Derived project_xml_path: {}",
        project_xml_path.display()
    );

    if !project_xml_path.exists() || !project_xml_path.is_file() {
        error!(
            "[import_table_file] Project XML file not found at: {}",
            project_xml_path.display()
        );
        return Err(CommandError::from(format!(
            "Project XML file not found: {}",
    ...
    let relative_path_for_xml = table_path
        .strip_prefix(project_xml_path.parent().ok_or_else(|| {
            CommandError::from("Could not get project base directory from project XML path")
        })?)?
        .to_string_lossy()
        .replace("\\", "/");
    ...
    let data = match extension.as_str() {
        ...
        _ => Err(CommandError::from(format!(
            "Unsupported table extension for loading: {}",
            extension
        ))),
    }?;
    ...
    if let Some(data_array) = data.get("data").and_then(|d| d.as_array()) {
        debug!(
            "[load_table_data] Successfully loaded {} rows.",
            data_array.len()
        );
    }
    Ok(data)
...
fn load_csv_data(
    ...
    let mut rdr = csv::ReaderBuilder::new()
        ...
        .from_path(path)
        .map_err(|e| {
            CommandError::from(format!("Failed to open CSV '{}': {}", path.display(), e))
        })?;
...
fn extract_xlsx_sheet_to_csv(
    ...
    let range: calamine::Range<Data> = match range_opt {
        Ok(r) => r,
        Err(calamine::XlsxError::WorksheetNotFound(msg)) => {
            return Err(CommandError::from(format!(
                "Sheet '{}' not found in XLSX: {}",
                sheet_name, msg
            )))
        }
        Err(e) => {
    ...
    let mut wtr = csv::Writer::from_path(final_table_path).map_err(|e| {
        CommandError::from(format!(
...
fn load_xlsx_data(
    ...
    let mut workbook: Xlsx<_> = open_workbook(path).map_err(|e| {
        CommandError::from(format!("Failed to open XLSX '{}': {}", path.display(), e))
    })?;
...
#[tauri::command]
pub async fn save_table_data(
    table_path_str: String,
    table_data: Vec<Value>,
    headers: Vec<String>,
) -> Result<(), CommandError> {
    ...
    match extension.as_str() {
        ...
        _ => Err(CommandError::from(format!(
            "Unsupported table extension for saving: {}",
            extension
        ))),
    }
}

fn save_xlsx_data_with_headers(
    path: &Path,
    data: Vec<Value>,
    headers: &[String],
) -> Result<(), CommandError> {
                    ...
                    if let Some(ref style_map) = styles {
                        ...
                        if let Some(style_val) = style_map.get(&coord_key) {
                            if let Some(style_obj) = style_val.as_object() {
                                if let Some(is_bold) =
                                    style_obj.get("bold").and_then(|v| v.as_bool())
                                {
                                    if is_bold {
                                        cell_format = cell_format.set_bold();
                                    }
                                }
                                if let Some(is_italic) =
                                    style_obj.get("italic").and_then(|v| v.as_bool())
                                {
                                    if is_italic {
                                        cell_format = cell_format.set_italic();
                                    }
                                }
                                if let Some(is_underline) =
                                    style_obj.get("underline").and_then(|v| v.as_bool())
                                {
                                    if is_underline {
                                        cell_format = cell_format.set_underline(
                                            rust_xlsxwriter::FormatUnderline::Single,
                                        );
                                    }
                                }
                                if let Some(text_color_val) =
                                    style_obj.get("textColor").and_then(|v| v.as_str())
                                {
                                    if !text_color_val.is_empty() {
                                        ...
                                        {
                                            cell_format = cell_format.set_font_color(
                                                rust_xlsxwriter::Color::RGB(color_num),
                                            );
                                        }
                                    }
                                }
                                if let Some(color_val) =
                                    style_obj.get("color").and_then(|v| v.as_str())
                                {
                                    if !color_val.is_empty() {
                                        ...
                                        if let Ok(color_num) = u32::from_str_radix(clean_color, 16)
                                        {
                                            cell_format = cell_format.set_background_color(
                                                rust_xlsxwriter::Color::RGB(color_num),
                                            );
                                        }
                                    }
...
fn get_headers(path: &Path) -> Result<Vec<String>, CommandError> {
    ...
    match extension.as_str() {
        ...
        _ => Err(CommandError::from(format!(
            "Unsupported table extension for getting headers: {}",
            extension
        ))),
    }
}
...
pub async fn rename_table_header(
    ...
    let loaded_value = match extension.as_str() {
        ...
        _ => {
            return Err(CommandError::from(format!(
                "Unsupported table extension for renaming header: {}",
                extension
            )))
        }
    };
    ...
    let original_headers: Vec<String> = serde_json::from_value(original_headers_val.clone())
        .map_err(|e| {
            CommandError::from(format!("Failed to parse headers from loaded data: {}", e))
        })?;
...
mod tests {
    ...
    async fn test_import_table_file_with_db_metadata() -> Result<(), Box<dyn std::error::Error>> {
        let (_temp_dir, project_dir, dummy_table_path, _conn, project_uuid) =
            setup_test_environment();
        let project_xml_path =
            create_dummy_project_xml(&project_dir, "TestTableProject", &project_uuid);
        ...
        let import_result =
            import_table_file(dummy_table_path_str.clone(), project_xml_path_str.clone()).await;
        assert!(
            import_result.is_ok(),
            "import_table_file failed: {:?}",
            import_result.err()
        );
        ...
        assert!(
            final_table_abs_path.exists(),
            "Imported table file should exist at {}",
            final_table_abs_path_str
        );
        ...
        assert!(
            !metadata_json_path.exists(),
            ".metadata.json file should NOT exist at {}",
        ...
        let expected_relative_path = final_table_abs_path
            .strip_prefix(&project_dir)?
            .to_string_lossy()
            .replace("\\", "/");

        assert_eq!(
            updated_project_data.table_files.files.len(),
            1,
            "Should be one table file in XML"
        );
        ...
        assert!(
            updated_project_data
                ...
                .files
                .is_empty(),
            "document_metadata_files should be empty in XML"
        );

        let loaded_meta_option: Option<FileMetadataWithCustomFieldsFromDb> =
            db_handler::load_asset_metadata("test-uuid", &expected_relative_path)
                .expect("Failed to load metadata from DB for assertion");

        assert!(
            loaded_meta_option.is_some(),
            "Metadata should be found in DB for relative path: {}",
...
pub async fn save_table_schema(
    ...
    info!(
        "[save_table_schema] Saving schema for table: {} in project {}",
...
pub async fn load_table_schema(
    ...
    let normalized_path = table_path.replace("\\", "/");
    debug!(
        "[load_table_schema] Loading schema for table: {} in project {}",
        normalized_path, project_id
    );
...
pub async fn create_new_table(
    ...
    info!(
        "[Backend Create Table] Start: project_xml_path={}, headers={:?}, has_schema={}",
        project_xml_path,
        headers,
        schema.is_some()
    );
    ...
    loop {
        ...
        let relative_path = format!(
            "{}/{}/{}/{}",
            HARVEY_FILES_DIR, TABLES_DIR, base_name, table_name
        );
        if !folder_path.exists()
            && !project_data
                ...
                .files
                .iter()
                .any(|f| f.relative_path == relative_path)
        {
            break;
        }
        i += 1;
    ...
    // Update project XML
    let relative_path_for_xml = new_table_path
        .strip_prefix(project_base_dir)?
        .to_string_lossy()
        .replace("\\", "/");
    ...
    if project_id_for_db.is_empty() {
        return Err(CommandError::Message(format!(
            "Project ID (UUID) is missing in the project file ({}).",
            xml_path.display()
        )));
    }
    ...
    info!(
        "[Backend Create Table] Success: new_table_path={}",
...
pub async fn export_formatted_table_to_csv(
    ...
    info!(
        "[export_formatted_table_to_csv] Exporting formatted table to: {}",
        output_path_str
    );
...
pub async fn export_formatted_table_to_xlsx(
    ...
    info!(
        "[export_formatted_table_to_xlsx] Exporting formatted table to: {}",
        output_path_str
    );
    ...
    if let Some(val) = styles {
        ...
        if let Some(map) = val.as_object() {
            ...
            info!(
                "[export_formatted_table_to_xlsx] Styles parameter was not an object: {:?}",
                val
            );
        }
    } else {
...
pub async fn export_table_to_csv(
    ...
    info!(
        "[export_table_to_csv] Exporting table from: {} to: {}",
        table_path_str, output_path_str
    );
    ...
    let headers_val = loaded_data
        .get("headers")
        .ok_or_else(|| CommandError::from("Missing 'headers' in table data"))?;
    let data_val = loaded_data
        .get("data")
        .ok_or_else(|| CommandError::from("Missing 'data' in table data"))?;
...
pub async fn export_table_to_xlsx(
    ...
    info!(
        "[export_table_to_xlsx] Exporting table from: {} to: {}",
        table_path_str, output_path_str
    );
