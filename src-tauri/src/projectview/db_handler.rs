...
pub fn save_asset_metadata(
    ...
    let sql = "
        ...
        ON CONFLICT(project_id, asset_relative_path) DO UPDATE SET
            -- project_id = excluded.project_id, -- Part of PK, cannot be updated here
            -- asset_relative_path = excluded.asset_relative_path, -- Part of PK
            file_name = excluded.file_name, -- file_name can change if file is replaced by another with same path but different actual name (rare)
            file_path = excluded.file_path, -- file_path can change (e.g. if project moved, though relative path is key)
            ...
            asset_type = excluded.asset_type, -- asset_type can change (e.g. if a placeholder becomes a specific type)
            updated_at = CURRENT_TIMESTAMP
        ;
    ";
