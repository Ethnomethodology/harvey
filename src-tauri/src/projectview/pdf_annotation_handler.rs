...
use log::{error, info, warn}; // debug removed
                              // PathBuf removed
...
pub async fn load_pdf_annotations(
    ...
    info!(
        "[PDF Annots DB] Loading for project_id {}: PDF key '{}'",
    ...
    match load_annotations_from_db(&project_id, &original_pdf_relative_path_str, "pdf") {
        ...
        Err(e) => {
            error!(
                "[PDF Annots DB] Error loading annotations for project_id {} - {}: {}",
                project_id, original_pdf_relative_path_str, e
            );
            Err(CommandError::from(format!(
                "Failed to load PDF annotations from DB: {}",
                e
            )))
        }
    }
...
pub async fn save_pdf_annotations(
    ...
    match save_annotations_to_db(
        ...
        &annotations_json_content,
        "pdf",
    ) {
        Ok(_) => {
            info!(
                "[PDF Annots DB] Annotations saved successfully for project_id {} - {}.",
                project_id, original_pdf_relative_path_str
            );
            Ok(())
        }
        Err(e) => {
            error!(
                "[PDF Annots DB] Error saving annotations for project_id {} - {}: {}",
                project_id, original_pdf_relative_path_str, e
            );
            Err(CommandError::from(format!(
                "Failed to save PDF annotations to DB: {}",
                e
            )))
        }
    }
