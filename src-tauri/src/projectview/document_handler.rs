...
use crate::projectview::shared_types::{
    DocumentEntryXml, FileMetadata, ProjectXml, DOCS_DIR, HARVEY_FILES_DIR, TEMP_SUBDIR_DOCS,
};
...
fn get_unique_temp_path_for_conversion(
    ...
    let file_name = format!(
        "{}_{}_{}.{}",
        sanitized_prefix, timestamp, unique_id, extension
    );
    debug!(
        "Generated unique temp path: {}",
        temp_dir.join(&file_name).display()
    );
...
pub async fn import_document<R: Runtime>(
    ...
    if project_id_for_db.is_empty() {
        ...
        return Err(CommandError::Message(format!(
            "Project ID (UUID) is missing in the project file ({}). Asset import cannot proceed.",
            project_xml_path.display()
        )));
    }
    ...
    let new_document_filename_with_ext =
        truncate_filename_stem(&original_source_filename_with_ext, MAX_FILENAME_STEM_LENGTH);
    info!(
        "[import_document] Original filename: '{}', Truncated filename for project: '{}'",
        original_source_filename_with_ext, new_document_filename_with_ext
    );

    let new_document_filename_stem_truncated = Path::new(&new_document_filename_with_ext)
        .file_stem()
        .and_then(|s| s.to_str())
        .ok_or_else(|| {
            CommandError::from(format!(
                "Could not get stem from truncated filename: {}",
                new_document_filename_with_ext
            ))
        })?;
    ...
    let (doc_folder, new_document_filename_with_ext, new_document_filename_stem) = loop {
        let current_stem = if folder_counter == 0 {
            ...
            format!(
                "{}_{}",
                new_document_filename_stem_truncated, folder_counter
            )
        };
        ...
        let name_conflict =
            project_data.document_files.files.iter().any(|f| {
        ...
        if !candidate_folder.exists() && !name_conflict {
            ...
            debug!(
                "[import_document] Found unique document folder: {} and filename: {}",
        ...
        if folder_counter > 1000 {
            return Err(CommandError::from(format!(
                "Could not find unique folder for document stem '{}' after 1000 attempts.",
                new_document_filename_stem_truncated
            )));
        }
    };
    ...
    let source_extension_lower = source_path
        .extension()
        .and_then(|s| s.to_str())
    ...
    match source_extension_lower.as_str() {
        "pdf" => {
            ...
            info!(
                "[import_document] Copying PDF from '{}' to '{}'",
                source_path.display(),
                final_pdf_path.display()
            );
            fs::copy(&source_path, &final_pdf_path)
                .map_err(|e| CommandError::from(format!("Failed to copy PDF: {}", e)))?;

            info!(
                "[import_document] Updating project XML to include PDF: {}",
                final_pdf_name
            );
            let xml_content = fs::read_to_string(&project_xml_path)?;
            ...
            if !project_data
                ...
                .files
                .iter()
                .any(|doc| doc.relative_path == relative_path_for_pdf_xml)
            {
            ...
            info!(
                "[import_document] PDF annotation file/XML entry is no longer created for PDF: {}",
            ...
            info!(
                "[import_document] PDF FileMetadata before save: created_at={:?}",
                pdf_file_metadata.created_at
            );
            ...
            info!(
                "[import_document] PDF import successful. Final path: {}",
                final_or_temp_path
            );
        }
        ...
        "txt" | "md" | "rtf" | "docx" => {
            ...
            let conversion_type = if source_extension_lower == "docx" {
                "docx"
            } else {
                "text/markdown/rtf"
            };
            info!(
                "[import_document] Handling Pandoc conversion for .{} file ({}) -> HTML",
                source_extension_lower, conversion_type
            );
            // Use new_document_filename_stem (truncated) for the temp file name prefix
            let temp_html_path: PathBuf = get_unique_temp_path_for_conversion(
                &doc_folder,
                &new_document_filename_stem,
                "html",
            )?;
            ...
            let script_path = app_handle
                ...
                .map_err(|e| {
                    CommandError::from(format!("Failed to resolve pandoc script path: {}", e))
                })?;
            ...
            info!(
                "[import_document] Running pandoc script: {} {}",
                script_path.display(),
                pandoc_args.join(" ")
            );
            ...
            let (mut rx, _child) = get_python_command(&app_handle)?
                ...
                .map_err(|e| {
                    CommandError::from(format!(
                        "Failed to spawn pandoc script for {}: {}",
                        conversion_type, e
                    ))
                })?;
            ...
            while let Some(event) = rx.recv().await {
                match event {
                    ...
                    CommandEvent::Error(msg) => {
                        error!("[Pandoc {} Error]: {}", conversion_type, msg);
                        tool_stderr.push_str(&format!("Exec Error: {}\n", msg));
                        exit_code = Some(-1);
                        break;
                    }
                    CommandEvent::Terminated(payload) => {
                        info!("[Pandoc {} Term]: {:?}", conversion_type, payload);
                        exit_code = payload.code;
                        if payload.signal.is_some() && exit_code.is_none() {
                            exit_code = Some(-1);
                        }
                        break;
                    }
            ...
            if exit_code != Some(0) {
                error!(
                    "[import_document] Pandoc {} failed. Code:{:?}\nStderr:{}",
                    conversion_type, exit_code, tool_stderr
                );
                ...
                return Err(CommandError::from(format!(
                    ...
                    tool_stderr.chars().take(300).collect::<String>()
                )));
            }
            if !temp_html_path.exists() || fs::metadata(&temp_html_path)?.len() == 0 {
                warn!(
                    "[import_document] Pandoc {} success but output missing/empty: {}",
                    conversion_type,
                    temp_html_path.display()
                );
                return Err(CommandError::from(format!(
                    "Pandoc {} success but output empty/missing.",
                    conversion_type
                )));
            }
            ...
            let asset_relative_path_for_db = conceptual_original_doc_path // Path uses truncated name
                ...
                .map_err(|_| {
                    CommandError::from(format!(
                        "Failed to strip prefix for DB relative path of document: {}",
                        conceptual_original_doc_path.display()
                    ))
                })?
                .to_string_lossy()
                .replace("\\", "/")
            ...
            info!(
                "[import_document] DOC FileMetadata before save: created_at={:?}",
                doc_file_metadata.created_at
            );
            match db_handler::save_asset_metadata(
                &project_id_for_db,           // Pass project_id
                &doc_file_metadata,
        ...
        _ => {
            error!(
                "[import_document] Unsupported file type for import: .{}",
