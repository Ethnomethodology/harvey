...
impl Drop for CancelGuard {
    fn drop(&mut self) {
        if self.state.remove(&self.job_id).is_some() {
            debug!(
                "[CancelGuard] Removed cancel flag for job '{}' on drop.",
                self.job_id
            );
        } else {
...
fn format_timestamp_for_table(seconds: f64) -> String {
    ...
    format!(
        "{:02}:{:02}.{:03}",
        total_minutes_part, secs_part, millis_part
    )
}
...
#[tauri::command]
pub async fn trim_media<R: Runtime>(
    ...
    info!(
        "[Trim Backend] Start: Path='{}', Start={:.3}, End={:.3}",
        original_media_path, start_time, end_time
    );
    ...
    let media_subdir = original_path
        .parent()
        .ok_or_else(|| CommandError::from("Could not get media parent dir"))?;
    ...
    let media_stem_dir = media_subdir
        .parent()
        .ok_or_else(|| CommandError::from("Could not get media stem dir"))?;
    let original_media_identifier = media_stem_dir
        .file_name()
        .and_then(|n| n.to_str())
        .ok_or_else(|| CommandError::from("Could not get media identifier"))?;
    let media_asset_dir = media_stem_dir
        .parent()
        .ok_or_else(|| CommandError::from("Could not get Media asset dir"))?;
    let parent_name = media_asset_dir.file_name().and_then(|n| n.to_str());
    if parent_name != Some(MEDIA_DIR)
        && parent_name != Some(AUDIOS_DIR)
        && parent_name != Some(VIDEOS_DIR)
    {
        return Err(CommandError::from(format!(
            "Media stem not in expected directory (Media, Audios, or Videos). Found: {:?}",
            parent_name
        )));
    }
    let harvey_files_dir = media_asset_dir
        .parent()
        .ok_or_else(|| CommandError::from(format!("Could not get '{}' dir", HARVEY_FILES_DIR)))?;
    if harvey_files_dir.file_name().and_then(|n| n.to_str()) != Some(HARVEY_FILES_DIR) {
        return Err(CommandError::from(format!(
            "Media asset dir not in '{}' dir",
            HARVEY_FILES_DIR
        )));
    }
    let project_base_dir = harvey_files_dir
        .parent()
        .ok_or_else(|| CommandError::from("Could not get project base dir"))?;
    let project_base_dir_name = project_base_dir
        .file_name()
        .and_then(|n| n.to_str())
        .ok_or_else(|| CommandError::from("Could not get project dir name"))?;
    ...
    let output_stem_dir_name = loop {
        ...
        if trim_counter > 999 {
            return Err(CommandError::from(
                "Could not find unique trim directory name after 999 attempts.",
            ));
        }
    };
        ...
        if let Ok(resource_dir) = app_handle.path().resource_dir() {
            ...
            if sidecars_path.exists() {
                ...
                if let Ok(existing_path) = std::env::var("DYLD_LIBRARY_PATH") {
                    command = command.env(
                        "DYLD_LIBRARY_PATH",
                        format!("{}:{}", sidecars_path_str, existing_path),
                    );
                } else {
    ...
    while let Some(event) = rx.recv().await {
        match event {
            ...
            tauri_plugin_shell::process::CommandEvent::Stderr(line) => {
                ...
                debug!("[FFmpeg Trim][stderr] {}", l.trim_end());
                ffmpeg_stderr.push(l);
            }
            tauri_plugin_shell::process::CommandEvent::Error(msg) => {
                error!("[FFmpeg Trim][error] {}", msg);
                ffmpeg_error = Some(msg);
                break;
            }
            tauri_plugin_shell::process::CommandEvent::Terminated(payload) => {
                info!(
                    "[FFmpeg Trim][term] Code:{:?}, Sig:{:?}",
                    payload.code, payload.signal
                );
                ffmpeg_exit_code = payload.code;
                break;
            }
            _ => {}
        }
    ...
    if ffmpeg_error.is_some() || ffmpeg_exit_code != Some(0) {
        error!(
            "[Trim Backend] FFmpeg fail. Code:{:?}, Err:{:?}\nStderr:\n{}",
            ffmpeg_exit_code, ffmpeg_error, stderr_output
        );
        let _ = fs::remove_dir_all(&output_stem_base_path);
        return Err(CommandError::from(format!(
            "ffmpeg trim failed. Code:{:?}. Error:{:?}",
            ffmpeg_exit_code, ffmpeg_error
        )));
    }
    if !output_media_path.exists() || output_media_path.metadata()?.len() == 0 {
        error!(
            "[Trim Backend] FFmpeg output missing or empty: {}",
            output_media_path.display()
        );
        let _ = fs::remove_dir_all(&output_stem_base_path);
        return Err(CommandError::from(format!(
    ...
    info!(
        "[Trim Backend] FFmpeg trim success: {}",
        output_media_path.display()
    );

    info!(
        "[Trim Backend] Updating XML: {}",
        project_xml_path.display()
    );
    ...
    let (original_speakers, _original_transcripts, is_video_source) = match original_entry {
        ...
        None => {
            warn!(
                "[Trim Backend] Original XML entry '{}' not found when trying to copy metadata.",
    ...
    let target_dir = if is_video_source {
        VIDEOS_DIR
    } else {
        AUDIOS_DIR
    };
    ...
    let new_media_entry = MediaFileEntryXml {
        ...
        speakers: original_speakers
            .clone()
            .or_else(|| Some(SpeakersXml::default())), // Clone here
        transcripts: Vec::new(),
    };

    if project_data.find_media(&new_media_entry.name).is_none() {
        info!(
            "[Trim Backend] Adding new media entry to XML: {}",
            new_media_entry.name
        );
        if is_video_source {
        ...
        warn!(
            "[Trim Backend] Trimmed media ID '{}' already exists in XML. Skipping XML update.",
            new_media_entry.name
        );
    }

    info!("[Trim Backend] Reloading project data...");
    let reloaded_project_data_result =
        super::core_commands::load_project_data(project_xml_path_str.clone()).await;
    ...
    if project_uuid_for_db.is_empty() {
        ...
        info!(
            "[Trim Backend] Project UUID for DB operations: {}",
            project_uuid_for_db
        );
        ...
        info!(
            "[Trim Backend] Running ffprobe for new trimmed media: {}",
            output_media_path.display()
        );
            ...
            .await
        {
            Ok(output) => {
                if output.status.success() {
                    ...
                    debug!(
                        "[Trim Backend] ffprobe output for {}: {}",
                        output_media_path.display(),
                        ffprobe_json_str
                    );
                    match serde_json::from_str::<FFProbeOutput>(&ffprobe_json_str) {
                        Ok(parsed_ffprobe_output) => {
                            duration_seconds_meta = parse_duration_str_to_seconds(
                            ...
                            bit_rate_overall_meta = parsed_ffprobe_output
                                .format
                                .bit_rate
                                .as_deref()
                            ...
                            for stream in parsed_ffprobe_output.streams {
                                ...
                                match stream.codec_type.as_deref() {
                                    Some("video") if width_meta.is_none() => {
                                        ...
                                        frame_rate_meta =
                                            parse_frame_rate_str(stream.avg_frame_rate.clone())
                                                .or_else(|| {
                            ...
                            info!(
                                "[Trim Backend] Successfully parsed ffprobe output for {}",
                                output_media_path.display()
                            );
                        }
                        Err(e) => {
                    ...
                    error!(
                        "[Trim Backend] ffprobe failed for {}. Code: {:?}, Stderr: {}",
                        output_media_path.display(),
                        output.status.code(),
                        stderr_str
                    );
            ...
            Err(e) => {
                error!(
                    "[Trim Backend] ffprobe execution error for {}: {}",
                    output_media_path.display(),
                    e
                );
            }
        ...
        let asset_type = if video_codec_meta.is_some() {
            "video"
        } else if audio_codec_meta.is_some() {
            "audio"
        } else {
            "media"
        }
        ...
        let waveform_peaks = match fs::read(&output_media_path) {
            Ok(audio_data) => match waveform_utils::generate_audio_peaks(&audio_data, 512) {
                Ok(peaks) => {
            ...
            Err(e) => {
                warn!(
                    "[Trim Backend] Failed to read media file for waveform generation: {}",
                    e
                );
                None
            }
        ...
        match db_handler::save_asset_metadata(
            ...
            Ok(_) => info!(
                "[Trim Backend] Successfully saved asset metadata to DB for trimmed media: {}",
                db_key_relative_path_trimmed
            ),
            Err(e) => warn!(
                "[Trim Backend] Failed to save asset metadata to DB for trimmed media {}: {}.",
                db_key_relative_path_trimmed, e
            ),
        }
        ...
        if let Err(e) = db_handler::save_media_transcript_data(
            ...
            warn!(
                "[Trim Backend] Failed to save media_transcript_data for trimmed media {}: {}",
                db_key_relative_path_trimmed, e
            );
        } else {
            info!(
                "[Trim Backend] Successfully saved media_transcript_data for trimmed media: {}",
                db_key_relative_path_trimmed
            );
        }

        // --- Start: Copy group associations from original media to trimmed media ---
        let original_media_relative_path = original_path
            .strip_prefix(project_base_dir)
            .map(|p| p.to_string_lossy().replace("\\", "/"))
            .map_err(|e| {
                CommandError::from(format!(
                    "Failed to get relative path for original media: {}",
                    e
                ))
            });
        ...
        if let Ok(orig_rel_path) = original_media_relative_path {
            ...
            match db_handler::get_db_path() {
                Ok(db_path_for_groups) => match rusqlite::Connection::open(&db_path_for_groups) {
                    Ok(conn) => {
                        match db_handler::get_groups_for_file_asset(
                            &conn,
                            &project_uuid_for_db,
                            &orig_rel_path,
                        ) {
...
pub async fn save_speaker_config(payload: SaveSpeakerConfigPayload) -> Result<(), CommandError> {
    ...
    if let Some(media_file) =
        project_data.find_media_by_relative_path_mut(&payload.media_relative_path)
    {
        info!(
            "[Backend SaveSpeakers] Found entry '{}'. Updating speakers.",
            payload.media_relative_path
        );
        ...
        for name in &validated_names {
            if !unique_names.insert(name.clone()) {
                warn!(
                    "Duplicate speaker name detected: '{}'. Frontend should ideally prevent this.",
                    name
                );
            }
        }
        ...
        info!(
            "[Backend SaveSpeakers] Saving validated config: {:?}",
            speakers_data
        );
    ...
    if !found_and_updated {
        return Err(CommandError::from(format!(
            "Media Rel Path '{}' not found in XML.",
            payload.media_relative_path
        )));
    }
    ...
    info!(
        "[Backend SaveSpeakers] Success for '{}'.",
        payload.media_relative_path
    );
    Ok(())
}
...
pub async fn load_transcript_json(transcript_path: String) -> Result<String, CommandError> {
    info!(
        "[Backend Load Full Transcript JSON] Path: {}",
        transcript_path
    );
    ...
    if file_path.extension().and_then(|e| e.to_str()) != Some("json") {
        return Err(CommandError::from(
            "Only .json transcripts are supported for loading.".to_string(),
        ));
    }
    ...
    match serde_json::from_str::<JsonValue>(&content) {
        ...
        Err(e) => Err(CommandError::from(format!(
            "Failed to parse transcript JSON: {}. File: {}",
            e, transcript_path
        ))),
    }
}
...
pub async fn save_transcript_json(
    ...
    use super::shared_utils;
    info!(
        "[Backend Save Full Transcript JSON] Transcript Path: {}",
        transcript_path
    );
    info!(
        "[Backend Save Full Transcript JSON] Project XML Path: {}",
        project_xml_path
    );
    let normalized_transcript_path_buf =
        shared_utils::normalize_path_for_comparison(&PathBuf::from(&transcript_path));
    let normalized_project_xml_path_buf =
        shared_utils::normalize_path_for_comparison(&PathBuf::from(&project_xml_path));

    let project_base_dir = normalized_project_xml_path_buf
        .parent()
    ...
    if let Some(parent) = normalized_transcript_path_buf.parent() {
        ...
        return Err(CommandError::from(format!(
            "Invalid transcript path (no parent directory): {}",
            transcript_path
        )));
    }
    ...
    match serde_json::from_str::<JsonValue>(&lexical_table_json_string) {
        Ok(json_value) => {
            if !(json_value.get("root").is_some()
                && json_value.get("root").unwrap().is_object()
                && json_value.get("root").unwrap().get("children").is_some()
                && json_value
                    .get("root")
                    .unwrap()
                    .get("children")
            ...
            if let Some(root_children) = json_value
                .get("root")
    ...
    writer
        .write_all(lexical_table_json_string.as_bytes())
        .map_err(|e| CommandError::from(format!("Failed to write transcript JSON: {}", e)))?;
    writer.flush()?;
    info!(
        "[Backend Save Full Transcript JSON] Saved Lexical Table JSON to disk: {}",
        normalized_transcript_path_buf.display()
    );

    let transcript_filename = normalized_transcript_path_buf
        .file_name()
        .and_then(|n| n.to_str())
        .ok_or_else(|| CommandError::from("Could not get transcript filename"))?
        .to_string();

    let (_item_type, media_identifier_opt, transcript_relative_path_buf) =
        shared_utils::get_item_details(&normalized_transcript_path_buf, project_base_dir)?;
    let media_identifier = media_identifier_opt.ok_or_else(|| {
        CommandError::from(format!(
            "Could not determine media identifier for transcript path: {}",
            transcript_path
        ))
    })?;
    let transcript_relative_path = transcript_relative_path_buf
        .to_string_lossy()
        .replace("\\", "/");
    let stem_rel_path = transcript_relative_path_buf
        .parent()
        .and_then(|p| p.parent())
        .map(|p| p.to_string_lossy().replace("\\", "/"))
    ...
    if let Some(media_entry) = project_data.find_media_by_stem_dir_mut(&stem_rel_path) {
        found_media = true;
        debug!(
            "[Backend Save Full Transcript JSON] Found media entry for stem '{}' in XML.",
            stem_rel_path
        );
        ...
        if !found_transcript_xml_entry {
            debug!(
                "[Backend Save Full Transcript JSON] Adding new transcript entry for '{}'.",
    ...
    if !found_media {
        ...
        return Err(CommandError::from(format!(
            "Media identifier '{}' not found in XML. Could not link saved transcript.",
            media_identifier
        )));
    }
    ...
    let media_entry_relative_path = project_data
        .find_media(&media_identifier)
        .unwrap()
        .relative_path
        .clone();
    ...
    let media_metadata =
        db_handler::load_asset_metadata(&project_uuid_for_db, &media_entry_relative_path)?;
...
pub(crate) fn prepare_output_paths(
    ...
    CommandError,
> {
    // Return signature matches new var names
    debug!(
    ...
    let media_filename_stem = media_path
        .file_stem()
        .and_then(|s| s.to_str())
        .ok_or_else(|| CommandError::from(format!("Invalid media filename: {}", media_path_str)))?
        .to_string();

    let media_subdir = media_path
        .parent()
        .ok_or_else(|| CommandError::from("Cannot get parent dir of media file"))?;
    if media_subdir.file_name().and_then(|n| n.to_str()) != Some(MEDIA_SUBDIR) {
        return Err(CommandError::from(format!(
            "Media file not in expected '{}' subdir.",
            MEDIA_SUBDIR
        )));
    }
    let media_stem_dir = media_subdir
        .parent()
        .ok_or_else(|| CommandError::from("Cannot get media stem dir"))?;
    let transcripts_dir = media_stem_dir.join(TRANSCRIPTS_SUBDIR);

    fs::create_dir_all(&transcripts_dir)?;
    debug!(
        "[prepare_output_paths][{}] Transcripts dir ensured: {:?}",
        job_id, transcripts_dir
    );

    // --- Paths for original transcript ---
    let temp_whisper_output_base_orig =
        transcripts_dir.join(format!("whisper_temp_{}_orig", job_id));
    let temp_whisper_output_base_orig_str =
    ...
    let mut final_transcript_path_orig =
        transcripts_dir.join(format!("{}_1.json", media_filename_stem));
    let mut counter = 2;
    while final_transcript_path_orig.exists() {
        final_transcript_path_orig =
            transcripts_dir.join(format!("{}_{}.json", media_filename_stem, counter));
        counter += 1;
    }

    let temp_rttm_base = transcripts_dir.join(format!("{}_diarization", media_filename_stem));
    ...
    if translate_to_english {
        let temp_whisper_output_base_en =
            transcripts_dir.join(format!("whisper_temp_{}_en", job_id));
        temp_whisper_output_base_en_str_opt =
            Some(temp_whisper_output_base_en.to_string_lossy().to_string());
        expected_whisper_temp_json_path_en_opt =
            Some(temp_whisper_output_base_en.with_extension("json"));

        let mut final_transcript_path_en =
            transcripts_dir.join(format!("{}.en.json", media_filename_stem));
        let mut counter = 1;
        while final_transcript_path_en.exists() {
            final_transcript_path_en =
                transcripts_dir.join(format!("{}_{}.en.json", media_filename_stem, counter));
            counter += 1;
        }
...
pub fn map_speaker_ids_to_names(segments: &mut Vec<TranscriptSegment>, user_names: &[String]) {
...
pub struct TranscriptionJobCompletedPayload {
    // Made pub
    job_id: String,
...
pub async fn transcribe_media_command<R: Runtime>(
    ...
    info!(
        "[Transcribe Command][{}] Received request: {:?}",
        job_id, payload
    );
    ...
    info!(
        "[Transcribe Command][{}] Registered with cancellation state.",
        job_id
    );
    ...
    if cancel_flag.load(AtomicOrdering::Relaxed) {
        warn!(
            "[Transcribe Command][{}] Cancelled immediately after job registration.",
            job_id
        );
        ...
        return Err(CommandError::from(
            "Transcription cancelled by user immediately.",
        ));
    }

    let media_path_for_filename = std::path::PathBuf::from(&payload.media_path_str);
    let media_filename_for_progress = media_path_for_filename.file_name().map_or_else(
    ...
    emit_progress_cmd(
        ...
        &job_id,
        0.0,
        &format!("Processing {}...", media_filename_for_progress),
    )?;
    ...
    .await
    {
        ...
        Err(e) => {
            ...
            if error_message.to_lowercase().contains("cancel") {
                warn!(
                    "[Transcribe Command][{}] WAV conversion cancelled by helper. Emitting event.",
                    job_id
                );
                let _ = app_handle.emit(
                    "custom_transcription_job_completed",
                ...
                error!(
                    "[Transcribe Command][{}] WAV conversion failed: {}. Emitting error event.",
    ...
    emit_progress_cmd(
        ...
        &job_id,
        5.0,
        &format!("Audio for {} prepared.", media_filename_for_progress),
    )?;

    let whisper_model_path_str = resolve_model_path_cmd(
        &payload.model_name,
        &job_id,
        payload.transcription_engine.as_deref(),
    )?;

    // Pre-execute_transcription_pass (Original Language) Cancellation Check
    if cancel_flag.load(AtomicOrdering::Relaxed) {
        warn!(
            "[Transcribe Command][{}] Cancelled before main processing pass.",
            job_id
        );
        ...
        let _ = app_handle.emit(
            ...
            TranscriptionJobCompletedPayload {
                ...
                translated_transcript_file_path: None,
                error_message: Some(
                    "Transcription cancelled by user before main processing pass.".to_string(),
                ),
            },
        );
        return Err(CommandError::from(
            "Transcription cancelled before main processing pass.",
        ));
    }
    ...
    emit_progress_cmd(
        ...
        &job_id,
        10.0,
        &format!("Transcribing {}...", media_filename_for_progress),
    )?;
    ...
    .await
    {
        ...
        Err(e) => {
            let error_message = e.to_string();
            warn!(
                "[Transcribe Command][{}] Original transcription pass failed: {}",
                job_id, error_message
            );
            // Cleanup temporary files from this pass
            if wav_media_path.to_string_lossy() != payload.media_path_str {
                // Check if WAV was temporary
                ...
                let _ = app_handle.emit(
                    ...
                    TranscriptionJobCompletedPayload {
                        ...
                        error_message: Some(format!(
                            "Original transcription pass failed: {}",
                            error_message
                        )),
                    },
                );
    ...
    if payload.translate_to_english {
        ...
        if cancel_flag.load(AtomicOrdering::Relaxed) {
            warn!(
                "[Transcribe Command][{}] Cancelled before translation pass.",
                job_id
            );
            if wav_media_path.to_string_lossy() != payload.media_path_str {
            ...
            let _ = app_handle.emit(
                ...
                TranscriptionJobCompletedPayload {
                    ...
                    translated_transcript_file_path: None,
                    error_message: Some(
                        "Transcription cancelled by user before translation pass.".to_string(),
                    ),
                },
            );
            return Err(CommandError::from(
                "Transcription cancelled before translation pass.",
            ));
        }
        ...
        if let (Some(base_en_str), Some(json_path_en), Some(final_path_en_pb)) = (
            ...
            emit_progress_cmd(
                ...
                &job_id,
                60.0,
                &format!("Translating {}...", media_filename_for_progress),
            )?;

            let source_language_for_translation = payload
                .language_code
                .clone()
                .unwrap_or_else(|| "auto".to_string());
            ...
            let mut translated_segments = match translation_result {
                ...
                Err(e) => {
                    let error_message = e.to_string();
                    warn!(
                        "[Transcribe Command][{}] Translation pass failed: {}",
                        job_id, error_message
                    );
                    if wav_media_path.to_string_lossy() != payload.media_path_str {
            ...
            align_speakers_to_translated_segments(
                &original_segments,
            ...
            if let Some(ref translated_names) = payload.translated_speaker_names {
                let contains_actual_names =
                    translated_names.iter().any(|name| !name.trim().is_empty());
            ...
            emit_progress_cmd(
                ...
                &job_id,
                90.0,
                &format!("Saving translation for {}...", media_filename_for_progress),
            )?;
            let lexical_json_en = create_lexical_table_from_segments(&translated_segments);
            let lexical_json_en_str =
                serde_json::to_string_pretty(&lexical_json_en).map_err(|e| {
                    CommandError::from(format!(
                        "Failed to serialize translated Lexical Table JSON: {}",
                        e
                    ))
                })?;
            ...
            .await?;
            info!(
                "[Transcribe Command][{}] Translated transcript saved.",
                job_id
            );

            if json_path_en.exists() {
                let _ = fs::remove_file(&json_path_en);
            }
        }
    }
    ...
    emit_progress_cmd(
        ...
        95.0,
        &format!(
            "Saving original transcript for {}...",
            media_filename_for_progress
        ),
    )?;
    let lexical_json_orig = create_lexical_table_from_segments(&original_segments);
    let lexical_json_orig_str = serde_json::to_string_pretty(&lexical_json_orig).map_err(|e| {
        CommandError::from(format!(
            "Failed to serialize original Lexical Table JSON: {}",
            e
        ))
    })?;
    ...
    let normalized_xml_path = crate::projectview::shared_utils::normalize_path_for_comparison(
        &PathBuf::from(&payload.project_xml_path),
    );
    if let Some(project_base_dir) = normalized_xml_path.parent() {
        ...
        if let Ok(relative_path) = media_path_buf.strip_prefix(project_base_dir) {
            ...
            if let Ok(project_data) = crate::projectview::core_commands::load_project_data(
                payload.project_xml_path.clone(),
            )
            .await
            {
                ...
                if let Some(media_entry) = project_data
                    .files
                    .iter()
                    .find(|f| f.path == payload.media_path_str)
                {
    ...
    emit_progress_cmd(
        ...
        &job_id,
        100.0,
        &format!("Successfully processed {}.", media_filename_for_progress),
    )?;
    ...
    let completion_payload = TranscriptionJobCompletedPayload {
        ...
        translated_transcript_file_path: final_transcript_path_en_for_payload
...
fn align_speakers_to_translated_segments(
    ...
    if original_segments.is_empty() || translated_segments.is_empty() {
        return;
    }
    info!(
        "[Align Speakers][{}] Aligning {} translated segments with {} original segments.",
        job_id,
    ...
    for t_seg in translated_segments.iter_mut() {
        ...
        for o_seg in original_segments {
            ...
            if current_overlap > 0.0 {
                ...
                let current_priority = if t_mid >= o_start && t_mid < o_end {
                    0.0
                } else {
                    (t_mid - o_mid).abs()
                };
                ...
                if current_overlap > max_overlap
                    || (current_overlap == max_overlap && current_priority < best_overlap_priority)
                {
...
pub(crate) async fn convert_to_wav_if_needed_cmd<R: Runtime>(
    ...
    info!(
        "[FFmpeg CMD][{}] Checking audio file: {}",
    ...
    if extension == "wav" {
        info!(
            "[FFmpeg CMD][{}] Input is already WAV. Skipping conversion.",
            job_id
        );
    ...
    if output_wav_path.exists() {
        match output_wav_path.metadata() {
            Ok(m) if m.len() > 0 => {
                info!(
                    "[FFmpeg CMD][{}] Target WAV file already exists and is not empty. Reusing.",
                    job_id
                );
    ...
    let _ = emit_progress_cmd(
        ...
        job_id,
        2.0,
        &format!("Converting {} to WAV...", media_filename_for_progress),
    )?;
    ...
    debug!(
        "[FFmpeg CMD][{}] Spawned FFmpeg process (PID: {:?})",
        job_id,
        child.pid()
    );
    ...
    loop {
        if cancel_flag.load(AtomicOrdering::Relaxed) {
            warn!(
                "[FFmpeg CMD][{}] Cancellation requested. Killing FFmpeg process...",
                job_id
            );
            let _ = child.kill();
            if output_wav_path.exists() {
                if let Err(e) = fs::remove_file(&output_wav_path) {
                    warn!(
                        "[FFmpeg CMD][{}] Failed to remove partial WAV file {:?}: {}",
                        job_id, output_wav_path, e
                    );
                } else {
                    info!(
                        "[FFmpeg CMD][{}] Removed partial WAV file {:?}",
                        job_id, output_wav_path
                    );
                }
            }
            return Err(CommandError::from(format!(
                "Audio conversion cancelled for job {}.",
                job_id
            )));
        }
    ...
    if ffmpeg_error.is_some() || ffmpeg_exit_code != Some(0) {
        error!(
            "[FFmpeg CMD][{}] FFmpeg process failed. Code: {:?}, Error: {:?}\nStderr:\n{}",
    ...
    if !output_wav_path.exists() {
        error!(
            "[FFmpeg CMD][{}] FFmpeg reported success, but output file is missing: {}",
            job_id,
            output_wav_path.display()
        );
        return Err(CommandError::from(format!(
    ...
    match output_wav_path.metadata() {
        Ok(m) if m.len() == 0 => {
            error!(
                "[FFmpeg CMD][{}] FFmpeg reported success, but output file is empty: {}",
                job_id,
                output_wav_path.display()
            );
            let _ = fs::remove_file(&output_wav_path);
        ...
        Err(e) => {
            error!(
                "[FFmpeg CMD][{}] FFmpeg reported success, but failed to get metadata for {}: {}",
                job_id,
                output_wav_path.display(),
                e
            );
    ...
    info!(
        "[FFmpeg CMD][{}] Successfully converted '{}' to WAV: {}",
        job_id,
        input_path_str,
        output_wav_path.display()
    );
...
pub(crate) fn find_model_file_cmd(model_dir: &Path) -> Result<PathBuf, CommandError> {
    debug!(
        "[Helper CMD] Searching for model file in directory: {:?}",
        model_dir
    );
    ...
    for entry_result in fs::read_dir(model_dir)? {
        match entry_result {
            ...
            Err(e) => {
                warn!(
                    "[Helper CMD] Failed to read directory entry in '{}': {}",
                    model_dir.display(),
                    e
                );
            }
...
pub(crate) fn resolve_model_path_cmd(
    ...
    if engine == Some("faster-whisper") {
        ...
        if !model_dir_path.exists() || !model_dir_path.is_dir() {
            let e_msg = format!(
                "Faster-whisper model directory not found: '{}'. Please download the model first.",
...
async fn run_diarization_script<R: Runtime>(
    ...
    info!(
        "[Diarization Script][{}] Starting for: {}, num_speakers: {}",
        job_id, media_path, num_speakers
    );
    ...
    let mut command =
        get_python_command(app_handle).map_err(|e| CommandError::from(e.to_string()))?;
    ...
    info!(
        "[Diarization Script][{}] Spawned python script (PID: {:?})",
        job_id,
        child.pid()
    );
    ...
    loop {
        if cancel_flag.load(AtomicOrdering::Relaxed) {
            warn!(
                "[Diarization Script][{}] Cancellation requested. Killing process...",
                job_id
            );
            let _ = child.kill();
            return Err(CommandError::from(format!(
                "Diarization process cancelled for job {}.",
                job_id
            )));
        }
    ...
    if process_error.is_some() || exit_code != Some(0) {
        ...
        let error_message = format!(
            "Diarization script failed. Exit: {:?}, Err: {:?}\nStderr:\n{}",
...
fn parse_rttm_file_cmd(rttm_path: &Path) -> Result<Vec<RttmRecord>, CommandError> {
    ...
    for line_result in reader.lines() {
        ...
        let start_time: f64 = parts[3]
            .parse()
            .map_err(|_| CommandError::from("Invalid RTTM start time"))?;
        let duration: f64 = parts[4]
            .parse()
            .map_err(|_| CommandError::from("Invalid RTTM duration"))?;
    ...
    info!(
        "[RTTM Parse CMD] Parsed {} records from {}",
        records.len(),
        rttm_path.display()
    );
    Ok(records)
...
// Adapted from local_handler/transcription.rs
fn merge_diarization_results_cmd(
...
pub(crate) async fn execute_transcription_pass<R: Runtime>(
    ...
    info!(
        "[Exec Pass][{}] DEBUG: Entered. Lang: {}, Translate: {}, NumSpeakers: {}, Engine: {:?}",
        job_id, language_code, is_translation_pass, num_speakers, transcription_engine
    );

    let output_dir = expected_whisper_json_output_path
        .parent()
    ...
    if diarization_on && !is_translation_pass {
        emit_progress_cmd(
            ...
            job_id,
            30.0,
            &format!("Diarizing {}...", media_filename_for_progress),
        )?;
        ...
        match parse_rttm_file_cmd(&rttm_path) {
            Ok(rttm_records) => {
                if !rttm_records.is_empty() {
                    ...
                    warn!(
                        "[Exec Pass][{}] Diarization produced no RTTM records.",
                        job_id
                    );
                }
            }
    ...
    info!(
        "[Exec Pass][{}] Pass complete. Segments: {}.",
        job_id,
        segments.len()
    );
    Ok(segments)
...
#[tauri::command]
pub async fn list_subtitle_files_command(
    media_path_str: String,
) -> Result<Vec<SubtitleFileEntry>, CommandError> {
    info!(
        "[list_subtitle_files_command] Listing subtitles for: {}",
        media_path_str
    );
    ...
    let media_file_parent_dir = media_path
        .parent()
        .ok_or_else(|| CommandError::from("Could not get media file parent dir"))?;
    if media_file_parent_dir.file_name().and_then(|n| n.to_str()) != Some(MEDIA_SUBDIR) {
        return Err(CommandError::from(format!(
            "Media file not in expected '{}' subdirectory.",
            MEDIA_SUBDIR
        )));
    }
    let media_stem_dir = media_file_parent_dir
        .parent()
        .ok_or_else(|| CommandError::from("Could not get media stem directory"))?;
    let transcripts_dir = media_stem_dir.join(TRANSCRIPTS_SUBDIR);

    let mut subtitle_files = Vec::new();
    if transcripts_dir.exists() && transcripts_dir.is_dir() {
        for entry in fs::read_dir(transcripts_dir)
            .map_err(|e| CommandError::from(format!("Failed to read transcripts dir: {}", e)))?
        {
            let entry = entry
                .map_err(|e| CommandError::from(format!("Error reading directory entry: {}", e)))?;
            let path = entry.path();
            if path.is_file() {
...
pub async fn convert_srt_to_vtt_command(srt_path_str: String) -> Result<String, CommandError> {
    info!(
        "[convert_srt_to_vtt_command] Converting SRT: {}",
        srt_path_str
    );
...
fn convert_ass_time_to_vtt(t: &str) -> String {
    ...
    if parts.len() >= 2 {
        ...
        let normalized_hms = if hms_parts.len() == 3 {
            format!(
                "{:0>2}:{:0>2}:{:0>2}",
                hms_parts[0], hms_parts[1], hms_parts[2]
            )
...
pub async fn convert_ass_to_vtt_command(ass_path_str: String) -> Result<String, CommandError> {
    info!(
        "[convert_ass_to_vtt_command] Converting ASS with robust state machine: {}",
        ass_path_str
    );
        ...
        if section == "[V4+ Styles]" || section == "[V4 Styles]" {
            if trimmed.starts_with("Format:") {
                ...
                if parts.len() == 2 && !format_cols_styles.is_empty() {
                    ...
                    if col_values.len() == format_cols_styles.len() {
                        ...
                        if let Some(idx) =
                            format_cols_styles.iter().position(|c| c == "primarycolour")
                        {
            ...
            if trimmed.starts_with("Format:") {
                ...
                if parts.len() == 2 && !format_cols_events.is_empty() {
                    ...
                    let col_values: Vec<&str> =
                        data_part.splitn(num_cols, ',').map(|s| s.trim()).collect();

                    if col_values.len() == num_cols {
                        let start_idx = format_cols_events
                            .iter()
                            .position(|c| c == "start")
                            .unwrap_or(1);
                        let end_idx = format_cols_events
                            .iter()
                            .position(|c| c == "end")
                            .unwrap_or(2);
                        let style_idx = format_cols_events
                            .iter()
                            .position(|c| c == "style")
                            .unwrap_or(3);
                        let text_idx = format_cols_events
                            .iter()
                            .position(|c| c == "text")
                            .unwrap_or(9);

                        if start_idx < col_values.len()
                            && end_idx < col_values.len()
                            && text_idx < col_values.len()
                        {
                            ...
                            events.push(DialogueEvent {
                                ...
                                style: if style_idx < col_values.len() {
                                    col_values[style_idx].to_string()
                                } else {
                                    "Default".to_string()
                                },
                                text: raw_text,
        ...
        let append_text =
            |out: &mut String,
             ...
             used_classes: &mut std::collections::HashMap<String, String>| {
                ...
                if let Some(c) = color {
                    let is_white = c.eq_ignore_ascii_case("#FFFFFF")
                        || c.eq_ignore_ascii_case("#FFF")
                        || c.eq_ignore_ascii_case("white");
                    if !is_white {
                        let safe_suffix = c
                            .trim_start_matches('#')
                            .chars()
                ...
                if bold {
                    prefix.push_str("<b>");
                    suffix.insert_str(0, "</b>");
                }
                if italic {
                    prefix.push_str("<i>");
                    suffix.insert_str(0, "</i>");
                }
                if underline {
                    prefix.push_str("<u>");
...
pub async fn cancel_transcription(
    ...
    // Ensure CommandError is in use
    info!(
        "[Transcribe Command][Cancel] Received cancellation request for job: {}",
        job_id
    );
    if let Some(flag_entry) = cancel_state.0.get(&job_id) {
        ...
        match cancel_flag.compare_exchange(
            false,
            true,
            ...
            Ok(_) => {
                info!(
                    "[Transcribe Command][Cancel] Cancellation flag successfully SET for job: {}",
                    job_id
                );
            }
            Err(_) => {
                info!(
                    "[Transcribe Command][Cancel] Cancellation flag was already SET for job: {}",
                    job_id
                );
            }
        }
...
pub async fn start_live_transcription(
    ...
    let mut command = app_handle.shell().command(binary_path_str.clone());
    ...
    info!(
        "[Live Transcription] Executing command '{}' with args: {:?}",
        binary_path_str, args
    );
    ...
    tokio::spawn(async move {
        ...
        while let Some(event) = rx.recv().await {
            ...
            match event {
                ...
                CommandEvent::Stderr(line) => {
                    error!(
                        "[Live Transcription][whisper-stream stderr]: {}",
                ...
                CommandEvent::Terminated(payload) => {
                    info!(
                        "[Live Transcription] Whisper-stream process terminated with payload: {:?}",
                        payload
                    );
                }
                _ => {}
...
pub async fn stop_live_transcription(
    ...
    if let (Some(active_doc_path_str), Some(project_uuid), Some(project_base_dir)) =
        (active_doc_path_opt, project_uuid_opt, project_base_dir_opt)
    {
        info!(
            "[Live Transcription] Processing saved audio for doc: {}",
            active_doc_path_str
        );
        ...
        if attachments_dir.exists() && attachments_dir.is_dir() {
            ...
            match fs::read_dir(&attachments_dir) {
                ...
                Err(e) => {
                    error!(
                        "[Live Transcription] Failed to read attachments directory: {}",
                        e
                    );
                }
            }
            ...
            for path in candidate_paths {
                ...
                if let Err(e) = trim_silence_from_wav(&app_handle, &path_str).await {
                    error!(
                        "[Live Transcription] Failed to trim silence from {}: {}",
                        path_str, e
                    );
                }
            ...
            if !audio_files.is_empty() {
                info!(
                    "[Live Transcription] Found {} audio files to attach.",
                    audio_files.len()
                );
                ...
                match db_handler::load_asset_metadata(&project_uuid, &relative_doc_path) {
                    Ok(Some(metadata_from_db)) => {
                        ...
                        let mut existing_attachments: Vec<String> = custom_fields
                            .iter()
                        ...
                        if let Err(e) = db_handler::save_asset_metadata(
                            ...
                            if let Err(e) =
                                app_handle.emit("metadata_updated", &active_doc_path_str)
                            {
                    ...
                    Err(e) => error!(
                        "[Live Transcription] Failed to get metadata for document '{}': {}",
                        relative_doc_path, e
                    ),
                }
            }
...
async fn trim_silence_from_wav(app_handle: &AppHandle, file_path_str: &str) -> Result<(), String> {
    ...
    info!(
        "[Live Transcription] Trimming silence: {:?} {}",
        ffmpeg_path,
        args.join(" ")
    );
    ...
    if output_path.exists() && output_path.metadata().map(|m| m.len() > 0).unwrap_or(false) {
        ...
        fs::rename(&output_path, &input_path)
            .map_err(|e| format!("Failed to rename trimmed file: {}", e))?;
        info!(
            "[Live Transcription] Successfully trimmed silence from {}",
            file_path_str
        );
    } else {
        return Err(format!(
            "Trimmed output file missing or empty for {}",
            file_path_str
        ));
    }
