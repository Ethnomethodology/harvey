...
use super::shared_utils::{
    ensure_base_asset_dirs, save_project_xml, truncate_filename_stem, MAX_FILENAME_STEM_LENGTH,
};
...
fn register_project_image(
    ...
    if project_data
        ...
        .files
        .iter()
        .any(|f| f.relative_path == relative_path_for_xml)
    {
    ...
    let file_metadata_for_db = FileMetadata {
        ...
        title: format!(
            "Screenshot from {} at {}s",
            original_media_stem_for_metadata,
            timestamp.round()
        ), // Use new parameter
        description: format!(
            "Screenshot captured from media '{}' at timestamp {} seconds.",
            original_media_stem_for_metadata, timestamp
        ), // Use new parameter
        summary: String::new(),
    ...
    if let Err(e) = db_handler::save_asset_metadata(
        ...
        return Err(CommandError::from(format!(
            "Failed to save screenshot metadata to DB: {}",
            e
        )));
    }
    info!(
        "[register_project_image] Saved screenshot metadata to DB for: {} (project_id: {})",
...
pub async fn save_screenshot(
    ...
    let project_base_dir = match project_xml_path.parent() {
        ...
        None => {
            return Err(format!(
                "Could not determine base directory from project XML path: {}",
    ...
    if !project_xml_path.exists() {
        return Err(format!(
            "Project XML file not found at the specified path: {}",
            project_xml_path_str
        ));
    }
    ...
    fs::create_dir_all(&image_folder_for_screenshot).map_err(|e| {
        format!(
            "Failed to create Images sub-directory for screenshot {}: {}",
            screenshot_base_folder_name, e
        )
    })?;
    ...
    loop {
        ...
        if counter > 100 {
            // Safety break
            return Err(format!(
                "Could not generate a unique filename for screenshot from {} at timestamp {}",
                original_media_stem, timestamp_str
            ));
        }
    }
    ...
    let image_bytes = STANDARD
        .decode(&image_data_base64) // Updated base64 decode
    ...
    info!(
        "[save_screenshot] Screenshot saved to: {}",
        file_path.display()
    );
...
pub async fn import_image_file(
    ...
    info!(
        "[import_image_file] Importing image from: {}",
        source_path_str
    );
    ...
    let original_source_filename_with_ext = source_path
        .file_name()
        .and_then(|s| s.to_str())
    ...
    let truncated_image_filename_with_ext =
        truncate_filename_stem(&original_source_filename_with_ext, MAX_FILENAME_STEM_LENGTH);
    info!(
        "[import_image_file] Original filename: '{}', Truncated filename for project: '{}'",
        original_source_filename_with_ext, truncated_image_filename_with_ext
    );

    let image_file_stem_truncated_original = Path::new(&truncated_image_filename_with_ext)
        .file_stem()
        .and_then(|s| s.to_str())
        .ok_or_else(|| {
            CommandError::from(format!(
                "Could not get stem from truncated image filename: {}",
                truncated_image_filename_with_ext
            ))
        })?;
    ...
    let (folder_path, final_image_name) = loop {
        ...
        let name_conflict =
            project_data.image_files.files.iter().any(|f| {
        ...
        if !candidate_folder.exists() && !name_conflict {
            ...
            debug!(
                "[import_image_file] Found unique image folder: {} and filename: {}",
        ...
        if folder_counter > 1000 {
            return Err(CommandError::from(format!(
                "Could not find unique folder for image stem '{}' after 1000 attempts.",
                image_file_stem_truncated_original
            )));
        }
    };
    ...
    // Copy the file
    info!(
        "[import_image_file] Copying image from '{}' to '{}'",
        source_path.display(),
        final_image_path.display()
    );
    fs::copy(&source_path, &final_image_path)
        .map_err(|e| CommandError::from(format!("Failed to copy image file: {}", e)))?;

    // Update the project XML
    info!(
        "[import_image_file] Updating project XML to include image: {}",
        final_image_name
    );
    let xml_content = fs::read_to_string(&project_xml_path)?;
    let mut project_data: ProjectXml = serde_json::from_str(&xml_content)?;
    ...
    project_data
        .image_files
        .files
        .sort_by(|a, b| a.name.cmp(&b.name)); // Keep the list sorted
    ...
    if let Err(e) = db_handler::save_asset_metadata(
        ...
        error!(
            "[import_image_file] Failed to save image metadata to DB for {} (project_id: {}): {}",
            relative_path_for_xml, project_id_for_db, e
        );
        // Attempt to clean up copied file if DB save fails?
        return Err(CommandError::from(format!(
            "Failed to save image metadata to DB: {}",
            e
        )));
    }
    info!(
        "[import_image_file] Saved image metadata to DB for: {} (project_id: {})",
        relative_path_for_xml, project_id_for_db
    );
...
#[tauri::command]
pub async fn load_image_annotations(
    project_id: String,
    image_relative_path_str: String,
    ...
    info!(
        "[DB Image Annots] Loading for project_id {}: image key '{}'",
        project_id, image_relative_path_str
    );
    match load_annotations_from_db(&project_id, &image_relative_path_str, "image") {
        ...
        Err(e) => {
            error!(
                "[DB Image Annots] Error loading annotations for project_id {} - {}: {}",
                project_id, image_relative_path_str, e
            );
            Err(CommandError::from(format!(
                "Failed to load image annotations from DB: {}",
                e
            )))
        }
    }
...
#[tauri::command]
pub async fn save_image_annotations(
    project_id: String,
    image_relative_path_str: String,
    annotations_json_string: String,
) -> Result<(), CommandError> {
    ...
    info!(
        "[DB Image Annots] Saving for project_id {}: image key '{}'",
        project_id, image_relative_path_str
    );
    ...
    match save_annotations_to_db(
        ...
        Ok(_) => {
            info!(
                "[DB Image Annots] Annotations saved successfully for project_id {} - {}.",
                project_id, image_relative_path_str
            );
            Ok(())
        }
        Err(e) => {
            error!(
                "[DB Image Annots] Error saving annotations for project_id {} - {}: {}",
                project_id, image_relative_path_str, e
            );
            Err(CommandError::from(format!(
                "Failed to save image annotations to DB: {}",
                e
            )))
        }
    }
...
mod tests {
    ...
    fn create_dummy_project_xml_for_image_test(
        project_dir: &Path,
        project_name: &str,
        project_uuid: &str,
    ) -> PathBuf {
    ...
    fn create_dummy_png_file(dir: &Path, filename: &str) -> PathBuf {
        ...
        file.write_all(&png_magic)
            .expect("Failed to write dummy png content");
        file_path
    }
    ...
    async fn test_import_image_file_with_db_metadata() -> Result<(), Box<dyn std::error::Error>> {
        let (_temp_dir, project_dir, _dummy_image_path, _conn, project_uuid) =
            setup_test_environment();
        let project_xml_path = create_dummy_project_xml_for_image_test(
            &project_dir,
            "TestImageProject",
            &project_uuid,
        );
        ...
        let import_result =
            import_image_file(dummy_image_path_str.clone(), project_xml_path_str.clone()).await;
        assert!(
            import_result.is_ok(),
            "import_image_file failed: {:?}",
            import_result.err()
        );
        ...
        assert!(
            final_image_abs_path.exists(),
            "Imported image file should exist at {}",
            final_image_abs_path_str
        );
        ...
        let expected_relative_path = final_image_abs_path
            .strip_prefix(&project_dir)?
            .to_string_lossy()
            .replace("\\", "/");

        assert_eq!(
            updated_project_data.image_files.files.len(),
            1,
            "Should be one image file in XML"
        );
        ...
        assert!(
            updated_project_data
                ...
                .files
                .is_empty(),
            "document_metadata_files should be empty in XML regarding this image"
        );

        // Database Assertions
        let loaded_meta_option =
            test_db_handler::load_asset_metadata("test-image-uuid", &expected_relative_path)
        ...
        if let Some(loaded_meta) = loaded_meta_option {
            ...
            assert!(
                loaded_meta.width.is_none(),
                "Width should be None as image_handler does not extract it"
            );
            assert!(
                loaded_meta.height.is_none(),
                "Height should be None as image_handler does not extract it"
            );
        }
