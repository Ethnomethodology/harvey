...
# The original function body is huge.
# We will create a hybrid approach:
# 1. Open the DB. Check if we have > 0 records for this project_id.
# 2. If NO records: Read the JSON manifest. Call a new function `ingest_manifest_to_db` (which will iterate the JSON structure, populate parent_asset_path, and insert rows).
# 3. Then, regardless, call `build_project_view_data_from_db` to construct the FileEntry tree for Svelte.

# To preserve the auto-healing and the exact Svelte structure mapping without rewriting 700 lines from scratch,
# I will extract the current parsing logic into `ingest_manifest_to_db`, tweaking it to INSERT instead of just building vectors.

# But wait, modifying 700 lines of complex nested loops in python scripting might be error-prone.
# A safer, more incremental refactor that perfectly fits "CQRS":
# I will implement `build_project_view_data_from_db` first.
# For now, I will keep the existing logic in `load_project_data` but wrap it in an `if asset_count == 0` block.
# This ensures it ONLY runs for new projects, seeds the DB, and then we return `build_project_view_data_from_db`.
...
with open("src-tauri/src/projectview/core_commands_cqrs.rs", "w") as f:
    f.write(new_code)
