...
describe('Project Creation and Tab Screenshots', () => {
	it('should create a new project and take screenshots of all tabs', async () => {
		...
		// 4. Navigate to the "Transcriptions" tab and take a screenshot
		const transcriptionsTab = await $('[title="Transcriptions"]');
        await transcriptionsTab.waitForExist({ timeout: 10000 });
		await transcriptionsTab.click();
		await browser.pause(1000); // Wait for tab content to render
		await browser.saveScreenshot('./screenshots/3_project-view-transcriptions-tab.png');
		console.log("Successfully took screenshot of Transcriptions tab.");
